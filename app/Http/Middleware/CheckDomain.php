...
class CheckDomain
{
    ...
    public function handle($request, Closure $next)
    {
	   ...
	   /* if($domain=='secdoc.seqrdoc.com'){
		...
		if($domain=='convocation.mitwpu.edu.in'){ 
			$domain = 'mitwpu.seqrdoc.com';
		}
		//Added By Aditya 2024-11-05 for anu domain change
		if ($domain == 'verification.anu.edu.in') {
			// Get the full current URL
			$currentRoute = \Request::url();
			// dd($currentRoute);
			// Check if 'bverify' is part of the current URL
			if (strpos($currentRoute, 'bverify') !== false) {
				// If 'bverify' is found, change the domain
				$domain = 'anu.seqrdoc.com';
			} else {
				// If 'bverify' is not found, return 'site_status' view
				return response()->view('site_status');
			}
		}
		...
		else if($subdomain[0] == 'convocation')
		{

			$dbName = 'seqr_d_mitwpu';
		}

		else if($subdomain[0] == 'certificate')
		{
