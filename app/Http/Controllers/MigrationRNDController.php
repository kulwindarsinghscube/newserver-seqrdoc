...
class MigrationRNDController extends Controller
{
    ...
    {   
        ...
        $FIELDS = FieldMaster::where('template_id',$id)->orderBy('field_position','ASC')->get();
        ...
        foreach($FIELDS as $key => $FID ) {
            ...
            if($FID->font_id != 0) {
                $font_data=FontMaster::find($FID->font_id)->toArray(); 
        ...
        $templateData = ExcelPDFTemplateMaster::select('id')->where('template_name',$template_name)->first();
        ...
        if($templateData){
            ...
            ExcelPDFTemplateMaster::where('id',$template_id)->update([ "placer_details"=>$placer_boxes,"ep_details"=>$ep_boxes,"print_bg_file"=>$bg_template_id,"print_bg_status"=>$print_bg_status,"verification_bg_file"=>$bg_template_id,"verification_bg_status"=>$verification_bg_status]);   
