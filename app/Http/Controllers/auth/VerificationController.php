...
public function showVerify($token)
{
    ...
    $site_data = Demosite::select('apple_app_url', 'android_app_url')
    ...
    $existing_user = DB::table('user_table')->where('token', $token)->first();

    if ($existing_user) {
        $email = $existing_user->email_id;
        $isVerified = $existing_user->is_verified ?? 0;
    } else {
        // Just check if token is valid
        $pendingUser = UserVerification::where('token', $token)->first();

        if ($pendingUser) {
            $email = $pendingUser->email_id;
            $isVerified = 0;
        } else {
    ...
    return view('auth.verify', compact('site_data', 'sitename', 'email', 'isVerified', 'error', 'token'));
}
...
public function verifyUser($token)
{
    ...
    $subdomain = explode('.', $domain)[0]; // take first part

    if (!empty($token)) {
        $existing = DB::table('user_table')->where('token', $token)->first();

        if ($existing) {
            $status = ($existing->is_verified == 1) ? 2 : 1;

            if ($status === 1) {
                // Add subdomain-specific logic if needed
                if (strtolower($subdomain) == 'mallareddyuniversity') {
                    // Example: you could set extra fields or perform logging
                    DB::table('user_table')->where('token', $token)->update([
                        'is_verified' => 1,
                        'organization_name' => $existing->organization_name ?? null
                    ]);
                } else {
            ...
            if ($pendingUser) {
                $insertData = [
                    ...
                    'email_id' => $pendingUser->email_id,
                    'mobile_no' => $pendingUser->mobile_no,
                    'verify_by' => $pendingUser->verify_by,
                    'status' => '1',
                    'device_type' => $pendingUser->device_type,
                    'token' => $pendingUser->token,
                    'OTP' => $pendingUser->OTP,
                    'site_id' => $pendingUser->site_id,
                    'is_verified' => 1,
                    'created_at' => now(),
                    'updated_at' => now(),
                ];

                // Add organization_name for mallareddyuniversity
                if (strtolower($subdomain) == 'mallareddyuniversity') {
                    $insertData['organization_name'] = $pendingUser->organization_name ?? null;
                }

                DB::table('user_table')->insert($insertData);
