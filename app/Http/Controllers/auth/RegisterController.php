   ...
   public function userRegister(WebUserRegisterRequest $request)
{
    ...
    // -----------------------------
    // Store temporarily in UserVerification table
    // -----------------------------
    $userData = [
    ...
    // Add organization_name only for Mallareddy University
    if (strtolower($subdomain[0]) == 'mallareddyuniversity') {
        $userData['organization_name'] = $request['organization_name'];
    }

    UserVerification::create($userData);

    // Send verification email if verify_by == 2
    ...
    return response()->json([
        'status' => true,
        'link' => URL::route('webapp.verify', $token),
    ]);
}
