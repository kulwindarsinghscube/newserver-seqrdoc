...
use Helper;
use App\Jobs\ValidateExcelKDKNJob;
use App\Jobs\pdfGenerateKdknJob;
// use App\Jobs\pdfGenerateJntuJob1;


class KDKNCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.kdkn.index');
        //newrrmu
    }
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                // if ($dropdown_template_id == 1) {
                    ...
                    // dd($excelData);
                    $response = $this->dispatch(new ValidateExcelKDKNJob($excelData));
                    // print_r($response);
                    // die();
...
// dd($request->all());
          ...
          if($dropdown_template_id==1){
            ...
            $link = $this->dispatch(new pdfGenerateKdknJob($pdfData));
        }
