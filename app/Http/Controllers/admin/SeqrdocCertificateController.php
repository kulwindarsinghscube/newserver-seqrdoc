...
use Helper;
use App\Jobs\ValidateExcelSeqrdocCertificateJob;
use App\Jobs\PdfGenerateSeqrdocGradeCertificate;

class SeqrdocCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.seqrdoc.records');
       //newrrmu
    }

    public function uploadpage(){

      return view('admin.seqrdoc.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSeqrdocCertificateJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateSeqrdocGradeCertificate($pdfData));
        } 
