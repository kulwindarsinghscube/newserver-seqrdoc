...
use App\Jobs\PdfGenerateKsgCustomJob;


class KsgCustomController extends Controller
{
    public function index(Request $request)
    {
    //    return view('admin.ksg.custom_index');
    $batches = DB::table('tbl_batch')->get();

    return view('admin.ksg.custom_index', compact('batches'));
    }
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $batch_name = KsgbatchModel::where('id', $batch_id)->value('name');
         $temp_id=TemplateMaster::where('template_name',$batch_name)->value('id');
            ...
            $pdfData=array('studentDataOrg'=>$rowData1,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'image_path'=> $temp_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));
      
   
        
            $link = $this->dispatch(new PdfGenerateKsgCustomJob($pdfData));
