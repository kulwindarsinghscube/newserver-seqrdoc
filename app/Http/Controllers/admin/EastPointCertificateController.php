...
use Helper;
use App\Jobs\ValidateExcelEastpointJob;
use App\Jobs\PdfGenerateEastpointJob;
use App\Jobs\PdfGenerateEastpointJob1;


class EastPointCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.eastpoint.index');
    }
...
// dd($rowData2);
                ...
                $response = $this->dispatch(new ValidateExcelEastpointJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==2){
             $pdfData=array('studentDataOrg'=>$rowData1,'subjectDataOrg'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));

          }
   
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateEastpointJob($pdfData));
        } 
          else if($dropdown_template_id=2){
               $link = $this->dispatch(new PdfGenerateEastpointJob1($pdfData));
