...
use Helper;
use App\Jobs\ValidateExcelSuranaJob;
use App\Jobs\PdfGenerateSuranaJob;
class SuranaCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.surana.index');
    }
    ...
    public function pdfGenerate(){
		...
		$excelfile =  'Surana_grade_card_sample_BCom.xlsx';
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSuranaJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
				...
				$PR_Alpha_signed_Grade_key=$last_col+12;
				$PR_Credits_Earned_key=$last_col+13;
				$PR_semester_percentage_Of_Marks_key=$last_col+14;
				$PR_Class_description_key=$last_col+15;
				$Date_key=$last_col+16;
				$Photo_key=$last_col+17;
				$subj_col = 14;
		...
		$pdfData=array('studentDataOrg'=>$rowData1, 'GrandTotal_CT_Max_key'=>$GrandTotal_CT_Max_key, 'GrandTotal_CT_Min_key'=>$GrandTotal_CT_Min_key, 'GrandTotal_CT_Sec_key'=>$GrandTotal_CT_Sec_key, 'GrandTotal_GP_key'=>$GrandTotal_GP_key, 'GrandTotal_CA_key'=>$GrandTotal_CA_key, 'GrandTotal_CP_key'=>$GrandTotal_CP_key, 'SR_SGPA_key'=>$SR_SGPA_key, 'SR_Alpha_signed_Grade_key'=>$SR_Alpha_signed_Grade_key, 'SR_Credits_Earned_key'=>$SR_Credits_Earned_key, 'SR_semester_percentage_Of_Marks_key'=>$SR_semester_percentage_Of_Marks_key, 'SR_Class_description_key'=>$SR_Class_description_key, 'PR_SGPA_key'=>$PR_SGPA_key, 'PR_Alpha_signed_Grade_key'=>$PR_Alpha_signed_Grade_key, 'PR_Credits_Earned_key'=>$PR_Credits_Earned_key, 'PR_semester_percentage_Of_Marks_key'=>$PR_semester_percentage_Of_Marks_key, 'PR_Class_description_key'=>$PR_Class_description_key, 'Date_key'=>$Date_key, 'Photo_key'=>$Photo_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'pdf_flag'=>$pdf_flag,'highestrow'=>($highestRow-1)); //For Custom Loader
		 
		if($dropdown_template_id==1){
            
            $link = $this->dispatch(new PdfGenerateSuranaJob($pdfData));
        }
