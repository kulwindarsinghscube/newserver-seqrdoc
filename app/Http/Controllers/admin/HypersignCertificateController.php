...
use Helper;
use App\Jobs\ValidateExcelHypersignJob;
use App\Jobs\PdfGenerateHypersignJob;

class HypersignCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.hypersign.index');
    }
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id == 1)
                {
                    ...
                    $response = $this->dispatch(new ValidateExcelHypersignJob($excelData));
                }
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        // exit;
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateHypersignJob($pdfData));
        }
