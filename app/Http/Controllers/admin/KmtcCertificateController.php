...
use Helper;
use App\Jobs\ValidateExcelKmtcJob;
//use App\Jobs\PdfGenerateKmtcJob;
//use App\Jobs\PdfGenerateKmtcJob2;
use App\Jobs\PdfGenerateKmtcJob3;
class KmtcCertificateController extends Controller
{
   
    public function index(Request $request)
    {
       return view('admin.kmtc.index');
    }
    ...
    public function uploadpage(){
        ...
        return view('admin.kmtc.index',compact(['ftp_flag','ftpHost']));
    }
     ...
     public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelKmtcJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $sheet3 = $objPHPExcel1->getSheet(2);
                ...
                $sheet4 = $objPHPExcel1->getSheet(3);
                $highestColumn4 = $sheet4->getHighestColumn();
                $highestRow4 = $sheet4->getHighestDataRow();
                $rowData4 = $sheet4->rangeToArray('A1:' . $highestColumn4 . $highestRow4, NULL, TRUE, FALSE);
                
                $sheet5 = $objPHPExcel1->getSheet(4);
                $highestColumn5 = $sheet5->getHighestColumn();
                $highestRow5 = $sheet5->getHighestDataRow();
                $rowData5 = $sheet5->rangeToArray('A1:' . $highestColumn5 . $highestRow5, NULL, TRUE, FALSE);
            }
        ...
        unset($rowData4[0]);
        $rowData4=array_values($rowData4);
        unset($rowData5[0]);
        $rowData5=array_values($rowData5);
        //store ghost image
        //$tmpDir = $this->createTemp(public_path().'\backend\images\ghosttemp\temp');
        $admin_id = \Auth::guard('admin')->user()->toArray();

        
        $pdfData=array('studentDataOrg'=>$rowData1,'yearOne'=>$rowData2,'yearTwo'=>$rowData3,'yearThree'=>$rowData4,'FQE'=>$rowData5,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile);
        
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateKmtcJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateKmtcJob2($pdfData));
        }
        elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateKmtcJob3($pdfData));
        }
