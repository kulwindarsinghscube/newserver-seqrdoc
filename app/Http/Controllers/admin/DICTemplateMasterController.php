...
use App\Jobs\DICPDFGenerateJob;
use App\Jobs\TemplateBackJob;

use App\models\BackgroundTemplateMaster;
use App\Events\BarcodeImageEvent;
use App\Events\TemplateEvent;
use App\Events\TemplateBackEvent;

use App\models\FontMaster;
use App\models\FieldMaster;
use App\models\FieldMasterBackTemplate;
...
use App\Jobs\PreviewBackPDFGenerateJob;
use App\Jobs\PreviewPDFGenerateJob;

use App\Jobs\PDFBothPreviewOnlyJob;
use App\Jobs\PDFBothGenerateJob;
use App\Jobs\PdfGenerateStudentWallet;
...
class DICTemplateMasterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            if($subdomain[0] == 'demo') {
                ...
                if($adminId != 1) {
                    $templateIds = DB::table('user_template_map')
                        ->where('user_id', $adminId)
                        ->where('template_type', 0)
                        ->pluck('template_id');

                    $templateIds = $templateIds->toArray();
                    $templateId = implode(',',$templateIds);

                    if (!empty($templateIds)) {
                        $where_str .= " AND template_master.id IN (" . $templateId . ")";   
                    } else {
                        $where_str .= " AND template_master.id = 0";   
                    }
                }
            }
            ...
            if($subdomain[0]=="demo"){
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'actual_template_name','id','status','bc_contract_address','is_back_template','sample_pdf','sample_pdf_last_updated_at'];
            }else{
        ...
        // $siteData = Site::on('mysql')->select('country')->where('site_id',$auth_site_id)->first();
        $siteData=DB::connection('mysql')->table("seqr_demo.sites")->select('country')->where('site_id',$auth_site_id)->first();
        $is_disable = true;
        if($siteData->country != 'India'){
            $is_disable = false;
        } 
    ...
    //edit back template
    public function backEdit($id,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        // die();
        //get field master data
        $FIELDS = FieldMasterBackTemplate::where('template_id',$id)->orderBy('field_position')->get();
        $field_count = count($FIELDS);
        ...
        //dd($FIELDS[0]["is_encrypted_text"]);
        return view('admin.addtemplates.back_canvas',compact('TEMPLATE','FIELDS','field_count','default_image','font_style','font_size','FONTS','text_opacity','uv_percentage_value','BGTEMPLATE','aws_canvas_upload_path','systemConfig','canvas_upload_path'));
    }


    public function backCanvas(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
       ...
       /* if($subdomain[0]=='cedp'){
        ...
        return view('admin.addtemplates.editBackcanvasmaker',compact('config','config_static','config_default','FONTS','BGTEMPLATE','bg_upload_path','systemConfig'));
    }


    //store template details
    public function backStore(TemplateMasterRequest $request){
        ...
        // $template_data = Event::dispatch(new TemplateBackEvent($get_template_data));
        
        $template_data = $this->dispatch(new TemplateBackJob($get_template_data));
    ...
    public function backSaveforpreview(Request $request){
        ...
        // Background template
        if(isset($template_data['back_bg_template_id']) && $template_data['back_bg_template_id'] != '')
            $FID['back_bg_template_id'] = $template_data['back_bg_template_id'];
    ...
    //previewpdf
    public function backPreviewpdf(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $back_bg_template_img = '';
        
        if(isset($FID['back_bg_template_id']) && $FID['back_bg_template_id'] != '') {
            if($FID['back_bg_template_id'] == 0) {
                $back_bg_template_width = $FID['template_width'];
                $back_bg_template_height = $FID['template_height'];
            } 
            else {
                $get_back_bg_template_data = BackgroundTemplateMaster::select('image_path','width','height')->where('id',$FID['back_bg_template_id'])->first();
                if($get_file_aws_local_flag->file_aws_local == '1'){  
                    $back_bg_template_img = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.bg_images').'/'.$get_back_bg_template_data['image_path'];
                }
                else{
                    $back_bg_template_img = \Config::get('constant.local_base_path').$subdomain[0].'/'.\Config::get('constant.bg_images').'/'.$get_back_bg_template_data['image_path'];
                }
                $back_bg_template_width = $get_back_bg_template_data['width'];
                $back_bg_template_height = $get_back_bg_template_data['height'];
        ...
        {
            $back_bg_template_img = '';
            $back_bg_template_width = 210;
            $back_bg_template_height = 297;
        }
       
        
        $this->dispatch(new PreviewBackPDFGenerateJob($isPreview,2,'',$FID,$Orientation,'','','','','',$back_bg_template_img,$back_bg_template_width,$back_bg_template_height,'','','',$tmpDir,'','','',$style2D,$style1D,$style1Da,$fonts_array,$font_name,$admin_id));
    }
    ...
    public function checktemplatemapped(Request $request){
        ...
        $TEMPLATE = TemplateMaster::where('id',$template_id)->first();
        $status = false;
        ...
        if($subdomain[0] == "demo"){
            ...
            if(isset($TEMPLATE->is_back_template) && $TEMPLATE->is_back_template == 1) {
                $get_back_template_fields = FieldMasterBackTemplate::select('is_mapped')->where('security_type','!=','QR Code')
                ->where('security_type','!=','ID Barcode')->where('template_id',$template_id)->orderBy('is_mapped','DESC')->get()->toArray();
                
                //print_r($get_back_template_fields);
                if ($get_back_template_fields[0]['is_mapped'] == 'excel' || $get_back_template_fields[0]['is_mapped'] == 'database'  ) {
                    // return json_encode($get_template_fields[0]);
                    $status = true;
                } 
                else
                {   
                    $status = false;
                    $get_template_fields = array('type'=> 'error', 'message' => 'Cant generate pdf from unmapped back template!');
                }
        ...
        // $get_back_template_fields = FieldMasterBackTemplate::select('is_mapped')->where('template_id',$template_id)->get()->toArray();
        // if ($get_back_template_fields[0]['is_mapped'] == 'excel' || $get_back_template_fields[0]['is_mapped'] == 'database'  ) {
        //     return json_encode($get_back_template_fields[0]);
        // }
        // else
        // {
        //     $get_back_template_fields = array('type'=> 'error', 'message' => 'Cant generate pdf from unmapped back template!');
        //     return json_encode($get_back_template_fields);
        // }
    ...
    public function excelcheck(Request $request){
        $domain = $request->getHost();
        $subdomain = explode('.', $domain);
        if($request->hasFile('dic_field_file')){
            //check extension
            $file_name = $request['dic_field_file']->getClientOriginalName();
            $ext = pathinfo($file_name, PATHINFO_EXTENSION);
            ...
            if($request['dic_field_file']->move($target_path,$excelfile)){
                ...
                // dd(count($firstRow[0]));
                //functional users template validation start
                //demo=>live 655(ST O&M)=>42 657(TPSSL)=>38
            if ($subdomain[0] == 'tpsdi') {
                ...
                $columnMap = [
                    ...
                    48 => ["Unique no","Watermark student data","Watermark TPSDI","student name","Enrollment NO","Program Title","Date","Student Image","Place","Result","Mobile Number","Email ID","Card Category","Hub Name"],
            ...
            //functional users template validation end
                $dataArray = [];
                ...
                // echo"<pre>";print_r(array_filter($dataArray));die();
                    ...
                    // dd($totalFoundCounts);
                    if(!empty($thirdIndexData) && $totalFoundCounts>0){
                        ...
                        $message = "We found ".$totalFoundCounts." already generated certificates, that will become Inactive.<br> Are you sure to continue?";
                        return response()->json(['success'=>2,'message'=>$message,'msg'=>$msg]);die();
                    }
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $check_option = $request->dic_pdf_local_option;
        ...
        if($subdomain[0]=="demo"){
            $template_data = TemplateMaster::select('id','unique_serial_no','template_name','bg_template_id','width','height','background_template_status','is_block_chain','bc_document_description','bc_document_type','is_back_template','back_bg_template_id')->where('id',$template_id)->first();
        } else {
        ...
        $FID['is_back_template'] = $template_data['is_back_template'];
        //}
        /**End Blockchain**/
        ...
        foreach ($field_data as $field_key => $field_value) {
            ...
            if($subdomain[0]=="demo" || $subdomain[0]=="dc"){
        ...
        if($subdomain[0]=="demo"){
            ...
            if($template_data['is_back_template'] == 1) {
                //get field data
                $field_data_back = FieldMasterBackTemplate::where('template_id',$template_id)->where('security_type','!=','QR Code')
                ->where('security_type','!=','ID Barcode')->orderBy('is_mapped','DESC')->get()->toArray();
                $check_back_mapped = array();

                // echo "<pre>";
                // print_r($field_data_back);
                // echo "</pre>";
                
                foreach ($field_data_back as $field_key => $field_value_back) {
                    // start code for static image hide from map
                    if($field_value_back['security_type'] != 'Copy' && $field_value_back['security_type'] != 'ID Barcode' && $field_value_back['security_type'] != 'Static Image' && $field_value_back['security_type'] != 'Static Text' && $field_value_back['security_type'] != 'Anti-Copy' && $field_value_back['security_type'] != '1D Barcode' && $field_value_back['security_type'] != 'Static Microtext Border' && $field_value_back['security_type'] != 'QR Code' ) {
                        if($field_value_back['mapped_name'] == '') {
                            $message = 'Can\'t generate pdf from unmapped back template!';
                            return response()->json(['success'=>false,'message'=>$message]);
                        }
                        if ($field_value_back['security_type'] == 'Static Image' || $field_value_back['security_type'] == 'Static Text' || $field_value_back['security_type'] == 'Anti-Copy' || $field_value_back['security_type'] == '1D Barcode' || $field_value_back['security_type'] == 'Static Microtext Border') {
                        }
                        else
                        {
                            $check_back_mapped[] = $field_value_back['mapped_name'];
                        }
                    }
                
                    
                    $FID_BACK['id'][] = $field_value_back['id'];
                    $FID_BACK['mapped_name'][] = $field_value_back['mapped_name'];
                    $FID_BACK['mapped_excel_col'][] = '';
                    $FID_BACK['data'][] = '';
                    $FID_BACK['security_type'][] = $field_value_back['security_type'];
                    $FID_BACK['field_position'][] = $field_value_back['field_position'];
                    $FID_BACK['text_justification'][] = $field_value_back['text_justification'];
                    $FID_BACK['x_pos'][] = $field_value_back['x_pos'];
                    $FID_BACK['y_pos'][] = $field_value_back['y_pos'];
                    $FID_BACK['width'][] = $field_value_back['width'];
                    $FID_BACK['height'][] = $field_value_back['height'];
                    $FID_BACK['font_style'][] = $field_value_back['font_style'];
                    $FID_BACK['font_id'][] = $field_value_back['font_id'];
                    $FID_BACK['font_size'][] = $field_value_back['font_size'];
                    $FID_BACK['font_color'][] = $field_value_back['font_color'];
                    $FID_BACK['font_color_extra'][] = $field_value_back['font_color_extra'];
                    $FID_BACK['text_opacity'][] = $field_value_back['text_opicity'];
                    $FID_BACK['visible'][] = $field_value_back['visible'];
                    $FID_BACK['visible_varification'][] = $field_value_back['visible_varification'];
                    
                    // start get data from db and store in array 
                    $FID_BACK['sample_image'][] = $field_value_back['sample_image'];
                    $FID_BACK['angle'][] = $field_value_back['angle'];
                    $FID_BACK['sample_text'][] = $field_value_back['sample_text'];
                    $FID_BACK['line_gap'][] = $field_value_back['line_gap'];
                    $FID_BACK['length'][] = $field_value_back['length'];
                    $FID_BACK['uv_percentage'][] = $field_value_back['uv_percentage'];
                    $FID_BACK['print_type'] = $request->print_type;
                    $FID_BACK['is_repeat'][] = $field_value_back['is_repeat'];
                    // end get data from db and store in array
                    $FID_BACK['is_mapped'][] = $field_value_back['is_mapped'];
                    $FID_BACK['infinite_height'][] = $field_value_back['infinite_height'];
                    $FID_BACK['include_image'][] = $field_value_back['include_image'];
                    $FID_BACK['grey_scale'][] = $field_value_back['grey_scale'];
                    $FID_BACK['is_uv_image'][] = $field_value_back['is_uv_image'];
                    $FID_BACK['is_transparent_image'][] = $field_value_back['is_transparent_image'];
                    $FID_BACK['is_font_case'][] = $field_value_back['is_font_case'];
                    $FID_BACK['combo_qr_text'][] = $field_value_back['combo_qr_text'];
                    
                    if($subdomain[0]=="demo"){
                        $FID_BACK['is_encrypted_qr'][] = $field_value_back['is_encrypted_qr'];
                        $FID_BACK['encrypted_qr_text'][] = $field_value_back['encrypted_qr_text'];
                    }
                
                    $FID_BACK['name'][] = $field_value_back['name'];
                
                    /**Blockchain**/
                    $FID_BACK['is_meta_data'][] = $field_value_back['is_meta_data'];
                    $FID_BACK['meta_data_label'][] = $field_value_back['meta_data_label'];
                    $FID_BACK['meta_data_value'][] = $field_value_back['meta_data_value'];
                    /**End Blockchain**/
                }
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($get_file_aws_local_flag->file_aws_local == '1'){
                    ...
                    //     $filename1 = \Storage::disk('s3')->url($excelfile);
                    // }
                    CoreHelper::sandboxingFileAWS($systemConfig,$fullpath,$template_data['id'],$excelfile);
                }
                else{
                    ...
                    // }
                    CoreHelper::sandboxingFile($systemConfig,$fullpath,$template_data['id'],$excelfile);
                }
                $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                ...
                $highestRow = $sheet->getHighestDataRow();
                // //wallet code start
                // if($subdomain[0]=="demo" && $isPreview==false && $template_data['id']==688){
                // $highestColumnIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($highestColumn);
                // $filterdata = [];
                ...
                //         }
                //         for ($col = 1; $col <= $highestColumnIndex; ++$col) {
                //             $actualData[] = $sheet->getCellByColumnAndRow($col, $row)->getValue();
                //         }
                //         $filterdata[] = array_filter($actualData);
                //     }
                // // dd(array_filter($filterdata));
                //     $this->dispatch(new PdfGenerateStudentWallet(array_filter($filterdata)));
                ...
                foreach ($rowData[0] as $key => $f) {
                    if($f != '') {
                        ...
                        if($subdomain[0]=="demo"){
                            //check mapped name with excel is in db or not
                            if($FID['is_back_template'] == 1 ) {
                                foreach ($FID_BACK['mapped_name'] as $i => $value) {
                                    if($value == $f) {
                                        $FID_BACK['mapped_excel_col'][$i] = $key;
                                    }
                                }
                            }
                        }
                ...
                if($subdomain[0]=="demo"){
                    if($FID['is_back_template'] == 1 ) {
                        $diff1 = array_diff($check_back_mapped, $fields);
                        ...
                        if(count($diff1) > 0)
                        {
                            ...
                            $diff_name1 = implode(", ", $diff1);
                            $message = Array('type' => 'fieldNotMatch', 'message' => '<table border="1"><tr><td style="padding:10px;">Following mapping back columns are missing : <br><b>'.$diff_name1.'</b></td></tr></table>');
                            return json_encode($message);
                        }
                    }
                }
                ...
                if($FID['back_bg_template_id'] == 0) {
                    ...
                    if($FID['printing_type'] == 'pdfGenerate'){
                        ...
                        else
                        {
                            ...
                            // echo \Config::get('constant.local_base_path').$subdomain[0].'/'.\Config::get('constant.bg_images').'/'.$get_back_bg_template_data['image_path'];
                            // echo "<br>";
                            $back_bg_template_width = $get_back_bg_template_data['width'];
                            $back_bg_template_height = $get_back_bg_template_data['height'];
                        }
        ...
        if($subdomain[0] =='demo') {
            $back_bg_template_img_generate = '';

            if(isset($FID['back_bg_template_id']) && $FID['back_bg_template_id'] != '') {
                if($FID['back_bg_template_id'] == 0) {
                    $back_bg_template_img_generate = '';
                    $back_bg_template_width_generate = $FID['template_width'];
                    $back_bg_template_height_generate = $FID['template_height'];
                } else {
                    $get_back_bg_template_data =  BackgroundTemplateMaster::select('image_path','width','height')->where('id',$FID['back_bg_template_id'])->first();
                    if($get_file_aws_local_flag->file_aws_local == '1'){ 
                        $back_bg_template_img_generate = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.bg_images').'/'. $get_back_bg_template_data['image_path'];
                    }
                    else{
                        $back_bg_template_img_generate = \Config::get('constant.local_base_path').$subdomain[0].'/'.\Config::get('constant.bg_images').'/'. $get_back_bg_template_data['image_path'];
                    }
                    $back_bg_template_width_generate = $get_back_bg_template_data['width'];
                    $back_bg_template_height_generate = $get_back_bg_template_data['height'];
                }
            } else {
                $back_bg_template_img_generate = '';
                $back_bg_template_width_generate = 210;
                $back_bg_template_height_generate = 297;
            }
        ...
        // echo $back_bg_template_img;
        if($subdomain[0] =='demo') {
            if($FID['is_back_template'] == 1) {
                $pdf_data = ['isPreview'=>$isPreview,'highestRow'=>$highestRow,'highestColumn'=>$highestColumn,'FID'=>$FID,'FID_BACK'=>$FID_BACK,'Orientation'=>$Orientation,'template_Width'=>$template_Width,'template_height'=>$template_height,'bg_template_img_generate'=>$bg_template_img_generate,'bg_template_width_generate'=>$bg_template_width_generate,'bg_template_height_generate'=>$bg_template_height_generate,'bg_template_img'=>$bg_template_img,'bg_template_width'=>$bg_template_width,'bg_template_height'=>$bg_template_height,'back_bg_template_img_generate'=>$back_bg_template_img_generate,'back_bg_template_width_generate'=>$back_bg_template_width_generate,'back_bg_template_height_generate'=>$back_bg_template_height_generate,'back_bg_template_img'=>$back_bg_template_img,'back_bg_template_width'=>$back_bg_template_width,'back_bg_template_height'=>$back_bg_template_height,'mapped_excel_col_unique_serial_no'=>$mapped_excel_col_unique_serial_no,'ext'=>$ext,'fullpath'=>$fullpath,'tmpDir'=>$tmpDir,'excelfile'=>$excelfile,'timezone'=>$timezone,'printer_name'=>$printer_name,'style2D'=>$style2D,'style1D'=>$style1D,'style1Da'=>$style1Da,'fonts_array'=>$fonts_array,'font_name'=>$font_name,'admin_id'=>$admin_id,'excel_row_num'=>$excel_row_num,'pdf_flag'=>$pdf_flag];
            } else {
    ...
    //         if($subdomain[0] =='demo') {
    //             if($FID['is_back_template'] == 1) {
    //                 $response = $this->dispatch(new PDFBothPreviewOnlyJob($pdf_data));
    //             } else {
    ...
    //         if($subdomain[0] =='demo') {
    //             if($FID['is_back_template'] == 1) {
    //                 $response = $this->dispatch(new PDFBothGenerateJob($pdf_data));
    //             } else {
    ...
    // }
      ...
      if ($check_option == 1) {
            // PREVIEW MODE
            if ($subdomain[0] == 'demo' && ($template_id == 742 || $template_id == 744)) {
                // If subdomain is 'demo', run Python script for preview
                $directoryUrlForward = "C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/excel2pdf/";
                $directoryUrlBackward = "C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\excel2pdf\\";
                $pyscript = $directoryUrlBackward . "Python_files\\batch_script.py";
        ...
        if($subdomain[0]=='demo'){
            ...
            $dbName = 'seqr_d_'.$subdomain[0];
                ...
                $python = 'C:\\inetpub\\vhosts\\seqrdoc.com\\httpdocs\\Python\\Python3.8.10Embeded\\python.exe';
                                    $pyscript = "C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\excel2pdf\\Python_files\\batch_script.py";

              $cmd = escapeshellarg($pyscript) . " " .
                escapeshellarg($template_id) . " " .
                escapeshellarg($fullpath) . " " .
                escapeshellarg($directoryUrlForward) . " " .
                escapeshellarg($serverName) . " " .
                escapeshellarg($dbUserName) . " " .    // ? Fixed
                escapeshellarg($dbPassword) . " " .    // ? Fixed
                escapeshellarg($dbName) . " " .
                escapeshellarg($subdomain[0]) . " " .
                escapeshellarg("False") . " 2>&1";
            ...
            //    die();
                ...
                $response = [
                    ...
                    'excel_row' => null,
                    'highestRow' => null,
                    'msg' => 'Something went wrong in script execution.'
                ];

                // âœ… Look for JSON response in Python output
                foreach ($output as $line) {
                    $trimmed = trim($line);

                    // Skip mupdf ICC profile warnings
                    if (strpos($trimmed, 'mupdf: ICC profile') !== false) {
                        continue;
                    }
                    ...
                    if (strpos($trimmed, '{') === 0 && strrpos($trimmed, '}') === strlen($trimmed) - 1) {
                        ...
                        if (json_last_error() === JSON_ERROR_NONE && isset($parsed['status'])) {
                            $response['success'] = ($parsed['status'] == 200);
                            $response['is_progress'] = $parsed['is_progress'] ?? null;
                            $response['excel_row'] = $parsed['excel_row'] ?? null;
                            $response['highestRow'] = $parsed['highestRow'] ?? null;

                            $filePath = $parsed['Preview_Big'] ?? $parsed['Printable'] ?? null;
// dd($filePath);

                            if ($filePath) {
                                $relativePath = str_replace("C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/", "", str_replace("\\", "/", $filePath));
                                $publicUrl = url($relativePath);

                                // dd($relativePath, $publicUrl);


                                $htmlMsg = "<b>Click <a href='{$publicUrl}' class='downloadpdf' download target='_blank'>Here</a> to download file<b>";
                                $response['msg'] = $htmlMsg;
                            } else {
                                $response['msg'] = $parsed['Message'] ?? 'PDF generated but no path returned.';
                            }
                        }
                ...
                // If subdomain is not demo, use Laravel Job for Preview
                $response = $this->dispatch(new PDFPreviewOnlyJob($pdf_data));
                return response()->json(['success'=>true,'is_progress'=>$response['is_progress'],'excel_row'=>$response['excel_row'],'highestRow'=>$response['highestRow'],'msg'=>$response['msg']]);
            }
        }
        else
        {
            // GENERATE MODE
            if ($subdomain[0] == 'demo'&& ($template_id == 742 || $template_id == 744)) {
                // If subdomain is 'demo', run Python script for generation
                    $directoryUrlForward = "C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/excel2pdf/";
                $directoryUrlBackward = "C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\excel2pdf\\";
                $pyscript = $directoryUrlBackward . "Python_files\\batch_script.py";
    ...
    escapeshellarg("True") . " 2>&1";
                ...
                // If subdomain is not demo, use Laravel Job for Generate
                // dd($pdf_data['fullpath']);
                $response = $this->dispatch(new DICPDFGenerateJob($pdf_data));
                // if($response['is_progress']=='no'){
                // $file_name = $request['field_file']->getClientOriginalName();
                // $excelfile =  date("YmdHis") . "_" . $file_name;
                // $target_path = public_path().'/backend/canvas/dummy_images/'.$template_data['template_name'];
                // $fullpath = $target_path.'/'.$excelfile;
   ...
   public function copyTemplate(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    ...
    //map template
    public function templateBackMap($id){
        $template_data = TemplateMaster::find($id);
        
        $fields = FieldMasterBackTemplate::where('template_id',$id)->where('security_type','!=','QR Code')
        ->where('security_type','!=','ID Barcode')->get()->toArray();

        
        
        return view('admin.templateMaster.back_template_map',compact('template_data','fields'));
    }
    // get Excel columns
    public function uploadMapFileBack(Request $request){
    ...
    // this function uploadMapColumns 
    public function uploadMapColumnsBack(Request $request)
    {     
        $databse_detail=$request->session()->get('databse_detail');
        $is_mapped=$request['is_mapped'];
        $field_value=$request['f_value'];
        ...
        // update Unique_serial_no 
        if($excel_serial_no) {
            $TemplateMaster=FieldMasterBackTemplate::where('id',$id)
            ->update(['Unique_serial_no'=>$excel_serial_no]);
        }
        ...
        foreach ($field_value as $key => $value) {
                ...
                $new_data=FieldMasterBackTemplate::where('id',$key)     
                            ->update(['mapped_name'=>$value,'is_mapped'=>$is_mapped]);        
            }
        }
        if($databse_detail[0]['is_mapped']==$is_mapped)
     ...
     // map columns from databse
    public function mapFromDatabaseBack(MappingDatabaseRequest $request)
    {
    ...
    /**
     * uploadSamplePdf Handle the upload of a sample PDF file.
     *
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function uploadSamplePdf(Request $request) {
        // Retrieve the uploaded file and template ID from the request
        $file = $request->file('sample_pdf');
        $templateId = $request->input('template_id');

        // Extract domain and subdomain from the request
        $domain = $request->getHost();
        $subdomain = explode('.', $domain);

        // Generate a unique file name using the current timestamp, template ID, and original file name
        $fileName = time() . '-' . $templateId . '-' . $file->getClientOriginalName();
        
        // Define the path to save the file locally
        $directoryPath = public_path($subdomain[0] . '/' . config('constant.template') . '/' . $templateId);
 
        // Move the uploaded file to the specified directory
        $file->move($directoryPath, $fileName);

        // Retrieve the template record from the database
        $template = TemplateMaster::find($templateId);

        if ($template) {
            // Remove the old PDF file if it exists
            if (!empty($template->sample_pdf)) {
                $oldFilePath = public_path($subdomain[0] . '/' . config('constant.template') . '/' . $templateId . '/' . $template->sample_pdf);
                if (file_exists($oldFilePath)) {
                    unlink($oldFilePath);
                }
            }

            // Update the template record with the new PDF file information
            $template->sample_pdf = $fileName;
            $file_uploaded_date = now()->format('Y-m-d H:i:s');
            $template->sample_pdf_last_updated_at = $file_uploaded_date;
            $template->save();

            // Construct the public URL for the newly uploaded PDF file
            $scheme = $request->secure() ? 'https' : 'http';
            $publicPath = "$scheme://$domain/{$subdomain[0]}/" . config('constant.template') . "/$templateId/$fileName";
