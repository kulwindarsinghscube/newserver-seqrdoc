...
use App\Jobs\PdfGenerateSdmJob;
use App\Jobs\PdfGenerateSdmPdcJob;
use App\Jobs\PdfGenerateSdmNursingPdcJob;
use App\Jobs\PdfGenerateSdmDentalPdcJob;
use App\Jobs\PdfGenerateSdmBdsJob;
use App\Jobs\PdfGenerateSdmBptJob;
use App\Jobs\PdfGenerateSdmMptIJob;
use App\Jobs\PdfGenerateSdmNursingJob;
use App\Jobs\PdfGenerateSdmMbbsJob;
use App\Jobs\PdfGenerateSdmMbbsIIIJob;
use App\Jobs\PdfGenerateSdmMbbsIIIMainJob;
use App\Jobs\PdfGenerateSdmMbbsIIIPartIIMainJob;
use App\Jobs\PdfGenerateSdmMbbsIIIPartIIJob;
use App\Jobs\PdfGenerateSdmMscBioMedicalJob;
use App\Jobs\PdfGenerateSdmBscMedicalLabTechnologyJob;
use App\Jobs\PdfGenerateSdmMscIIIBioMedicalJob;
use App\Jobs\PdfGenerateSdmMdsIIJob;
use App\Jobs\PdfGenerateSdmMdsIJob;
use App\Jobs\pdfGenerateSdmBscNursingJob;
use App\Jobs\pdfGenerateSdmBscNursingUGJob;
...
use App\Jobs\PdfGenerateSdmMdsIISubjectSixJob;
use App\Jobs\PdfGenerateSdmMbbs4060Job;
use App\Jobs\PdfGenerateSdmBscAlliedJob;
use App\Jobs\PdfGenerateSdmBscAlliedJob1;
use App\Jobs\PdfGenerateSdmMbbsIIIJob_3subjects;

class SdmCertificateController extends Controller
{
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                /**  Load $inputFileName to a Spreadsheet Object  **/
                if($dropdown_template_id != "3" && $dropdown_template_id != "5" && $dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "12" && $dropdown_template_id != "13" && $dropdown_template_id != "16" && $dropdown_template_id != "28" && $dropdown_template_id != "28" && $dropdown_template_id != "18" && $dropdown_template_id != "26"){
					$objPHPExcel1 = $objReader->load($fullpath);
					$sheet1 = $objPHPExcel1->getSheet(0);
					$highestColumn1 = $sheet1->getHighestColumn();
					$highestRow1 = $sheet1->getHighestDataRow();
                    if($dropdown_template_id == "27" || $dropdown_template_id == "22"){
					...
					if($dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "13" && $dropdown_template_id != "12" && $dropdown_template_id != "26"){
						//3,5,16 template
						if($dropdown_template_id==16|| $dropdown_template_id==18){
                            ...
                            $subjStart=6; 
                        }else if($dropdown_template_id==28){
                            // $subjCols=19;  
                            $highestColumnSet=($course_counts+1)*23;
                            $last_col=6+($course_counts*24);                        
                            $check_last_col=6+($course_counts-1)*23; 
                            $subjStart=6; 
                        }else{
                            $highestColumnSet=($course_counts+1)*22;
                            $last_col=5+($course_counts*23);						
                        ...
                        if($dropdown_template_id==5 ) {
                            //extract cell values from row 2
                            // for ($row = 2; $startRow <= $highestRow; $row++) {
                                ...
                                for ($row =  $startRow; $row <= $endRow; $row++) {
                                    ...
                                    for ($col = 1; $col <= $highestColumnIndex; ++$col) {
                                        if($c>=$vc && $vc != $vcc){ 	
                                            ...
                                            if($dropdown_template_id==16 ||$dropdown_template_id==28 || $dropdown_template_id==18){
                                                $vc+=$vcc;
                                            }else{
                                                $vc+=$vcc;
                                            }
                            ...
                            for ($row = 2; $row <= $highestRow; $row++) {
                                ...
                                for ($col = 1; $col <= $highestColumnIndex; ++$col) {
                                    if($c>=$vc && $vc != $vcc){ 	
                                        ...
                                        if($dropdown_template_id==28){
                                          $th=$worksheet->getCellByColumnAndRow($vc+2, $row)->getValue();
                                        $pr=$worksheet->getCellByColumnAndRow($vc+14, $row)->getValue();  
                                    }else{
                                        $th=$worksheet->getCellByColumnAndRow($vc+2, $row)->getValue();
                                        $pr=$worksheet->getCellByColumnAndRow($vc+12, $row)->getValue();
                                    }
                        ...
                        if($dropdown_template_id==16 || $dropdown_template_id==18){
                            ...
                            $cgpa=$last_col+1;
                            $class_grade=$last_col+2;
                            $total_credits=$last_col+9;
                            $total_credit_point=$last_col+10;
                            $credit_ex_subj=$last_col+11;
                            $total_ex_credit=$last_col+12;
                            $total_cp_earned=$last_col+13;
                            $exam_result=$last_col+14;
                            // $Grand_Total_In_words_key=$last_col+3;
                            // $Percentage_key=$last_col+4;
                        ...
                        if($dropdown_template_id==26){
                            $subjCols=14;    
                        }
                        if($dropdown_template_id==29){
                            $subjCols=12;    
                        }
						...
						if($dropdown_template_id==11  || $dropdown_template_id==13){
                        ...
                        }elseif($dropdown_template_id==26 || $dropdown_template_id==29){
                            $GT_Max_key=$last_col;
                            $GT_Min_key=$last_col+1;
                            $GT_Sec_key=$last_col+2;
                            $Grand_Total_In_words_key=$last_col+3;
                            ...
                            $DOB_key=$last_col+10;
                            $course_key=$last_col+11;    
                            $LT65_key=$last_col+12;    
                            $remark_LT65_key=$last_col+13;
                        }else{
                            $GT_Max_key=$last_col;
                            $GT_Min_key=$last_col+1;
                            $GT_Sec_key=$last_col+2;
		...
		if($dropdown_template_id != "3" && $dropdown_template_id != "5" && $dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "12" && $dropdown_template_id != "13" && $dropdown_template_id != "16" && $dropdown_template_id != "28" && $dropdown_template_id != "18" && $dropdown_template_id != "26"){
			...
			if($dropdown_template_id==11 || $dropdown_template_id==13){
                ...
                $pdfData=array(
                    ...
                    'Credit_Ex_Subj' =>$credit_ex_subj, 
                    'Total_Ex_Credit' =>$total_ex_credit, 
                ...
                if($dropdown_template_id==26 || $dropdown_template_id==29){
                    $pdfData=array('studentDataOrg'=>$rowData1, 'GT_Max_key'=>$GT_Max_key, 'GT_Min_key'=>$GT_Min_key, 'GT_Sec_key'=>$GT_Sec_key, 'Grand_Total_In_words_key'=>$Grand_Total_In_words_key, 'Percentage_key'=>$Percentage_key, 'Result_key'=>$Result_key, 'Date_key'=>$Date_key, 'Student_image_key'=>$Student_image_key, 'Batch_of_key'=>$Batch_of_key, 'Aadhar_No_key'=>$Aadhar_No_key, 'DOB_key'=>$DOB_key, 'course_key'=>$course_key, 'LT65_key'=>$LT65_key, 'remark_LT65_key'=>$remark_LT65_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token); 
                }else{
                    $pdfData=array('studentDataOrg'=>$rowData1, 'GT_Max_key'=>$GT_Max_key, 'GT_Min_key'=>$GT_Min_key, 'GT_Sec_key'=>$GT_Sec_key, 'Grand_Total_In_words_key'=>$Grand_Total_In_words_key, 'Percentage_key'=>$Percentage_key, 'Result_key'=>$Result_key, 'Date_key'=>$Date_key, 'Student_image_key'=>$Student_image_key, 'Batch_of_key'=>$Batch_of_key, 'Aadhar_No_key'=>$Aadhar_No_key, 'DOB_key'=>$DOB_key, 'course_key'=>$course_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
                }
            }
        }
        // echo "<prE>";
        ...
        elseif($dropdown_template_id==11){ 
            // print_r($pdfData);
            $link = $this->dispatch(new PdfGenerateSdmMscBioMedicalJob($pdfData));
        } 
		elseif($dropdown_template_id==12){
            $link = $this->dispatch(new PdfGenerateSdmBscMedicalLabTechnologyJob($pdfData));
        }
        elseif($dropdown_template_id==13){
            $link = $this->dispatch(new PdfGenerateSdmMscIIIBioMedicalJob($pdfData));
        }
		elseif($dropdown_template_id==14){
            $link = $this->dispatch(new PdfGenerateSdmMdsIIJob($pdfData));
        }
		elseif($dropdown_template_id==15){
            $link = $this->dispatch(new PdfGenerateSdmMdsIJob($pdfData));
        }
		elseif($dropdown_template_id==16){
            $link = $this->dispatch(new pdfGenerateSdmBscNursingJob($pdfData));
        }
        elseif($dropdown_template_id==28){
            $link = $this->dispatch(new pdfGenerateSdmBscNursingUGJob($pdfData));
        }
        ...
        elseif($dropdown_template_id==22){
            $link = $this->dispatch(new PdfGenerateSdmMdsIISubjectSixJob($pdfData));
        }elseif($dropdown_template_id==23){
            $link = $this->dispatch(new PdfGenerateSdmMbbs4060Job($pdfData));
        }elseif($dropdown_template_id==24){
            $link = $this->dispatch(new PdfGenerateSdmMbbsIIIMainJob($pdfData));
        }elseif($dropdown_template_id==25){
            $link = $this->dispatch(new PdfGenerateSdmMbbsIIIPartIIMainJob($pdfData));
        }
        elseif($dropdown_template_id==26){
            $link = $this->dispatch(new PdfGenerateSdmBscAlliedJob($pdfData));
        }
        elseif($dropdown_template_id==29){
            $link = $this->dispatch(new PdfGenerateSdmBscAlliedJob1($pdfData));
        }
        elseif($dropdown_template_id==27){
            $link = $this->dispatch(new PdfGenerateSdmMbbsIIIJob_3subjects($pdfData));
        } 
