...
use Helper;
use App\Jobs\ValidateExcelSaiuJob;
use App\Jobs\PdfGenerateSaiuJob;
use App\Jobs\PdfGenerateSaiuGradeJob;
use App\Jobs\PdfGenerateSaiuDegreeJob;
use App\Jobs\PdfGenerateSaiuGrade3Job;
use App\Jobs\PdfGenerateSaiuGrade4Job;
use App\Jobs\PdfGenerateSaiuGrade5Job;
use App\Jobs\PdfGenerateSaiuGrade6Job;
use App\Jobs\PdfGenerateSaiuGrade7Job;
use App\Jobs\PdfGenerateSaiuGrade8Job;
use App\Jobs\PdfGenerateSaiuGrade9Job;




class SaiuCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.saiu.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSaiuJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $template_id = 100;
        $dropdown_template_id = $request['template_id'];
        $dropdown_merging_type = $request['merging_type'];
        $dropdown_verification_bg = $request['verification_bg'];
        /* 1=Basic */
		...
		$pdfData=array('studentDataOrg'=>$rowData1, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'dropdown_merging_type'=>$dropdown_merging_type,'dropdown_verification_bg'=>$dropdown_verification_bg,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token); //For Custom Loader
         
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateSaiuGradeJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateSaiuDegreeJob($pdfData));
        }elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateSaiuGrade3Job($pdfData));
        }elseif($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateSaiuGrade4Job($pdfData));
        }elseif($dropdown_template_id==5){
            $link = $this->dispatch(new PdfGenerateSaiuGrade5Job($pdfData));
        }elseif($dropdown_template_id==6){
            $link = $this->dispatch(new PdfGenerateSaiuGrade6Job($pdfData));
        }elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateSaiuGrade7Job($pdfData));
        }elseif($dropdown_template_id==8){
            $link = $this->dispatch(new PdfGenerateSaiuGrade8Job($pdfData));
        }
        elseif($dropdown_template_id==9){
            $link = $this->dispatch(new PdfGenerateSaiuGrade9Job($pdfData));
        }
