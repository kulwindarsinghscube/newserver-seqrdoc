...
use Helper;
use App\Jobs\ValidateExcelSrwcnagpurJob;
use App\Jobs\PdfGenerateSrwcnagpurJob;

class SrwcnagpurCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.srwcnagpur.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSrwcnagpurJob($excelData));
                // print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $highestColumn1 = $sheet1->getHighestColumn();
                $highestRow1 = $sheet1->getHighestDataRow();
                $highestRow = $sheet1->getHighestDataRow();
        ...
        $pdfData=array('studentDataOrg'=>$rowData1,'subjectsMark'=>$rowData2, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>($highestRow-1)); //For Custom Loader
        /* 1=Memorandum, 2=Consolidated, 3=UG-CGM, 4=UG Memorandum, 5=Provisional Certificate */
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateSrwcnagpurJob($pdfData));
        }
