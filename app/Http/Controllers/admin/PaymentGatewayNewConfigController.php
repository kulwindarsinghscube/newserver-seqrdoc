 ...
 *  Author : Ketan valand 
 *   Date  : 23/11/2019
 *   Use   : show and update and get information of PaymentGatewayNewConfig    
 *
**/
...
class PaymentGatewayNewConfigController extends Controller
{
     /**
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
           ...
           $pg_data= PaymentGatewayNew::select('pg_name','id')->where('site_id',$site_id)->where('status',1)->get()->toArray();
           return $pg_data;
        }
    	return view('admin.paymentGatewayNewConfig.index');
    }

     /**
     * Display the specified PaymentGatewayNewConfig Detail.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function show(Request $request)
    {
        ...
        {
        	$pg_config_data=PaymentGatewayNewConfig::select('pg_status','amount','crendential')
    ...
    /**
     * Update the specified PaymentGatewayNewConfig in storage.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    ...
    public function update(PaymentGatewayNewConfigRequest $request)
    {
        $pg_status=$request['opt_pg'];
        $amount=$request['amt_charge'];
        $crendential=$request['opt_crenden'];
        
        $pg_update=PaymentGatewayNewConfig::where('pg_id',$request['opt_wl'])
                  ->update(['pg_status'=>$pg_status,'amount'=>$amount,
                  	'crendential'=>$crendential]);

        $pg_data=PaymentGatewayNew::where('id',$request['opt_wl'])
                        ->update(['status'=>$pg_status]);

        //check is paytem request  
        $nameExit=PaymentGatewayNew::select('pg_name')->where('id',$request['opt_wl'])->first();
       
        // PaymentGatewayNew name available or not
        ...
        { 
            ...
            if($nameExit==="paytm")
            {
                ...
                $data=PaymentGatewayNew::select('payment_gateway.pg_name','payment_gateway_config.crendential')
               ...
               // PaymentGatewayNew production crendential
               ...
               // PaymentGatewayNew local crendential 
