...
use Helper;
use App\Jobs\ValidateExcelGhrusaikhedaJob;
use App\Jobs\pdfGenerateGhrusaikhedaJob;
// use App\Jobs\pdfGenerateJntuJob1;


class GhrusaikhedaCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.ghrusaikheda.index');
        //newrrmu
    }
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                // if ($dropdown_template_id == 1) {
                    ...
                    // dd($excelData);
                    $response = $this->dispatch(new ValidateExcelGhrusaikhedaJob($excelData));
                    // print_r($response);
                    // die();
...
// dd($request->all());
          ...
          if($dropdown_template_id==1){
            ...
            $link = $this->dispatch(new pdfGenerateGhrusaikhedaJob($pdfData));
        }
