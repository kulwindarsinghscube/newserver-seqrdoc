...
use Helper;
use App\Jobs\ValidateExcelAtriauJob;
use App\Jobs\PdfGenerateAtriauJob1;
use App\Jobs\PdfGenerateAtriauJob3;
use App\Jobs\pdfGenerateAtriaugrade2Job;




class AtriauCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.atriau.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelAtriauJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $admin_id = \Auth::guard('admin')->user()->toArray();
		if($dropdown_template_id==1 || $dropdown_template_id==7||$dropdown_template_id==8||$dropdown_template_id==9 ){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateAtriauJob1($pdfData));
        }

         elseif($dropdown_template_id==2){
            $link = $this->dispatch(new pdfGenerateAtriaugrade2Job($pdfData));
            // dd($pdfData);
        }
        elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateAtriauJob3($pdfData));
        }
