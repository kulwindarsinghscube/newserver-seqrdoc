...
namespace App\Http\Controllers\admin\excel2pdf;
...
{   
       ...
       // TemplateMaster::where('create_refresh',1)->update(['create_refresh'=>0]);
        ...
        if($request->ajax()){
            ...
            if($status==1)
            {
                ...
                $where_str.= " and (uploaded_pdfs.map_type =1)";
            }
            else if($status==0)
            {
                $status=0;
            ...
            if($subdomain[0] == 'demo') {
                ...
                if($adminId != 1) {
                    ...
                    if (!empty($templateIds)) {
                        $where_str .= " AND uploaded_pdfs.id IN (" . $templateId . ")";   
                    } else {
                        $where_str .= " AND uploaded_pdfs.id = 0";   
                    }
                }
            }
            ...
            }elseif($subdomain[0]=="demo"||$subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi"){
        ...
        return view('admin.excel2pdf.templateMaster.index');
    }

    public function templateMaker(Request $request)
    {   
        $FontMasterList = FontMaster::get();


        $template_data=array("action"=>"add");
        

        return view('admin.excel2pdf.addtemplate.index',compact('template_data','FontMasterList'));
    }
        ...
        if($request->hasFile('pdf_upload')){
            ...
            if($request['pdf_upload']->move($target_path,$pdfFile)){
                ...
                if(!empty($template_id)){
                    ...
                    $templateDataCount = TemplateMaster::select('id')->where('template_name',$template_name)->count();
                    if($templateDataCount==0){
            ...
            if($templateDataCount==0){
                ...
                if($subdomain[0]=="icat"){
                    ...
                    DB::table('excel2pdf_log')->insertGetId($dataLogInsert);                        
                }
                return response()->json(['success'=>"success",'message'=>'Template updated successfully.','type'=>'toaster','rstatus' => 'edit']);
            }else{
                return response()->json(['success'=>false,'message'=>'Template already exist with same name!','type'=>'toaster','rstatus' => 'exist']);
    ...
    public function processPdf(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        // $directoryUrlForward="C:/wamp64/www/uneb/public/excel2pdf/";
        // $directoryUrlBackward="C:\\wamp64\\www\\uneb\\public\\excel2pdf\\";

        $directoryUrlForward=\Config::get('constant.EXCEL2PDF_DirUrlForward');
        $directoryUrlBackward=\Config::get('constant.EXCEL2PDF_DirUrlBack');
        ...
        $blockchainPdf=(isset($_POST['blockchainPdf'])&&$_POST['blockchainPdf']!="undefined")?$_POST['blockchainPdf']:0;
            ...
            if(in_array($file_extension,$image_ext)){
              ...
              if(move_uploaded_file($_FILES['file']['tmp_name'],$location)){
                ...
                if($pdf_flag==''){
                    ...
                    if(($subdomain[0]=="demo") && $previewPdf == 1) {
                    // if($subdomain[0]=="demo") {
                        $pyscript = $directoryUrlBackward."Python_files\\excel_place_single_preview.py";
                    }
                    else{
                        $pyscript = $directoryUrlBackward."Python_files\\excel_place_single.py";
                    }
                }else{
                    $pyscript = $directoryUrlBackward."Python_files\\excel_place_single.py";
                }
                ...
                // if($subdomain[0]=="demo" || $admin_id = 39){
                    // echo $cmd;
                    // echo "<br>";
                    // print_r($output);
    ...
    public function processPdfAgain(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($pdf_page=="Single"){
            ...
            // }else if($subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi"){
    ...
    public function test(Request $request){
         ...
         $directoryUrlForward="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/excel2pdf/";
        $directoryUrlBackward="C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\excel2pdf\\Python_files\\mpkv_single_pdf_blockchain.py";
        //$cmd="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/excel2pdf/pdf_env/Scripts/python.exe $directoryUrlBackward";
        //$output = exec ("C:/Users/admin/AppData/Local/Programs/Python/Python38/python.exe C:/wamp64/www/seqrdoc/public/excel2pdf/Python_files/session.py");
    ...
    public function imageFormSave(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $location = public_path().'/'.$subdomain[0].'/backend/templates/excel2pdf_images/'.$filename;
        $target_path = public_path().'/'.$subdomain[0].'/backend/templates/excel2pdf_images';   
         //if directory not exist make directory
        if(!is_dir($target_path)){
            mkdir($target_path, 0777,true);
        }
    ...
    public function imageList(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        $files = glob(public_path().'/'.$subdomain[0].'/backend/templates/excel2pdf_images/*.*'); //basename($image)
        $imagePath='http://'.$domain.'/'.$subdomain[0].'/backend/templates/excel2pdf_images/';
        $counter=1;
    ...
    public function AssignLab(){ 
        ...
        return view('admin.excel2pdf.templateMaster.assignlab',compact('LabData','id','template_name','LabDataAssigned'));
    }
    public function AssignLabSave(Request $request){         
        ...
        //return redirect('/admin/excel2pdf-template-list');
        return redirect('/admin/excel2pdf-assign/'.$id)->with('success','Record is saved successfully.');;
    }
    ...
    public function updateTitle(Request $request,$id){
        ...
        if($id != ''){ 
            if($bc_flag==1){
                ...
                CoreHelper::checkContactAddress($id,'excel2PDFTEMPLATE');                
    ...
    public function excelToPdf(){
		return view('admin.excel2pdf.excel2pdf.index');
    } 
}
