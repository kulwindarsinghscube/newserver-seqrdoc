...
use App\Jobs\PDFPreviewOnlyJobV1;
...
class TemplateMasterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $siteData = Site::select('country')->where('site_id',$auth_site_id)->first();
        $siteData=DB::connection('mysql')->table("seqr_demo.sites")->select('country')->where('site_id',$auth_site_id)->first();

        $is_disable = true;
        if($siteData->country != 'India' && $subdomain[0] == "po" ){
            $is_disable = false;
        }
    ...
    public function excelcheck(Request $request){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                for ($col = 1; $col <= $highestColumnIndex; $col++) {
                    $columnNames[] = $sheet->getCellByColumnAndRow($col, 1)->getValue();
                }
                if($subdomain[0] == "demo" || $subdomain[0] == "nita"){
                    $template_id = $request->id; 
                    $image_not_found = [];  
                    $total_image_not_found = 0;
                    $field_master = FieldMaster::select('mapped_name')->where('template_id',$template_id)->where('security_type','Dynamic Image')->get();
                    // dd($field_master);
                    if(!empty($field_master)){
                        foreach($field_master as $value){
                            $column_name = $value->mapped_name ;
                            $columnIndex = array_search($column_name, $columnNames);
                            if($columnIndex){
                                foreach($dataArray as $value){
                                    $foldername = public_path("$subdomain[0]/backend/templates/$template_id/");
                                    $image_name = CoreHelper::getFileWithExtension($value[$columnIndex],$foldername.$value[$columnIndex]);
                                    // dd($image_name);
                                    if(!$image_name){
                                        $image_name = $value[$columnIndex];
                                    }
                                    $imagefilepath = $foldername.$image_name;
                                    if (!file_exists($imagefilepath)) {
                                        // $notfoundimage[] = $image_name;
                                        $image_not_found[] =  $image_name;
                                        $total_image_not_found++;
                                    }
                                    
                                }
                            }
                        }
                        if(!empty($image_not_found)){
                            ...
                            \File::append($file, implode(PHP_EOL, $image_not_found).PHP_EOL);
                            $path = \Request::getScheme().'://'.$subdomain[0].'.'.$subdomain[1].'.com/';
                            $msg= "<br><b>Click <a href='".$path.$subdomain[0].'/test.txt'."'class='downloadpdf' download target='_blank'>Here</a> to download file contains mising image names.<b>";
                            $message = "We found that ".$total_image_not_found." images are missing";
                            return response()->json(['type'=>'error','success'=>2,'message'=>$message.$msg,'msg'=>$msg]);
...
//echo $value;
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
       ...
       if($check_option == 1){
            ...
            if($subdomain[0] == 'ghrcacsp' || $subdomain[0] == 'ghruamravati' || $subdomain[0] == 'aiimsnagpur' || $subdomain[0] == 'ghristu' || $subdomain[0] ==  'ghricacsn') {
                $response = $this->dispatch(new PDFPreviewOnlyJobV1($pdf_data));
            } else {
            ...
            if($subdomain[0] == 'ghrcacsp' || $subdomain[0] == 'ghruamravati' || $subdomain[0] == 'aiimsnagpur' || $subdomain[0] == 'ghristu' || $subdomain[0] ==  'ghricacsn' ) {
                $response = $this->dispatch(new PDFGenerateJobV1($pdf_data));
            } else {
