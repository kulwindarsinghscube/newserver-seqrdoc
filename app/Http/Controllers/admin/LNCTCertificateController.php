...
use Helper;
use App\Jobs\ValidateExcelLnctJob;
use App\Jobs\ValidateExcelLnctPhdJob;
use App\Jobs\PdfGenerateLnctJob;
use App\Jobs\PdfGenerateLnctHindiJob;
use App\Jobs\PdfGenerateLnctPhdJob;

use App\Jobs\PdfGenerateLnctDiplomaJob;
use App\Jobs\PdfGenerateLnctDiplomaHindiJob;


class LNCTCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.lnct.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id==1 || $dropdown_template_id==2 || $dropdown_template_id==4 || $dropdown_template_id==5){
                    $response = $this->dispatch(new ValidateExcelLnctJob($excelData));
                } else if($dropdown_template_id==3){
                    $response = $this->dispatch(new ValidateExcelLnctPhdJob($excelData));
                }
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateLnctJob($pdfData));
        } else if($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateLnctHindiJob($pdfData));
        } else if($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateLnctPhdJob($pdfData));
        } else if($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateLnctDiplomaJob($pdfData));
        } else if($dropdown_template_id==5){
            $link = $this->dispatch(new PdfGenerateLnctDiplomaHindiJob($pdfData));
        } 
