...
use Helper;
use App\Jobs\ValidateExcelsbcityJob;
use App\Jobs\PdfGeneratesbcityJob1;



class SbcityCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.sbcity.index');
    }
    ...
    //             $recordToGenerate = $highestRow1 - 3;
    ...
    //             $response = $this->dispatch(new ValidateExcelsbcityJob($excelData));
    //             //print_r($response);
    ...
    //     // $startRow = $request->get('startRow', 4); // Default to row 1 if not provided
    //     // $endRow = $startRow + $batchSize - 1;

    //     $batchSize = 1; // Rows per batch
    //     // $startRow = $request->get('startRow', 4);
    //     $requestedStart = (int)$request->get('startRow', 4);
    //     $startRow = max(4, $requestedStart);
    //     $endRow = $startRow + $batchSize - 1;

    //     // Prevent accidental re-processing of previous rows
    //     if ($startRow < 4) {
    //         $startRow = 4;
    //     }
    ...
    //                 if ($highestRow1 >= $startRow) { //added by mandar
    //                     $course = array();
    //                     $cols_name = array();
    //                     for ($row = 1; $row <= 1; $row++) {
    ...
    //                     $records = array();
    //                     $r = 1;
    //                     $effectiveStart = $startRow < 4 ? 4 : $startRow;

    //                     for (
    //                         $row = $effectiveStart;
    //                         $row <= min($endRow, $highestRow1);
    //                         $row++
    //                     ) {
    ...
    //                 $c = $last_col - 5;
    //                 $total = $last_col - 6;
    //                 $credit_earned = $last_col - 7;
    //                 $result = $last_col - 8;
    //                 $sgpi = $last_col - 9;
    //                 $subj_col = 22;
    //                 $subj_start = 54;
    //                 $subj_end = $subj_col * 12;
    //             }
    //         }
    //     } else {
    //         return response()->json(['success' => false, 'message' => 'File not found!']);
    ...
    //     $admin_id = \Auth::guard('admin')->user()->toArray();
    //     if ($dropdown_template_id == 1 || $dropdown_template_id == 2) {
    //         $pdfData = array('studentDataOrg' => $rowData1, 'note_key' => $cg, 'credit_register_key' => $c, 'total_credit_hours_key' => $total, 'total_credit_points_key' => $credit_earned, 'cgpa_key' => $result, 'sgpa_key' => $sgpi, 'subj_col' => $subj_col, 'subj_start' => $subj_start, 'subj_end' => $subj_end, 'auth_site_id' => $auth_site_id, 'template_id' => $template_id, 'dropdown_template_id' => $dropdown_template_id, 'previewPdf' => $previewPdf, 'excelfile' => $excelfile, 'loader_token' => $loader_token, 'pdf_flag' => $pdf_flag, 'highestrow' => (($highestRow1 ? $highestRow1 : $highestRow) - 1)); //For Custom Loader
    //     } else {
    ...
    //         // dd($pdfData);
    //         $link = $this->dispatch(new PdfGeneratesbcityJob1($pdfData));
    //         // dd($pdfData);
    //     }
    ...
    public function validateExcel(Request $request, CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                if ($dropdown_template_id == 4 || $dropdown_template_id == 7 || $dropdown_template_id == 9) {
                    $sheet2 = $objPHPExcel1->getSheet(1);
                    $rowData2 = $sheet2->toArray();
                    $excelData = array('rowData1' => $rowData1, 'subjectsMark' => $rowData2, 'auth_site_id' => $auth_site_id, 'dropdown_template_id' => $dropdown_template_id);
                } else {
                    $excelData = array('rowData1' => $rowData1, 'auth_site_id' => $auth_site_id, 'dropdown_template_id' => $dropdown_template_id);
                }
    ...
    public function uploadfile(Request $request, CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        // Start Update code for batchwise genration
        $batchSize = 1; // Rows per batch
        $startRow = $request->get('startRow', 1); // Default to row 1 if not provided
        $endRow = $startRow + $batchSize - 1;
        // End Update code for batchwise genration
        // End Update code for batchwise genration
        // dd($endRow);
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                if ($dropdown_template_id == 1 || $dropdown_template_id == 2 || $dropdown_template_id == 3 || $dropdown_template_id == 4 || $dropdown_template_id == 5 || $dropdown_template_id == 7 || $dropdown_template_id == 8 || $dropdown_template_id == 9) {
                    ...
                    if ($highestRow1 >= $startRow) {
                        ...
                        // dd($rowData1);
                        // $rowData1 = $sheet1->rangeToArray('A1:' . $highestColumn1 . $highestRow1, NULL, TRUE, FALSE);
                        if ($dropdown_template_id == 1 || $dropdown_template_id == 3 || $dropdown_template_id == 7 || $dropdown_template_id == 8 || $dropdown_template_id == 9) {
                            ...
                            if ($sheetCount > 1) {
                                ...
                                $rowData2 = array(); // Prevent crash if second sheet not present
                            };
                        }
                    } else {
                        // dd(1);
        ...
        // }
        elseif ($dropdown_template_id == 2 || $dropdown_template_id == 5) {
            ...
            $pdfData = array('studentDataOrg' => $rowData1, 'subjectsMark' => $rowData2, 'auth_site_id' => $auth_site_id, 'template_id' => $template_id, 'dropdown_template_id' => $dropdown_template_id, 'previewPdf' => $previewPdf, 'excelfile' => $excelfile, 'loader_token' => $loader_token, 'highestrow' => (($highestRow1 ? $highestRow1 : $highestRow) - 1));
        }

        if ($dropdown_template_id == 1 || $dropdown_template_id == 5) {
            // dd($pdfData);
            $link = $this->dispatch(new PdfGeneratesbcityJob1($pdfData));
            // dd($pdfData);
            // dd($)
