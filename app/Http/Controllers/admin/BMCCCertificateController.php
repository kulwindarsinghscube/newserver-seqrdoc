...
// Ghost image
    ...
    public function pdfSample1()
    {
        $pdf = new TCPDF();
        $pdf->AddPage();
         $arial = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Arial.TTF', 'TrueTypeUnicode', '', 96);
        $text = "Test "; // Example text
        $maxWidth = 190; // Maximum width in PDF units
        $fontFamily = 'arial';
       ...
       // $this->printTextWithinWidth($pdf, $text, $maxWidth);
       // $this->printTextWithRepeatOrTruncate($pdf, $text, $maxWidthText);
         $this->printExactStringWithinWidth($pdf, $text, $assignedWidth);
        $text = "365371585680 "; // Example text
        $x=10;
        $y=50;
        $pdf->SetXY($x, $y);
        $pdf->SetFont($fontFamily, $fontStyle, $fontSize);
       ...
       // $this->printTextWithRepeatOrTruncate($pdf, $text, $maxWidthText);
        ...
        $text = "GOVERNMENT OF TELANGANA "; // Example text
        $x=10;
        $y=55;
        $pdf->SetXY($x, $y);
        $pdf->SetFont($fontFamily, $fontStyle, $fontSize);
      ...
      //   $this->printExactStringWithinWidth($pdf, $text, $assignedWidth);
        ...
        $pdf->Output('text_within_width_repeat.pdf', 'I');
    }

    private function printExactStringWithinWidth(TCPDF $pdf, string $text, int $assignedWidth)
    {
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        $textWidth = round($pdf->GetStringWidth($text),2);

        if ($textWidth <= $assignedWidth) {
            $repeatedText = "";
            $remainingWidth = $assignedWidth;

            while (round($pdf->GetStringWidth($repeatedText . $text),2) <= $remainingWidth) {
                $repeatedText .= $text;
            }

            if (round($pdf->GetStringWidth($repeatedText),2) > $assignedWidth) {
                $trimmedRepeatedText = "";
                for ($i = 0; $i < strlen($repeatedText); $i++) {
                    $testTrimmed = $trimmedRepeatedText . $repeatedText[$i];
                    if (round($pdf->GetStringWidth($testTrimmed),2) <= $assignedWidth) {
                        $trimmedRepeatedText = $testTrimmed;
                    } else {
                        break;
                    }
                }
                $pdf->Text($x, $y, $trimmedRepeatedText);
            } else {
                $pdf->Text($x, $y, $repeatedText);
            }

        } else {
            $trimmedText = "";
            for ($i = 0; $i < strlen($text); $i++) {
                $testTrimmed = $trimmedText . $text[$i];
                if (round($pdf->GetStringWidth($testTrimmed),2) <= $assignedWidth) {
                    $trimmedText = $testTrimmed;
                } else {
                    break;
                }
            }
            $pdf->Text($x, $y, $trimmedText);
        }
    }

      private function printTextWithRepeatOrTruncate(TCPDF $pdf, string $text, int $maxWidth)
    {
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        ...
        if ($textWidth <= $maxWidth) {
            $pdf->Text($x, $y, $text);
            return; // Exit if it fits
        }

        // Truncate if too wide
        $truncatedText = "";
        for ($i = 0; $i < strlen($text); $i++) {
            $testTruncatedText = $truncatedText . $text[$i];
            if ($pdf->GetStringWidth($testTruncatedText) <= $maxWidth) {
                $truncatedText = $testTruncatedText;
            } else {
                break;
            }
        }
        $pdf->Text($x, $y, $truncatedText);

        // Repeat if too short
        if ($textWidth < $maxWidth) {
            $repeatedText = "";
            $remainingWidth = $maxWidth - $pdf->GetStringWidth($truncatedText);
            if ($truncatedText !== ""){
                $x += $pdf->GetStringWidth($truncatedText);
            }

            while ($pdf->GetStringWidth($repeatedText . $text) <= $remainingWidth) {
                $repeatedText .= $text;
            }
            $pdf->Text($x, $y, $repeatedText);
        }
    ...
    private function printTextWithinWidth(TCPDF $pdf, string $text, int $maxWidth)
    {
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        $textWidth = $pdf->GetStringWidth($text);
        ...
        if ($textWidth <= $maxWidth) {
            // Text fits within the width
            $pdf->Text($x, $y, $text);
        } else {
            // Text exceeds the width, truncate
            $truncatedText = "";
            for ($i = 0; $i < strlen($text); $i++) {
                $testTruncatedText = $truncatedText . $text[$i];
                if ($pdf->GetStringWidth($testTruncatedText) <= $maxWidth) {
        ...
        if($textWidth < $maxWidth){
            $repeatedText = "";
            while($pdf->GetStringWidth($repeatedText . $text) <= $maxWidth){
                $repeatedText .= $text;
            }
            $pdf->Text($x, $y, $repeatedText);
        }
    ...
    function getMicrotext($str, $microFontSize,$width){
           
               //  $str = preg_replace('/\s+/', '', $str); //added by Mandar
                $textArray = imagettfbbox($microFontSize, 0, public_path().'/backend/fonts/Arial.ttf', $str);
                $strWidth = ($textArray[2] - $textArray[0]);
                $strHeight = $textArray[6] - $textArray[1] / $microFontSize;
                ...
                //Cut string in required characters (final string)
                return $array = substr($microlinestrRep,0,$microlinestrCharReq);
...
// $bg_yellow_patch = public_path().'\\backend\canvas\yellowpatchimages\YELLOW_BG.jpg';
// $font_file=public_path().'\\bmcc\backend\canvas\fonts\Arial Bold.TTF';
// list($width, $height, $type, $attr) = getimagesize($bg_yellow_patch);
        ...
        //  $filename = "Input Data Format For Card Printing v1.xlsx";
        // $pathImport = public_path() . '/demo/';
        ...
        //$pdf = new TCPDF('L', 'mm', array('482.6', '330.2'), true, 'UTF-8', false);
       // $pdf = new TCPDF('P', 'mm', array('210.058','296.926'), true, 'UTF-8', false);
       ...
       // set fonts
        ...
        $century = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Century.ttf', 'TrueTypeUnicode', '', 96);

        $nirmalUIN = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\NotoSansTelugu-Regular.ttf', 'TrueTypeUnicode', '', 96);
        ...
        'stretchtext' => 3
          ...
          // add spot colors
        //$pdf->AddSpotColor('Special', 30, 100, 90, 10); 
        $pdf->AddSpotColor('Special', 62, 2, 3, 0); 
         $pdf->AddPage();
          ...
          $qrCodex=15;
            $qrCodey=15;
            $enrollment_font_size = '15.4';//487

            // Ensure the text is in UTF-8 (just in case)
           
            $pdf->SetTextColor(0,0, 0);
            $pdf->SetFont($nirmalUIN, false, $enrollment_font_size, '', false);
            $pdf->SetXY($qrCodex, $qrCodey);
            //$pdf->Cell(0, 0, 'ధనలక్ష్మి', 0, false, '');
            $pdf->Write(0, 'ధనలక్ష్మి');

         $pdf->SetFont($arial, false, 12, '', false);
          ...
          /********INVISIBLE Start****/
            ...
            $pdf->SetTextSpotColor('Special', 100);
            $pdf->SetXY(178, 207);
            $pdf->Rotate(90);
            $pdf->MultiCell(169, 0, 'Hi this is demo text', '', 'L', 0, 1);
            $pdf->Rotate(0);
            ...
            // Define a spot color
$spotcolor = $pdf->AddSpotColor('MySpotRed', 0, 100, 100, 0); // CMYK

// Set fill spot color for shapes (not for bitmap images)
$pdf->SetFillSpotColor($spotcolor, 100); // 100% tint

// Draw a filled rectangle
$pdf->Rect(10, 50, 40, 20, 'F');
...
// $centerY = 148;
// $radius = 50;
// $text = "TEXT IN CIRCULAR ANGLE";

// // Calculate spacing
// $charCount = strlen($text);
// $totalAngle = 180; // degrees for semi-circle
// $angleStep = $totalAngle / ($charCount - 1);
// $startAngle = -90 - ($totalAngle / 2); // Centered on top
...
//     // Rotate character upright along the circle path
//     $pdf->StartTransform();
//     $pdf->Rotate($angle + 90, $x, $y);
//     $pdf->Text($x - 1.5, $y - 1.5, $char);
//     $pdf->StopTransform();
// }

        //  $fileNameJob="415_100001_101000";
        //  for($i=1;$i<=10;$i++){
        // //$i=0;
        // $serial_number=109000+$i;  

        // $y=20*$i;
        // // $serial_number2=109500+$i;    
       

        //      /*======First UP======================================================================*/    
        //     $name="DSRRAU/".$serial_number; 

        //     $qrCodex=15;
        //     $qrCodey=15;
        //     $enrollment_font_size = '10.4';//487
        //     $pdf->SetTextColor(0,0, 0);
        //     $pdf->SetFont($arial, false, $enrollment_font_size, '', false);
        //     $pdf->SetXY($qrCodex, $qrCodey);
        //     $pdf->Cell(0, 0, "Overlay 4", 0, false, '');


        //     $bg_yellow_patch = public_path().'\\backend\canvas\yellowpatchimages\RND\Yellow_Patch_BG.png';

        //     $qrCodex=16.4;
        //     $qrCodey=14.6+$y;
        //     $qrCodeWidth="35";
        //     $qrCodeHeight="8";
        //     $pdf->Image($bg_yellow_patch, $qrCodex, $qrCodey, $qrCodeWidth,  $qrCodeHeight, 'PNG', '', '', false, 600);
            
        //     $enrollment_font_size = '11';
        //     $alpha=0.3+(0.01*$i);
        ...
        //     $pdf->SetFont($arialb, 'B', $enrollment_font_size, '', false);
        //     $pdf->SetXY($qrCodex, $qrCodey);
        //     $pdf->Cell(0, 0, $name, 0, false, '');

        //     $pdf->SetAlpha(1);


        //    // $pdf->setImageScale(1.66); 
        //     $bg_yellow_patch = public_path().'\\backend\canvas\yellowpatchimages\RND\yellow patch trial_4.png';

        //     $qrCodex=16.4;
        //     $qrCodey=14.6+$y;
        //     $qrCodeWidth="35";
        //     $qrCodeHeight="8";
        //     $pdf->Image($bg_yellow_patch, $qrCodex, $qrCodey, $qrCodeWidth,  $qrCodeHeight, '', '', '', false, 600);

    //          $microFontSize=6.4;
    //     $pdf->SetFont($arial, '', $microFontSize, '', false);

    //     // Example data (text strings) and their respective positions and widths
    //     $lines = [
    //         ['text' => '365371585680 ', 'x' => 10, 'y' => 20, 'width' => 200],
    //         ['text' => 'DHANA LAXMI ', 'x' => 10, 'y' => 25, 'width' => 200],
    //         ['text' => '1450360 ', 'x' => 10, 'y' => 30, 'width' => 200],
    //         ['text' => 'GOVERNMENT OF TELANGANA ', 'x' => 10, 'y' => 35, 'width' => 200],
    //     ];
    // $padding = 2; // Padding around the text
    //     $borderWidth = 0.5; // Border line width
    ...
    //             'D' // 'D' for draw only, 'DF' for draw and fill
    //         );

    //         // Set position for this line
    //         $pdf->SetXY($x, $y);

    //         // Calculate the string width using the current font and size
    //         $stringWidth = $pdf->GetStringWidth($text);

    //         // Check if the string fits the defined width
    //         if ($stringWidth > $width) {
    //             // If the string is too long, trim it to fit
    //             $maxChars = floor($width / ($stringWidth / strlen($text))); // Calculate max chars that can fit
    //             $trimmedText = substr($text, 0, $maxChars); // Trim the text
    //             $pdf->Text($x, $y, $trimmedText); // Output the trimmed text
    //         } else {
    //             // If the string is too short, repeat it until it fits
    //             $repeatedText = $text;
    //             $repeatedWidth = $pdf->GetStringWidth($repeatedText);

    //             // Keep repeating the text until it fits the width
    //             while ($repeatedWidth < $width) {
    //                 $repeatedText .= ' ' . $text; // Repeat the text
    //                 $repeatedWidth = $pdf->GetStringWidth($repeatedText); // Recalculate the width
    //             }

    //             // Trim the text to fit exactly within the width if it exceeds
    //             if ($repeatedWidth > $width) {
    //                 $excessLength = $repeatedWidth - $width;
    //                 $repeatedText = substr($repeatedText, 0, strlen($repeatedText) - $excessLength); // Trim excess
    //             }

    //             // Output the repeated (or trimmed) text
    //             $pdf->Text($x, $y, $repeatedText);
    //         }
    //     }
            ...
            // $pdf->StartTransform(); // Start a transformation sequence
            // $pdf->Rect($x, 50, $actualWidth, 59, 'CNZ'); // Define a clipping rectangle
            // // $pdf->Clip(); // Apply the clipping
            // $pdf->setCellPaddings( $left = '0', $top = '', $right = '', $bottom = '');

            // $str =$this->getMicrotext($partyName,$microFontSize,$width);
               
            // $pdf->SetFont($arial, '', $microFontSize, '', false);
            // $pdf->setXY($x, 50);//265
            // $pdf->Cell(190, 0, $str, 1, 1, 'L', 0, '', 0);
         

            //  $partyName="DHANA LAXMI ";
            // $str =$this->getMicrotext($partyName,$microFontSize,$width);
            ...
            // $partyName="GOVERNMENT OF TELANGANA ";
            // $str =$this->getMicrotext($partyName,$microFontSize,$width);
            ...
            //     $bg_yellow_patch = public_path().'\\backend\canvas\yellowpatchimages\RND\yellow patch trial_tff.tif';
            ...
            // $enrollment_font_size = '10.4';//487
            // $pdf->SetTextColor(0,0, 0);
            // $pdf->SetFont($century, false, $enrollment_font_size, '', false);
            // $pdf->SetXY($qrCodex, $qrCodey);
            // $pdf->Cell(0, 0, "Folio No. : ".$name, 0, false, '');


            // $bg_yellow_patch = public_path().'\\backend\canvas\yellowpatchimages\Image_34.7mm_x_8.5mm.jpg';
            ...
            // $pdf->SetFont($century, false, $enrollment_font_size, '', false);
            // $pdf->SetXY($qrCodex, $qrCodey);
            // $pdf->Cell(0, 0, "Folio No. : ".$name2, 0, false, '');
       ...
       //    $bg_yellow_patch = public_path().'\\backend\bhavin_shah_graph.png';
       ...
       //      $pdf->Image($bg_yellow_patch, $qrCodex, $qrCodey, $qrCodeWidth,  $qrCodeHeight, '', '', '', false, 600);
        ...
        // $myPath = public_path().'/demo/backend/rajsthan_ayurveda/pdf/';
        //    $dt = date("_ymdHis");
        //     // print_r($pdf);
        //     // print_r("$tmpDir/" . $name."".$ghost_font_size.".png");
        // $pdf->output($myPath . DIRECTORY_SEPARATOR .$fileNameJob.'.pdf', 'F');

        //     $file_name=$fileNameJob.".pdf";
        //     $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/demo/backend/rajsthan_ayurveda/0415_Dr_S_R_Rajasthan_Ayurved_University_Grade_Card_BG_A4_24_04.pdf";
        //     //0486_Dr_S_R_Rajasthan_Ayurved_University_Migration_Certificate_DIGITAL
        //     //0488_Dr_S_R_Rajasthan_Ayurved_University_Passing_Certificate_for_DIGITAL
        //    // 0487_Dr_S_R_Rajasthan_Ayurved_University_Provisional_Certificate
        //     $mergePath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/demo/backend/rajsthan_ayurveda/pdf_withbg/";
        //     $directoryUrlBackward="C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\pdf2pdf\\";
        //     $pyscript = $directoryUrlBackward."Python_files\\rajsthan_merge.py";
      ...
      //  0488 Dr. S.R Rajasthan Ayurved University Passing Certificate for DIGITAL
    }
