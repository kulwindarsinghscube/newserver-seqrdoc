...
use App\Jobs\PdfGenerateSdmMbbs4060ResitJob;
use App\Jobs\PdfGenerateSdmMbbsThirdResitJob;
use App\Jobs\PdfGenerateSdmMbbsThirdPartIIResitJob;
use App\Jobs\PdfGenerateSdmBscAlliedResitJob;
use App\Jobs\PdfGenerateSdmBscAlliedResitNoVivaJob;
use App\Jobs\PdfGenerateSdmMbbsIIIResitJob_3subjects;
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                /**  Load $inputFileName to a Spreadsheet Object  **/
                if($dropdown_template_id != "3" && $dropdown_template_id != "5" && $dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "12" && $dropdown_template_id != "13" && $dropdown_template_id != "16" && $dropdown_template_id != "18" && $dropdown_template_id != "26" && $dropdown_template_id != "28"){
					$objPHPExcel1 = $objReader->load($fullpath);
					$sheet1 = $objPHPExcel1->getSheet(0);
					$highestColumn1 = $sheet1->getHighestColumn();
					$highestRow1 = $sheet1->getHighestDataRow();
					...
					if($dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "13" && $dropdown_template_id != "12" && $dropdown_template_id != "26" && $dropdown_template_id != "28"){
						//3,5,16 template
                        ...
                        }elseif($dropdown_template_id==26  || $dropdown_template_id==28){
                            $GT_Max_key=$last_col; 
                            $GT_Min_key=$last_col+1;
                            $GT_Sec_key=$last_col+2;
                            $Grand_Total_In_words_key=$last_col+3;
		...
		if($dropdown_template_id != "3" && $dropdown_template_id != "5" && $dropdown_template_id != "7" && $dropdown_template_id != "11" && $dropdown_template_id != "12" && $dropdown_template_id != "13" && $dropdown_template_id != "16" && $dropdown_template_id != "18" && $dropdown_template_id != "26" && $dropdown_template_id != "28"){
			...
			if($dropdown_template_id==11 || $dropdown_template_id==13){
                ...
                if($dropdown_template_id==26 || $dropdown_template_id==28){
                    ...
                    $pdfData=array('studentDataOrg'=>$rowData1, 'GT_Max_key'=>$GT_Max_key, 'GT_Min_key'=>$GT_Min_key, 'GT_Sec_key'=>$GT_Sec_key, 'Grand_Total_In_words_key'=>$Grand_Total_In_words_key, 'Percentage_key'=>$Percentage_key, 'Result_key'=>$Result_key, 'Date_key'=>$Date_key, 'Student_image_key'=>$Student_image_key, 'Batch_of_key'=>$Batch_of_key, 'Aadhar_No_key'=>$Aadhar_No_key, 'DOB_key'=>$DOB_key, 'course_key'=>$course_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>($highestRow-1)); //For Custom Loader
                }
            }
        }
        // print_r($pdfData);
        ...
        elseif($dropdown_template_id==23){
            $link = $this->dispatch(new PdfGenerateSdmMbbs4060ResitJob($pdfData));
        }
        elseif($dropdown_template_id==24){
            $link = $this->dispatch(new PdfGenerateSdmMbbsThirdResitJob($pdfData));
        }
        elseif($dropdown_template_id==25){
            $link = $this->dispatch(new PdfGenerateSdmMbbsThirdPartIIResitJob($pdfData));
        }
        elseif($dropdown_template_id==26){
            $link = $this->dispatch(new PdfGenerateSdmBscAlliedResitJob($pdfData));
        } 
        elseif($dropdown_template_id==27){
            $link = $this->dispatch(new PdfGenerateSdmMbbsIIIResitJob_3subjects($pdfData));
        }
        elseif($dropdown_template_id==28){
            $link = $this->dispatch(new PdfGenerateSdmBscAlliedResitNoVivaJob($pdfData));
        }
        // return response()->json(['success'=>true,'message'=>'Certificates generated successfully.','link'=>$link]);

        return response()->json(['success'=>true,'message'=>'Certificates generated successfully.','link'=>$link,'startRow'=> $startRow,'endRow'=> $endRow,'highestRow'=> ($highestRow)]);
