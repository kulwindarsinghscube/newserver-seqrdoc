...
class DSRRAU_Rajsthan415CertificateController extends Controller
{
...
// Ghost image
    ...
    public function pdfGenerate(){
       ...
       // $pdf->SetCreator('TCPDF');
        ...
        //set fonts
        $arial = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Arial.TTF', 'TrueTypeUnicode', '', 96);
        $arialb = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Arialb.TTF', 'TrueTypeUnicode', '', 96);
        $ariali = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\ARIALI.TTF', 'TrueTypeUnicode', '', 96);

        $arialNarrowB = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\ARIALNB.TTF', 'TrueTypeUnicode', '', 96);

       $timesNewRoman = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Times-New-Roman.TTF', 'TrueTypeUnicode', '', 96);
        $timesNewRomanB = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Times-New-Roman-Bold.TTF', 'TrueTypeUnicode', '', 96);
        $timesNewRomanBI = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Times-New-Roman-Bold-Italic.TTF', 'TrueTypeUnicode', '', 96);
        $timesNewRomanI = TCPDF_FONTS::addTTFfont(public_path().'\bmcc\backend\canvas\fonts\Times New Roman_I.TTF', 'TrueTypeUnicode', '', 96);
...
// // Define paths for image and font
// $imagePath = $bg_yellow_patch;
// $fontPath = $font_file;

// // Check if image and font exist
// if (!file_exists($imagePath) || !file_exists($fontPath)) {
//   die("Error: Image or font file not found!");
// }

// // Create image resource from the file
// $image = imagecreatefromjpeg($imagePath);

// // Allocate a color for the text (white in this example)
// //$textColor = imagecolorallocate($image, 255, 255, 255);
// $textColor = imagecolorallocatealpha($image, 255,255, 0, 85);
// // Text to be placed on the image
// $text = "DSRRAU/100001";
// $name = "DSRRAU_100001_85";
// // Get the image dimensions
// $imageWidth = imagesx($image);
// $imageHeight = imagesy($image);

// // Calculate text placement (adjust as needed for better positioning)
// $textX = ($imageWidth - imagettfbbox($fontSize, 0, $fontPath, $text)[4]) / 2;
// $textY = $imageHeight / 2 + 10; // Adjust vertical position as needed
...
// imagejpeg($image,public_path()."\\". $name.".jpg");
...
// /public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Arial Bold.TTF
...
//         $textcolor = imagecolorallocatealpha($image, 230, 230, 230, 100);
       ...
       // $pdf = new TCPDF('P', 'mm', array('210', '297'), true, 'UTF-8', false);
        ...
        $fileNameJob="415_110001_111000";
        for($i=1;$i<501;$i++){
        //$i=0;
        $serial_number=110000+$i;  

        $serial_number2=110500+$i;    
        $pdf->AddPage();
       ...
       // $fontEBPath=public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\E-13B_0.php';
        ...
        // }
            ...
            $bg_yellow_patch_overlay = public_path().'\\backend\canvas\yellowpatchimages\RND\yellow patch trial_4.png';
            ...
            $qrCodeWidth="35";
            $qrCodeHeight="8";
            $pdf->Image($bg_yellow_patch_overlay, $qrCodex, $qrCodey, $qrCodeWidth,  $qrCodeHeight, '', '', '', false, 600);
            ...
            /*============================================================================*/    
             ...
             /*======Second UP======================================================================*/  
            // $serial_number=$serial_number+500;  
            // $name="DSRRAU/".$serial_number; 
      ...
      */
       ...
       //$pdf->Output('sample.pdf', 'I');  
        ...
        $pdf->output($myPath . DIRECTORY_SEPARATOR .$fileNameJob.'.pdf', 'F');
            ...
            $bgPath="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/demo/backend/rajsthan_ayurveda/0415_Dr_S_R_Rajasthan_Ayunred_University_Grade_Card_Final_Print_DIGITAL.pdf";
