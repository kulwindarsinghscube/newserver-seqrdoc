...
use Helper;
use App\Jobs\ValidateExcelCambridgeJob;

use App\Jobs\PdfGenerateCambridgeGradeCardJob;

class CambridgeCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.cambridge.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                //updated by kamini  for skip records when pdf_file is no
                if($dropdown_template_id==1){
                      $recordToGenerate = 0;
                    for ($i = 1; $i < count($rowData1); $i++) { 
                        if (isset($rowData1[$i][6]) && strtolower(trim($rowData1[$i][6])) === 'yes' ) {
                            $recordToGenerate++;
                        }
                    }
                ...
                $response = $this->dispatch(new ValidateExcelCambridgeJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateCambridgeGradeCardJob($pdfData));
        }
