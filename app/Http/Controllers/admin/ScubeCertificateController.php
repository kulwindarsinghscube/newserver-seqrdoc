...
use Helper;
use App\Jobs\ValidateExcelSCUBEJob;
use App\Jobs\pdfGenerateScubeJob;
// use App\Jobs\pdfGenerateJntuJob1;


class ScubeCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.scube.index');
        //newrrmu
    }
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                // if ($dropdown_template_id == 1) {
                    ...
                    // dd($excelData);
                    $response = $this->dispatch(new ValidateExcelSCUBEJob($excelData));
                    // print_r($response);
                    // die();
...
// dd($request->all());
          ...
          if($dropdown_template_id==1){
            ...
            $link = $this->dispatch(new pdfGenerateScubeJob($pdfData));
        }
