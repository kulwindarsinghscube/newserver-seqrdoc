...
use Helper;
use App\Jobs\ValidateExcelBestiuEDocumentJob;
use App\Jobs\PdfGenerateBestiuEDocumentJob1;
use App\Jobs\PdfGenerateBestiuEDocumentJob2;
use App\Jobs\PdfGenerateBestiuEDocumentJob3;
use App\Jobs\PdfGenerateBestiuEDocumentJob4;
use App\Jobs\PdfGenerateBestiuEDocumentJob5;
use App\Jobs\PdfGenerateBestiuEDocumentJob5_10s;
use App\Jobs\PdfGenerateBestiuEDocumentJob5_8s;
use App\Jobs\PdfGenerateBestiuEDocumentJob5_6s;
use App\Jobs\PdfGenerateBestiuEDocumentJob7;
use App\Jobs\PdfGenerateBestiuEDocumentJob8;
use App\Jobs\PdfGenerateBestiuEDocumentJob6;
use App\Jobs\PdfGenerateBestiuEDocumentJob6_8s;
use App\Jobs\PdfGenerateBestiuEDocumentJob6_10s;
class BestiuEDocumentController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.bestiu.e_document_index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelBestiuEDocumentJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id==1 || $dropdown_template_id==2||$dropdown_template_id==3||$dropdown_template_id==4||$dropdown_template_id==5||$dropdown_template_id==6||$dropdown_template_id==7||$dropdown_template_id==8){
                    ...
                    if($highestRow1>=$startRow){
                        ...
                        $rowData1 = $sheet1->rangeToArray($range, NULL, TRUE, FALSE);
                         ...
                         if($dropdown_template_id==7||$dropdown_template_id==6||$dropdown_template_id==5) {
        ...
        if (!is_dir(public_path().'/'.$subdomain[0].'/backend/tcpdf/examples/textfile')) {
            mkdir(public_path().'/'.$subdomain[0].'/backend/tcpdf/examples/textfile', 0755, true); // Create folder with permissions
        }

        $record_textfile = $request->record_textfile;
        ...
        $admin_id = \Auth::guard('admin')->user()->toArray();
		if($dropdown_template_id==1 || $dropdown_template_id==2|| $dropdown_template_id==3 || $dropdown_template_id==4 ){
            $pdfData=array('studentDataOrg'=>$rowData1, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'record_textfile' =>$record_textfile,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
        }elseif($dropdown_template_id==7||$dropdown_template_id==6||$dropdown_template_id==5){
			$pdfData=array('studentDataOrg'=>$rowData1,'subjectDataOrg'=>$rowData2, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
        } else{
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob1($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob2($pdfData));
        }
          elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob3($pdfData));
        }
         elseif($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob4($pdfData));
        }
        elseif($dropdown_template_id==5){
            if($request->subjects==6){
             $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob5_6s($pdfData));
            }else if($request->subjects==8){
             $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob5_8s($pdfData));
            }else if($request->subjects==10){
             $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob5_10s($pdfData));
            }else{
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob5($pdfData));
            } 
        }
        elseif($dropdown_template_id==6){
            if($request->subjects==8){
             $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob6_8s($pdfData));
            }else if($request->subjects==10){
             $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob6_10s($pdfData));
            }else{
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob6($pdfData));
            }
        }
        elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob7($pdfData));
        }elseif($dropdown_template_id==8){
            $link = $this->dispatch(new PdfGenerateBestiuEDocumentJob8($pdfData));
        }
