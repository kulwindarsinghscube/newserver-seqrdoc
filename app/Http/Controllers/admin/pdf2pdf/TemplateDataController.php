...
{   
      ...
      public function index(Request $request)
    {
       ...
       // TemplateMaster::where('create_refresh',1)->update(['create_refresh'=>0]);
        ...
        if($request->ajax()){
            ...
            $where_str.= " and (uploaded_pdfs.map_type =0)";
            ...
            }elseif($subdomain[0]=="demo"||$subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi" ||$subdomain[0]=="abyssinia" || $subdomain[0]=="alarduni"){
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'template_name','pdf_page','id','file_name','publish',DB::raw('IFNULL(`template_title`,"") AS template_title'),'print_bg_file', 'print_bg_status', 'verification_bg_file', 'verification_bg_status', 'is_block_chain', 'bc_contract_address'];  
            }
            else{
            ...
            if($subdomain[0]=="po"){
                $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'template_name','pdf_page','id','file_name','publish',DB::raw('IFNULL(`template_title`,"") AS template_title'),'print_bg_file', 'print_bg_status', 'verification_bg_file', 'verification_bg_status','verification_type']; 
            }  
   ...
   public function edit(){
    $id=$_POST['id'];
        ...
        if($subdomain[0] == 'po'){
            $templateData=DB::select(DB::raw('SELECT template_name,file_name,pdf_page,template_title,verification_type FROM `uploaded_pdfs` WHERE id ="'.$id.'"'));                      
        }else{
        ...
        $template_data=array("action"=>"edit","id"=>$id,"template_name"=>$templateData->template_name,"file_name"=>$templateData->file_name,"pdf_page"=>$templateData->pdf_page,"template_title"=>$templateData->template_title,'verification_type'=>@$templateData->verification_type);
    return view('admin.pdf2pdf.addtemplate.index',compact('template_data'));
   }
...
public function testS3(Request $request)
{
    ...
    $s3=\Storage::disk('s3');
    $pdf_file_directory = "public/".$subdomain[0].'/backend/pdf_file/Inactive_PDF/';
...
public function uploadFilesToS3(Request $request)
{
    ...
    $status=false;
    $deleteFiles=[];
    $pdf_paths_arr=explode(', ',$pdf_paths);
    foreach($pdf_paths_arr as $file_path){
        $file_path = preg_replace('/\\\\/', '\\', $file_path);
        $myFile = pathinfo($file_path);
        //Show the file name
        $filename=$myFile['basename'];
        $temp_pdf_file = public_path().'/'.$subdomain[0].'/backend/pdf_file/'.$filename;
        array_push($deleteFiles, $temp_pdf_file);
        //$aws_qr = \Storage::disk('s3')->put('/public/'.$subdomain[0].'/backend/pdf_file/'.$filename, file_get_contents($temp_pdf_file), 'public');

        $s3 = \Storage::disk('s3');
        $livePdf = 'public/'.$subdomain[0].'/backend/pdf_file/'.$filename;
        ...
        if($s3->exists($livePdf)) {
            ...
            if($s3->exists($awsInactivePDFFolder)) {
                $newFileNamePdf = 'public/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF/'.$currDateTime.'_'.$filename;
        ...
        if(!$s3->exists($livePdf)) {
            $s3->put($livePdf, file_get_contents($temp_pdf_file));
        }
    $status=true;
    }

    if($status==true){
         foreach($deleteFiles as $file_path){
            $file_path = preg_replace('/\\\\/', '\\', $file_path);
...
public function uploadFilesToS3_council(Request $request)
{
    ...
    $domain = \Request::getHost();
    $subdomain = explode('.', $domain);
    if($subdomain[0]!="council"){$subdomains="council";}
    $s3=\Storage::disk('s3');
    $pdf_file_directory = "public/".$subdomain[0].'/backend/pdf_file/';
    ...
    foreach($pdf_paths_arr as $file_path){
        ...
        if($s3->exists($livePdf)) {
            //$student = StudentTable::where('status',1)->where('serial_no',$serial_no)->value('id');
            //$awsInactivePDFFolder = 'public/'.$subdomain[0].'/varify';
            $awsInactivePDFFolder = 'public/'.$subdomains.'/varify';
            
            $currDateTime=date('YmdHis');
            // check folder 
            if($s3->exists($awsInactivePDFFolder)) {
                //$newFileNamePdf = 'public/'.$subdomain[0].'/varify/'.$currDateTime.'_'.$filename;
                $newFileNamePdf = 'public/'.$subdomains.'/varify/'.$currDateTime.'_'.$filename;                
                $s3->move($livePdf, $newFileNamePdf);
            } else {
                // folder create
                $s3->makeDirectory($awsInactivePDFFolder, 0777);
        ...
        if($request->hasFile('pdf_upload')){
            ...
            if($request['pdf_upload']->move($target_path,$pdfFile)){
                ...
                if(!empty($template_id)){
                    ...
                    if($subdomain[0] == 'po'){
                        $insertRequest->verification_type=$verification_type;
                    }
                    $insertRequest->save();
               ...
               return response()->json(['success'=>false,'message'=>'Error while uploading file!','type'=>'toaster','rstatus' => 'invalid']); 
                    ...
                    if($templateDataCount==0){
                        ...
                        TemplateMaster::where('id',$template_id)->update(["extractor_details"=>$extractor_boxes,"placer_details"=>$placer_boxes,"ep_details"=>$ep_boxes,"generated_by"=>$admin_id['id']]);
                        
                        $folder=public_path().'/'.$subdomain[0].'/documents/';
                        
                        $folder2=public_path().'/'.$subdomain[0].'/documents/copy_json';
                        ...
                        if($subdomain[0]=="icat"){
                            $dataLogInsert=array();
                            $dataLogInsert['tbl_name'] ='uploaded_pdfs';
                            $dataLogInsert['data_id'] =$template_id;
                            $dataLogInsert['template_name'] =$template_name;
                            $dataLogInsert['action'] ='Update';
                            $dataLogInsert['ip_address'] =$_SERVER['REMOTE_ADDR'];
                            $dataLogInsert['user_id'] =$admin_id['id'];
                            $dataLogInsert['created_at'] =date('Y-m-d H:i:s');
                            DB::table('pdf2pdf_log')->insertGetId($dataLogInsert);                        
                        }
                        return response()->json(['success'=>"success",'message'=>'Template updated successfully.','type'=>'toaster','rstatus' => 'edit']);
                    }else{
                        return response()->json(['success'=>false,'message'=>'Template already exist with same name!','type'=>'toaster','rstatus' => 'exist']);
...
/*
    ...
    public function processPdf(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        ini_set('max_execution_time', '600'); // Optional: Increase max script execution time
        ...
        $directoryUrlForward=\Config::get('constant.PDF2PDF_DirUrlForward');
        $directoryUrlBackward=\Config::get('constant.PDF2PDF_DirUrlBack');
        $serverName=\Config::get('constant.DB_HOST');
        $dbUserName=\Config::get('constant.DB_UN');
        $dbPassword=\Config::get('constant.DB_PW');
        ...
        if(in_array($subdomain[0], $awsS3Instances)){ 
            $awsS3InstancesStr=implode('#', $awsS3Instances);
        ...
        $systemConfig = $get_system_config_data[0]['sandboxing'];
        ...
        $template_id = $_POST['template_id'];
        $pdf_page = $_POST['pdf_page'];
        $pdf_flag = (isset($_POST['pdf_flag']))? $_POST['pdf_flag'] : "";
        $entry_type="Fresh";
        $response = 0;
        $progressfile=$_POST['progress_file'];
        $previewPdf=(isset($_POST['previewPdf'])&&$_POST['previewPdf']!="undefined")?$_POST['previewPdf']:0;
        ...
        if($pdf_page=="Single"){
            ...
            if($subdomain[0]=="demo" || $subdomain[0]=="test"){
                if($get_system_config_data[0]['sandboxing'] == '1') {
                    $target_path = public_path().'/'.$subdomain[0].'/uploads/data/sandbox/';
                } else {
            ...
            if(in_array($file_extension,$image_ext)){
              ...
              if(move_uploaded_file($_FILES['file']['tmp_name'],$location)){
                ...
                if($pdf_flag==''){
                    ...
                    //  if($subdomain[0]=="pjlcp") {
                    //     $pyscript = $directoryUrlBackward."Python_files\\extract_and_place.py";

                    // }else
                    if(($subdomain[0]=="demo"||$subdomain[0]=="newserver"|| $subdomain[0]=="mmk") && $previewPdf == 1) {
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_preview_rohit.py";
                    }else if($subdomain[0]=="newserver"){                      
                       // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_niit_sikkim.py";
                       //  $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket.py";
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_compressed_test.py";    
                    }else if($subdomain[0]=="test1"){                      
                       // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_niit_sikkim.py";
                         $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_test2.py";
                    }else if($subdomain[0]=="icat"){                      
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place.py";
                    }else if(in_array($subdomain[0], $awsS3Instances)){                      
                       // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_niit_sikkim.py";
                         $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket.py";
                    
                    }else if($subdomain[0]=="mpkv"){
                        //  $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_mpkv.py"; 29-07-2025
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_mpkv_v1.py";
                    
                    }
                    // else if($subdomain[0]=="konkankrishi" && $template_id==9){
                    //      $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_v1.py";
                    
                    // }
                    else if($subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi" ||$subdomain[0]=="abyssinia" ||$subdomain[0]=="alarduni"){
                        // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain.py"; 29-07-2025
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_v1.py"; 
                         
                    
                    }else if($subdomain[0]=="newserver"||$subdomain[0]=="test"){
                        //  $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain.py"; 29-07-2025
                         $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_v1.py";
                    }
                    /*else if($subdomain[0]=="kessc" || $subdomain[0]=="mitwpu" || $subdomain[0]=="lnctbhopal" || $subdomain[0]=="wilson" || $subdomain[0]=="ghribmjal" || $subdomain[0]=="ghrietn"){
                        //echo "Asd";
                         $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_kessc.py";
                    }*/
                    else if($subdomain[0]=="council"){                      
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_council.py";
                    }else if($subdomain[0]=="demo"||$subdomain[0]=="newserver"){                      
                        // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain.py"; 29-07-2025
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_v1.py";
                    }
                    else if($subdomain[0]=="po"){                      
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_compressed_po.py";
                    }
                    else{
                        $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_compressed.py";
                        //$pyscript = $directoryUrlBackward."Python_files\\extract_and_place.py";
                    }
                ...
                if(($subdomain[0]=="demo"||$subdomain[0]=="newserver"||$subdomain[0]=="mmk") && $previewPdf == 1){
                    $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr $systemConfig $previewPdf 2>&1";
                }else if($subdomain[0]=="newserver"){
                    $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $systemConfig 2>&1";

                }else if($subdomain[0]=="demo"||$subdomain[0]=="newserver"||$subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi" || $subdomain[0]=="abyssinia" || $subdomain[0]=="alarduni"){
                    $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr $systemConfig $previewPdf 2>&1";
                }else if(in_array($subdomain[0], $awsS3Instances)){  
                $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr $systemConfig 2>&1";
     ...
     public function processPdfAgain(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($pdf_page=="Single"){
            ...
            /*else if($subdomain[0]=="kessc" || $subdomain[0]=="mitwpu" || $subdomain[0]=="lnctbhopal" || $subdomain[0]=="wilson" || $subdomain[0]=="ghribmjal" || $subdomain[0]=="ghrietn"){
                ...
                // $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain.py";
                $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_s3_bucket_blockchain_v1.py";
            }
            else{
                $pyscript = $directoryUrlBackward."Python_files\\extract_and_place_compressed.py";
            ...
            // if($subdomain[0]=="demo"||$subdomain[0]=="newserver"||$subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi"){
            //         $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr $systemConfig $previewPdf 2>&1";
            
            // }else if(in_array($subdomain[0], $awsS3Instances)){  
            //     $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr 2>&1";
            ...
            if(($subdomain[0]=="demo"||$subdomain[0]=="newserver"||$subdomain[0]=="mmk") && $previewPdf == 1){
                ...
                $cmd = "$pyscript $template_id $py_location $user_id $entry_type $progressfile $dbName $subdomain[0] $directoryUrlForward $directoryUrlBackward $serverName $dbUserName $dbPassword $site_id $user_name $printer_name $awsS3InstancesStr $systemConfig $compressionInstancesStr 2>&1";
            ...
            // exec($cmd, $output, $return);
            exec('C:/Users/Administrator/AppData/Local/Programs/Python/Python38/python.exe '.$cmd, $output, $return);
            // if($subdomain[0]=="konkankrishi"){
            //     print_r($output);
            // }
            // if($subdomain[0]=="abyssinia"){
            //     echo $cmd;
            //     echo "<br>";
            //     print_r($output);
            // }
    ...
    public function updateTitle(Request $request,$id){
        ...
        $id=$request->id;
        $bc_flag=$request->bc_flag;
        $bc_document_description=$request->bc_document_description;
        $bc_document_type=$request->bc_document_type;
        if($id != ''){ 
            if($bc_flag==1){
                //Blockchain Integration
                CoreHelper::checkContactAddress($id,'PDF2PDFTEMPLATE');                
				DB::select(DB::raw('UPDATE `uploaded_pdfs` SET template_title="'.$template_title.'", bc_document_description="'.$bc_document_description.'", bc_document_type="'.$bc_document_type.'", is_block_chain=1 WHERE id="'.$id.'"'));
			}else{
	...
	public function pdfPreview(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($pdf_page=="Single"){ 
            // if($subdomain[0] == 'pjlcp') {
            //     $pyscript = $directoryUrlBackward."Python_files\\preview_single_pjlcp.py";
            // } else {
    ...
    public function mintNonMintedRecords(Request $request){



           // $directoryUrlBackward=\Config::get('constant.PDF2PDF_DirUrlBack')."Python_files\\blockchain_session.py";
           //  //$directoryUrlBackward="C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\pdf2pdf\\Python_files\\mint_blockchain_non_minted_records.py";
           //  $cmd=$directoryUrlBackward;
           //  exec($cmd, $output, $return);

           //  print_r($output);
           //  exit;

   //  $student_data=DB::connection($mysql2)->table($dbName.".student_table")->where('template_id','7')->where('status','1')->get()
     //    ->toArray();
      //   $directoryUrlForward="C:/Inetpub/vhosts/seqrdoc.com/httpdocs/demo/public/pdf2pdf/";
        // $directoryUrlBackward="C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\pdf2pdf\\Python_files\\mint_blockchain_non_minted_records.py";
        // $cmd=$directoryUrlBackward;
        ...
        // $student_data=DB::connection($mysql2)->table($dbName.".student_table")->where('template_id','8')->where('status','1')->get()->toArray();
        ...
        // do{
            ...
            $directoryUrlBackward=\Config::get('constant.PDF2PDF_DirUrlBack')."Python_files\\mint_blockchain_non_minted_records.py";
            //$directoryUrlBackward="C:\\Inetpub\\vhosts\\seqrdoc.com\\httpdocs\\demo\\public\\pdf2pdf\\Python_files\\mint_blockchain_non_minted_records.py";
            $cmd=$directoryUrlBackward;
            // exec($cmd, $output, $return);
            exec('C:/Users/Administrator/AppData/Local/Programs/Python/Python38/python.exe '.$cmd, $output, $return);
        ...
        //     $student_data=DB::connection($mysql2)->table($dbName.".student_table")->where('template_id','7')->whereNull('bc_txn_hash')->where('status','1')->get()->toArray();
   ...
   public function getTextVerification(Request $request)
   {
        
      
       $id = $request->id;
      
       $data = TemplateMaster::find($id);
   
       if (!$data) {
           return response()->json(['error' => 'Template not found'], 404);
       }
   
       $extractorDetails = json_decode($data->extractor_details, true);
       $activeDetails = json_decode($data->active_text_verification, true);
   
       if (!is_array($extractorDetails)  ) {
           return response()->json(['error' => 'Invalid extractor or active details'], 400);
       }
   
       // Create a map of active names with is_status = 1
       $activeMap = [];
       if(is_array($activeDetails)){

       
       foreach ($activeDetails as $activeItem) {
           if (isset($activeItem['name'], $activeItem['is_status']) && $activeItem['is_status'] === 1) {
               $activeMap[$activeItem['name']] = true;
           }
       }
    }
   
       // Filter and map the extractor details
       $result = array_values(array_filter(array_map(function ($item, $index) use ($activeMap) {
           if (isset($item['text_verification']) && $item['text_verification'] === "use") {
               return [
                    'id'=> $item['id'],
                   'is_status' => isset($activeMap[$item['text_verification_lable'] ?? '']) ? 1 : 0,
                   'name' => $item['text_verification_lable'] ?? null,
                   'index' => $index // Include the index in the output
               ];
           }
           return null; // Explicitly return null for non-matching items
       }, $extractorDetails, array_keys($extractorDetails))));
   ...
   public function saveTextVerification(Request $request)
   {
       // Retrieve the TemplateMaster record
    //    $id = 134; // You may want to pass this dynamically
       $id = $request->id;
    //    dd($id);
       $data = TemplateMaster::find($id);
       ...
       // Get the selected names from the request
       $selectedNames = $request->input('dynamicCheckbox', []);
       $selectedId = $request->input('dynamicCheckboxId', []);

       // Prepare the new active_text_verification data
       $activeDetails = [];
   
       // Loop over the selected names and create the new data for active_text_verification
       foreach ($selectedNames as $index=>$name) {
           $activeDetails[] = [ 
               'id'=> $selectedId[$index],
               'name' => $name,
               'is_status' => 1, // Mark it as active
           ];
       }
   
       // Update the active_text_verification column
       $data->active_text_verification = json_encode($activeDetails);
   
       // Save the record
       $data->save();
   
       // Return a success response
       return response()->json(['message' => 'Text verification updated successfully'], 200);
   }
