...
class AdminManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            $where_str = "1 = ?";
			if($subdomain[0] == 'sgrsa'){		
				$admin_id = Auth::guard('admin')->user()->id;
				$role_id = Auth::guard('admin')->user()->role_id;  //1 Admin, 2 Sub Agent, 3 Supplier
...
$user_permission = DB::connection($new_connection)->query()
        ...
        $auth_current_permissions = UserPermission::where('user_id',Auth::guard('admin')->user()->id)->pluck('route_name')->toArray();

        return view('admin.adminManagement.edit',compact('user','roles','user_permission','role_current_permissions','auth_current_permissions','id'));
    }
    ...
    public function update(UserRequest $request, $id)
    {
        ...
        $user_details = $request->all();
        $generate_password = Hash::make($request->password);
    ...
    public function RouteList(){
        ...
        foreach ($routes as $value) {
            ...
            if ($route != '') {
                ...
                $route_save = AclPermission::on('mysql_new')->firstOrNew(['route_name'=>$route]);
                ...
                // // old server seqrdemo acl_permissions_group
                // $acl_permission = DB::connection('mysql_old')->table("acl_permissions_group")->select('sub_route')->where('sub_route',$route_save->route_name)->get()->toArray();
                // if(count($acl_permission)<=0){
                //          $aclGroup=array(); 
                //         $aclGroup['main_route'] = $route_save->route_name;
                //         $aclGroup['sub_route'] = $route_save->route_name;
                ...
                //         $aclGroup['sub_step_id'] = 0;
                //         $aclGroup['visibility_on'] = 1;
                //         DB::connection('mysql_old')->table("acl_permissions_group")->insert($aclGroup);
...
$user_permission = DB::connection($new_connection)->query()
	...
	public function AssignSupplier(){ 
        ...
        $fullname=$LA[0]->fullname;    
        $SupplierData=DB::select(DB::raw('SELECT id,company_name FROM `suppliers` WHERE publish=1 order by company_name')); 
        return view('admin.adminManagement.assignsupplier',compact('id','fullname','SupplierData','supplier_id'));
    }

	public function AssignSupplierSave(Request $request){         
        $id=$request->id;
        $supplier_id=$request->sbTwo;
		if($id != '' && $supplier_id != ''){
        DB::select(DB::raw("UPDATE `admin_table` SET supplier_id='".$supplier_id."' WHERE id='".$id."'"));
		}
        return redirect('admin/adminmaster/adminsupplier-assign/'.$id)->with('success','Record is saved successfully.');;
    }	
	...
	public function AssignAgent(){ 
		...
		$LA = DB::table('admin_table')->select('supplier_id','agent_id','fullname')->where('id', '=', $id)->get();
        $agent_id=$LA[0]->agent_id;
        $fullname=$LA[0]->fullname;  
		if($role_id == 3){
			$AgentData=DB::select(DB::raw('SELECT id,name FROM `agents` WHERE admin_id='.$admin_id.' and publish=1 order by name'));
		}else{
			$AgentData=DB::select(DB::raw('SELECT id,name FROM `agents` WHERE publish=1 order by name'));
		}	
         
		$supplier_id=$LA[0]->supplier_id;
		$SupplierData=DB::select(DB::raw('SELECT id,company_name FROM `suppliers` WHERE publish=1 order by company_name')); 
        return view('admin.adminManagement.assignagent',compact('id','fullname','AgentData','agent_id','SupplierData', 'supplier_id', 'role_id'));
    }

	public function AssignAgentSave(Request $request){         
        $id=$request->id;        
        $agent_id=$request->sbTwo;
		if($id != '' && $agent_id != ''){
			if($role_id == 3){
				$admin_id=Auth::guard('admin')->user()->id;        
				DB::select(DB::raw("UPDATE `admin_table` SET supplier_id='".$admin_id."', agent_id='".$agent_id."' WHERE id='".$id."'"));
			}else{
				$supplier_id=$request->supplier_id;
				DB::select(DB::raw("UPDATE `admin_table` SET supplier_id='".$supplier_id."', agent_id='".$agent_id."' WHERE id='".$id."'"));
			}
		}
        return redirect('admin/adminmaster/adminagent-assign/'.$id)->with('success','Record is saved successfully.');;
    }
}
