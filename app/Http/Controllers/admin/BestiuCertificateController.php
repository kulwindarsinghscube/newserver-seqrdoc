...
use App\Jobs\PdfGenerateBestiuJob5;
use App\Jobs\PdfGenerateBestiuJob6;
use App\Jobs\PdfGenerateBestiuJob7;
use App\Jobs\PdfGenerateBestiuJob8;
use App\Jobs\PdfGenerateBestiuJob9;
use App\Jobs\PdfGenerateBestiuJob10;
use App\Jobs\PdfGenerateBestiuJob11;
use App\Jobs\PdfGenerateBestiuJob12;
use App\Jobs\PdfGenerateBestiuJob13;
use App\Jobs\PdfGenerateBestiuJob14;
use App\Jobs\PdfGenerateBestiuJob15;
use App\Jobs\PdfGenerateBestiuJob16;
use App\Jobs\PdfGenerateBestiuJob17;
use App\Jobs\PdfGenerateBestiuJob18;
use App\Jobs\PdfGenerateBestiuJob19;
use App\Jobs\PdfGenerateBestiuJob20;

class BestiuCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.bestiu.index');
    }
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id==1 || $dropdown_template_id==2|| $dropdown_template_id==18|| $dropdown_template_id==3|| $dropdown_template_id==5 || $dropdown_template_id==6 || $dropdown_template_id==7|| $dropdown_template_id==11 || $dropdown_template_id==12 || $dropdown_template_id==15|| $dropdown_template_id==16 || $dropdown_template_id==17||$dropdown_template_id==19){
				...
				if( $dropdown_template_id==10|| $dropdown_template_id==13|| $dropdown_template_id==14){
					$rowData1 = $sheet1->rangeToArray('A1:' . $highestColumn1 . $highestRow1, NULL, TRUE, FALSE);
					unset($rowData1[0]);
				}
                //Commented by Mandar 25-06-2025
                ...
                if($dropdown_template_id==4){
                    ...
                    if($highestRow>=$startRow){ //added by mandar
				...
				if($dropdown_template_id==9){
					...
					$note_key=$last_col-3;
					$total_credit_points_key=$last_col-4;
					$total_credit_hours_key=$last_col-5;					
					$cgpa_key=$last_col-6;
					$sgpa_key=$last_col-7;
					$subj_col = 5;
					$subj_start = 7;        
                ...
                if($dropdown_template_id==8 || $dropdown_template_id==20){
					$worksheet = $objPHPExcel1->getSheet(0);
					$highestRow = $worksheet->getHighestRow(); 
					$highestColumn = $worksheet->getHighestColumn(); 
					$highestColumnIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($highestColumn); 
					...
					$cgpa_key=$last_col-4;
					$sgpa_key=$last_col-5;
					$credit_register_key=$last_col-6;
					$subj_col = 5;
					$subj_start = 12;        
					$subj_end = $subj_col*$course_counts;
				}
        ...
        $admin_id = \Auth::guard('admin')->user()->toArray();
		if($dropdown_template_id==4 || $dropdown_template_id==9|| $dropdown_template_id==3|| $dropdown_template_id==6|| $dropdown_template_id==7 || $dropdown_template_id==11|| $dropdown_template_id==12){
			$pdfData=array('studentDataOrg'=>$rowData1, 'note_key'=>$note_key, 'total_credit_hours_key'=>$total_credit_hours_key, 'total_credit_points_key'=>$total_credit_points_key, 'cgpa_key'=>$cgpa_key, 'sgpa_key'=>$sgpa_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
		} else if($dropdown_template_id==8 || $dropdown_template_id==20){
			$pdfData=array('studentDataOrg'=>$rowData1, 'note_key'=>$note_key, 'credit_register_key'=>$credit_register_key , 'total_credit_hours_key'=>$total_credit_hours_key, 'total_credit_points_key'=>$total_credit_points_key, 'cgpa_key'=>$cgpa_key, 'sgpa_key'=>$sgpa_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'pdf_flag'=>$pdf_flag,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
		}else{
			$pdfData=array('studentDataOrg'=>$rowData1, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1)); //For Custom Loader
        }
        ...
        elseif($dropdown_template_id==6){
            $link = $this->dispatch(new PdfGenerateBestiuJob6($pdfData));
        }
        elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateBestiuJob7($pdfData));
        }
        elseif($dropdown_template_id==8){
            $link = $this->dispatch(new PdfGenerateBestiuJob8($pdfData));
        }
        elseif($dropdown_template_id==9){
            $link = $this->dispatch(new PdfGenerateBestiuJob9($pdfData));
        }
        elseif($dropdown_template_id==10){
            $link = $this->dispatch(new PdfGenerateBestiuJob10($pdfData));
        }elseif($dropdown_template_id==11){
            $link = $this->dispatch(new PdfGenerateBestiuJob11($pdfData));
        }elseif($dropdown_template_id==12){
            $link = $this->dispatch(new PdfGenerateBestiuJob12($pdfData));
        }elseif($dropdown_template_id==13){
            $link = $this->dispatch(new PdfGenerateBestiuJob13($pdfData));
        }elseif($dropdown_template_id==14){
            $link = $this->dispatch(new PdfGenerateBestiuJob14($pdfData));
        }
        elseif($dropdown_template_id==15){
            $link = $this->dispatch(new PdfGenerateBestiuJob15($pdfData));
        } 
        elseif($dropdown_template_id==16){
            $link = $this->dispatch(new PdfGenerateBestiuJob16($pdfData));
        }
        elseif($dropdown_template_id==17){
            $link = $this->dispatch(new PdfGenerateBestiuJob17($pdfData));
        }elseif($dropdown_template_id==18){
            $link = $this->dispatch(new PdfGenerateBestiuJob18($pdfData));
        }  
        elseif($dropdown_template_id==19){
            $link = $this->dispatch(new PdfGenerateBestiuJob19($pdfData));
        }    
         elseif($dropdown_template_id==20){
            $link = $this->dispatch(new PdfGenerateBestiuJob20($pdfData));
        }
