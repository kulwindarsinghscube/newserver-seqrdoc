...
use Helper;
use App\Jobs\ValidateExcelChanakyaConvocationJob;
use App\Jobs\ValidateExcelChanakyaTranscriptJob;
use App\Jobs\ValidateExcelChanakyaPhdJob;
use App\Jobs\PdfGenerateChanakyaConvocation;
use App\Jobs\PdfGenerateChanakyaTranscriptJob;
use App\Jobs\PdfGenerateChanakyaPhdJob;
use App\Jobs\ValidateExcelChanakyaTranscriptNewJob;
use App\Jobs\PdfGenerateChanakyaTranscriptnewJob;
use App\Jobs\PdfGenerateChanakyaMbaTrimesterJob;
use App\Jobs\ValidateExcelChanakyaMbaTrimesterob;
use App\Jobs\PdfGenerateChanakyaUGWith6SemesterJob;
use App\Jobs\PdfGenerateChanakyaUG6SemLateralJob;

class ChanakyaertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.chanakya.index');
       //newrrmu
    }
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($dropdown_template_id == 1)
                {
                    ...
                    // dd($excelData);
                    $response = $this->dispatch(new ValidateExcelChanakyaConvocationJob($excelData));
                ...
                else if($dropdown_template_id == 2)
                {
                    ...
                    //dd($excelData);
                    $response = $this->dispatch(new ValidateExcelChanakyaTranscriptJob($excelData));
                 ...
                 else if ($dropdown_template_id == 3)
                {
                    ...
                    // 🔹 Dispatch a new validation job for the new template
                    $response = $this->dispatch(new ValidateExcelChanakyaTranscriptNewJob($excelData));
                    $responseData = $response->getData();
                } else if ($dropdown_template_id == 4) 
                {
                    // dd($dropdown_template_id);

                    // ===== Chanakya PhD Template =====
                    $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                    ...
                    $highestColumn1 = $sheet1->getHighestColumn();
                    $highestRow1 = $sheet1->getHighestDataRow();
                    $rowData1 = $sheet1->rangeToArray(
                        'A1:' . $highestColumn1 . $highestRow1,
                        NULL,
                        TRUE,
                        FALSE
                    );
                    ...
                    $response = $this->dispatch(
                        new ValidateExcelChanakyaPhdJob($excelData)
                    );

                    $responseData = $response->getData();
                }
                   else if ($dropdown_template_id == 5||$dropdown_template_id == 6||$dropdown_template_id == 7)
                {
                    $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                    $objPHPExcel1 = $objReader->load($fullpath);
                    $sheet1 = $objPHPExcel1->getSheet(0);
                    ...
                    $response = $this->dispatch(new ValidateExcelChanakyaMbaTrimesterob($excelData));
                    $responseData = $response->getData();
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                if($get_file_aws_local_flag->file_aws_local == '1'){
                    ...
                    $rowData1 = $sheet1->rangeToArray('A1:' . $highestColumn1 . $highestRow1, NULL, TRUE, FALSE);
                     ...
                     if($dropdown_template_id==1||$dropdown_template_id==2|| $dropdown_template_id == 5||$dropdown_template_id==6||$dropdown_template_id == 7){
                            ...
                            if($dropdown_template_id==2 || $dropdown_template_id==3|| $dropdown_template_id == 5||$dropdown_template_id==6||$dropdown_template_id == 7) {
                    ...
                    if ($dropdown_template_id == 3 || $dropdown_template_id == 4) {
                        $sheet2 = $objPHPExcel1->getSheet(1);
                        $rowData2 = $sheet2->toArray();
                    }
    ...
    //  dd( $rowData1);
        ...
        if($dropdown_template_id==1){
            ...
            $link = $this->dispatch(new PdfGenerateChanakyaConvocation($pdfData));
            ...
            // dd($rowData1);
         $pdfData=array('studentDataOrg'=>$rowData1, 'subjectsMark'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'no_of_semister'=>$no_of_semister,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));


            $link = $this->dispatch(new PdfGenerateChanakyaTranscriptJob($pdfData));
            ...
            $rowData2=array_values($rowData2);
            // dd($rowData2);
            $pdfData=array('studentDataOrg'=>$rowData1, 'subjectsMark'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'no_of_semister'=>$no_of_semister);

        //   dd($pdfData);
            $link = $this->dispatch(new PdfGenerateChanakyaTranscriptnewJob($pdfData));
        ...
        //   dd($pdfData);
            $link = $this->dispatch(new PdfGenerateChanakyaPhdJob($pdfData));
          ...
          // dd($rowData2);
            $pdfData=array('studentDataOrg'=>$rowData1, 'subjectsMark'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'no_of_semister'=>$no_of_semister,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));
            $link = $this->dispatch(new PdfGenerateChanakyaMbaTrimesterJob($pdfData));
            ...
            $rowData2=array_values($rowData2);
            $pdfData=array('studentDataOrg'=>$rowData1, 'subjectsMark'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'no_of_semister'=>$no_of_semister,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));
            $link = $this->dispatch(new PdfGenerateChanakyaUGWith6SemesterJob($pdfData));
            ...
            $rowData2=array_values($rowData2);
            $pdfData=array('studentDataOrg'=>$rowData1, 'subjectsMark'=>$rowData2,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'no_of_semister'=>$no_of_semister,'highestrow'=>(($highestRow1?$highestRow1:$highestRow)-1));
            $link = $this->dispatch(new PdfGenerateChanakyaUG6SemLateralJob($pdfData));
