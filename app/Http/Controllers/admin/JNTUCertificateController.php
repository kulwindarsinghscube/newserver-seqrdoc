...
use Helper;
use App\Jobs\ValidateExcelJNTUJob;
use App\Jobs\pdfGenerateJntuJob;
use App\Jobs\pdfGenerateJntuJob1;


class JNTUCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.jntu.index');
        //newrrmu
    }
    ...
    public function validateExcel(Request $request, CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if ($request->hasFile('field_file')) {
            ...
            //  dd($request['field_file']->move($target_path,$excelfile));           
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                // if ($dropdown_template_id == 1) {
                    ...
                    // dd($excelData);
                    $response = $this->dispatch(new ValidateExcelJNTUJob($excelData));
...
// dd($request->all());
        ...
        if ($dropdown_template_id == 2) {
            $pdfData = array('studentDataOrg' => $rowData1, 'auth_site_id' => $auth_site_id, 'template_id' => $template_id, 'dropdown_template_id' => $dropdown_template_id, 'previewPdf' => $previewPdf, 'excelfile' => $excelfile, 'loader_token' => $loader_token, 'highestrow' => ($highestRow1 - 1));


            $link = $this->dispatch(new pdfGenerateJntuJob1($pdfData));//12subject//
          ...
          elseif($dropdown_template_id==1){
            ...
            $link = $this->dispatch(new pdfGenerateJntuJob($pdfData));
        }
