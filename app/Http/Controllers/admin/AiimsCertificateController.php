...
use Helper;
use App\Jobs\ValidateExcelAiimsJob;

use App\Jobs\PdfGenerateAiimsJob;
use App\Jobs\PdfGenerateAiimsMBBSJob;
use App\Jobs\PdfGenerateAiimsCharacterJob;
use App\Jobs\PdfGenerateAiimsProvisionalMBBSJob;
use App\Jobs\PdfGenerateAiimsExperienceJob;
use App\Jobs\PdfGenerateAiimsInternshipJob;
use App\Jobs\PdfGenerateAiimsMigrateJob;
use App\Jobs\PdfGenerateAiimsBonafideJob;
use App\Jobs\PdfGenerateAiimsMBBSConvocationJob;
use App\Jobs\PdfGenerateAiimsMDConvocationJob;


class AiimsCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.aiims.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelAiimsJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader($inputFileType);
                /**  Load $inputFileName to a Spreadsheet Object  **/
                if($dropdown_template_id == "1" || $dropdown_template_id == "2" || $dropdown_template_id == "3" || $dropdown_template_id == "4" || $dropdown_template_id == "5" || $dropdown_template_id == "6" || $dropdown_template_id == "7" || $dropdown_template_id == "8" || $dropdown_template_id == "9" || $dropdown_template_id == "10"){
					$objPHPExcel1 = $objReader->load($fullpath);
					$sheet1 = $objPHPExcel1->getSheet(0);
					$highestColumn1 = $sheet1->getHighestColumn();
					$highestRow1 = $sheet1->getHighestDataRow();
		...
		if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateAiimsJob($pdfData));
        } 
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateAiimsMBBSJob($pdfData));
        }
        elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateAiimsCharacterJob($pdfData));
        }
        elseif($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateAiimsProvisionalMBBSJob($pdfData));
        }
        elseif($dropdown_template_id==5){
            $link = $this->dispatch(new PdfGenerateAiimsExperienceJob($pdfData));
        }
        elseif($dropdown_template_id==6){
            $link = $this->dispatch(new PdfGenerateAiimsInternshipJob($pdfData));
        } 
		elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateAiimsMigrateJob($pdfData));
        }
        elseif($dropdown_template_id==8){
            $link = $this->dispatch(new PdfGenerateAiimsBonafideJob($pdfData));
        } 
        elseif($dropdown_template_id==9){
            $link = $this->dispatch(new PdfGenerateAiimsMBBSConvocationJob($pdfData));
        }
        elseif($dropdown_template_id==10){
            $link = $this->dispatch(new PdfGenerateAiimsMDConvocationJob($pdfData));
        } 
    ...
    //   exit;
        ...
        for ($excel_row = 5; $excel_row <= 40; $excel_row++)//$highestRow
        {
           ...
           //  exit;
            ...
            // echo date('dmYHis');
            // exit;
            //$dt = "20250328184502";//date('dmYHis');//"10102023121802" //"21112023124500" //
           ...
           // print_r($check_data);
            ...
            if(!$check_dataFlag&&!empty($encryptedKey)&&!empty($certName)){
                ...
                if (file_exists($pdf_path)) {
                    ...
                    $mintData['description'] = "AIIMS Nagpur";
                    
                    $mintData['metadata1'] = ["label" => "Enrollment No", "value" => $rowData1[0][1]];
                    $mintData['metadata2'] = ["label" => "Name of Student", "value" => $rowData1[0][2]];
                    $mintData['metadata3'] = ["label" => "Name of Degree", "value" =>$rowData1[0][5]];
                    $mintData['metadata4'] = ["label" => "Subject", "value" => $rowData1[0][6]];
                    $mintData['metadata5'] = ["label" => "Exam Month/Year", "value" => $rowData1[0][7]];


                    //  $mintData['metadata1'] = ["label" => "Enrollment No", "value" => $rowData1[0][1]];
                    // $mintData['metadata2'] = ["label" => "Name of Student", "value" => $rowData1[0][2]];
                    // $mintData['metadata3'] = ["label" => "Name of Degree", "value" => "MBBS"];
                    // $mintData['metadata4'] = ["label" => "Exam Month/Year", "value" => $rowData1[0][9]];
                    // $mintData['metadata5'] = ["label" => "Internship Completion Month/Year", "value" =>$rowData1[0][7]];

                    $mintData['uniqueHash'] = $encryptedKey;

                    $mintData['pdf_file'] = $pdf_path;
                    $mintData['template_id'] = $template_id;
                    $mintData['bc_contract_address'] = "0x80c62d9e50c7F221fE31c46939165532699b09A2";
                    ...
                    if ($response['status'] == 200) {
                        ...
                        if (isset($response['pinataIpfsHash'])) {
                         ...
                         $resultu = StudentTable::where('id',$check_data->id)->update(['bc_txn_hash'=>$bc_txn_hash,'bc_ipfs_hash'=>$bc_ipfs_hash,'pinata_ipfs_hash'=>$pinata_ipfs_hash,"is_block_chain"=>1]);
