...
use Helper;
use App\Jobs\ValidateExcelSpitJob;
use App\Jobs\PdfGenerateSpitJob;
class SpitCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.spit.index');
    }
    ...
    public function pdfGenerate(){
        ...
        $bg_img="SPIT_BG.jpg"; //SPIT_BG, SPIT_BG_DATA
     ...
     public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelSpitJob($excelData));
                //print_r($response);
            ...
            $recordToGenerate = ($new_rows??0)+($old_rows??0);
    ...
    public function uploadfile(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $rowData1 = $sheet1->rangeToArray("A$startRow:" . $highestColumn1 . $endRow, NULL, TRUE, FALSE);
                }else{
                    $last_iteration = 1;
                    $rowData1=array(); 
                    $last_col=0;
                ...
                //$rowData2=$sheet2->toArray(); // get second worksheet rows
                // unset($rowData1[0]);
                // unset($rowData1[1]);
                // unset($rowData1[2]);
                // unset($rowData1[3]);
                ...
                //extract cell values from row 8
                $course=array();    
                $ctr_sgpa=0;
                $next_sgpa=1;
                for ($row = 6; $row <= 6; $row++) {
                ...
                if($next_sgpa > 8){
                    ...
                    $next_sgpa=$next_sgpa;
                    $remain_sgpa=8-$ctr_sgpa;
                }
                
                for ($row = 8; $row <= 8; $row++) {
                    for ($col = 1; $col <= $highestColumnIndex; ++$col) {
                        $value = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                        if($value){ $course[]=trim($value); }
                    }                   
                } 
                
                $counts = array_count_values($course);
                $course_counts = $counts['Course credits'];             
                $highestColumnSet=($course_counts+1)*5;
                $add_col=$course_counts*2; //code, course name
                $last_col=7+($course_counts)*5+$add_col;
                $start_key=$highestColumnIndex-2;
                //$cols_count_upto_totalcgp=$last_col+3;
                //$total_array_count=$highestColumnIndex+$add_col;                
                //$array_remain_cols=$total_array_count-$cols_count_upto_totalcgp;
                
                $course_code=array();
                $cc=8;
                for ($row = 6; $row <= 6; $row++) {
                    for ($col = 8; $col <= $highestColumnSet; ++$col) {
                        $value = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                        if($value){ $course_code[$cc]=trim($value); $cc+=5;}
                    }
                }
                //print_r($course_code);               
                $course_name=array();
                ...
                for ($row = 7; $row <= 7; $row++) {
                    for ($col = 8; $col <= $highestColumnSet; ++$col) {
                        $value = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                        if($value){ $course_name[$cn]=trim($value); $cn+=5; }
                ...
                $function_cols=array();
                $g=12;
                for ($row = 1; $row <= $course_counts; $row++) {
                    $function_cols[]=$g;
                    $g+=5;
                }
                //print_r($function_cols); exit;
                $records=array();
                $r=1;
                //extract cell values from row 9
                for ($row = $startRow; $row <= $endRow; $row++) {
                    ...
                    for ($col = 1; $col <= $highestColumnIndex; ++$col) {
                        if($c>=$vc && $vc != 5){ 
                            $value["code".$vc] = $course_code[$vc]; 
                            $value["subject_name".$vc] = $course_name[$vc];
                            if($vc==$highestColumnSet-2){
                                $vc=0;
                            } 
                            $vc+=5;
                        }
                        if (in_array($c, $function_cols)){
                            $value[$c] = $worksheet->getCellByColumnAndRow($col, $row)->getCalculatedValue();
                        }else{
                            $value[$c] = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                            //insert remaining SGPA
                            if($c==$start_key){
                                for ($scol = 1; $scol <= $remain_sgpa; ++$scol) {
                                    $value["SGPA".$next_sgpa] = '';
                                    $next_sgpa++;
                                }
                            }
                            /*if($c==7){
                                $date_value = $worksheet->getCellByColumnAndRow($col, $row)->getValue();                                
                                $timestamp =  \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($date_value); //excelToDateTimeObject
                                $exam_held = gmdate( 'd-m-Y', $timestamp ); //$exam_held = gmdate( 'F-Y', $timestamp );                                
                                $value[$c] =  $exam_held;
                            }else{
                                $value[$c] = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                            }*/
        ...
        // dd($rowData1);
        $total_course_credit_key=$last_col;
        $total_credits_earned_key=$last_col+1;
        $total_cgp_key=$last_col+2;
        $last_values = array_slice($rowData1[0], -2, 2, true); 
        $rowKey=array_keys($last_values);
        $sgpa_key_start=$rowKey[0]-8;
        $cgpa_key=$rowKey[0];
        $remark_key=$rowKey[1];
        $subj_col = 7;
        $subj_start = 7;        
        ...
        //,'subjectsMark'=>$rowData2
        $pdfData=array('studentDataOrg'=>$rowData1, 'total_course_credit_key'=>$total_course_credit_key, 'total_credits_earned_key'=>$total_credits_earned_key, 'total_cgp_key'=>$total_cgp_key, 'subj_col'=>$subj_col, 'subj_start'=>$subj_start, 'subj_end'=>$subj_end, 'sgpa_key'=>$sgpa_key_start, 'cgpa_key'=>$cgpa_key, 'remark_key'=>$remark_key, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>($highestRow1-1));
        \Log::info($pdfData);
        /* 1=Basic SPIT_BG_DATA */
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateSpitJob($pdfData));
        }
        ...
        return response()->json(['success'=>true,'message'=>'Certificates generated successfully.','link'=>$link,'startRow'=> $startRow,'endRow'=> $endRow,'highestRow'=> $highestRow1]);
    }
