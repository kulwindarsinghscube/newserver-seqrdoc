...
use Helper;
use App\Jobs\ValidateExcelEMeesevaCertificateUtilityJob;

use App\Jobs\PdfGenerateEMeesevaCertificateUtilityJob;
use App\Jobs\PdfGenerateEMeesevaCertificatePrinterJob;

class EMeesevaCertificateController extends Controller
{
    
    public function uploadpageprinter(){
        return view('admin.e_meeseva.printer'); 
    }
    ...
    public function validateExcelPrinter(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            $jsonArr['totalSecondsForGeneration'] =0;
            $loaderData=CoreHelper::createLoaderJson($jsonArr,1);            
            return response()->json(['success'=>true,'type' => 'success', 'message' => 'success','old_rows'=>0,'new_rows'=>0,'loaderFile'=>$loaderData['fileName'],'loader_token'=>$loaderData['loader_token']]);
    ...
    public function uploadfilePrinter(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateEMeesevaCertificatePrinterJob($pdfData));
    ...
    public function uploadpageutility(){
        return view('admin.e_meeseva.utility'); 
    }

    public function validateExcelUtility(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
       
        // exit;
        $template_id=100;
        $dropdown_template_id = $request['template_id'];
        if((!$request->hasFile('field_file_bg')&&empty($request['printer_name']))&&$dropdown_template_id!=3&&$dropdown_template_id!=4){
            return response()->json(['success'=>false,'type' => 'error', 'message'=>'Please select printer or upload backround pdf file only']);
        }
        $get_file_aws_local_flag = $checkUploadedFileOnAwsOrLocal->checkUploadedFileOnAwsORLocal();
        ...
        //         $destPath=public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$template_id.'/'.$excelfile;
        //         //$aws_excel = \File::copy($fullpath,$destPath);
        //         $destPath=$fullpath;
        //     }else{
        //          return response()->json(['success'=>false,'type' => 'error', 'message'=>'Json data file not uploaded']);
        //     }
                   
        // }else{
        //     switch($dropdown_template_id){
        //         case '1' : 
        //             $fileName="NonJudicial.json";
        //             break;
        //         case '2' : 
        ...
        //             $fileName="NonJudicial.json";
        //     }
        //     $destPath =public_path().'/'.$subdomain[0].'/backend/sample_excel/'.$fileName;
        ...
        // $response = $this->dispatch(new ValidateExcelEMeesevaCertificateUtilityJob($excelData));
        //print_r($response);
        // $responseData =$response->getData();
    ...
    public function uploadfileUtility(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        $get_file_aws_local_flag = $checkUploadedFileOnAwsOrLocal->checkUploadedFileOnAwsORLocal();
        ...
        $template_id = 100;
        $dropdown_template_id = $request['template_id'];
        $template_printer_name=$request['printer_name'];
        /* 1=Basic */
        $previewPdf = array($request['previewPdf'],$request['previewWithoutBg']);
        ...
        // if($request->hasFile('field_file_bg')){
        //     //check extension
        //     $file_name = $request['field_file_bg']->getClientOriginalName();
        //     $ext = pathinfo($file_name, PATHINFO_EXTENSION);
        ...
        //     $target_path = public_path().'/backend/canvas/dummy_images/'.$template_id;
        //     $fullpathPdf = $target_path.'/'.$pdffile;

        //     if(!is_dir($target_path)){
        //         mkdir($target_path, 0777);
        //     }

        //     if($request['field_file_bg']->move($target_path,$pdffile)){
        //         //get excel file data
        ...
        //         $destPath=public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$template_id.'/'.$pdffile;
        //         $aws_excel = \File::copy($fullpathPdf,$destPath);
        ...
        // }
        $auth_site_id=Auth::guard('admin')->user()->site_id;
        $bg_pdf_file=   public_path().'\\'.$subdomain[0]."/backend/canvas/bg_pdfs/e_Meeseva_Certificate_Bg.pdf";
        $excelfile = '';
        $admin_id = \Auth::guard('admin')->user()->toArray();
        // exit;
        $rowData1 = [];
        $pdfData=array('studentDataOrg'=>$rowData1,'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token, 'template_printer_name'=>$template_printer_name, "bg_pdf_file"=>$bg_pdf_file, 'subj_col' => '', 'subj_start'=> '', 'subj_end'=> ''); //For Custom Loader
        
        
        switch($dropdown_template_id){
            case '1' : 
                $link = $this->dispatch(new PdfGenerateEMeesevaCertificateUtilityJob($pdfData));
                break;
            default:
                $link = $this->dispatch(new PdfGenerateEMeesevaCertificateUtilityJob($pdfData));
        }
        ...
        if(!empty($template_printer_name)){
                ...
                if (!empty($result)) {
                    # Found a link.
                    //$base64= $result['href'][0];
                    // $base64Data = chunk_split(base64_encode(file_get_contents($result['href'][0])));
                    
                    $base64Data = 'data:application/pdf;base64,'.base64_encode(file_get_contents($result['href'][0]));

                    $base64DataEncy = $this->encrypt_data($base64Data,'G7b4y$D8kqT3pF9@H2LmX7wRzC5NvQ3!');

                    $base64 = $base64DataEncy;

                    // $base64Data = 'data:application/pdf;base64,'.base64_encode(file_get_contents($result['href'][0]));
                    // $encryptionKey = 'G7b4y$D8kqT3pF9@H2LmX7wRzC5NvQ3!';
                    // $encryptedData = $this->encrypt_data($base64Data, $encryptionKey);
                    
                    // $base64 = $encryptedData;

                    $fileName = basename($result['href'][0]);
                }else{
                    $base64="";
                    $fileName ="";
                }
        ...
        return response()->json(['success'=>true,'message'=>'Certificates generated successfully.','link'=>$link, 'base64'=>$base64,"fileName"=>$fileName]);
