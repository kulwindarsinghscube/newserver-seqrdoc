...
class OldDocumentsController extends Controller
{
						...
						if ($error) {
							...
							if (!empty($res)) {
								...
								if(empty($doc_year[$i])){
									...
									$doc_year_val = $doc_year[$i];
								}

								$result = VerificationDocuments::where('id',$document_id[$i])->update(['result_found_status'=>$resultFound[$i],'remark'=>$remark[$i],'exam_name'=>$exam[$i],'semester'=>$semester[$i],'doc_year'=>$doc_year_val,'last_updated_by'=>$last_updated_by]);
					...
					if ($refundStatus) {
						if (empty($pendingStatus)) {
							...
							if ($requestData) {
								...
								$requestData->offer_letter = (!empty($requestData->offer_letter)) ? '<a href="' . Config::get('constant.local_base_path').$subdomain[0] .'/backend/'. $requestData->offer_letter . '" target="_blank">Link</a>' : 'Not Uploded';
								$requestDetailData = array();
								foreach ($requestDetails as $readData) {
									$requestDetailData[] = array("document_type" => $readData->document_type,
										"document_path" => '<a href="' .  Config::get('constant.local_base_path').$subdomain[0].'/backend/' . $readData->document_path . '" target="_blank">Link</a>',
						...
						$this->sendMailCompleteVerificaitonAdmin($request_id);
	...
	function sendMailCompleteVerificaitonAdmin($request_id) {

		$domain = \Request::getHost();
	    $subdomain = explode('.', $domain);
		$requestData = (array)$this->getRequest($request_id);
		...
		if ($requestData) {
			...
			if ($requestData['verification_status'] == "Completed") {
				...
				$userData = User::where('id',$user_id)->first();
				$requestDetails = (array)$this->getRequestDetails($request_id);
				...
				//	exit;
					$requestDetailData[] = array("document_type" => $readData['document_type'],
						"document_path" => $baseUrl . $readData['document_path'],
						"result" => (!empty($readData['result_found_status'])) ? $readData['result_found_status'] : '-',
						"remark" => (!empty($readData['remark'])) ? $readData['remark'] : '-',
						"exam" => (!empty($readData['exam_name'])) ? $readData['exam_name_display'] : '-',
						"semester" => (!empty($readData['semester'])) ? $readData['semester_name_display'] : '-',
				...
				$params["offer_letter"] = (!empty($requestData['offer_letter'])) ? $baseUrl . $requestData['offer_letter'] : 'Not Uploded';
				/*print_r($userData);
				echo "<br>";
				print_r($requestData);
				echo "<br>";
				print_r($requestDetailData);*/

				 $params['request_number']=$requestData['request_number'];
				 $params['student_name']=$requestData['student_name'];
				 $params['institute']=$requestData['institute'];
				 $params['degree_name']=$requestData['degree_name'];
				 $params['branch_name_long']=$requestData['branch_name_long'];
				 $params['registration_no']=$requestData['registration_no'];//$userData['registration_no'];
				 $params['passout_year']=$requestData['passout_year'];
				 $params['name_of_recruiter']=$requestData['name_of_recruiter'];
				 $params['created_date_time']=$requestData['created_date_time'];
				 
				 $params['payment_transaction_id']=$requestData['payment_transaction_id'];
				 $params['amount']=$requestData['amount'];
				 $params['payment_date_time']=$requestData['payment_date_time'];

				 $params['requestDetailData']=$requestDetailData;

				// print_r($params);
				 if($subdomain[0] == "galgotias"){
				 $mail_view = 'mail.verify.complete_verification_admin_galgotias';
				 }else{
				 $mail_view = 'mail.verify.complete_verification_admin';

				 }
				 
				
				 $mail_subject = '#' . $params['request_number'] . ' Education verification details';
                 $user_email = $userData['email_id'];//'dev12@scube.net.in' 

				 //$this->dispatch(new SendMailJob($mail_view,$user_email,$mail_subject,$params));

                 Mail::send($mail_view, ['user_data'=>$params], function ($m) use ($user_email,$mail_subject){
			                    $m->to($user_email)->subject($mail_subject)->cc(array('principal.ghrce@raisoni.net','registrar_ghrce@raisoni.net'));
			                });
...
*/
		...
		$gotData = DB::select(DB::raw("SELECT DISTINCT vr.branch,bm.branch_name_long FROM verification_requests AS vr INNER JOIN branch_master AS bm ON vr.branch=bm.id WHERE vr.payment_status='Paid'". $extraWhere));

/*echo "SELECT tm.template_name,count(*) as totalCount,sum(sd.amount) as totalAmount FROM scanning_requests as sr INNER JOIN scanned_documents as sd ON sr.id=sd.request_id INNER JOIN student_table as st ON sd.document_key = st.key INNER JOIN template_master as tm ON st.template_id=tm.id INNER JOIN transactions as tr ON sr.request_number=tr.student_key AND tr.trans_status=1 WHERE sr.payment_status='Paid' " . $extraWhere."group by tm.template_name";		
exit;*/
/*SELECT DISTINCT vr.branch,bm.branch_name_long FROM verification_requests AS vr INNER JOIN branch_master AS bm ON vr.branch=bm.id WHERE vr.payment_status='Paid'*/
		...
		if (!empty($gotData)) {
			...
			if (isset($_POST['fromDate']) && isset($_POST['toDate']) && !empty($_POST['fromDate']) && !empty($_POST['toDate'])) {
				...
				$extraWhereSub .= " AND (DATE(created_date_time) >= '" . $fromDate . "' AND DATE(created_date_time) <= '" . $toDate . "')";
			...
			foreach ($gotData as $row) {
				$wrsheet->SetCellValueByColumnAndRow(1, $i, $recordCount);
				$wrsheet->SetCellValueByColumnAndRow(2, $i, $row->branch_name_long);
				
				


				$gotDataDetails = DB::select(DB::raw("SELECT count(*) as totalCount,sum(total_amount) as totalAmount FROM verification_requests  WHERE payment_status='Paid' AND branch='" . $row->branch . "'" . $extraWhereSub));
				//print_r($gotDataDetails);
				//exit;
				//$sql = $GLOBALS['conn']->prepare("SELECT count(*) as totalCount,sum(total_amount) as totalAmount FROM verification_requests  WHERE payment_status='Paid' AND branch='" . $branch_id . "'" . $extraWhere);

				$wrsheet->SetCellValueByColumnAndRow(3, $i, $gotDataDetails[0]->totalCount);
				$wrsheet->SetCellValueByColumnAndRow(4, $i, $gotDataDetails[0]->totalAmount);
				$i++;
				if ($i == 65537) {
					$headerIndex = 1;
	...
	public function testEmail2(){
		...
		// exit;
				...
				//$params=array();
                 ...
                 Mail::send($mail_view, ['user_data'=>$params], function ($m) use ($user_email,$mail_subject){
			                    $m->to($user_email)->subject($mail_subject)->cc(array('neelam.ak@vibrantscreen.com','principal.ghrce@raisoni.net','registrar_ghrce@raisoni.net'));
			                });
				// $user_email="as@scube.net.in";
				// Mail::send($mail_view, ['user_data'=>$params], function ($m) use ($user_email,$mail_subject){
			    //                 $m->to($user_email)->subject($mail_subject)->cc(array('gmandar4@gmail.com'));
			    //             });
