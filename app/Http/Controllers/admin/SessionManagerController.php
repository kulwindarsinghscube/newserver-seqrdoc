        ...
        if($request->ajax()){
            ...
            if (!empty($request->input('sSearch')))
            {
                ...
                if($subdomain[0] == 'mallareddyuniversity') {
                    $where_str .= " or (user_table.organization_name like \"%{$search}%\" )";
                }

                                    // . " or user_table.organization_name like \"%{$search}%\"" 
           ...
           DB::statement(DB::raw('set @rownum=0'));   
            if($status==1)
            {
                ...
                if($subdomain[0] == 'mallareddyuniversity') {
                    $columns = [DB::raw('@rownum  := @rownum  + 1 AS rownum'),'user_table.username as name','user_table.fullname as fullname','user_table.organization_name as organization','session_manager.ip','session_manager.login_time','session_manager.logout_time','session_manager.device_type','session_manager.id'];
                } else {
            ...
            foreach ($session_manager_list->toArray() as $key => $value) {
                ...
                $session_manager_list[$key]['fullname'] = $check_name['fullname'];
                                    $session_manager_list[$key]['organization'] = $value['organization'];
