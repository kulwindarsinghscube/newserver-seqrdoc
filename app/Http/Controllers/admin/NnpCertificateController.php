...
use Helper;
use App\Jobs\ValidateExcelNnpJob;
use App\Jobs\PdfGenerateCSCACS2Job1;
use App\Jobs\PdfGenerateNnpJob;


class NnpCertificateController extends Controller
{
    public function index(Request $request)
    {
        return view('admin.nnp.index');
    }
    ...
    public function validateExcel(Request $request, CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if ($request->hasFile('field_file')) {
            ...
            if ($request['field_file']->move($target_path, $excelfile)) {
                ...
                $response = $this->dispatch(new ValidateExcelNnpJob($excelData));
                //print_r($response);
    ...
    public function uploadfile(Request $request, CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if ($dropdown_template_id == 1) {
            $link = $this->dispatch(new PdfGenerateCSCACS2Job1($pdfData));
        } elseif ($dropdown_template_id == 2) {
            // dd($dropdown_template_id);

            $link = $this->dispatch(new PdfGenerateNnpJob($pdfData));
        }
