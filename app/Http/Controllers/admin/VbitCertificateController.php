...
use Helper;
use App\Jobs\ValidateExcelVbitJob;
use App\Jobs\PdfGenerateVbitJob;
use App\Jobs\PdfGenerateVbitJob2;
use App\Jobs\PdfGenerateVbitJob3;
use App\Jobs\PdfGenerateVbitJob4;
use App\Jobs\PdfGenerateVbitJob5;
use App\Jobs\PdfGenerateVbitJob6;
use App\Jobs\PdfGenerateVbitJob7;
use App\Jobs\PdfGenerateVbitJob8;
class VbitCertificateController extends Controller
{
    public function index(Request $request)
    {
       return view('admin.vbit.index');
    }
    ...
    public function validateExcel(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal){
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $response = $this->dispatch(new ValidateExcelVbitJob($excelData));
                // print_r($response);
      ...
      // $start_memory = memory_get_usage();
        ...
        if($request->hasFile('field_file')){
            ...
            if($request['field_file']->move($target_path,$excelfile)){
                ...
                $highestColumn1 = $sheet1->getHighestColumn();
                $highestRow1 = $sheet1->getHighestDataRow();
                // \Log::debug("data : -$startRow, $endRow,$highestRow1");
                ...
                // unset($rowData1[0]);
                // unset($rowData2[0]);
                // \Log::debug("data",['data'=>$rowData1,'data1'=>$rowData2]);
                if($subdomain[0]=="newserver"){
                    $objPHPExcel1->disconnectWorksheets();
                    unset($objPHPExcel1);
                }
        ...
        $pdfData=array('studentDataOrg'=>$rowData1,'subjectsMark'=>$rowData2, 'auth_site_id'=>$auth_site_id,'template_id'=>$template_id,'dropdown_template_id'=>$dropdown_template_id,'previewPdf'=>$previewPdf,'excelfile'=>$excelfile,'loader_token'=>$loader_token,'highestrow'=>($highestRow1-1)); //For Custom Loader
        /* 1=Memorandum, 2=Consolidated, 3=UG-CGM, 4=UG Memorandum, 5=Provisional Certificate */
        if($dropdown_template_id==1){
            $link = $this->dispatch(new PdfGenerateVbitJob($pdfData));
        }
        elseif($dropdown_template_id==2){
            $link = $this->dispatch(new PdfGenerateVbitJob2($pdfData));
        }
        elseif($dropdown_template_id==3){
            $link = $this->dispatch(new PdfGenerateVbitJob3($pdfData));
        }
        elseif($dropdown_template_id==4){
            $link = $this->dispatch(new PdfGenerateVbitJob4($pdfData));
        }     
        elseif($dropdown_template_id==5){
            $link = $this->dispatch(new PdfGenerateVbitJob5($pdfData));
        }elseif($dropdown_template_id==6){
            $link = $this->dispatch(new PdfGenerateVbitJob6($pdfData));
        }elseif($dropdown_template_id==7){
            $link = $this->dispatch(new PdfGenerateVbitJob7($pdfData));
        } elseif($dropdown_template_id==8){
            $link = $this->dispatch(new PdfGenerateVbitJob8($pdfData));
        } 
