...
class DashboardAjaxController extends Controller
{
    ...
    public function appGrandCount() {
        ...
        $digital_cards = DB::table('student_table')
                        ->where('status', 1)
                        ->where('publish', 1)
                        ->where('site_id', $auth_site_id)
                        ->where('tpsdidoc_type', 'card')
                        ->whereNotNull('functional_user_id')
                        ->count();
        $inactive_func_users = DB::table('functional_users')
                            ->whereNull('PIN')
                            ->count();
        $active_func_users = DB::table('functional_users')
                            ->whereNotNull('PIN')
                            ->count();
                            // dd($active_func_users);
        }
        ...
        return response()->json([
            ...
            'digital_cards'=> ($subdomain[0]=='tpsdi')?$digital_cards:0,
            'inactive_func_users'=> ($subdomain[0]=='tpsdi')?$inactive_func_users:0,
            'active_func_users'=> ($subdomain[0]=='tpsdi')?$active_func_users:0,
        ],200);
    ...
    public function scanData(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal) {
        ...
        if($scan_data){
            ...
            foreach ($scan_data as $readData) {
                ...
                // !is_file($filePath) || 
                if(!file_exists($local_server_qr_path)){                
