    ...
    public function getMonthlyConsumptionData(Request $request){
        ...
        foreach ($data as $key => $value) {
            ...
            if(!empty($year)&&!empty($month)){
                $active=DB::connection($dbCred)->table($dbName.".student_table")->where('status','1')->whereYear('created_at', '=', $year)
                ->whereMonth('created_at', '=', $month)->where('status','1')->count();
                $in_active=DB::connection($dbCred)->table($dbName.".student_table")->where('status','0')->whereYear('created_at', '=', $year)
                ->whereMonth('created_at', '=', $month)->count();
            
            }else{
                $active=DB::connection($dbCred)->table($dbName.".student_table")->where('status','1')->count();
                $in_active=DB::connection($dbCred)->table($dbName.".student_table")->where('status','0')->count();
            }
