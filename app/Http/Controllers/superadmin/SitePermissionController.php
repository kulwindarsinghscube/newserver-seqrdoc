...
//echo $readPermission;
    ...
    public function createTableDynamically($new_connection,$admin_data_array,$acl_permission_data,$super_admin_data_array,$system_config_data,$user_permission_data,$site_data,$site_id,$super_admin_data,$site_permission_data,$site_url,$rolePermissionData,$payment_gateway_data,$payment_gateway_config_data){
        ...
        //genration request table 
        \Artisan::call('migrate', ['--database' => $new_connection,'--path'=>'database/migrations/2024_11_19_002308_create_generation_request_table.php']);

        //create request student table
        \Artisan::call('migrate', ['--database' => $new_connection,'--path'=>'database/migrations/2024_11_19_012308_create_request_student_table.php']);

        //request printing details table 
        \Artisan::call('migrate', ['--database' => $new_connection,'--path'=>'database/migrations/2024_11_19_022308_create_request_printing_details_table.php']);
...
//print_r($role_permission_data);
    ...
    public function getFilesDetails(Request $request){
        ...
        $res = $client->get('https://'.$subdomain[0].'.seqrdoc.com/superadmin/getMigrateDataDetails?subdomain='.$sub_domain.'&type='.$type);
        
        if ($res->getStatusCode() == 200) { // 200 OK
            $response_data = $res->getBody()->getContents();
        }
        $data = json_decode(stripslashes($response_data));

        return response()->json(['success'=>true,'msg'=>'success',"data"=>$data]);
        
    }

    public function oldgetFilesDetails(Request $request){
    ...
    function getMigrateDataDetails(){
        $sub_domain = $_GET['subdomain'];
        $type = $_GET['type'];

        
        switch ($type) {
            case 'pdf_file':
                            $directory_pdf_file = public_path().'/'.$sub_domain.'/backend/pdf_file/';
                            $file_count_pdf_file = 0;
                            $files = glob($directory_pdf_file . "*.{pdf,PDF}",GLOB_BRACE);
                            $file_size_pdf_file=0;
                            if ($files){
                           ...
                           // echo $size;
                            ...
                            // AWS COUNT AND SIZE
                            $aws_directory_pdf_file = 'public/'.$sub_domain.'/backend/pdf_file/';
                            $disk = \Storage::disk('s3');
                            $list = $disk->allFiles($aws_directory_pdf_file);
                            $size = 0;
                            if(count($list) >0) {
            ...
            case 'pdf_inactive':
                                $directory_pdf_file = public_path().'/'.$sub_domain.'/backend/pdf_file/inactive_PDF/';
                                $file_count_pdf_file_inactive = 0;
                                $files = glob($directory_pdf_file . "*.{pdf,PDF}",GLOB_BRACE);
                                $file_size_pdf_file_inactive=0;
                                if ($files){
                                ...
                                // //AWS COUNT AND SIZE
                                $aws_directory_pdf_file = 'public/'.$sub_domain.'/backend/pdf_file/Inactive_PDF';
                                $disk = \Storage::disk('s3');
                                $list = $disk->allFiles($aws_directory_pdf_file);
                                $size = 0;
                                if(count($list) >0) {
            ...
            case 'pdf_live':
                            $directory_pdf_file = public_path().'/'.$sub_domain.'/backend/tcpdf/examples/';
                            $file_count_pdf_file_live = 0;
                            $files = glob($directory_pdf_file . "*.{pdf,PDF}",GLOB_BRACE);
                            $file_size_pdf_file_live=0;
                            if ($files){
                            ...
                            // AWS COUNT AND SIZE
                            $aws_directory_pdf_file = 'public/'.$sub_domain.'/backend/tcpdf/examples';
                            $disk = \Storage::disk('s3');
                            $list = $disk->allFiles($aws_directory_pdf_file);
                            $size = 0;
                            if(count($list) >0) {
            ...
            case 'pdf_preview':
                                $directory_pdf_file = public_path().'/'.$sub_domain.'/backend/tcpdf/examples/preview/';
                                $file_count_pdf_file_preview= 0;
                                $files = glob($directory_pdf_file . "*.{pdf,PDF}",GLOB_BRACE);
                                $file_size_pdf_file_preview=0;
                                if ($files){
                                ...
                                // AWS COUNT AND SIZE
                                $aws_directory_pdf_file = 'public/'.$sub_domain.'/backend/tcpdf/examples/preview';
                                $disk = \Storage::disk('s3');
                                $list = $disk->allFiles($aws_directory_pdf_file);
                                $size = 0;
                                foreach ($list as $file) {
            ...
            case 'templates':
                            $directory_pdf_file = public_path().'/'.$sub_domain.'/backend/templates/';
                            $templates=CoreHelper::getDirContents($directory_pdf_file);
                           ...
                           /* $file_count_pdf_file = 0;
                            ...
                            //AWS COUNT AND SIZE
                            $aws_directory_pdf_file = 'public/'.$sub_domain.'/backend/templates';
                            $disk = \Storage::disk('s3');
                            $list = $disk->allFiles($aws_directory_pdf_file);
                            $size = 0;
                            if(count($list) >0) {
            ...
            case 'database_log':
                        if($sub_domain == 'demo')
                        {
                            $dbName = 'seqr_'.$sub_domain;
                        }
                        else if($sub_domain == 'apponly'||$sub_domain == 'master')
                        {
                            $dbName = 'seqr_demo';
                        }
                        else{
                            $dbName = 'seqr_d_'.$sub_domain;
                        }
                        $result = DB::select(DB::raw('SELECT table_name AS "Table",
                            (data_length + index_length) AS "Size"
                            FROM information_schema.TABLES
