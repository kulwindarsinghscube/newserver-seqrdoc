...
use Helper;
class PdfGenerateNeelkanthJobApi
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('NEELKANTHDR');
        $card_serial_no=$cardDetails->next_serial_no;
        
        // echo  'ser '.$card_serial_no;
        // die();
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            if($inputSource == 'Excel') {
                ...
                $customerId = trim($studentData['customer_id']);
                
                $name = trim($studentData['CUST_NAME']);
                $panNo = trim($studentData['panNopan_number']);

                $fdSrNo = trim($studentData['TDT_FDRECSERL']);

                $accPrefix = trim($studentData['TDT_SCHMCD']);
                $accNo = trim($studentData['TDT_FDACNO']);
                $recPrefix = trim($studentData['TDT_FDACSERL']);
                $recNo = trim($studentData['TDT_FDRECNO']);
                $nomineeName = trim($studentData['Nominee Name']);
                
                $depositeAmt = trim($studentData['TDT_ORGAMOUNT']);
                $depositeDate = trim($studentData['TDT_FDISSRNWDT']);
                //$depositePeriod = trim($studentData[11]);
                $depositeMonthPeriod = trim($studentData['TDT_PRDMN']);
                $depositeDayPeriod = trim($studentData['TDT_PRDDY']);
                $MaturityDate = trim($studentData['TDT_FDMATDT']);
                $intRate = trim($studentData['TDT_RTOFINT']);
                $MaturityAmt = trim($studentData['TDT_MATAMT']);
            } else {
           ...
           // $amountInword = "Rupees Five Lakhs Only";
            ...
            // echo $name;
            // echo "<br>";
            // echo $ghost_font_size;
            // echo "<br>";
            // die();
            // uncomment when live  below 4 lines
			...
			$pdf->setPageMark();
			$pdfBig->setPageMark();
            // end this section ---uncomment when live  below 4 lines
