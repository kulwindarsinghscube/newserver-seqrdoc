                ...
                if($studentData){
                    ...
                    if (!isset($decodedResponse['errors']) || !is_array($decodedResponse['errors'])) {
                        ...
                        foreach ($traits as $key => $trait) {
                            // print_r($trait['value']);
                            // echo "<br>";
                            // if (isset($trait['value']) && strtolower($trait['value']) === 'nan') {
                            //     unset($traits[$key]);
                            //     continue; // Skip rest of the loop for this trait
                            // }
                        ...
                        if (($subdomain[0] ?? '') === 'anu') {
                            ...
                            $mainUrl    = "{$protocol}://{$domain}/{$subdomain[0]}/backend/pdf_file/{$studentData['certificate_filename']}";
                            $fallbackUrl = "https://anu.seqrdoc.com/{$subdomain[0]}/backend/pdf_file/{$studentData['certificate_filename']}";


                            // Check if main URL exists
                            if ($this->urlExists($mainUrl)) {
                                $dataR['dirPdfUrl'] = $mainUrl;
                            } else {
                                $dataR['dirPdfUrl'] = $fallbackUrl;
                            }
