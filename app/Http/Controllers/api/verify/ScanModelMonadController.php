                   ...
                   //print_r($qrCodes);
                    if(!empty($user_id))
                    {
                        if (!empty($total_files_count) && !empty($qrCodes)&& ($verificationType=="marksheet" || ($verificationType=="degree"&&$total_files_count==1)) && ($dataType=="qr" || $dataType=="serialno"))
                        {
                            ...
                            $scanning_request->total_amount= ($user_id==25)?1:$totalAmount;
                            ...
                            if (!empty($id)) {
                                ...
                                if (($totalAmountBackend == $totalAmount || $user_id==25) && $total_files_count == $totalFilesBackend ) {
                                    ...
                                    $message = array('status' => 200,'success' => true,'message' => 'Request submitted successfully.', "request_number" => $request_number, "request_id" => $id, "amount" => $totalAmount, 'showPdf' => $showPdf, 'dataPdf' => $dataPdf ,'url'=>"https://".$_SERVER['HTTP_HOST'].'/verify/payment/paytm');
                                }else{
