...
class PaymentScanController extends Controller
{
            ...
            if (1==1) 
            {
                ...
                if (!empty($studentData)) {
                    if($studentData['status']=='1'){
                        ...
                        if (isset($site['files_storage']) && isset($studentData['files_storage']) && 
                            $site['files_storage'] == 1 && $studentData['files_storage'] == 1) 
                        { 
                        ...
                        if (count($transaction)>=1) {
                            $payment_status = true;
                            $scan_data['payment_status'] = $payment_status;
                        ...
                        else
                        {
                            ...
                            if($paymentList){
                                foreach ($paymentList as $payment_key => $value) {
                                    ...
                                    if(!isset($studentData['student_name']) || empty($studentData['student_name']) || $$studentData['student_name'] == null){
                                        $studentData['student_name'] = 'Test Student';
                                    }
                                    ...
                                    if($pg_name == "paytm"){
                                        $png = \Config::get("constant.payment_image")."/paytm_btn.png";
                                        $url = url('verify/payment/new-paytm?UID='.$user_id.'&payment_from=mobile&key_payment='.$key);
                                        $paymentUrlArray []= array('pg_name'=>$pg_name,'pg_logo'=>$png,'pg_url'=>$url);
                                    }
                                    
                                    if($pg_name == "omniware"){
                                        $png = \Config::get("constant.payment_image")."/omniware_btn.png";
                                        $url = url('verify/payment/new-omniware?UID='.$user_id.'&payment_from=mobile&key_payment='.$key);
                                        $paymentUrlArray []= array('pg_name'=>$pg_name,'pg_logo'=>$png,'pg_url'=>$url);
                                    }

                                    if($pg_name == "paystack"){
                                        $png = \Config::get("constant.payment_image")."/paystack_btn.png";
                                        $url = url('verify/payment/new-paystack?UID='.$user_id.'&payment_from=mobile&key_payment='.$key);
                                        $paymentUrlArray []= array('pg_name'=>$pg_name,'pg_logo'=>$png,'pg_url'=>$url);
                                    }
                                    
                                    // if($pg_name == "payubiz"){
                                    //     $png = \Config::get("constant.payment_image")."/payu_btn.png";
                                    //     $url = url('webapp/rohit-payubiz/'.$key.'/'.$amount.'/'.$studentData['student_name']);
                                    //     $paymentUrlArray []= array('pg_name'=>$pg_name,'pg_logo'=>$png,'pg_url'=>$url);

                                    // }
                                    // if($pg_name == "payumoney"){
                                    //     $png = \Config::get("constant.payment_image")."/payu_money_btn.jpg";
                                    //     $url = url('webapp/rohit-payumoney/'.$key.'/'.$amount.'/'.$studentData['student_name']);
                                    //     $paymentUrlArray []= array('pg_name'=>$pg_name,'pg_logo'=>$png,'pg_url'=>$url);
                                    // }
                                ...
                                $scanndHistory->save();
                                return $message = array('success' => true,'status'=>200, 'message' => 'Add Payment gateway First for this instance');    
                            }
                            // return $message = array('success' => false,'status'=>400, 'message' => 'Success','data' => $paymentUrlArray);
                            $scanndHistory = new ScannedHistory();
                            $scanndHistory['date_time'] = $date;
                            $scanndHistory['device_type'] = $data['device_type'];
                            $scanndHistory['scanned_data'] = $key;
                            ...
                            $scanndHistory['document_status'] = $document_status;
                            $scanndHistory->save();
                            return $message = array('success' => true,'status'=>200, 'message' => 'Success','payment_data' => $paymentUrlArray,'data' => $scan_data);
