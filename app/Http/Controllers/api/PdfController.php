            ...
            if($type==1){
                    ...
                    if($subtype==1){
                      // print_r($subdomain);
                        /*$studentData = StudentTable::where('key',$key)
                                                ...
                                                ->orderBy('id','DESC')
                                                ->first();*/
                        $studentData = StudentTable::where('serial_no',$token)
                                                ->where('status',1)
                                                ->where('site_id',$site_id)
                                                ->orderBy('id','DESC')
                                                ->first(); 
                       ...
                       // if($studentData){
                            ...
                            if($subdomain[0]=="test"){
                                ...
                                //echo "abc11231";
                                  $content = \Storage::disk('s3')->get('public/'.$subdomain[0].'/backend/pdf_file/'.$studentData['certificate_filename']);
                                 //exit;
                                 //print_r($content);
                               //exit;
                                ...
                                //echo "xyz";
                                 $file = $storagePath.'/'.$subdomain[0].'/backend/pdf_file/'.$studentData['certificate_filename'];
                                //exit;
                       ...
                       /* }else{
                        ...
                        if($studentData){*/
                            if($subdomain[0]=="test"){
                                 $content = \Storage::disk('s3')->get('public/'.$subdomain[0].'/backend/pdf_file/sandbox/'.$studentData['certificate_filename']);

                                return response($content)->header('Content-Type', 'application/pdf');
                            }else{

                            $file = $storagePath.'/'.$subdomain[0].'/backend/pdf_file/sandbox/'.$studentData['certificate_filename'];
                            }
                        /*}else{
                            $file= $notFoundPdf;
                        }*/
                ...
                switch ($subtype) {
                    case '1':
                        ...
                        $file =$storagePath.'/'.$subdomain[0].'/backend/tcpdf/examples/'.$studentData['certificate_filename'];
                        // $file =$storagePath.'/'.$subdomain[0].'/backend/tcpdf/examples/'.$token.'.pdf';
                        break;
                    case '2':
                        $file =$storagePath.'/'.$subdomain[0].'/backend/tcpdf/examples/sandbox/'.$studentData['certificate_filename'];
                        // $file =$storagePath.'/'.$subdomain[0].'/backend/tcpdf/examples/sandbox/'.$token.'.pdf';
                        break;
                    case '3':
                        $file =$storagePath.'/'.$subdomain[0].'/backend/tcpdf/examples/preview/'.$studentData['certificate_filename'];
