...
use App\Jobs\ValidateExcelNeelkanthJob;
use App\Jobs\PdfGenerateNeelkanthJob;
use App\Jobs\PdfGenerateNeelkanthSCJob;
use App\Utility\ApiSecurityLayerNeelkanthBank;
use App\models\SessionManager;
use Illuminate\Support\Facades\Auth;
use App\models\Admin;
use App\models\NeelkanthBank;
...
class NeelkanthBankcertificate extends Controller
{
    public function login(Request $request){
        ...
        if(ApiSecurityLayerNeelkanthBank::checkAuthorization()){
            ...
            if ($validator->fails()) {
                  ...
                  $message = array('status' => 400, 'message' => ApiSecurityLayerNeelkanthBank::getMessage($validator->errors()));
                   return ApiSecurityLayerNeelkanthBank::apiResponse($message,false,$requestUrl,$requestMethod,$requestParameter);
            }else{
            ...
            // $res=Auth::guard('admin')->attempt($credential);
            // dd($res);
            if(Auth::guard('admin')->attempt($credential))
            {  
                $result = Auth::guard('admin')->user(); 
                $session_id = ApiSecurityLayerNeelkanthBank::generateAccessToken();
                ...
                $sessionData->device_type = 'neelkanthBank';
                $sessionData->ip = $ip;
                $sessionData->save();
                ...
                $message = array('status'=>200,'message'=>'You have logged in successfully.','data'=>$userData);  
                 return ApiSecurityLayerNeelkanthBank::apiResponse($message,array('accesstoken'=>$session_id),$requestUrl,$requestMethod,$requestParameter);
            ...
            }else
            {
             ...
             $message = array('status' => 403, 'message' => 'Access forbidden.1');
              return ApiSecurityLayerNeelkanthBank::apiResponse($message,false,$requestUrl,$requestMethod,$requestParameter);
        }
    ...
    public function generateDocuments(Request $request){
        ...
        if (ApiSecurityLayerNeelkanthBank::checkAuthorization()){    
            ...
            if(isset($request->inputSource)&&($request->inputSource=="Excel"||$request->inputSource=="JSON")){
                ...
                if($request->inputSource=="Excel"){
                    ...
                    $messages = [
                        ...
                        'generation_type.required' => 'Generation type is required',
            ...
            if ($validator->fails()) {
                ...
                if (!empty($user_id)) 
                {
                    $userData=ApiSecurityLayerNeelkanthBank::checkAccessTokenAdmin($user_id);
            ...
            if($userData){
                ...
                if($template_id=='NEELKANTHDR'||$template_id=='NEELKANTHSC'){
                ...
                $inProcesssGenerationCounts = NeelkanthBank::select('id')->where('template_id',$template_id)->where('status','In Process')->count();
                // dd($inProcesssGenerationCounts);
                ...
                $superAdminUpdate = SuperAdmin::where('property','print_limit')->update(['current_value'=>$studentTableCounts]);
                ...
                if(empty($customTemplateId)){
                    $templateCheck = TemplateMaster::where('id',$template_id)->first();
                        ...
                        if($request['file']->move($target_path,$excelfile)){
                            ...
                            switch ($customTemplateId) {
                                case 'NEELKANTHDR':
                                    ...
                                    $excelData=array('rowData1'=>$rowData1,'rowData2'=>$rowData2,'auth_site_id'=>$auth_site_id,'dropdown_template_id'=>$template_id,'type'=>'Excel');
                                                    $response = $this->dispatch(new ValidateExcelNeelkanthJob($excelData));
                                ...
                                case 'NEELKANTHSC':
                                    ...
                                    $excelData=array('rowData1'=>$rowData1,'auth_site_id'=>$auth_site_id,'type'=>'Excel');
                                    $response = $this->dispatch(new PdfGenerateNeelkanthSCJob($excelData));
                    ...
                    if(empty($customTemplateId)){
                        ...
                        switch ($customTemplateId) {
                            case 'NEELKANTHDR':
                                ...
                                if(!isset($jsonData->FdData)){
                                    $message = array('status'=>422,'message' => 'Please enter valid JSON data.');
                                    return ApiSecurityLayerNeelkanthBank::apiResponse($message,false,$requestUrl,$requestMethod,$requestParameter);
                                }

                                $excelData=array('rowData1'=>$jsonData->FdData,'auth_site_id'=>$auth_site_id,'dropdown_template_id'=>$template_id,"type"=>'JSON');
                                // dd($excelData);
                                $response = $this->dispatch(new ValidateExcelNeelkanthJob($excelData));
                ...
                if(!empty($customTemplateId)){
                    ...
                    $insertRequest = new NeelkanthBank();
                    ...
                    NeelkanthBank::where('id',$request_id)->update(["updated_at"=>$updated,"response"=>json_encode($message)]);
                    ...
                    $url = "http://".$domain."/api/seqrdoc-process-docs-neelkanth";
                    $ch = curl_init();
    ...
    public function processDocuments(){
       ...
       if(ApiSecurityLayerNeelkanthBank::checkAccessTokenAdmin($array['user_id'])){

        $pdf_data=$array['pdf_data'];
        $response = $this->dispatch(new PdfGenerateNeelkanthJob($pdf_data));
    ...
    public function processDocumentsCustomTemplates(){
        ...
        $check=ApiSecurityLayerNeelkanthBank::checkAuthorization();
        // dd($check);exit;
        // if (ApiSecurityLayerNeelkanthBank::checkAuthorization()){
           $json = file_get_contents('php://input');
           $request=json_decode($json);
        ...
        //    if(ApiSecurityLayerNeelkanthBank::checkAccessTokenAdmin($array['user_id'])){
                ...
                switch ($pdf_data['customTemplateId']) {
                    case 'NEELKANTHDR':
                        ...
                        $response = $this->dispatch(new PdfGenerateNeelkanthJob($pdfData));
                    ...
                    $pdfData=array('studentDataOrg'=>$studentDataOrg,'auth_site_id'=>$pdf_data['auth_site_id'],'template_id'=>$pdf_data['template_id'],'previewPdf'=>$pdf_data['previewPdf'],'excelfile'=>$pdf_data['excelfile'],'request_id'=>$pdf_data['request_id'],'generation_from'=>$pdf_data['generation_from'],'admin_id'=>$pdf_data['admin_id'],'call_back_url'=>$pdf_data['call_back_url']);
                        $link = $this->dispatch(new PdfGenerateNeelkanthSCJob($pdfData));
                        return $message = array('status' => 200, 'message' => 'success',"data"=>$response);
                    break;
                    default:
                        return $message = array('status' => 403, 'message' => 'Access forbidden4.');
                        break;
                }
            
            // }else
            // {
            //     return $message = array('status' => 403, 'message' => 'Access forbidden5.');
            // }
    
        // }else
        // {
        //     return $message = array('status' => 403, 'message' => 'Access forbidden6.');
        // }
    }
...
public function uploadfile(Request $request)
{
    ...
    // if ($request->hasFile('input_excel')) {
    //     dd('File exists:', $request->file('input_excel'));
    // } else {
    //     dd('No file received');
    // }
    ...
    $rules = [
        ...
        'generation_type' => 'required',
        'input_excel' => 'required|file|mimes:xlsx,xls',
    ];

    $messages = [
        'template_name.required' => 'Template is required',
        'generation_type.required' => 'Generation type is required',
        'input_excel.required' => 'Excel file is required',
        'input_excel.mimes' => 'The file must be an Excel file (xlsx or xls)',
    ];
    ...
    if ($validator->fails()) {
        ...
        $template_id = 100;
        $dropdown_template_id = $request['template_name'];
        
        if($request->hasFile('input_excel')){

            $file_name = $request['input_excel']->getClientOriginalName();
            $ext = pathinfo($file_name, PATHINFO_EXTENSION);

            //excel file name
            $excelfile =  date("YmdHis") . "_" . $file_name;
            $target_path = public_path().'/backend/canvas/'.$template_id;
            $fullpath = $target_path.'/'.$excelfile;

            if(!is_dir($target_path)){
                mkdir($target_path, 0777);
            }
            if($request['input_excel']->move($target_path,$excelfile)){
                if($ext == 'xlsx' || $ext == 'Xlsx'){
                    $inputFileType = 'Xlsx';
                }
                else{
        ...
        return response()->json(['msg' => 'File uploaded successfully'], 200);
    }
}
