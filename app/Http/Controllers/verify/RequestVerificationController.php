...
class RequestVerificationController extends Controller
{
    	...
    	if(isset($request['key_payment'])){
    		...
    		if (isset($request['payment_from']) && $request['payment_from'] == "mobile") {
    				...
    				if (isset($request['UID']) && !empty($request['UID'])) {
						...
						$paramList["INDUSTRY_TYPE_ID"] = 'Retail';
						$paramList["CHANNEL_ID"] = 'WEB';
						$paramList['CALLBACK_URL'] = $this->getPaystackSuccessUrl("mobile",$request['key_payment']);
    	...
    	$ORDER_ID = 'SeQR_PU_' . 1 . '_' . strtotime("now");
			...
			if(!empty($last_id)){
		       ...
		       $payment=array(
		          ...
		          'email' => $email_id,
		          'amount' => $amount,
		          'callback_url' => $this->getPaystackSuccessUrl($payment_from,$request['key_payment'])
		        );
		        return $payment;
	...
	public function getPaystackSuccessUrl($deviceType,$key_payment){
		...
		if ($isQrCodeverification == 1) {
			...
			if ($deviceType == "web") {
				...
				$url = "/verify/payment/paystack/response-success-qr?key=".$key_payment;
			} else {

				$url = "/verify/payment/paystack/response-success-mobile-qr?key=".$key_payment;
			}
		} else {
			if ($deviceType == "web") {

				$url = "/verify/payment/paystack/response-success?key=".$key_payment;
			} else {
				$url = "/verify/payment/paystack/response-success-mobile?key=".$key_payment;
	 ...
	 public function paystackResponseSuccess(Request $request){
        ...
        $user_id = Auth::guard('webuser')->user()->id;
       	...
       	return view('verify.payment.paystack_success',compact('inputInfo','session_key','verification_requests','request_number','user_id'));
    }

	    public function paystackResponseSuccessQR(Request $request){
        ...
        $email_id= Auth::guard('webuser')->user()->email_id;
        $mobile_no= Auth::guard('webuser')->user()->mobile_no;
       	return view('verify.payment.paystack_success_qr',compact('inputInfo','session_key','verification_requests','request_number','email_id','mobile_no','user_id'));
    	...
    	return view('verify.payment.paystack_success_mobile',compact('inputInfo','session_key','verification_requests','request_number','student_name','user_id'));
    }
	 ...
	 public function paystackResponseSuccessMobileQR(Request $request){
    	...
    	return view('verify.payment.paystack_success_mobile_qr',compact('inputInfo','session_key','scanning_requests','request_number','student_name','user_id'));
	...
	public function paytmTransStatusUpdatCron(Request $request){
		...
		foreach ($transactions as $readTransaction) {
		    ...
		    \Log::info('PaytmWallet Response on '.now()->format('Y-m-d H:i:s'), $response);
		    // echo "<pre>";print_r($response);
		    if ($status->isSuccessful() && $response && $response['STATUS'] === 'TXN_SUCCESS') {
		        $updates = [
		            // 'sr_no' => $readTransaction->sr_no,
		            'trans_id_gateway' => $response['TXNID'],
		            'payment_mode'     => $response['PAYMENTMODE'],
		            'amount'           => $response['TXNAMOUNT'],
		            'trans_status'     => 1,
		            'updated_at'       => $transaction_date,
		        ];
		        // dd($updates,$readTransaction);
		        $transaction=Transactions::where('sr_no', $readTransaction->sr_no)->update($updates);
		        if($transaction>0){
		        	$verification_requests = VerificationRequests::where('request_number',$readTransaction->student_key)->first();
		        	$scanning_requests = ScanningRequests::where('request_number',$readTransaction->student_key)->first();
			        if (!empty($verification_requests)) {
			        	$this->getverificationrequestdata($verification_requests->user_id,$readTransaction);
			        }else if (!empty($scanning_requests)) {
			        	// dd($readTransaction);
			        	$this->getscanningrequestdata($scanning_requests->user_id,$readTransaction);
			        }
		      	}
		    }
		}
	...
	public function getverificationrequestdata($user_id,$readTransaction){
		$domain = \Request::getHost();
	    $subdomain = explode('.', $domain);
	    $student_key=$readTransaction->student_key;
		VerificationRequests::where('request_number',$student_key)->update(['payment_status'=>'Paid']);//3024 "student_key" => "OCV-R-2806"
		$users = User::where('id',$user_id)->first();
			$params["email_id"] = $users['email_id'];
			$params["amount"] = $readTransaction->amount;
			$params["trans_id"] = $readTransaction->trans_id_ref;
			$params["date"] = $readTransaction->created_at;
			$params["key"] = $student_key;
		$requestData = DB::select( DB::raw("SELECT vr.*,b.branch_name_long,d.degree_name FROM verification_requests as vr
									INNER JOIN branch_master as b ON b.id=vr.branch
									INNER JOIN degree_master as d ON d.id=vr.degree
		...
		if (!empty($requestData)) {
            ...
            if($subdomain[0] == "raisoni"||$subdomain[0] == "demo"){
				...
				$mail_view_institute = 'mail.verify.raisoni_institute_req_recv';
				$mail_subject_institute = '#' . $params['key'] .' // '.$params["student_name"] .' // Candidate education details received for verification';
            
				$this->dispatch(new SendMailJob($mail_view_institute,$user_email,$mail_subject_institute,$params));
			}
		
	}
	public function getscanningrequestdata($user_id,$readTransaction){
		$student_key=$readTransaction->student_key;
		ScanningRequests::where('request_number',$student_key)->update(['payment_status'=>'Paid']);
		$array = explode('_', $readTransaction->trans_id_ref);
		if ($array[1] == 'PU') {
			$gate = 'PAYU MONEY';
		} else if ($array[1] == 'PT') {
			$gate = 'PAYTM';
		...
		$users = User::where('id',$user_id)->first();
			...
			$params["date"] = $readTransaction->created_at;
			$params["key"] = $student_key;
			$params["gateway_id"] = $readTransaction->trans_id_gateway;
			$params["gateway"] = $gate;
			$params["mode"] = $readTransaction->payment_mode;
			$params["name"] = $users['fullname'];
			if($subdomain[0] == "raisoni"||$subdomain[0] == "demo"){
				$mail_view = 'mail.verify.success_scan';
				$mail_subject = '#' . $params['key'] . ' Verification request fees payment received successfully ';
	...
	public function paytmTransStatusCheck(Request $request){
				...
				//refund code
			// 	$trans_id_ref='SeQR_PU_1_1752142289';
			// echo	$refundReferenceId = 'REFUND_' . uniqid();
			// 	$refund = PaytmWallet::with('refund');
			// 	//$status->prepare(['order' => $trans_id_ref]);
			// 	$refund->prepare([
			// 		'order' => $trans_id_ref,
			// 		'reference' => $refundReferenceId, // provide refund reference for your future reference (should be unique for each order)
			// 		'amount' => 500, // refund amount 
			// 		'transaction' => '20250710210540000150093635485207817' // provide paytm transaction id referring to this order 
			// 	]);
			...
			// 	die();
				// original code
				 $trans_id_ref='SeQR_PU_1_1756889113';
				$status = PaytmWallet::with('status');
			    $status->prepare(['order' => $trans_id_ref]);
			    $status->check();
