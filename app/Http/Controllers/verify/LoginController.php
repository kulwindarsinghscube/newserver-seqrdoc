    ...
    public function login(Request $request)
    {
        if (!empty($request->username) && !empty($request->password)) {
           ...
           /* $userData = Users::where('username',$username)
            ...
            $userData = Users::where(function ($query) {
            ...
            if(!empty($userData)) {
                ...
                if (Auth::guard('webuser')->attempt($credential1)) {
                    ...
                    if($userData['is_verified'] == 1){
                        ...
                        return response()->json(['status' => false,'message' => 'Verification pending! Redirecting to verification page in 5 seconds','UID'=>$userData['id'], 'mobile_no' => $userData['mobile_no'], 'code' => 300]);
                    }
                }else{
...
if ($id) {
                            $mail_data = Users::select('fullname', 'email_id', 'mobile_no','OTP')
                                    ->where('id', $id)
                                    ->first();

                            $data = ['fullname' => $mail_data['fullname'], 'OTP' => $mail_data['OTP']];
                            try {
                                    if ($request->student_verify_type == 'm') {
                                    $this->sendSMS($student_mob_no, "Your OTP for SeQR App Verification is " . $OTP . " - Team SSSL");
                                    } else {
                                    Mail::to($mail_data['email_id'])->send(new LoginOtpMail($data));
                                    //    Mail::to('dev1@scube.net.in')->send(new LoginMail($data));
                                    }

                                return response()->json(['status' => true, 'message' => 'Registration done successfully.', 'verification_method' => $verify_by, 'mobile_no' => $student_mob_no, 'code' => 200, 'verification_type' => $request->student_verify_type,'email_id'=>$mail_data['email_id']]);
            ...
            elseif($registration_type == 1)
            {
                ...
                if (!empty($employer_name) && !empty($employer_reg_no) && !empty($employer_working_sector) && (!empty($employer_address) || $subdomain[0]=="demo" || $subdomain[0]=="monad") && !empty($employer_mob_no) && !empty($employer_email)) {
                    ...
                    if (!$userData) {
                        ...
                        if ($id)
                        {
                            ...
                            $data = ['fullname' => $mail_data['fullname'], 'OTP' => $mail_data['OTP']];
                            // dd($mail_data);
                            //CoreHelper::sendSMS($employer_mob_no, "Your OTP for SeQR App Verification is " . $OTP." - Team SSSL");
                        try {

                                if ($request->emp_verify_type == 'm') {
                                    $this->sendSMS($employer_mob_no, "Your OTP for SeQR App Verification is " . $OTP . " - Team SSSL");
                                } else {
                                       Mail::to($mail_data['email_id'])->send(new LoginOtpMail($data));
                                    // Mail::to('dev1@scube.net.in')->send(new LoginMail($data));
                            ...
                            return response()->json(['status' => true, 'message' => 'Registration done successfully.', 'verification_method' => $verify_by, 'mobile_no' => $employer_mob_no, 'code' => 200, 'verification_type' => $request->emp_verify_type,'email_id'=>$mail_data['email_id']]);
            ...
            elseif($registration_type == 2)
            {
                ...
                if (!empty($agency_name) && !empty($agency_reg_no) && !empty($agency_working_sector) && (!empty($agency_address) || $subdomain[0]=="demo" || $subdomain[0]=="minad") && !empty($agency_mob_no) && !empty($agency_email)) {
                    ...
                    if (!$userData) {
                        ...
                        if ($id) {
                                    ...
                                    $data = ['fullname' => $mail_data['fullname'], 'OTP' => $mail_data['OTP']];
                            try {
                                    if ($request->agency_verify_type == 'm') {
                                    ...
                                    return response()->json(['status' => true, 'message' => 'Password has been sent to your registered email.', 'code' => 200, 'verification_method' => $verify_by, 'mobile_no' => $agency_mob_no, 'verification_type' => $request->agency_verify_type,'email_id'=>$mail_data['email_id']]);
    ...
    public function updateVerification(Request $request) {
        ...
        $mobile_no = (isset($request->mobile_no_verification)) ? $this->sanitizeVar($request->mobile_no_verification) : '';
        $otp = (isset($request->otp)) ? $request->otp : '';

        $otp = str_replace(',', '', implode(',', $otp));

        if (!empty($mobile_no) && !empty($otp)) {
            $userData = Users::where('mobile_no',$mobile_no)
                        ->where('publish', 1)
                        ->first();
...
$password = $this->genRandomStr(8);
                    ...
                    DB::table('user_table')
                        ...
                        ->update(['is_verified' => 1,'password'=>$pwd]);
$data = ['fullname'=>$userData['fullname'],'email_id' => $userData['email_id'],$password];

Mail::to($userData['email_id'])->send(new LoginMail($data));
                    return response()->json(['message' => 'Verified', 'status' => true]);   
                    
                }else{
                    return response()->json(['message' => 'You have entered wrong otp.', 'status' => false]);
                }
            }else if($userData && $userData['is_verified'] == 1){
                return response()->json(['message' => 'User is already verified.', 'status' => false]);
            }else{
                
                return response()->json(['message' => 'User not found with this mobile number.', 'status' => false]);
            }             
        }else{
            return response()->json(['message' => 'Data params missing', 'status' => false]);
        }
    }

    public function resend_otp(Request $request) {

            $mobile_no = (isset($request->mobile_no)) ? $this->sanitizeVar($request->mobile_no) : '';
            ...
            if (!empty($mobile_no)){
                    ...
                    if ($userData && $userData['is_verified'] == 0) {
                        ...
                        DB::table('user_table')
                                ->where('id',$userData['id'])
                                ->update(['otp' => $OTP]);
$data = ['fullname' => $userData['fullname'], 'OTP' => $OTP];

if($request->verification_type=="m"){
                        $this->sendSMS($mobile_no, "Your OTP for SeQR App Verification is " . $OTP . " - Team SSSL");
}else{
                    Mail::to($userData['email_id'])->send(new LoginOtpMail($data));
                }
                        return response()->json(['message' => 'OTP has been sent to your registered mobile number.', 'status' => true]);        

                    }else if($userData && $userData['is_verified'] == 1){
                        return response()->json(['message' => 'This user is already verified.', 'status' => false]);
                    }else{   
                        return response()->json(['message' => 'User not found with this mobile number.', 'status' => false]);
                    }            
    ...
    public function ForgotPassword(Request $request)
    {
        ...
        if (!empty($email_id)) {
            ...
            ->where('publish', 1)
            ->first();
          //  if($email_id=="erika.turpin@dciresources.com"||$email_id=="dev12@scube.net.in"){
               // print_r($userData);
               // exit;
         ...
         //   }
            if ($userData && $userData['is_verified'] == 1) 
            {
                ...
                try {
                        ...
                        //         Mail::to($email_id)->send(new LoginMail($data));
                        //     }else{
