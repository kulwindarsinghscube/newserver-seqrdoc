...
use App\Imports\KmtcDiplomaImport;
...
use ZipArchive;
class ConvoAdminKmtcController extends Controller
{
    ...
    {   
        ...
        return view('convodataverification.admin.kmtc.index',compact('other_student_completion_year','phd_student_completion_year', 'other_student_program','phd_student_program','other_student_specialization','phd_student_specialization','other_student_faculty','phd_student_faculty')); // Or the view you want to show
    ...
    public function uploadStudent(Request $request)
    {
        ...
        try {
            ...
            if($request->input('student_type') == 1){
                ...
                $student_import = new KmtcDiplomaImport();
            }
           //  dd($student_import);
    ...
    public function edit($id)
    {
        ...
        return view('convodataverification.admin.kmtc.edit', compact('is_transaction_failed','is_transaction_pending','student', 'payments', 'student_ack_logs','convo_student_logs'));
    }
    ...
    public function exportCustomStudent1(Request $request)
    {
        ...
        foreach ($facultyGroups as $facultyGroup) {
            ...
            $file_name_new =  str_replace(' ', "_", $facultyGroup['faculty_name']) . "_PART_1.xlsx";
            ...
            try {
                Excel::store(new ConvoCustomStudentExport($filters), "temp/$file_name_new", 'local'); // Store in the local disk under temp
            } catch (\Exception $e) {
        ...
        $subdomain = explode('.', $domain);
          foreach ($uploadedFiles as $file) {
              ...
              if (count($fileNameParts) > 1) {
                    ...
                    // If no hyphen is present, use the full original file name
                    $prn = pathinfo($originalFileName, PATHINFO_FILENAME);
                }


              $find = ConvoStudent::where('prn', $prn)->first(); 
              if ($find){
                   $storedFiles[] = $originalFileName;
                   $path = public_path() . '/' . $subdomain[0] . '/backend/students/';
                    ...
                    $result= $this->updatePKLFile($request,  $originalFileName);
                    $response = $result[3];
                    $response = str_replace("'", '"', $response);
                    $responseArray = json_decode($response, true);
                    //dd($responseArray['Message']);
                    $update = ConvoStudent::where('prn', $prn)->update(['is_face_encoded' => 1]);


              }
              else{
                $notStoredFiles[] = $originalFileName;
         ...
         return response()->json([
            ...
            'not_uploaded_files' => $notStoredFiles,
        ]);

    }

   public function updatePKLFile(Request $request, $storedFiles)
    {
        ...
        //     $subdomain[0] = 'mitwpu';
        // }
        $imageName=$storedFiles;
         $imagePath=public_path()."\\".$subdomain[0]."\backend\students\\".$imageName;
        //   $pklFile = 'G:\wamp64\www\uneb\public\openCV\known_faces.pkl';
        //  $python_script = 'G:\wamp64\www\uneb\public\openCV\update_pkl_file.py '.$pklFile." ".$imagePath;
        
        // $cmd = 'G:\wamp64\www\uneb\public\openCV\myenv\Scripts\python.exe '.$python_script.' 2>&1';

    
          $pklFile = 'C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\public\\'.$subdomain[0].'\openCV\known_faces.pkl';
         $python_script = 'C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\public\openCV\update_pkl_file.py '.$pklFile." ".$imagePath;
        
        $cmd = 'C:\inetpub\vhosts\seqrdoc.com\httpdocs\face_verification\Scripts\python.exe '.$python_script.' 2>&1';
        exec($cmd, $output, $return);
        //print_r($output);
        return $output;
    }

    public function nonVerifiedStudentData(Request $request){

         $domain = \Request::getHost();
        $subdomain = explode('.', $domain);
        $find = ConvoStudent::select('prn')->where('is_face_encoded', 0)->get(); 
        $imagePath = public_path() . "\\" . $subdomain[0] . "\\backend\\students\\"; 
        ...
        foreach ($find as $student) {
            ...
            foreach ($extensions as $extension) {
                $imageFileName1 = $student->prn . $extension;  
                $fullImagePath1 = $imagePath . $imageFileName1;
                if (file_exists($fullImagePath1)) {
                    
                    $imageFound = true; 
                     $imagename[] = [$student->prn => $imageFileName1,];
                     //$responsedata[]=[$student->prn => 'Image Not Found'];
                    break;
                }

            }
            if (!$imageFound) {
                $imageFileName2Pattern = $student->prn . '-'; 
                $files = scandir($imagePath); 
                $matchingFiles = array_filter($files, function ($file) use ($imageFileName2Pattern, $extensions) {
                    foreach ($extensions as $extension) {
                        if (strpos($file, $imageFileName2Pattern) === 0 && strpos($file, $extension) !== false) {
                            return true;
                        }
                    }
                    return false;
                ...
                if (count($matchingFiles) > 0) {
                    $matchedImage = reset($matchingFiles); 
                    // $imagename[0]=$matchedImage; 
                    //  $imagename[1]=$student->prn; 
                     $imagename[] = [$student->prn => $matchedImage,]; 
                                   
                } 
                else {
                    // If no image is found, add the PRN to the missingPrns array
                    $missingPrns[] = $student->prn;
                    $responsedata[]=[$student->prn => 'Image Not Found.'] ; 
                }
            }
        }
        //update pkl file for found image 
          foreach ($imagename as $imageData) {
            foreach ($imageData as $prn => $image) {
                $result = $this->updatePKLFile($request, $image);
                $response = $result[3];
                $response = str_replace("'", '"', $response);
                $responseArray = json_decode($response, true);
                //dd($responseArray);  // This will show the response array for debugging

                if ($responseArray['Status'] == 200) {
                    // Update the student record if the status is 200
                    $update = ConvoStudent::where('prn', $prn)->update(['is_face_encoded' => 1]);
                } else {
                    
                    $responsedata[] = [$prn => $responseArray['Message']];
                }
            }
        }
        // dd($responsedata);

         // Initialize arrays to store PRNs with different reasons
            $imagesNotFound = [];
            $dataNotProcessed = [];
            ...
            // Iterate over each PRN
            foreach ($prns as $prn) {
                $reasonFound = false; 

                // Iterate through the response data to find a matching reason
                foreach ($responsedata as $response) {
                    if (isset($response[$prn])) {
                        // Add the PRN to the respective array based on the reason
                        if ($response[$prn] == 'Image Not Found.') {
                            $imagesNotFound[] = $prn;
                        } else {
                            $dataNotProcessed[] = $prn;
                        }
                        $reasonFound = true;
                        break; // Exit inner loop once the reason is found
                    }
                }

                // If no reason was found for the PRN, add it to the "Data Not Processed" list
                if (!$reasonFound) {
                    $dataNotProcessed[] = $prn;
                }
            }
            ...
            $fileContent = "=================================================================\n";
            $fileContent .= "Below PRNs Images Not Found :\n";
            $fileContent .= json_encode($imagesNotFound) . "\n"; // Simple JSON without pretty print
            $fileContent .= "=================================================================\n";
            $fileContent .= "Below PRNs Data Not Processed for Face Verification:\n";
            $fileContent .= json_encode($dataNotProcessed) . "\n";

            // Define the file path where you want to store the text file
            $filePath = public_path('not_verified_images.txt');
