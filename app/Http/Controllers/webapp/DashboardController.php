...
public function paystackCron()
{
        ...
        // $settlementResponse = json_decode($response, true);
        // dd($settlementResponse);
        ...
        // // Example: Take first settlement
        // $settlement_id = $settlementResponse['data'][0]['id'] ?? null;
      ...
      $settlement_id = "1";
        ...
        if (!$settlement_id) {
            return "No settlement found";
        }

        /*
        ----------------------------------
        STEP 2 : Get Settlement Transactions
        ----------------------------------
        */
        $ch = curl_init("https://api.paystack.co/settlement/".$settlement_id."/transactions");
        ...
        if(isset($_POST['qrcode'])){
            ...
            if($student_count > 0){
                ...
                if($student['status'] == '0'){
                    ...
                    if($transaction > 0  || $subdomain[0]=="test" || $subdomain[0]=="vesasc" || $subdomain[0]=="lnctbhopal"|| $subdomain[0]=="surana"|| $subdomain[0]=="srit"||$subdomain[0]=="mitwpu"|| $subdomain[0]=="po" || $subdomain[0]=="kessc" || $subdomain[0]=="jssaher" || $subdomain[0]=="vbit" || $subdomain[0]=="bestiu" || $subdomain[0]=="iyc" || $subdomain[0]=="bmcc" ||$subdomain[0]=="anu" ||$subdomain[0]=="ghrcemp" ||$subdomain[0]=="ghruamravati" ||$subdomain[0]=="iscnagpur" ||$subdomain[0]=="ghrusaikheda" ||$subdomain[0]=="lnctindore" ||$subdomain[0]=="ghrietp" || $noPaymentGateway==1){
                    	if($subdomain[0]=="demo"){
    ...
    public function instaMojoResponse(Request $request){
     ...
     public function paystack(Request $request){
        ...
        $amount = $request->segment(4);
        $student_name = $request->segment(5);
       // dd($key,$amount,$student_name);
        return view('webapp.payment.paystackPost',compact('key','amount','student_name'));
...
{
    $requestData = $request->all();
    $amount = $requestData['amount']; // fallback amount
    $key    = $requestData['key'] ?? null;
    $user = \Auth::guard('webuser')->user();
    $email  = 'dev11@scube.net.in';
    $amountInKobo = $amount * 100; // Paystack expects amount in kobo

    // Save key for later use in callback
    Session::put('student_key', $key);

    $paymentGateway = PaymentGatewayNew::where('pg_name', 'paystack')->first();
    $paystackKey = $paymentGateway->test_salt;
    $pg_id = $paymentGateway->id;
    // Callback URL
    ...
    $ORDER_ID = 'SeQR_PS_'.strtotime("now");

    $transactions = new Transactions;
    $transactions->pay_gateway_id = $pg_id;
    $transactions->trans_id_ref = $ORDER_ID;
    $transactions->amount = $amount;
    // $transactions->payment_mode = 'paystack';
    $transactions->user_id = $user->id;
    $transactions->student_key = $key;
    ...
    if (!isset($result['status']) || $result['status'] !== true) {
        return redirect()->back()->with('error', $result['message'] ?? 'Unable to initialize Paystack payment');
    }
...
{
    $key  = Session::get('student_key');
    $userId = \Auth::guard('webuser')->user()->id;
 
    // $pg_id = 1;
    $paymentGateway = PaymentGatewayNew::where('pg_name', 'paystack')->first();
    $paystackKey = $paymentGateway->test_salt;
    // dd($paymentGateway->id);
    $pg_id = $paymentGateway->id;
    ...
    // $ORDER_ID = 'SeQR_PS_'.strtotime("now");
     $txn = Transactions::where('student_key', $key)->where('user_id',$userId)->where('trans_status',0)->first();
    ...
    $inputInfo = [
        ...
        'PAYMENTMODE'   => $result['data']['channel'],
        'TXNAMOUNT'     =>  $result['data']['amount'] / 100,
    ];
    ...
    return view(
        'webapp.payment.paystackStatus',
        compact('inputInfo', 'key', 'userId')
    );
...
// public function addPaystackTransaction($payment_params)
// {
