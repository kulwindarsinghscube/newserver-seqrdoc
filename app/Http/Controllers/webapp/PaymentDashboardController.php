...
class PaymentDashboardController extends Controller
{
    ...
    public function dashboard(Request $request){
    	...
    	return view('webapp.payment_dashboard.show');
            ...
            if($student_count > 0){
                ...
                if($student['status'] == '0'){
                    ...
                    if($transaction > 0){
                        ...
                        $flag = 1;
                        $selectColumns = ['payment_gateway_new.pg_name','payment_gateway_new_config.pg_id','payment_gateway_new_config.pg_status','payment_gateway_new_config.amount'];
    ...
    public function paytm(Request $request){
        ...
        return view('webapp.rohit_payment.payTmPost',compact('key','amount','student_name','paymentGateway'));
    }
    ...
    public function PayTmResponse(Request $request){
        ...
        return view('webapp.rohit_payment.payTmStatus',compact('inputInfo','session_key','user_id'));
    }


    // PayUGiz payment gateway
    public function PayUBiz(Request $request){
        $key = $request->segment(3);
        $amount = $request->segment(4);
        $student_name = $request->segment(5);
        $pg_id = $request->segment(6);
        ...
        $site_id = \Auth::guard('webuser')->user()->site_id;
        $paymentGateway = PaymentGatewayNew::where('id',$pg_id)->where('site_id',$site_id)->first();
        // $paymentGateway = PaymentGatewayNew::where('pg_name','PayuBiz')->where('site_id',$site_id)->first();

        return view('webapp.rohit_payment.payUBizPost',compact('key','amount','student_name','paymentGateway'));
    }


    public function PayUBizResponse(Request $request){
        ...
        return view('webapp.rohit_payment.payUbizStatus',compact('inputInfo','user_id'));
    }
    ...
    public function PayuMoney(Request $request){
        ...
        $payment_key = Session::put('payment_key',$key);
        $site_id = \Auth::guard('webuser')->user()->site_id;
        // $paymentGateway = PaymentGatewayNew::where('pg_name','payumoney')->where('site_id',$site_id)->first();
        $paymentGateway = PaymentGatewayNew::where('id',$pg_id)->where('site_id',$site_id)->first();
        return view('webapp.rohit_payment.payuMoneyPost',compact('key','amount','student_name','paymentGateway'));
    }
    ...
    public function PayuMoneyResponse(Request $request){
        ...
        return view('webapp.rohit_payment.payuMoneyStatus',compact('inputInfo','user_id'));
    }
    ...
    public function omniware(Request $request){
        $key = $request->segment(3);
        $amount = $request->segment(4);
        $student_name = $request->segment(5);
        $pg_id = $request->segment(6);
        ...
        return view('webapp.rohit_payment.omniwarePost',compact('key','amount','student_name','paymentGateway'));
    }

    public function omniwareResponse(Request $request){
        $user_id = \Auth::guard('webuser')->user()->id;
        $session_key = Session::get('payment_key');
        $inputInfo = $_POST;
        return view('webapp.rohit_payment.omniwareStatus',compact('inputInfo','session_key','user_id'));
    }
    ...
    public function instaMojo(Request $request){
        ...
        $payment_key = Session::put('payment_key',$key);
        $site_id = \Auth::guard('webuser')->user()->site_id;
        $paymentGateway = PaymentGatewayNew::where('pg_name','instamojo')->where('site_id',$site_id)->first();

        return view('webapp.rohit_payment.instaMojoPost',compact('key','amount','student_name','paymentGateway'));
    }
    ...
    public function paymentinstaMojo(Request $request){
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER,
                    array("X-Api-Key:5f2efba7d0e687ccc7f2b84f0209432d",
                        "X-Auth-Token:edca9294ebac6f771db43e94f7016cb3"));
        $payload = Array(
            'purpose' => 'FIFA 16',
            'amount' => '2500',
            'phone' => '9999999999',
        ...
        $payment = DB::table('payment_gateway_new')
                    ->select('*')
                    ->join('payment_gateway_new_config', 'payment_gateway_new.id', '=', 'payment_gateway_new_config.pg_id')
                    ->where('payment_gateway_new.pg_name', '=', 'instaMojo')
                    ->where('payment_gateway_new.site_id', '=', $site_id)
                    ->get();
        ...
        if(strtolower($payment[0]->payment_mode) == 'test')
        {
            $X_Api_Key = 'd82016f839e13cd0a79afc0ef5b288b3';
            $X_Auth_Token = '3827881f669c11e8dad8a023fd1108c2';
        ...
        $payload = Array(
            ...
            'phone' => $mobile_no,
            'buyer_name' => $fullname,
            'redirect_url' => url('webapp/payment-gateway/rohit-instamojo/response'),
            'email' => $email_id,
            'allow_repeated_payments' => false
        );
    ...
    public function instaMojoResponse(Request $request){
        ...
        return view('webapp.rohit_payment.instaMojoStatus',compact('response','session_key','user_id'));
    }



    // Easypay payment gateway
    public function eazypay(Request $request){
        ...
        if($scan_data['id'] > 0){
            ...
            curl_setopt($ch, CURLOPT_URL,"https://iitjmupg.seqrdoc.com/easypaywebapp.php");//http://seqronline.com/iitj/easypaywebapp.php ///https://iitjmupg.seqrdoc.com/easypaywebapp.php
