...
class BlockChain
{
	...
	{	
		...
		try {
			...
			for($i=1;$i<=5;$i++){
				...
				if(isset($data['metadata'.$i])){
					...
					$data['metadata'.$i] = array_map(function($value) {
							    return strval($value);
							}, $data['metadata'.$i]);

					$requestParameters['metadata'.$i]=json_encode($data['metadata'.$i],JSON_FORCE_OBJECT);	
				}else{
					$requestParameters['metadata'.$i]=json_encode(["label"=> "", "value"=> ""]);
				}
			}

			/*if($data['uniqueHash']=="CFD20A17F9C0CFF93D114844C52B7512"){
			print_r($filePath);
			exit;
			}*/
			...
			if($info['http_code'] === 200){
			    ...
			    if(isset($response['success'])){
			    	...
			    	if($response['success']==200){
			    		...
			    		$respArr['ipfsHash']=$response['ipfsHash'];
			    		$respArr['pinataIpfsHash']=$response['pinataIpfsHash'];
        ...
        $responseTime = microtime(true) - LARAVEL_START;
		// dd($requestUrl,$requestMethod,$requestParameters,$response,$status,$responseTime,"mintData");
	...
	{	
		...
		try {
			...
			if(isset($data['uniqueHash'])&&isset($data['walletID'])&&!empty($data['uniqueHash'])&&!empty($data['walletID'])){
				...
				// if($data['uniqueHash']=="3433C65331CFB83A101464F46E019E5D"){
				// 	dd($requestUrl);
				// }
				 ...
				 //   print_r($response);
				    ...
				    if(isset($response['success'])){
				    	...
				    	if($response['success']==200){
				    		...
				    		$respArr = Self::retreiveDetailsOld($mode,$data); 
				    		// $respArr['status']=$response['success'];
				    		// if(isset($response['message'])){
				    		// 	$respArr['message']=$response['message'];
				    		// }else{
	...
	public static function retreiveDetailsOld($mode,$data) 
	{	
