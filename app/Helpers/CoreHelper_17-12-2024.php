    ...
    public static function mintPDF($data){//NORMALTEMPLATE, PDF2PDFTEMPLATE
        ...
        if(!empty($siteData&&!empty($siteData['bc_wallet_address'])&&$data['pdf_file'])&&!empty($data['uniqueHash'])&&!empty($data['template_id'])){
            ...
            if($checkContact&&!empty($checkContact['bc_contract_address'])){
                ...
                if($subdomain[0]=="mpkv"||$subdomain[0]=="ksu"||$subdomain[0]=="konkankrishi"||$subdomain[0]=="mitwpu"){
                    $mode=1;//1:live
                }else if($data['template_id']==2||$data['template_id']==698||$data['template_id']==718||$data['template_id']==719||$subdomain[0]=="anu"||($subdomain[0]=="demo"&&$data['template_id']==137)){
                $mode=1;//1:live 0:testnet
                ...
                if($response&&$response['status']==200&&isset($response['txnHash'])&&!empty($response['txnHash'])){
                    ...
                    $response=["status"=>$response['status'],"txnHash"=>$response['txnHash'],"ipfsHash"=>$response['ipfsHash'],"pinataIpfsHash"=>$response['pinataIpfsHash']];
    ...
    public static function retreiveDetails($data){
        ...
        // dd($data);
         if(!empty($data['template_id'])&&!empty($data['walletID'])&&!empty($data['uniqueHash'])){
                ...
                }else if($data['template_id']==2||$data['template_id']==698||$data['template_id']==718||$data['template_id']==719||$subdomain[0]=="anu"||$subdomain[0]=="verification"||($subdomain[0]=="demo"&&$data['template_id']==137)){
