...
class PhdStudentsImport implements ToCollection, WithHeadingRow, WithValidation
{
    public $errors = [];
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            $topic_hindi = $this->textTranslator->transliterateToHindi(@$row['topic']);
            $topic_krutidev = $this->textTranslator->unicodeToKrutiDev($topic_hindi);
           ...
           if(!empty(@$row['completion_date'])){
            ...
            //   echo  $issue_date  = $this->parseDate($row['issue_date']);
            //   die();

            $issue_date = date('Y-m-d',strtotime($this->parseDate($row['issue_date'])));
            // dd($issue_date);
            $issue_date=false;
    ...
    'cgpa' => 0.00, // required field
    'cgpa_hindi' => null,
    'cgpa_krutidev' => null,
    'topic'=> @$row['topic'],
    'topic_hindi' => $topic_hindi,
    'topic_krutidev' => $topic_krutidev,
    'student_type'=> 1,
    'gender' => trim($gender),
