    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
				...
				$SEM7=trim($studentData[366]);
				$SEM7_TOTAL_CREDITS_CREDIT_POINTS=trim($studentData[367]);
				$VIII_SEM=trim($studentData[368]);
				$SEM8_COURSE_CODE_1=trim($studentData[369]);
				$SEM8_COURSE_TITLE_1=trim($studentData[370]);
				$SEM8_CREDIT_HOURS_1=trim($studentData[371]);
				$SEM8_GRADE_POINT_1=trim($studentData[372]);
				$SEM8_CREDIT_POINTS_1=trim($studentData[373]);
				$SEM8_COURSE_CODE_2=trim($studentData[374]);
				$SEM8_COURSE_TITLE_2=trim($studentData[375]);
				$SEM8_CREDIT_HOURS_2=trim($studentData[376]);
				$SEM8_GRADE_POINT_2=trim($studentData[377]);
				$SEM8_CREDIT_POINTS_2=trim($studentData[378]);
				$TOTAL_CREDITS_CREDIT_HOURS=trim($studentData[379]);
				$TOTAL_CREDITS_CREDIT_POINTS=trim($studentData[380]);
				$OVERALL_GRADE_POINT_AVERAGE=trim($studentData[381]);
				$DATE=trim($studentData[382]);
				$Photo=trim($studentData[383]);
