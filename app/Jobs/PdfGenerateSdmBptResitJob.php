...
use Helper;
class PdfGenerateSdmBptResitJob
{
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
				...
				*/
				...
				$imageName = str_replace('/','_',$studentData[0]);

				$upload_COE = $COE;
                $pathInfo = pathinfo($COE);
                $COE = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_$imageName.".$pathInfo['extension'];
