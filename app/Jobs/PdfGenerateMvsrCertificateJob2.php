...
use Carbon\Carbon;
class PdfGenerateMvsrCertificateJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $OLDENGL = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\OLDENGL.ttf', 'TrueTypeUnicode', '', 96);
         $Maiandra_Demi_Bold=TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Maiandra GD Demi Bold.ttf', 'TrueTypeUnicode', '', 96);
       
        $preview_serial_no=1;
		//$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('MvsrPC');
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
                ...
                if( $dropdown_template_id == 2){
                    $high_res_bg="MVSR Engineering College Hyderabad Provisional Certificate.jpg"; // MVSR Engineering College Hyderabad Provisional Certificate, TranscriptData.jpg
				    $low_res_bg="MVSR Engineering College Hyderabad Provisional Certificate.jpg";

                }
                else if($dropdown_template_id == 6){
                    $high_res_bg="MVSR_Engineering_College_Hyderabad_Transcript.jpg"; // MVSR Engineering College Hyderabad Provisional Certificate, TranscriptData.jpg
				    $low_res_bg="MVSR_Engineering_College_Hyderabad_Transcript.jpg";
                }
			...
			//  echo"<pre>";print_r($studentData);    die();                			
				...
				$REF_No=trim($studentData[1]);
				$Roll_NO=trim($studentData[2]);
				$STUDENT_NAME=trim($studentData[3]);
				$Father_Name=trim($studentData[4]);
				$MOTHER_NAME=trim($studentData[5]);
				$Degree=trim($studentData[6]);
				$PROGRAME=trim($studentData[7]);
				// $Date=trim($studentData[8]);
				$Result=trim($studentData[9]);
				$CGPA=trim($studentData[10]);
				$Photo=trim($studentData[11]);
                // $ResultDate=trim($studentData[12]);
                 $ResultDate = $this->convertExcelDate($studentData[12]);
			
                $excelValue = $studentData[8]; 

                if (is_numeric($excelValue)) {
                    // $Date = Date::excelToDateTimeObject($excelValue)->format('M-Y'); // Result: May-2025
                    $Date = strtoupper(Date::excelToDateTimeObject($excelValue)->format('M/Y')); // 
                } else {
                        $Date = strtoupper($excelValue);// In case it's already a proper string
                }
				...
				//Start pdfBig  
                if($dropdown_template_id == 6){
                    $pdfBig->SetFont($Maiandra_Demi_Bold, 'B', 13, '', false); 
                    $pdfBig->SetTextColor(0, 0, 0);
                    $pdfBig->SetXY(0, 37);
                    $pdfBig->Cell(0, 0, ' TRANSCRIPT ', 0, false, 'C');
                    $pdfBig->SetXY(0, 44);
                    $pdfBig->Cell(0, 0, ' PROVISIONAL CERTIFICATE', 0, false, 'C');
                ...
                $pdfBig->SetFont($OLDENGL, '', 14, '', false); 
				$pdfBig->SetTextColor(0, 0, 0);    
				$pdfBig->SetXY($left_pos, 80);
                $pdfBig->Cell(20, 0, 'Ref. No.', 0, false, 'L');
                $pdfBig->SetXY($left_pos, 90);
                $pdfBig->Cell(20, 0, 'Roll No.', 0, false, 'L');
				$pdfBig->SetFont($timesb, 'B', 14, '', false); 
                $pdfBig->SetXY($left_pos+25, 80);
                $pdfBig->Cell(186, 0, $REF_No, 0, false, 'L');
                $pdfBig->SetXY($left_pos+25, 90);
                $pdfBig->Cell(186, 0, $Roll_NO, 0, false, 'L');

                $pdfBig->SetFont($OLDENGL, '', 14);
                $pdfBig->SetXY(23, 100);
                $pdfBig->Write(0, "This is to certify that _________________________________________________");
                $pdfBig->SetXY(23, 112);
                $pdfBig->Write(0, "Father's Name ____________________________________________________");
                $pdfBig->SetXY(23, 124);
                $pdfBig->Write(0, "Mother's Name ____________________________________________________");


                $pdfBig->SetXY(23, 136);
                $pdfBig->Write(0, "Passed the_________________________________________________________");


                $pdfBig->SetXY(23, 148);
                //$pdfBig->Write(0, "___________________________________ Examination of MVSR Engineering");
                $pdfBig->Write(0, "__________________________________________________________________");

                $pdfBig->SetXY(23, 160);
                $pdfBig->Write(0, "Examination of MVSR Engineering College,");
                
                $pdfBig->SetXY(23, 172);
                $pdfBig->Write(0, "affiliated to Osmania University, held in _________________________________");
               
                $pdfBig->SetXY(23, 184);
                $pdfBig->Write(0, "and that he/she was placed in __________________________________________");
             
                $pdfBig->SetXY(23, 196);
                $pdfBig->Write(0, "__________________________________________________________________");
               
                $pdfBig->SetXY(23, 208);
                $pdfBig->Write(0, "He/She has satisfied all the requirements for the award of the degree.");
                $pdfBig->SetXY(23, 220);
                $pdfBig->Write(0, "The date of declaration of result :");


               $pdfBig->SetXY(67, 98);
                $pdfBig->SetFont( $timesb, 'B', 13);
                $pdfBig->Cell(150, 10, $STUDENT_NAME, 0, 1, 'L');
                $pdfBig->SetXY(58, 110);
                $pdfBig->Cell(150, 10, $Father_Name, 0, 1, 'L');
                $pdfBig->SetXY(58, 122);
                $pdfBig->Cell(150, 10, $MOTHER_NAME, 0, 1, 'L');
                $pdfBig->SetXY(47, 134);
                $pdfBig->Cell(150, 10, $Degree, 0, 1, 'L');
                $pdfBig->SetXY(25, 146);
                $pdfBig->Cell(150, 10, $PROGRAME, 0, 1, 'L');
                $pdfBig->SetXY(105, 170);
                $pdfBig->Cell(150, 10, $Date, 0, 1, 'L');
                $pdfBig->SetXY(84, 182);
                $pdfBig->Cell(150, 10, strtoupper($Result), 0, 1, 'L');
                $pdfBig->SetXY(26, 194);
                $pdfBig->Cell(150, 10,' WITH CGPA : '. $CGPA, 0, 1, 'L');
                $pdfBig->SetXY(90, 218);
                $pdfBig->Cell(150, 10, $ResultDate, 0, 1, 'L');


                //invisible text
                // $pdfBig->SetXY(180, 38);
                // $pdfBig->SetTextColor(255, 255, 0); // RGB for yello
                // $pdfBig->SetFont($times, '', 10);
                // $pdfBig->Cell(15, 10,$STUDENT_NAME, 0, 1, 'R');
				//End pdfBig 
				//Start pdf
                  if($dropdown_template_id == 6){
                    $pdf->SetFont($Maiandra_Demi_Bold, 'B', 13, '', false); 
                    $pdf->SetTextColor(0, 0, 0);
                    $pdf->SetXY(0, 37);
                    $pdf->Cell(0, 0, ' TRANSCRIPT ', 0, false, 'C');
                    $pdf->SetXY(0, 44);
                    $pdf->Cell(0, 0, ' PROVISIONAL CERTIFICATE', 0, false, 'C');

                }
				$pdf->SetFont($OLDENGL, '', 14, '', false); 
                $pdf->SetTextColor(0, 0, 0);    
                $pdf->SetXY($left_pos, 80);
                $pdf->Cell(20, 0, 'Ref. No.', 0, false, 'L');
                $pdf->SetXY($left_pos, 90);
                $pdf->Cell(20, 0, 'Roll No.', 0, false, 'L');
                $pdf->SetFont($timesb, 'B', 14, '', false); 
                $pdf->SetXY($left_pos+25, 80);
                $pdf->Cell(186, 0, $REF_No, 0, false, 'L');
                $pdf->SetXY($left_pos+25, 90);
                $pdf->Cell(186, 0, $Roll_NO, 0, false, 'L');

                $pdf->SetFont($OLDENGL, '', 14);
                $pdf->SetXY(23, 100);
                $pdf->Write(0, "This is to certify that _________________________________________________");
                $pdf->SetXY(23, 112);
                $pdf->Write(0, "Father's Name ____________________________________________________");
                $pdf->SetXY(23, 124);
                $pdf->Write(0, "Mother's Name ____________________________________________________");


                $pdf->SetXY(23, 136);
                $pdf->Write(0, "Passed the_________________________________________________________");


                $pdf->SetXY(23, 148);
                //$pdf->Write(0, "___________________________________ Examination of MVSR Engineering");
                $pdf->Write(0, "__________________________________________________________________");

                $pdf->SetXY(23, 160);
                $pdf->Write(0, "Examination of MVSR Engineering College,");
                
                $pdf->SetXY(23, 172);
                $pdf->Write(0, "affiliated to Osmania University, held in _________________________________");
               
                $pdf->SetXY(23, 184);
                $pdf->Write(0, "and that he/she was placed in __________________________________________");
             
                $pdf->SetXY(23, 196);
                $pdf->Write(0, "__________________________________________________________________");
               
                $pdf->SetXY(23, 208);
                $pdf->Write(0, "He/She has satisfied all the requirements for the award of the degree.");
                $pdf->SetXY(23, 220);
                $pdf->Write(0, "The date of declaration of result :");


               $pdf->SetXY(67, 98);
                $pdf->SetFont( $timesb, 'B', 13);
                $pdf->Cell(150, 10, $STUDENT_NAME, 0, 1, 'L');
                $pdf->SetXY(58, 110);
                $pdf->Cell(150, 10, $Father_Name, 0, 1, 'L');
                $pdf->SetXY(58, 122);
                $pdf->Cell(150, 10, $MOTHER_NAME, 0, 1, 'L');
                $pdf->SetXY(47, 134);
                $pdf->Cell(150, 10, $Degree, 0, 1, 'L');
                $pdf->SetXY(25, 146);
                $pdf->Cell(150, 10, $PROGRAME, 0, 1, 'L');
                $pdf->SetXY(105, 170);
                $pdf->Cell(150, 10, $Date, 0, 1, 'L');
                $pdf->SetXY(84, 182);
                $pdf->Cell(150, 10, strtoupper($Result), 0, 1, 'L');
                $pdf->SetXY(26, 194);
                $pdf->Cell(150, 10,' WITH CGPA : '. $CGPA, 0, 1, 'L');
                $pdf->SetXY(90, 218);
                $pdf->Cell(150, 10, $ResultDate, 0, 1, 'L');


                //invisible text
                // $pdf->SetXY(180, 38);
                // $pdf->SetTextColor(255, 255, 0); // RGB for yello
                // $pdf->SetFont($times, '', 10);
                // $pdf->Cell(15, 10,$STUDENT_NAME, 0, 1, 'R');
				//End pdf			
				...
				$codeContents = $STUDENT_NAME.", ".$Degree.", ".$PROGRAME.", ".$excelValue." "."\n\n". $encryptedString;
                 //$codeContents = $STUDENT_NAME . ", " . $Degree . ", " . $PROGRAME . ", " . $excelValue . "<br>" . $encryptedString;
                ...
                //     $pdfBig->Image($profile_path_org, $profilex, $profiley, $profileWidth, $profileHeight, 'PNG', '', true, false);
                //     $pdf->Image($profile_path_org, $profilex, $profiley, $profileWidth, $profileHeight, 'PNG', '', true, false);
        ...
        if($previewPdf!=1){
			//$this->updateCardNo('MvsrPC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MvsrPC'.date("Ymdhms")).'.pdf';
