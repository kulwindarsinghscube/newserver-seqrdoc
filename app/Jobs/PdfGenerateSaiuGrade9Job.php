...
class PdfGenerateSaiuGrade9Job
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $Academic_year_1 = trim($studentData[83]);
            $Academic_year_2 = trim($studentData[84]);
            $Academic_year_3 = trim($studentData[85]);
            ...
            // End Subject Data Box 
            
          
            
            $tableY = 90;
            foreach ($subjectData as $sem) {
                // Subject Data  Header Box
                // Left
                if(empty($sem['semester'])){
                    break;
                }

                $pdfBig->setCellPaddings( $left = '1', $top = '0.5', $right = '', $bottom = ''); 
                $pdfBig->SetFont($arialNarrowB, '',9, '', false);
                $pdfBig->SetXY(11, $tableY);
                $pdfBig->MultiCell(188, 6, $sem['semester'], 0, "C", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();
                // $tableY = $tableY+4;
                ...
                $pdfBig->MultiCell(40, 8, 'Course Code', 1, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialNarrowB, '',9, '', false);
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY() );
                $pdfBig->MultiCell(102, 8, 'Course Title', 1, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialNarrowB, '',9, '', false);
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY() );
                $pdfBig->MultiCell(24, 8, 'Credits', 1, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialNarrowB, '',9, '', false);
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY() );
                $pdfBig->MultiCell(22, 8, 'Grade', 1, "C", 0, 0, '', '', true, 0, true);
                ...
                foreach ($sem['subjects'] as $subject) {
                    ...
                    $pdfBig->SetXY(11, $pdfBig->getY());
                    $pdfBig->MultiCell(40, 8, $subject['course_code'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetFont($arialNarrow, '',9, '', false);
                    $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                    $pdfBig->MultiCell(102, 8, $subject['course_name'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetFont($arialNarrow, '',9, '', false);
                    $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                    $pdfBig->MultiCell(24, 8, $subject['credit'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetFont($arialNarrow, '',9, '', false);
                    $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                    $pdfBig->MultiCell(22, 8, $subject['grade'], 1, "C", 0, 0, '', '', true, 0, true);
                    $pdfBig->ln();
                }
                ...
                $pdfBig->SetFont($arialNarrowB, '',8, '', false);
                $pdfBig->SetXY(11, $pdfBig->getY());
                $pdfBig->MultiCell(188, 5, 'SGPA : '.$sem['sgpa'], 'LRB', "L", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();

                $tableY = $pdfBig->getY()+6;
            }
            ...
            foreach ($subjectData as $sem) {
                ...
                $pdf->SetXY(11, $tableY);
                $pdf->MultiCell(188, 6, $sem['semester'], 0, "C", 0, 0, '', '', true, 0, true);
                $pdf->ln();
                // $tableY = $tableY+4;
                ...
                $pdf->MultiCell(40, 8, 'Course Code', 1, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($arialNarrowB, '',9, '', false);
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(102, 8, 'Course Title', 1, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($arialNarrowB, '',9, '', false);
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(24, 8, 'Credits', 1, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($arialNarrowB, '',9, '', false);
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(22, 8, 'Grade', 1, "C", 0, 0, '', '', true, 0, true);
                ...
                foreach ($sem['subjects'] as $subject) {
                    ...
                    $pdf->SetXY(11, $pdf->getY());
                    $pdf->MultiCell(40, 8, $subject['course_code'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetFont($arialNarrow, '',9, '', false);
                    $pdf->SetXY($pdf->getX(), $pdf->getY());
                    $pdf->MultiCell(102, 8, $subject['course_name'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetFont($arialNarrow, '',9, '', false);
                    $pdf->SetXY($pdf->getX(), $pdf->getY());
                    $pdf->MultiCell(24, 8, $subject['credit'], 1, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetFont($arialNarrow, '',9, '', false);
                    $pdf->SetXY($pdf->getX(), $pdf->getY());
                    $pdf->MultiCell(22, 8, $subject['grade'], 1, "C", 0, 0, '', '', true, 0, true);
                    $pdf->ln();
                }
                ...
                $pdf->SetXY(11, $pdf->getY());
                $pdf->MultiCell(188, 5, 'SGPA : '.$sem['sgpa'], 'LRB', "L", 0, 0, '', '', true, 0, true);
                $pdf->ln();
