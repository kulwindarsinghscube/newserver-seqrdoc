...
class PdfGenerateAtriauJob3 
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $tulu = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\NIRMALA.TTF', 'TrueTypeUnicode', '', 96);
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) 
            {
				...
				$high_res_bg="AU Degree Certificate.jpg"; // bestiu_pdc_bg, TranscriptData
				$low_res_bg="AU Degree Certificate.jpg";
				$pdfBig->AddPage();
                ...
                $Sr_no=trim($studentData[2]);
                $STUDENT_NAME=trim($studentData[3]);
                $degreeRaw = trim($studentData[4]); 
                $result = trim($studentData[5]); 
                $completion_date = trim($studentData[6]);
                $date = trim($studentData[7]);   
                $Photo = trim($studentData[8]);   
                ...
                $pdfBig->SetXY(140, 14);
                $pdfBig->Cell(0, 0, 'D.C. NO.: '.$unique_id, 0, false, 'L');

                $pdfBig->SetFont($arial, '', 6, '', false); 
                $pdfBig->SetXY(22, 241);
                $pdfBig->Cell(0, 0, 'REG NO.: '.$Reg_NO, 0, false, 'L');  
                

                $pdfBig->SetFont($arialb, '', 29.04, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, 100);
                $pdfBig->Cell(186, 0, $STUDENT_NAME, 0, false, 'C');
                $pdfBig->SetFont($arial, '', 14.52, '', false); 
                $pdfBig->SetXY(12, 128);			
                $pdfBig->Cell(186, 0, 'Is hereby conferred the degree of', 0, false, 'C');
                // $pdfBig->SetFont($arialb, '', 25.56, '', false);
                // $pdfBig->SetXY(12, 150);

                if (stripos($degreeRaw, 'IN') !== false) {
                    $parts = explode('IN', $degreeRaw, 2);

                    $mainDegree = trim($parts[0]);
                    $subDegree  = '(in ' . trim($parts[1]) . ')';

                } else {
                    $mainDegree = $degreeRaw;
                    $subDegree  = '';
                }
               ...
               $pdfBig->SetFont($arialb, '', 25.56, '', false);
                $pdfBig->SetXY(12, 150);
                $pdfBig->Cell(186, 0, $mainDegree, 0, 1, 'C');

                // Subtitle (Smaller)
                if ($subDegree != '') {
                  $pdfBig->SetFont($arialb, '', 15, '', false);
                    $pdfBig->Cell(186, 8, $subDegree, 0, 1, 'C');
                }
                ...
                // $pdfBig->MultiCell(186, 0, $degree, 0, 'C');


                $pdfBig->SetFont($arial, '', 14.52, '', false); 
                $pdfBig->SetXY(12, 180);
                $pdfBig->MultiCell(186, 0, ' Having fulfilled the prescribed requirements in '.$completion_date.' with '.$result, 0, 'C');
               
                $pdfBig->SetXY(13, 193);
                $pdfBig->Cell(184, 0, "", 'T');

                $pdfBig->SetFont($tulu, '', 11, '', false); 
                $pdfBig->SetXY(12, 197);
                $pdfBig->MultiCell(190, 0, 'ಮೆಲ್ಲಿಂದ ಹಿರಿಯರವರನ್ನು ಅಧ್ಯಕ್ಷರ ಮಂದಿರದಲ್ಲಿ ಶ್ರೀಕೃಷ್ಣ ಪರಮಾತ್ಮನ ಶಾಂತಿಗೆ ಮೊಳಗೆ, ಮೊಳಗಿಸಿದವು ಎಂದು ಎದ್ದೇಳಿದವರಿಗೆ ಉತ್ಸಾಹ ಆಘಾತಗೊಳ್ಳುತ್ತಿದ್ದಂತೆ ಅಂಗೀಕರಿಸಿದರು.', 0, 'C');

              
                $pdfBig->SetXY(150, 228);
                $pdfBig->Cell(41, 0, "", 'T');
                $pdfBig->SetXY(153, 230);
                $pdfBig->Cell(0, 0, 'ಉಪ-ಜಿಲ್ಲಾಪಾಲ', 0, false, 'L');  
                

                
                $pdfBig->SetFont($arialb, '', 12.48, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);  

                $pdfBig->SetXY(153, 236);
                $pdfBig->Cell(0, 0, 'Vice-Chancellor', 0, false, 'L');  
                $pdfBig->SetFont($arial, '', 12.48, '', false); 
                $pdfBig->SetXY(153, 242);
                ...
                $pdf->SetXY(140, 14);
                $pdf->Cell(0, 0, 'D.C. NO.: '.$unique_id, 0, false, 'L');

                $pdf->SetFont($arial, '', 6, '', false); 
                $pdf->SetXY(22, 241);
                $pdf->Cell(0, 0, 'REG NO.: '.$Reg_NO, 0, false, 'L');  
                

                $pdf->SetFont($arialb, '', 29.04, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, 100);
                $pdf->Cell(186, 0, $STUDENT_NAME, 0, false, 'C');
                $pdf->SetFont($arial, '', 14.52, '', false); 
                $pdf->SetXY(12, 128);			
                $pdf->Cell(186, 0, 'Is hereby conferred the degree of', 0, false, 'C');
                // $pdf->SetFont($arialb, '', 25.56, '', false);
                // $pdf->SetXY(12, 150);
               ...
               $pdf->SetFont($arialb, '', 25.56, '', false);
                $pdf->SetXY(12, 150);
                $pdf->Cell(186, 0, $mainDegree, 0, 1, 'C');

                // Subtitle (Smaller)
                if ($subDegree != '') {
                  $pdf->SetFont($arialb, '', 15, '', false);
                    $pdf->Cell(186, 8, $subDegree, 0, 1, 'C');
                }
                ...
                // $pdf->MultiCell(186, 0, $degree, 0, 'C');


                $pdf->SetFont($arial, '', 14.52, '', false); 
                $pdf->SetXY(12, 180);
                $pdf->MultiCell(186, 0, ' Having fulfilled the prescribed requirements in '.$completion_date.' with '.$result, 0, 'C');
               
                $pdf->SetXY(13, 193);
                $pdf->Cell(184, 0, "", 'T');

                $pdf->SetFont($tulu, '', 11, '', false); 
                $pdf->SetXY(12, 197);
                $pdf->MultiCell(190, 0, 'ಮೆಲ್ಲಿಂದ ಹಿರಿಯರವರನ್ನು ಅಧ್ಯಕ್ಷರ ಮಂದಿರದಲ್ಲಿ ಶ್ರೀಕೃಷ್ಣ ಪರಮಾತ್ಮನ ಶಾಂತಿಗೆ ಮೊಳಗೆ, ಮೊಳಗಿಸಿದವು ಎಂದು ಎದ್ದೇಳಿದವರಿಗೆ ಉತ್ಸಾಹ ಆಘಾತಗೊಳ್ಳುತ್ತಿದ್ದಂತೆ ಅಂಗೀಕರಿಸಿದರು.', 0, 'C');

              
                $pdf->SetXY(150, 228);
                $pdf->Cell(41, 0, "", 'T');
                $pdf->SetXY(153, 230);
                $pdf->Cell(0, 0, 'ಉಪ-ಜಿಲ್ಲಾಪಾಲ', 0, false, 'L');  
                

                
                $pdf->SetFont($arialb, '', 12.48, '', false); 
                $pdf->SetTextColor(0, 0, 0);  

                $pdf->SetXY(153, 236);
                $pdf->Cell(0, 0, 'Vice-Chancellor', 0, false, 'L');  
                $pdf->SetFont($arial, '', 12.48, '', false); 
                $pdf->SetXY(153, 242);
            ...
            $pdfBig->Rotate(90, 12, 278); // Rotate around (x=25, y=148) to make vertical
            $pdfBig->Text(12, 278, $STUDENT_NAME); // Adjust position to align vertically
