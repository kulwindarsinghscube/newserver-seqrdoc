...
class PdfGenerateSaiuDegreeJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('SaiuDegree');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$serial_no = trim($studentData[1]); //system unique id
			//$serial_no = str_pad($serial_no, 5, '0', STR_PAD_LEFT);
			$candidate_name = trim($studentData[2]);
			$year = trim($studentData[3]);
            if (in_array(strtolower(str_replace(" ", "", $candidate_name)), ["abhivyaktisinha", "vedavenkateshiyer", "dakshajaakashyap", "davulurisrinavya"])) {
                $academic_year_text = "2021-2024";
            } else {
                $academic_year_text = " $year"; // Ensure $year is defined earlier
            }
            
			$specialization = trim($studentData[5]);
			$specialRecognition = trim($studentData[5]);
            $prn = trim($studentData[6]);
            $course_name = trim($studentData[4]);
            $convocation_date = trim($studentData[7]);
            $main_font_size = 16; 
            ...
            $pdfBig->MultiCell(297, 7, "bearing Permanent Registration Number (PRN) $prn, the degree of", 0, "C", 0, 0, '', '', true, 0, true); 
            $pdfBig->ln(); 
            ...
            $pdfBig->MultiCell(207, 7, "The degree was awarded at the convocation ceremony held on the 
            $convocation_date in recognition of successful fulfillment of all graduation
            requirements prescribed under the Regulations during $academic_year_text for which we bear witness and hereunder place our
            ...
            $pdfBig->ln(); 
            $pdfBig->SetXY(35,181); 
            $pdfBig->MultiCell(38, 7, " S Sundara Rajan", 0, "C", 0, 0, '', '', true, 0, true);
            $pdfBig->SetXY(130,181); 
            $pdfBig->MultiCell(38, 7, " Dr. Ajith Abraham", 0, "C", 0, 0, '', '', true, 0, true);
            $pdfBig->SetXY(229,181); 
            ...
            $pdf->ln(); 
            $pdf->SetFont($englishtownb, '',$student_name_font_size, '', false);
            $pdf->SetFont($englishtownb, '', $student_name_font_size, '', true, 1.5);
            $pdf->SetXY(0, $pdf->getY()+2);
            $pdf->MultiCell(297, 7, $candidate_name, 0, "C", 0, 0, '', '', true, 0, true);
            $pdf->ln();
            $pdf->SetFont($englishtown, '',$main_font_size, '', false);
            $pdf->SetXY(0,$pdf->getY()+1);
            $pdf->MultiCell(297, 7, "bearing Permanent Registration Number (PRN) $prn, the degree of", 0, "C", 0, 0, '', '', true, 0, true); 
            $pdf->ln(); 
            ...
            $pdf->MultiCell(207, 7, "The degree was awarded at the Convocation 2025 held on the 
            $convocation_date in recognition of successful fulfillment of all graduation
            requirements prescribed under the Regulations during 
             $academic_year_text for which we bear witness and hereunder place our
            names and signatures along with the official seal of Sai University.", 0, "C", 0, 0, '', '', true, 0, true); 
            $pdf->SetFont($arialb, '',$sign_content_size, '', false); 
            $pdf->ln(); 
            $pdf->SetXY(35,181); 
            $pdf->MultiCell(35, 7, " S Sundara Rajan", 0, "C", 0, 0, '', '', true, 0, true);
            $pdf->SetXY(130,181); 
            ...
            // start invisible data
            $pdf->SetFont($englishtownb, '', 12, '', false); 
        ...
        if($previewPdf!=1){
            $this->updateCardNo('SaiuDegree',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
