...
class PdfGenerateSaiuJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        //set fonts
        /*$arial = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\Arial.TTF', 'TrueTypeUnicode', '', 96);
        $arialb = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\Arialb.TTF', 'TrueTypeUnicode', '', 96);
        $arialNarrow = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\arialn.TTF', 'TrueTypeUnicode', '', 96);
        $arialNarrowB = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\ARIALNB.TTF', 'TrueTypeUnicode', '', 96);
        $times = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\Times-New-Roman.ttf', 'TrueTypeUnicode', '', 96);
        $timesb = TCPDF_FONTS::addTTFfont(public_path().'\\backend\canvas\fonts\Times-New-Roman-Bold.ttf', 'TrueTypeUnicode', '', 96);
        */ 
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('SaiuC');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->MultiCell(395, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arial.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
			...
			$pdfBig->SetFont($arial, '', 21, '', false); 
            $pdfBig->SetTextColor(0, 0, 0);        
            $pdfBig->SetXY(12, 185); //199.5
            $pdfBig->Cell(395, 0, 'having successfully completed on '.$completion_date.',', 0, false, 'C');
			...
			$pdf->MultiCell(395, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arial.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetXY(12, 152.5);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('SaiuC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'SaiuC'.date("Ymdhms")).'.pdf';
