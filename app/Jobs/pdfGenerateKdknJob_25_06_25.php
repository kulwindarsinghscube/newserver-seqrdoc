    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			if($studentData[1]!=''){
                ...
                $student_name = trim($studentData[2]);
                $mother_name = trim($studentData[3]);
                $father_name = trim($studentData[4]);
                $register_id = trim($studentData[5]);
                $university_enrollment_no = trim($studentData[6]);
                $term = trim($studentData[7]);
                $academic_year = trim($studentData[8]);
                $session = trim($studentData[9]);
                $exam_name = trim($studentData[10]);
                $programme = trim($studentData[11]);
                $gender = trim($studentData[12]);
                $branch = trim($studentData[13]);

                $date_of_issue = trim($studentData[88]);
                $result = trim($studentData[89]);
                $sgpa = trim($studentData[89]);
                $cumulative_credits = trim($studentData[90]);
                $cumulative_egp = trim($studentData[91]);
                $cgpa = trim($studentData[92]);
...
$y_student_label = $y_academic + 2; // small gap before student name line
...
$y = $y_student_label + $main_info_height;
    ...
    $pdfBig->MultiCell(33, $main_info_height, "Registration No.", 0, "L", 0, 0);
    ...
    $pdfBig->SetFont($times, '', $content_font_size);
    $pdfBig->MultiCell(30, $main_info_height, ': ' . $session, 0, "L", 0, 0);
    $y += $main_info_height;

    // === LINE 3: Exam Roll No. & Degree ===
    $pdfBig->SetFont($timesb, '', $label_font_size);
    $pdfBig->SetXY($x_left, $y);
    $pdfBig->MultiCell(33, $main_info_height, "Exam Roll No.", 0, "L", 0, 0);
    ...
    $pdfBig->SetFont($times, '', $content_font_size);
    $pdfBig->MultiCell(50, $main_info_height, ': ' . $programme, 0, "L", 0, 0);
    $y += $main_info_height;
    ...
    $pdfBig->SetFont($times, '', $content_font_size);
    $pdfBig->MultiCell(65, $main_info_height, ': ' . $exam_name, 0, "L", 0, 0);
    ...
    $pdfBig->SetFont($times, '', $content_font_size);
    $pdfBig->MultiCell(75, $main_info_height, ': ' . $branch, 0, "L", 0, 0);
    $y += $main_info_height;
    ...
    $y = $pdfBig->GetY();
    $pdfBig->SetY($y + 1); // Reduce to 1mm or 2mm if you want a little breathing room
    ...
    $col_widths = [
        ...
        $pdfBig->setCellPaddings(1.5, 2, 1.5, 2);

        $startY = $pdfBig->GetY() + 4;
        $pdfBig->SetXY($table_x, $startY);

        $pdfBig->MultiCell($col_widths['sr_no'], 10, 'Sr.No.', 1, 'C', 0, 0);
        $pdfBig->MultiCell($col_widths['code'], 10, 'Course Code', 1, 'C', 0, 0);
        $pdfBig->MultiCell($col_widths['name'], 10, 'Course Name', 1, 'C', 0, 0);
        $pdfBig->MultiCell($col_widths['credit'], 10, 'Course Credit', 1, 'C', 0, 0);
        $pdfBig->MultiCell($col_widths['grade'], 10, 'Grade Earned', 1, 'C', 0, 0);
        $pdfBig->Ln();

        $pdfBig->SetFont($times, '', $contentFontSize);

        $subjectStartIndex = 22; // Starting from COURSE CODE
        $columnsPerSubject = 5;
        ...
        for ($s = 0; $s < 15; $s++) {
            ...
            $SubjectCode = trim($studentData[$baseIndex+1]);        // COURSE CODE
            $SubjectTitle = trim($studentData[$baseIndex + 2]);   // COURSE NAME
            $Credit = trim($studentData[$baseIndex + 4]);         // CREDITS
            $Grade = trim($studentData[$baseIndex + 5]);          // GRADES
            ...
            $pdfBig->MultiCell($col_widths['grade'], $rowHeight, $Grade, 1, 'C', 0, 0);
            $pdfBig->Ln();
        }
...
$table_width = 185;
$table_x = ($pdfBig->getPageWidth() - $table_width) / 2; // Centered X
$gapBetweenTables = 8;

$tableWidth = ($table_width - $gapBetweenTables) / 2; // Width of each SGPA/CGPA table
$cellCount = 4;
$cellWidth = $tableWidth / $cellCount;

$leftStartX = $table_x; // Align with subject table
$rightStartX = $leftStartX + $tableWidth + $gapBetweenTables;
$startY = $pdfBig->GetY();


        // === Fetch values from Excel data ===
        $earned_credits      = isset($studentData[15]) ? trim($studentData[15]) : '-';
        $grade_points_earned = isset($studentData[16]) ? trim($studentData[16]) : '-';
        $sgpa                = isset($studentData[17]) ? trim($studentData[17]) : '-';
        $cum_credits         = isset($studentData[18]) ? trim($studentData[18]) : '-';
        $cum_egp             = isset($studentData[19]) ? trim($studentData[19]) : '-';
        $cgpa                = isset($studentData[20]) ? trim($studentData[20]) : '-';

        // === Table Data Arrays ===
        $leftTableData = [
            ['Current Semester Performance'],
            ["Credits\nEarned", "Grade Points\nEarned", "Incentive Grade\nPoints Earned", 'SGPA'],
            [$earned_credits, $grade_points_earned, '-', $sgpa],
        ];

        $rightTableData = [
            ['Cumulative Performance'],
            ["Credits\nEarned", "Grade Points\nEarned", "Incentive Grade\nPoints Earned", 'CGPA'],
            [$cum_credits, $cum_egp, '-', $cgpa],
        ];

        // === Draw Tables ===
        $pdfBig->SetFont($timesb, '', 7.8);
        $pdfBig->setCellPaddings(0.5, 0.5, 0.5, 0.5);
        ...
        for ($row = 0; $row < 3; $row++) {
            ...
            if ($row === 0) {
                $pdfBig->Cell($cellWidth * 4, 6.8, $leftTableData[$row][0], 1, 0, 'C');
                $rowHeight = 6.8;
            } elseif ($row === 1) {
                for ($col = 0; $col < 4; $col++) {
                    $x = $pdfBig->GetX();
                    $y = $pdfBig->GetY();
                    $pdfBig->MultiCell($cellWidth, 13, $leftTableData[$row][$col], 1, 'C', 0, 0);
                    $pdfBig->SetXY($x + $cellWidth, $y);
                }
                $rowHeight = 13;
            } else {
                for ($col = 0; $col < 4; $col++) {
                    $pdfBig->Cell($cellWidth, 6.8, $leftTableData[$row][$col], 1, 0, 'C');
                }
                $rowHeight = 6.8;
            }
            ...
            if ($row === 0) {
                $pdfBig->Cell($cellWidth * 4, 6.8, $rightTableData[$row][0], 1, 0, 'C');
            } elseif ($row === 1) {
                for ($col = 0; $col < 4; $col++) {
                    $x = $pdfBig->GetX();
                    $y = $pdfBig->GetY();
                    $pdfBig->MultiCell($cellWidth, 13, $rightTableData[$row][$col], 1, 'C', 0, 0);
                    $pdfBig->SetXY($x + $cellWidth, $y);
                }
            } else {
                for ($col = 0; $col < 4; $col++) {
                    $pdfBig->Cell($cellWidth, 6.8, $rightTableData[$row][$col], 1, 0, 'C');
                }
            }
...
$table_width = 185;
$table_x = ($pdfBig->getPageWidth() - $table_width) / 2; // Same as subject table

$bottomStartX = $table_x; // Align left edge with subject table
$firstColWidth = 35;
$otherColWidth = ($table_width - $firstColWidth) / 8; // Distribute remaining space evenly

$defaultCellHeight = 6.8;
        ...
        foreach ($bottomTableData as $rowIdx => $rowData) {
            $pdfBig->SetXY($bottomStartX, $bottomStartY + ($rowIdx * $defaultCellHeight));

            if ($rowIdx === 0) {
                $pdfBig->SetFont($timesb, '', 8);
                $pdfBig->Cell($firstColWidth + ($otherColWidth * 8), $defaultCellHeight, $rowData[0], 1, 0, 'C');
            } else {
                for ($col = 0; $col < count($rowData); $col++) {
                    $cellText = $rowData[$col];
                    if ($col === 0) {
                        $pdfBig->SetFont($timesb, '', 8);
                        $pdfBig->Cell($firstColWidth, $defaultCellHeight, $cellText, 1, 0, 'C');
                    } else {
                        $pdfBig->SetFont($times, '', 8);
                        $pdfBig->Cell($otherColWidth, $defaultCellHeight, $cellText, 1, 0, 'C');
                    }
                }
            }
        }
        ...
        $result      = isset($studentData[21]) ? trim($studentData[21]) : '-';
        $date_value  = isset($studentData[22]) ? trim($studentData[22]) : '-';
...
$y = $y_student_label + $main_info_height;
    ...
    $pdf->SetXY($x_left, $y);
    $pdf->MultiCell(33, $main_info_height, "Registration No.", 0, "L", 0, 0);
    ...
    $pdf->SetFont($times, '', $content_font_size);
    $pdf->MultiCell(30, $main_info_height, ': ' . $session, 0, "L", 0, 0);
    $y += $main_info_height;

    // === LINE 3: Exam Roll No. & Degree ===
    $pdf->SetFont($timesb, '', $label_font_size);
    $pdf->SetXY($x_left, $y);
    $pdf->MultiCell(33, $main_info_height, "Exam Roll No.", 0, "L", 0, 0);
    ...
    $pdf->SetFont($times, '', $content_font_size);
    $pdf->MultiCell(50, $main_info_height, ': ' . $programme, 0, "L", 0, 0);
    $y += $main_info_height;
    ...
    $pdf->SetFont($times, '', $content_font_size);
    $pdf->MultiCell(65, $main_info_height, ': ' . $exam_name, 0, "L", 0, 0);
    ...
    $pdf->SetFont($times, '', $content_font_size);
    $pdf->MultiCell(75, $main_info_height, ': ' . $branch, 0, "L", 0, 0);
    $y += $main_info_height;
    ...
    $y = $pdf->GetY();
    $pdf->SetY($y + 1); // Reduce to 1mm or 2mm if you want a little breathing room
            ...
            $pdf->MultiCell($col_widths['sr_no'], 10, 'Sr.No.', 1, 'C', 0, 0);
            $pdf->MultiCell($col_widths['code'], 10, 'Course Code', 1, 'C', 0, 0);
            $pdf->MultiCell($col_widths['name'], 10, 'Course Name', 1, 'C', 0, 0);
            $pdf->MultiCell($col_widths['credit'], 10, 'Course Credit', 1, 'C', 0, 0);
            $pdf->MultiCell($col_widths['grade'], 10, 'Grade Earned', 1, 'C', 0, 0);
            $pdf->Ln();
            ...
            for ($s = 0; $s < 15; $s++) {
                ...
                $pdf->MultiCell($col_widths['grade'], $rowHeight, $Grade, 1, 'C', 0, 0);
                $pdf->Ln();
            }
...
// === Layout configuration ===
$table_width = 185;
$table_x = ($pdf->getPageWidth() - $table_width) / 2; // Centered X
$gapBetweenTables = 8;
...
$startY = $pdf->GetY();
            ...
            for ($row = 0; $row < 3; $row++) {
                ...
                if ($row === 0) {
                    $pdf->Cell($cellWidth * 4, 6.8, $leftTableData[$row][0], 1, 0, 'C');
                    $rowHeight = 6.8;
                } elseif ($row === 1) {
                    for ($col = 0; $col < 4; $col++) {
                        $x = $pdf->GetX();
                        $y = $pdf->GetY();
                        $pdf->MultiCell($cellWidth, 13, $leftTableData[$row][$col], 1, 'C', 0, 0);
                        $pdf->SetXY($x + $cellWidth, $y);
                    }
                    $rowHeight = 13;
                } else {
                    for ($col = 0; $col < 4; $col++) {
                        $pdf->Cell($cellWidth, 6.8, $leftTableData[$row][$col], 1, 0, 'C');
                    }
                    $rowHeight = 6.8;
                }
                ...
                if ($row === 0) {
                    $pdf->Cell($cellWidth * 4, 6.8, $rightTableData[$row][0], 1, 0, 'C');
                } elseif ($row === 1) {
                    for ($col = 0; $col < 4; $col++) {
                        $x = $pdf->GetX();
                        $y = $pdf->GetY();
                        $pdf->MultiCell($cellWidth, 13, $rightTableData[$row][$col], 1, 'C', 0, 0);
                        $pdf->SetXY($x + $cellWidth, $y);
                    }
                } else {
                    for ($col = 0; $col < 4; $col++) {
                        $pdf->Cell($cellWidth, 6.8, $rightTableData[$row][$col], 1, 0, 'C');
                    }
                }
...
$table_width = 185;
$table_x = ($pdf->getPageWidth() - $table_width) / 2; // Same as subject table
...
$defaultCellHeight = 6.8;
        ...
        // === Draw Grade Conversion Table ===
        foreach ($bottomTableData as $rowIdx => $rowData) {
            $pdf->SetXY($bottomStartX, $bottomStartY + ($rowIdx * $defaultCellHeight));

            if ($rowIdx === 0) {
                $pdf->SetFont($timesb, '', 8);
                $pdf->Cell($firstColWidth + ($otherColWidth * 8), $defaultCellHeight, $rowData[0], 1, 0, 'C');
            } else {
                for ($col = 0; $col < count($rowData); $col++) {
                    $cellText = $rowData[$col];
                    if ($col === 0) {
                        $pdf->SetFont($timesb, '', 8);
                        $pdf->Cell($firstColWidth, $defaultCellHeight, $cellText, 1, 0, 'C');
                    } else {
                        $pdf->SetFont($times, '', 8);
                        $pdf->Cell($otherColWidth, $defaultCellHeight, $cellText, 1, 0, 'C');
                    }
                }
            }
        }
...
$pdfBig->Image($qr_code_path, $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight, 'png', '', true, false);
$pdfBig->setPageMark();
$microline_str = strtoupper(str_replace(' ', '', $student_name)); // e.g., AFFANGAFFARPATHAN
$fontSize = 3;
$lineWidth = 40;
$microY = $qrCodey - 1;
...
// Center the text over QR
$microX = $qrCodex + ($qrCodeWidth - $textWidth) / 2;

$pdfBig->SetTextColor(0, 0, 0);
$pdfBig->StartTransform();
$pdfBig->SetXY($microX, $microY);
$pdfBig->Cell($textWidth, 0, $microline_str, 0, false, 'L');
