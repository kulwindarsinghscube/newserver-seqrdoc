...
use Carbon\Carbon;
class PdfGenerateMvsrCertificateJob4
{
...
// dd($generated_documents );
        ...
        $MAIAN = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\MAIAN.ttf', 'TrueTypeUnicode', '', 96);
        ...
        $subjectsArr = array();
        if($subjectDataOrg&&!empty($subjectDataOrg)){
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
				...
				$high_res_bg="MVSR_Engineering_College_Hyderabad_Transcript.jpg"; // MVSR_Engineering_College_Hyderabad_Transcript, TranscriptData.jpg
                ...
                $subjectData = array_slice($studentData, 7, 40);
                $subjectChunks = array_chunk($subjectData, 4);
				   $subjectsData=$subjectsArr[$studentData[9]];
                ...
                $pdfBig->SetFont($Maiandra_Demi_Bold, 'B', 15, '', false); 
				$pdfBig->SetTextColor(0, 0, 0);
                $pdfBig->SetXY(0, 38);
                $pdfBig->Cell(0, 0, 'SEMESTER GRADE REPORT', 0, false, 'C');
                $pdfBig->SetFont($MAIAN, '', 12, '', false); 
				$pdfBig->SetXY($left_pos, 50);
                $pdfBig->Cell(20, 0, 'EXAMINATION      : '.$Examination, 0, false, 'L');
                $pdfBig->SetXY($left_pos, 56);
                $pdfBig->Cell(20, 0, 'REF NO.                 : '.$REF_No, 0, false, 'L');
                $pdfBig->SetXY($left_pos, 62);
                $pdfBig->Cell(20, 0, 'NAME                    : '.$STUDENT_NAME, 0, false, 'L');
                $pdfBig->SetXY($left_pos, 68);
                $pdfBig->Cell(20, 0, 'FATHER\'S NAME    : '.$Father_Name, 0, false, 'L');

                $pdfBig->SetXY($left_pos, 74);
                $pdfBig->Cell(20, 0, 'MOTHER\'S NAME  : '.$MOTHER_NAME, 0, false, 'L');
                 $pdfBig->SetXY(145, 42.5);
                $pdfBig->Cell(20, 0, 'ROLL NO: '.$Roll_NO, 0, false, 'L');

                $pdfBig->SetFont($MTCORSVA, '', 15, '', false); 
                $pdfBig->setCellPaddings( $left = '', $top = '3', $right = '', $bottom = '');
                $pdfBig->SetXY($left_pos, 81.5);	
				$pdfBig->MultiCell(32, 13.5, 'Subject Code', 'LT', 'C', 0, 0, '', '', true, 0, true);
				$pdfBig->MultiCell(98, 13.5, 'Subject', 'LT', 'C', 0, 0, '', '', true, 0, true);
				$pdfBig->MultiCell(25, 13.5, 'Credits', 'LT', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
				$pdfBig->MultiCell(26, 13.5, 'Grade Awarded', 'LRT', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdfBig->SetXY($left_pos, 95);
                $pdfBig->MultiCell(181, 0, '', 'T', 'C', 0, 0);
                $pdfBig->SetXY($left_pos, 95);
               
                $pdfBig->MultiCell(32, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
				$pdfBig->MultiCell(98, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
				$pdfBig->MultiCell(25, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
				$pdfBig->MultiCell(26, 120, '', 'LRB', 'C', 0, 0, '', '', true, 0, true);
                ...
                foreach ($subjectsData as $term => $term_array) {
                    $subHegight=6;
                    $pdfBig->startTransaction();
                    
                    $lines = $pdfBig->MultiCell(98, $subHegight, $term_array[3], 'LR', 'L', 0, 0, '', '', true, 0, false,true, 0);
                    ...
                    $value = number_format((float) $term_array[7], 1, '.', '');
                    $pdfBig->setCellPaddings( $left = '1', $top = '2', $right = '', $bottom = '');
                    $pdfBig->SetXY($left_pos, $dataY);
                    $pdfBig->MultiCell(32, $subHegight, $term_array[2], 0, 'L', 0, 0, '', '', true);
                    $pdfBig->SetX($pdfBig->GetX());
                    $pdfBig->MultiCell(98, $subHegight, $term_array[3], 0, 'L', 0, 0, '', '', true);
                    $pdfBig->SetX($pdfBig->GetX());
                    $pdfBig->MultiCell(25, $subHegight, $value , 0, 'C', 0, 0, '', '', true);
                    $pdfBig->SetX($pdfBig->GetX());
                    $pdfBig->MultiCell(26, $subHegight, $term_array[4], 0, 'C', 0, 0, '', '', true);
                    $pdfBig->Ln();
                    $dataY=$pdfBig->GetY();
                }
                $pdfBig->Ln();
                $dataY=$pdfBig->GetY();
                $pdfBig->SetFont($MAIAN, '', 13, '', false); 
                $pdfBig->SetXY($left_pos, 217);
                $pdfBig->Cell(20, 0, 'S.G.P.A   : '.$studentData[19].'('.$studentData[26].')', 0, false, 'L');
                $pdfBig->SetX($left_pos+140);
                $pdfBig->Cell(20, 0, 'Result : '.$studentData[17], 0, false, 'L');
                $pdfBig->SetXY($left_pos, 225);
                $pdfBig->Cell(20, 0, 'The date of declaration of result : '.$ResultDate, 0, false, 'L');
                $pdfBig->SetFont($arialb, '', 10, '', false); 
                $pdfBig->SetXY(89, 265);
                $pdfBig->Cell(20, 0, 'Section-Incharge', 0, false, 'L');

                $pdfBig->SetXY(143, 265);
                $pdfBig->Cell(20, 0, 'Controller of Examinations', 0, false, 'L');
				
				//End pdfBig 
				//Start pdf
				  $pdf->SetFont($Maiandra_Demi_Bold, 'B', 15, '', false); 
                $pdf->SetTextColor(0, 0, 0);
                $pdf->SetXY(0, 38);
                $pdf->Cell(0, 0, 'SEMESTER GRADE REPORT', 0, false, 'C');
                $pdf->SetFont($MAIAN, '', 12, '', false); 
                $pdf->SetXY($left_pos, 50);
                $pdf->Cell(20, 0, 'EXAMINATION      : '.$Examination, 0, false, 'L');
                $pdf->SetXY($left_pos, 56);
                $pdf->Cell(20, 0, 'REF NO.                 : '.$REF_No, 0, false, 'L');
                $pdf->SetXY($left_pos, 62);
                $pdf->Cell(20, 0, 'NAME                    : '.$STUDENT_NAME, 0, false, 'L');
                $pdf->SetXY($left_pos, 68);
                $pdf->Cell(20, 0, 'FATHER\'S NAME    : '.$Father_Name, 0, false, 'L');

                $pdf->SetXY($left_pos, 74);
                $pdf->Cell(20, 0, 'MOTHER\'S NAME  : '.$MOTHER_NAME, 0, false, 'L');
                 $pdf->SetXY(145, 42.5);
                $pdf->Cell(20, 0, 'ROLL NO: '.$Roll_NO, 0, false, 'L');

                $pdf->SetFont($MTCORSVA, '', 15, '', false); 
                $pdf->setCellPaddings( $left = '', $top = '3', $right = '', $bottom = '');
                $pdf->SetXY($left_pos, 81.5);    
                $pdf->MultiCell(32, 13.5, 'Subject Code', 'LT', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(98, 13.5, 'Subject', 'LT', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(25, 13.5, 'Credits', 'LT', 'C', 0, 0, '', '', true, 0, true);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdf->MultiCell(26, 13.5, 'Grade Awarded', 'LRT', 'C', 0, 0, '', '', true, 0, true);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdf->SetXY($left_pos, 95);
                $pdf->MultiCell(181, 0, '', 'T', 'C', 0, 0);
                $pdf->SetXY($left_pos, 95);
               
                $pdf->MultiCell(32, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(98, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(25, 120, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(26, 120, '', 'LRB', 'C', 0, 0, '', '', true, 0, true);
                ...
                foreach ($subjectsData as $term => $term_array) {
                    ...
                    $lines = $pdf->MultiCell(98, $subHegight, $term_array[3], 'LR', 'L', 0, 0, '', '', true, 0, false,true, 0);
                    ...
                    $pdf->MultiCell(32, $subHegight, $term_array[2], 0, 'L', 0, 0, '', '', true);
                    $pdf->SetX($pdf->GetX());
                    $pdf->MultiCell(98, $subHegight, $term_array[3], 0, 'L', 0, 0, '', '', true);
                    $pdf->SetX($pdf->GetX());
                    $pdf->MultiCell(25, $subHegight, $value , 0, 'C', 0, 0, '', '', true);
                    $pdf->SetX($pdf->GetX());
                    $pdf->MultiCell(26, $subHegight, $term_array[4], 0, 'C', 0, 0, '', '', true);
                    $pdf->Ln();
                    $dataY=$pdf->GetY();
                }
                $pdf->Ln();
                $dataY=$pdf->GetY();
                $pdf->SetFont($MAIAN, '', 13, '', false); 
                $pdf->SetXY($left_pos, 217);
                $pdf->Cell(20, 0, 'S.G.P.A   : '.$studentData[19].'('.$studentData[26].')', 0, false, 'L');
                $pdf->SetX($left_pos+140);
                $pdf->Cell(20, 0, 'Result : '.$studentData[17], 0, false, 'L');
                $pdf->SetXY($left_pos, 225);
                $pdf->Cell(20, 0, 'The date of declaration of result : '.$ResultDate, 0, false, 'L');
				...
				$codeContents =  $STUDENT_NAME . ", " . $studentData[6]  . ",".$studentData[7] .", " .$studentData[3]." "."\n\n". $encryptedString;
        ...
        $file_name =  str_replace("/", "_",'MvsrSGR'.date("Ymdhms")).'.pdf';
