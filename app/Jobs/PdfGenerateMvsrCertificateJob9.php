...
use Carbon\Carbon;
use Helper;
class PdfGenerateMvsrCertificateJob9
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
            ...
            //    $monthYearExam= $this->convertExcelDate($monthYearExam);
              $monthYearExam = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($monthYearExam)->format('M-y');


				$ref_no=trim($studentData[5]);
				$course_name=trim($studentData[6]);
				$branch=trim($studentData[7]);
				$gender=trim($studentData[8]);
				$Roll_No=trim($studentData[9]);
				$hall_ticket_no=trim($studentData[9]);
				$collegeCode=trim($studentData[10]);
				$classAwarded=trim($studentData[11]);
				$serial=trim($studentData[12]);
				$CGPASecured=trim($studentData[20]);
				$CGPARegister=trim($studentData[14]);
				$CreditSecured=trim($studentData[15]);
				$place=trim($studentData[16]);
				$Result_DATE=trim($studentData[22]);
				$Result_DATE=$this->convertExcelDate($studentData[22]);

				$Photo=trim($studentData[23]);
				$marksheet=trim($studentData[24]);
				// $note=trim($studentData[19]);
				$course_year=trim($studentData[20]);
				$Mother_Name=trim($studentData[25]);
				
				$CGPA_Word=trim($studentData[26]);
				$Percentage_marks=trim($studentData[27]);
				$division=trim($studentData[28]);
                $heading1=trim($studentData[30]);
                $heading2=trim($studentData[31]);
				$course=trim($studentData[29]);
				...
				$pdfBig->SetXY(13, 37);             
                $pdfBig->MultiCell(175, 2,  $heading1, 0, 'C', 0, 1, '', '', true);       
                $pdfBig->SetXY(13, $pdfBig->GetY());             
                $pdfBig->MultiCell(175, 2,  $heading2, 0, 'C', 0, 1, '', '', true);
                 $X=12;
                $pdfBig->SetXY($X, $pdfBig->GetY()); 
				$pdfBig->SetFont($arialb, '', 7, '', false); 
				// $pdfBig->SetXY(12, 42.7);
			
                 $pdfBig->MultiCell(175, 2,  $course_name, 0, 'C', 0, 1, '', '', true);
			    $pdfBig->SetXY($X, $pdfBig->GetY());
              ...
              $tableSecondStart=  $pdfBig->GetY();
				...
				foreach($subjects as $term => $term_array){
					...
					if ($term == "I SEMESTER" || $term == "III SEMESTER" || $term == "V SEMESTER" || $term == "VII SEMESTER" ) {
						...
						foreach($term_array as $key => $val){
							...
							$lines = $pdfBig->MultiCell(54, 0, trim($val[4]), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
							$pdfBig=$pdfBig->rollbackTransaction(); // restore previous object
							if($lines>1){
								$subjectTableHeight = 10;
							} else {
							...
							$pdfBig->MultiCell(54, $subjectTableHeight,trim($val[4]), '', 'L', 0, 0, '', '', true, 0, true);
						...
						foreach($term_array as $key => $val){
							...
							$pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
							$pdfBig->MultiCell(54, $subjectTableHeight, trim($val[4]), '0', 'L', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetXY(13, 37);             
                $pdf->MultiCell(175, 2,  $heading1, 0, 'C', 0, 1, '', '', true);       
                $pdf->SetXY(13, $pdf->GetY());             
                $pdf->MultiCell(175, 2,  $heading2, 0, 'C', 0, 1, '', '', true);
                 $X=12;
                $pdf->SetXY($X, $pdf->GetY()); 
                $pdf->SetFont($arialb, '', 7, '', false); 
                // $pdf->SetXY(12, 42.7);
            
                 $pdf->MultiCell(175, 2,  $course_name, 0, 'C', 0, 1, '', '', true);
                $pdf->SetXY($X, $pdf->GetY());
                ...
                foreach($subjects as $term => $term_array){
                    ...
                    if ($term == "I SEMESTER" || $term == "III SEMESTER" || $term == "V SEMESTER" || $term == "VII SEMESTER" ) {
                        ...
                        foreach($term_array as $key => $val){
                            ...
                            $pdf->SetFont($arial, '', 8, '', false);  
                            // get the number of lines
                            $lines = $pdf->MultiCell(54, 0, trim($val[4]), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                            ...
                            $pdf->MultiCell(54, $subjectTableHeight,trim($val[4]), '', 'L', 0, 0, '', '', true, 0, true);
                        ...
                        foreach($term_array as $key => $val){
                            ...
                            $pdf->SetXY($pdf->getX(), $pdf->getY());
                            $pdf->MultiCell(54, $subjectTableHeight, trim($val[4]), '0', 'L', 0, 0, '', '', true, 0, true);
        ...
        if($previewPdf!=1){
			//$this->updateCardNo('MvsrMinor',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MvsrMinor'.date("Ymdhms")).'.pdf';
