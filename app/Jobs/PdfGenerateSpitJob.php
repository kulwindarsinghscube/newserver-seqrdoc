    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if ($studentDataOrg && !empty($studentDataOrg)) {
            ...
            foreach ($studentDataOrg as $studentData) {
                ...
                $upload_sign3_org = $CheckedBy;
                $pathInfo = pathinfo($CheckedBy);
                $CheckedBy = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_".rand(1,1000).".".$pathInfo['extension'];
                \File::copy($upload_sign3_org,$CheckedBy);
                ...
                $upload_sign1_org = $VerifiedBy;
                $pathInfo = pathinfo($VerifiedBy);
                $VerifiedBy = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_".rand(1,1000).".".$pathInfo['extension'];
                \File::copy($upload_sign1_org,$VerifiedBy); 
                ...
                $COE_y = 255;
                $COE_Width = 31.75; //26;
                $COE_Height = 10.847916667; //16;


                $upload_sign2_org = $COE;
                $pathInfo = pathinfo($COE);
                $COE = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_".rand(1,1000).".".$pathInfo['extension'];
                \File::copy($upload_sign2_org,$COE); 
                ...
                */
                ...
                $codeContents .= "\n";
                $codeContents .= $examination;
                $codeContents .= "\n";
                $codeContents .= $academic_year;
                $codeContents .= "\n";
                $codeContents .= $seat_number;
                $codeContents .= "\n\n" . strtoupper(md5($str));
                ...
                $pixel_Size = 1;
                $frame_Size = 1;
                //\PHPQRCode\QRcode::png($codeContents, $qr_code_path);
