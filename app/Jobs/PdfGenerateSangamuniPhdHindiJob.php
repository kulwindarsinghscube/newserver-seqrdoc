...
class PdfGenerateSangamuniPhdHindiJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $Krutidev_N = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Krutidev_N.TTF', 'OpenTypeUnicode', '', 96);
        $Krutidev_100 = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Kruti_Dev_010.ttf', 'OpenTypeUnicode', '', 96);
        
        //$card_serial_no="";
		$preview_serial_no=1;
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('SangamCert');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="sangam_blank.jpg"; 
                $low_res_bg="sangam_blank.jpg";
                $pdfBig->AddPage();
                ...
                $enrolmentNo = $studentData[1];
                $serialNumber = $studentData[2];
                $studentName = $studentData[3];
                $fatherName = $studentData[4];
                $degree = $studentData[5];
                $courseAndBranch = $studentData[6];
                $specialization = $studentData[7];
                $year = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($studentData[8])->format('jS F, Y');;
                $cgpa = $studentData[9];
                $title = $studentData[10];
                $date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($studentData[11])->format('jS F, Y');
                $photo = $studentData[12];

                // $pdfBig reference
                if(request()->template_id != 6){
                    $pdfBig->SetFont('calibri', '', 11.3, '', false); 
                    $pdfBig->setXY(14,14);
                    $pdfBig->MultiCell(28, 0, "Enrolment No. ", '', 'L', 0, 1);

                    $pdfBig->SetFont('Verdana', '', 10, '', false); 
                    $pdfBig->setXY(38.5,14);
                    $pdfBig->MultiCell(50, 0, $enrolmentNo, '', 'L', 0, 1);
                }


                // $pdfBig->SetFont('calibri', '', 11.3, '', false); 
                // $pdfBig->setXY(180,14);
                // $pdfBig->MultiCell(40, 0, $serialNumber, '', 'L', 0, 1);
                ...
                $style1D = array(
                 ...
                 $barodeHeight = 8;
                 $pdfBig->SetAlpha(1);
                 // $pdfBig->write1DBarcode(trim($enrolmentNo), 'C39', $barcodex, $barcodey, $barcodeWidth, $barodeHeight, 0.4, $style1D, 'N');
    			// $pdfBig->SetFont('Arial', '', 8, '', false);
    			// $pdfBig->SetXY(14, 26);
                //          $pdfBig->MultiCell(27, 0, trim($print_serial_no), 0, 'C', 0, 0);
                // Qr Code
                $pdfBig->setPageMark(); 
                ...
                // Micro text with font size 1.2
                $pdfBig->setXY(83,104);
                $pdfBig->SetFont('Arial', '', 1.2, '', false); 
                $pdfBig->MultiCell(18, 4, $studentName, '', 'C', 0, 1);
                ...
                $pdfBig->SetFont('Clearface', '', 18, '', false); 
                $pdfBig->setXY(10,114);
                $pdfBig->MultiCell(186, 4, "The Board of Management of the University\non the recommendation of the Academic Council\nhereby confers upon", '', 'C', 0, 1);


                $pdfBig->SetFont('Times New Roman', 'B', 22, '', false); 
                $pdfBig->setXY(10,138);
                $pdfBig->MultiCell(186, 4, $studentName, '', 'C', 0, 1);


                $pdfBig->SetFont('Clearface', '', 18, '', false); 
                $pdfBig->setXY(10,150);
                $pdfBig->MultiCell(186, 4, "Son/Daughter of ", '', 'C', 0, 1);


                $pdfBig->SetFont('Times New Roman', 'B', 22, '', false); 
                $pdfBig->setXY(10,158);
                $pdfBig->MultiCell(186, 4, $fatherName, '', 'C', 0, 1);

                $pdfBig->SetFont('Clearface', '', 18, '', false); 
                $pdfBig->setXY(10,169);
                if($specialization && $specialization != ""){
                    $pdfBig->MultiCell(186, 4, "the degree of", '', 'C', 0, 1);
                }else{
                    $honorText = "";
                    if(request()->template_id == 6){
                        $honorText = "the honorary degree of";
                    }else{
                        $honorText = "the degree of";
                    }
                    $pdfBig->MultiCell(186, 4, $honorText, '', 'C', 0, 1);
                }

    			// store current object
    			$pdfBig->startTransaction();
    			$pdfBig->SetFont('Times New Roman', 'B', 20, '', false);
    			// get the number of lines
    			$lines = $pdfBig->MultiCell(186, 4, $degree, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
    			// restore previous object
    			$pdfBig=$pdfBig->rollbackTransaction();	
    			
                if($lines>1){
    				$pdfBig->SetFont('Times New Roman', 'B', 15, '', false); 
    			}else{
    				$pdfBig->SetFont('Times New Roman', 'B', 20, '', false); 
    			}
                ...
                $degreeCount = explode("\n",$degree);
                if(count($degreeCount) > 1){
                    $pdfBig->setXY(10,182);
                    $pdfBig->SetFont('Times New Roman', 'B', 22, '', false);
                    $pdfBig->MultiCell(186, 4, $degreeCount[0], '', 'C', 0, 1);

                    $pdfBig->SetFont('Times New Roman', 'B', 15, '', false);
                    $pdfBig->MultiCell(186, 4, $degreeCount[1], '', 'C', 0, 1);
                }else{
                    $pdfBig->MultiCell(186, 4, $degree, '', 'C', 0, 1);
                }

                if($specialization && $specialization != ""){
                    $pdfBig->SetFont('Times New Roman', 'B', 20, '', false); 
                    $pdfBig->setXY(10,191);
                    $pdfBig->MultiCell(186, 4, "(".$specialization.")", '', 'C', 0, 1);
                }
                ...
                $pdfBig->setXY(10, 200);
                if($date && $date != "1st January, 1970" ){
                    $pdfBig->SetFont('Clearface', '', 17, '', false); 
                    if(request()->template_id == 6){
                        $pdfBig->MultiCell(186, 4, "in recognition of the research work completed on ".$date, '', 'C', 0, 1);
                    }else{
                        $pdfBig->MultiCell(186, 4, "in recognition of the research work completed on ".$date, '', 'C', 0, 1);
                    }
                    ...
                    $pdfBig->SetFont('Clearface', '', 17, '', false); 
                    if(request()->template_id == 6){
                        $pdfBig->MultiCell(186, 4, "in recognition of his/her outstanding contributions in the field of", '', 'C', 0, 1);
                    }else{
                        $pdfBig->MultiCell(186, 4, "in recognition of his outstanding contributions in the field of", '', 'C', 0, 1);
                    }
                ...
                // $pdfBig->startTransaction();
                // $pdfBig->SetFont('Times New Roman', 'B', 15, '', false);
                //$pdfBig->SetFont($devanagari, 'B', 15, '', false);
                // get the number of lines
                //$pdfBig->SetFont('kruti_dev_010','',24,'', false);

                // $lines = $pdfBig->MultiCell(186, 0, $title, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                // // restore previous object
                // $pdfBig=$pdfBig->rollbackTransaction(); 
                // if($lines>3){
                //     $pdfBig->SetFont('Times New Roman', 'B', 13, '', false); 
                // }else{
                //     $pdfBig->SetFont('Times New Roman', 'B', 15, '', false); 
                // }
                $pdfBig->SetFont($Krutidev_N, 'B', 15, '', false); 
                $pdfBig->startTransaction();
                $lines = $pdfBig->MultiCell(186, 0, $title, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                // restore previous object
                $pdfBig=$pdfBig->rollbackTransaction(); 
                if($lines>3){
                    $pdfBig->SetFont($Krutidev_N, 'B', 13, '', false); 
                }else{
                    $pdfBig->SetFont($Krutidev_N, 'B', 15, '', false); 
                }
                ...
                $pdfBig->setXY(10,209);
                $pdfBig->MultiCell(186, 0, $title, '', 'C', 0, 1);


                $pdfBig->SetFont('Clearface', '', 18, '', false); 
                $pdfBig->setXY(10, 228);
                $pdfBig->MultiCell(186, 0, "\nand awarded on ".$year."\nunder the seal of the Sangam University, Bhilwara", '', 'C', 0, 1);
                ...
                // $pdfBig reference
                if(request()->template_id != 6){
                    $pdf->SetFont('calibri', '', 11.3, '', false); 
                    $pdf->setXY(14,14);
                    $pdf->MultiCell(28, 0, "Enrolment No. ", '', 'L', 0, 1);

                    $pdf->SetFont('Verdana', '', 10, '', false); 
                    $pdf->setXY(38.5,14);
                    $pdf->MultiCell(50, 0, $enrolmentNo, '', 'L', 0, 1);
                }
                ...
                // $pdf->SetFont('calibri', '', 11.3, '', false); 
                // $pdf->setXY(180,14);
                // $pdf->MultiCell(40, 0, $serialNumber, '', 'L', 0, 1);

                // $pdf->setPageMark(); 
                //1D Barcode
                
                // $pdf->SetAlpha(1);
                // $pdf->write1DBarcode(trim($enrolmentNo), 'C39', $barcodex, $barcodey, $barcodeWidth, $barodeHeight, 0.4, $style1D, 'N');
                // $pdf->SetFont('Arial', '', 8, '', false);
                // $pdf->SetXY(14, 26);
                // $pdf->MultiCell(27, 0, trim($print_serial_no), 0, 'C', 0, 0);
                // Qr Code
                $pdf->setPageMark(); 
                $pdf->write2DBarcode($codeContents, 'QRCODE,Q', $qrCodex, $qrCodey, $qrCodeWidth, $qrCodeHeight, $style1Da, 'N');
                

                // Micro text with font size 1.2
                $pdf->setXY(83,104);
                $pdf->SetFont('Arial', '', 1.2, '', false); 
                $pdf->MultiCell(18, 4, $studentName, '', 'C', 0, 1);
                ...
                $pdf->SetFont('Clearface', '', 18, '', false); 
                $pdf->setXY(10,114);
                $pdf->MultiCell(186, 4, "The Board of Management of the University\non the recommendation of the Academic Council\nhereby confers upon", '', 'C', 0, 1);


                $pdf->SetFont('Times New Roman', 'B', 22, '', false); 
                $pdf->setXY(10,138);
                $pdf->MultiCell(186, 4, $studentName, '', 'C', 0, 1);


                $pdf->SetFont('Clearface', '', 18, '', false); 
                $pdf->setXY(10,150);
                $pdf->MultiCell(186, 4, "Son/Daughter of ", '', 'C', 0, 1);


                $pdf->SetFont('Times New Roman', 'B', 22, '', false); 
                $pdf->setXY(10,158);
                $pdf->MultiCell(186, 4, $fatherName, '', 'C', 0, 1);

                $pdf->SetFont('Clearface', '', 18, '', false); 
                $pdf->setXY(10,169);
                if($specialization && $specialization != ""){
                    $pdf->MultiCell(186, 4, "the degree of", '', 'C', 0, 1);
                }else{
                    if(request()->template_id == 6){
                        $honorText = "the honorary degree of";
                    }else{
                    ...
                    $pdf->MultiCell(186, 4, $honorText, '', 'C', 0, 1);
                }

                // store current object
                $pdf->startTransaction();
                $pdf->SetFont('Times New Roman', 'B', 20, '', false);
                // get the number of lines
                $lines = $pdf->MultiCell(186, 4, $degree, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                // restore previous object
                $pdf=$pdf->rollbackTransaction(); 
                
                if($lines>1){
                    $pdf->SetFont('Times New Roman', 'B', 15, '', false); 
                }else{
                    $pdf->SetFont('Times New Roman', 'B', 20, '', false); 
                }
                if($specialization && $specialization != ""){
                ...
                if(count($degreeCount) > 1){
                    $pdf->setXY(10,182);
                    $pdf->SetFont('Times New Roman', 'B', 20, '', false);
                    $pdf->MultiCell(186, 4, $degreeCount[0], '', 'C', 0, 1);

                    $pdf->SetFont('Times New Roman', 'B', 15, '', false);
                    $pdf->MultiCell(186, 4, $degreeCount[1], '', 'C', 0, 1);
                }else{
                    $pdf->MultiCell(186, 4, $degree, '', 'C', 0, 1);
                }

                if($specialization && $specialization != ""){
                    $pdf->SetFont('Times New Roman', 'B', 20, '', false); 
                    $pdf->setXY(10,191);
                    $pdf->MultiCell(186, 4, "(".$specialization.")", '', 'C', 0, 1);
                }




                $pdf->SetFont('Clearface', '', 17, '', false); 
                $pdf->setXY(10, 200);
                if($date && $date != "1st January, 1970" ){
                    if(request()->template_id == 6){
                        $pdf->MultiCell(186, 4, "in recognition of the research work completed on ".$date, '', 'C', 0, 1);
                    }else{
                        $pdf->MultiCell(186, 4, "in recognition of the research work completed on ".$date, '', 'C', 0, 1);
                    }
                }else{
                    if(request()->template_id == 6){
                        $pdf->MultiCell(186, 4, "in recognition of his/her outstanding contributions in the field of", '', 'C', 0, 1);
                    }else{
                        $pdf->MultiCell(186, 4, "in recognition of his outstanding contributions in the field of", '', 'C', 0, 1);
                    }
                }
                ...
                $pdf->SetFont('Times New Roman', 'B', 15, '', false);
                // get the number of lines
                $lines = $pdf->MultiCell(186, 0, $title, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                // restore previous object
                $pdf=$pdf->rollbackTransaction(); 
                
                if($lines>3){
                    $pdf->SetFont('Times New Roman', 'B', 13, '', false); 
                }else{
                    $pdf->SetFont('Times New Roman', 'B', 15, '', false); 
                }

                $pdf->setXY(10,209);
                $pdf->MultiCell(186, 0, $title, '', 'C', 0, 1);



                $pdf->SetFont('Clearface', '', 18, '', false); 
                $pdf->setXY(10, 228);
                $pdf->MultiCell(186, 0, "and awarded on ".$year."\nunder the seal of the Sangam University, Bhilwara", '', 'C', 0, 1);
                ...
                if($previewPdf!=1){
                    ...
                    $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'SangamCert',$admin_id,$card_serial_no);
        ...
        if($previewPdf!=1){
            $this->updateCardNo('SangamCert',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'SangamCert'.date("Ymdhms")).'.pdf';
