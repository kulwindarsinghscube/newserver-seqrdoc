...
class PdfGenerateAnuFinalAFCACertificateJob
{
      ...
      public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) 
            {
				...
				$high_res_bg="AFCA_Certificate_Background_old.jpg"; // bestiu_pdc_bg, TranscriptData.jpg
				$low_res_bg="AFCA_Certificate_Background.jpg";
				$pdfBig->AddPage();
                ...
                $pdfBig->SetXY($X, $Y);  
                $pdfBig->MultiCell(0, 10, $studentData[2], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[3], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetFont($EBGaramondRegular, '', 28, '', false); 
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[4], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetFont($EBGaramondRegular, '', 18, '', false); 
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[5], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetFont($EBGaramondExtraBold, '', 26.77, '', false); 
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[6], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetFont($EBGaramondRegular, '', 18, '', false); 
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[7], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[8], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 10, $studentData[9], 0, 'C', 0, 0, '', '', true, 0, true);


                $pdfBig->SetFont($EBGaramondRegular, '', 12, '', false); 
                 $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 5, $studentData[10], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 5, $studentData[11], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $pdfBig->SetXY($X, $pdfBig->GetY());  
                $pdfBig->MultiCell(0, 5, $studentData[12], 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                if(!empty($studentData[13])){
                 ...
                 if(!empty($studentData[15])){
                    ...
                    if(!empty($studentData[17])){
                        ...
                        $pdfBig->Cell(40, 0, $studentData[18], 0, false, 'C');
                   }
             ...
             $pdf->SetTextColor(0, 0, 0);
                ...
                $pdf->SetXY($X, $Y);  
                $pdf->MultiCell(0, 10, $studentData[2], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[3], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                 $text = str_replace(["\r\n", "\r", "\n"], "\n", $studentData[4]);
                $pdf->SetFont($EBGaramondRegular, '', 28, '', false); 
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10,  $text, 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetFont($EBGaramondRegular, '', 18, '', false); 
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[5], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetFont($EBGaramondExtraBold, '', 26.77, '', false); 
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[6], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetFont($EBGaramondRegular, '', 18, '', false); 
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[7], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[8], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 10, $studentData[9], 0, 'C', 0, 0, '', '', true, 0, true);


                $pdf->SetFont($EBGaramondRegular, '', 12, '', false); 
                 $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 5, $studentData[10], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 5, $studentData[11], 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $pdf->SetXY($X, $pdf->GetY());  
                $pdf->MultiCell(0, 5, $studentData[12], 0, 'C', 0, 0, '', '', true, 0, true);
                 ...
                 if(!empty($studentData[15])){
                    ...
                    if(!empty($studentData[17])){
                        ...
                        $pdf->Cell(40, 0, $studentData[17], 0, false, 'C');
                         $pdf->SetFont($MontserratRegular, '', 7.78, '', false);
                        $pdf->SetXY(85.3, 267);
                        $pdf->Cell(40, 0, $studentData[18], 0, false, 'C');
                   }
                ...
                } elseif(strpos($name1,'Sanjay Bhatanagar') !== FALSE) {
            ...
            if(!empty($studentData[15])){
                ...
                } elseif(strpos($name2,'Sanjay Bhatanagar') !== FALSE) {
                ...
                elseif(strpos($name2,'Dr Sanjeev Vidyarthi') !== FALSE) {
                   //$sign2 = public_path().'\\'.$subdomain[0].'\backend\canvas\images\Sanjeev.png';
            ...
            if(!empty($studentData[17])){
                ...
                } elseif(strpos($name3,'Sanjay Bhatanagar') !== FALSE) {
                ...
                elseif(strpos($name3,'Dr Sanjeev Vidyarthi') !== FALSE) {
                   //$sign3 = public_path().'\\'.$subdomain[0].'\backend\canvas\images\Sanjeev.png';
            ...
            //Bottom Blockchain URL Link in image By rohit 21-07-2025
                ...
                $mintData['metadata3']=["label"=> "Student Name", "value"=>$studentData[4]];
                $mintData['metadata4']=["label"=> "Year", "value"=> $year];
                $mintData['metadata5']=["label"=> "Title", "value"=> $studentData[6]];
         ...
         if($previewPdf!=1){
            $this->updateCardNo('anuAFCA',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'anuAFCA'.date("Ymdhms")).'.pdf';
