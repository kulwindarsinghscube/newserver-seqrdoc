...
class PdfGenerateBestiuEDocumentJob1
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('BestiuEG');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $CREDITS_EARNED = trim($studentData[64]);
                $SGPA=trim($studentData[65]);
                $CGPA=trim($studentData[66]);
                $TOTAL_CREDITS_CREDIT_HOURS=trim($studentData[67]);
                $TOTAL_CREDITS_CREDIT_POINTS=trim($studentData[68]);
                
                $schoolName=trim($studentData[69]);
                ...
                $pdfBig->SetFont($arialb, '', 12, '', false); 
                $pdfBig->SetXY(10, 47);
                $pdfBig->MultiCell(190, 0, 'SEMESTER GRADE CARD', 0, "C", 0, 0, '', '', true, 0, true);
                ...
                $lines = $pdfBig->MultiCell(55, 0, $PROGRAMME, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY(12, 62);
                $pdfBig->MultiCell(110, $height, 'Student Name : '.$candidate_name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), 62);
                $pdfBig->MultiCell(22, $height, 'Programme : ', 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY(143.5, 62);
                $pdfBig->MultiCell(55, $height, $PROGRAMME, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->ln();

                $pdfBig->SetXY(12, $pdfBig->getY());
                $pdfBig->MultiCell(110, 8, 'Father’s Name : '.$Fathers_Name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(77, 8, 'Year of Admission : '.$batch, 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();

                $pdfBig->SetXY(12, $pdfBig->getY());
                $pdfBig->MultiCell(190, 0, 'Examination : '.$examination, 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();

                $subjectY = $pdfBig->getY() +8;

                $pdfBig->setCellPaddings( $left = '', $top = '1.5', $right = '', $bottom = '');
                $pdfBig->SetFont($arialb, '', 12, '', false); 
                $pdfBig->SetXY(12, $subjectY);
                $pdfBig->MultiCell(186, 8, 'SEMESTER – ' .$semester, 1, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY(12, $subjectY);
                $pdfBig->MultiCell(35, 11, 'Course Code', 'LRTB', "C", 0, 0, '', '', true, 0, true);

                $pdfBig->setCellPaddings( $left = '', $top = '3.8', $right = '', $bottom = '');
                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                $pdfBig->MultiCell(102, 11, 'Course Title', 'LRTB', "C", 0, 0, '', '', true, 0, true);

                $pdfBig->setCellPaddings( $left = '', $top = '3.8', $right = '', $bottom = '');
                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                $pdfBig->MultiCell(15, 11, 'Credits', 'LRTB', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                $pdfBig->MultiCell(17, 11, 'Credit Points', 'LRTB', "C", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();
                ...
                for ($i = 1; $i < $length; $i++) {
                    ...
                    $lines = $pdfBig->MultiCell(102, $subHegight, $courseTitle, 'LR', 'L', 0, 0, '', '', true, 0, false,true, 0);
                    ...
                    if (empty($courseTitle) || $isLast) {
                        $border = 'LRB';
                    } else {
                        $border = 'LR';
                    }
                    // // Skip row only if not last and title is empty
                    // if (!$isLast && ($courseTitle === null || $courseTitle === '')) {
                    //     break;
                    // }
                    ...
                    $pdfBig->SetXY(12, $subjectY);
                    $pdfBig->MultiCell(35, $subHegight, $courseCode, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                    $pdfBig->MultiCell(102, $subHegight, $courseTitle, $border, "L", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                    $pdfBig->MultiCell(15, $subHegight, $courseCreditHours, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                    $pdfBig->MultiCell(17, $subHegight, $courseGradePoint, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdfBig->SetXY($pdfBig->getX(), $subjectY);
                    $pdfBig->MultiCell(17, $subHegight, $creditPoint, $border, "C", 0, 0, '', '', true, 0, true);
                    ...
                    // $totalCredit += is_numeric($courseCreditHours) ? (float)$courseCreditHours : 0;
                    // $totalCreditPoint += is_numeric($creditPoint) ? (float)$creditPoint : 0;
                }
                ...
                $pdfBig->SetXY(12, $y);
                $pdfBig->MultiCell(137, 0, 'TOTAL', 1, "R", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $y);
                $pdfBig->MultiCell(15, 0, $TOTAL_CREDITS_CREDIT_HOURS, 1, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $y);
                $pdfBig->MultiCell(17, 0, '', 1, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $y);
                $pdfBig->MultiCell(17, 0, $TOTAL_CREDITS_CREDIT_POINTS, 1, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetFont($arialb, '', 11, '', false); 
                $pdfBig->SetXY(12, $y);
                $pdfBig->MultiCell(93, 0, 'SGPA : '.$SGPA, 1, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $y);
                $pdfBig->MultiCell(93, 0, 'CGPA : '.$CGPA, 1, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 12, '', false); 
                $pdf->SetXY(10, 47);
                $pdf->MultiCell(190, 0, 'SEMESTER GRADE CARD', 0, "C", 0, 0, '', '', true, 0, true);
                ...
                // get the number of lines
                $lines = $pdf->MultiCell(55, 0, $PROGRAMME, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY(12, 62);
                $pdf->MultiCell(110, $height, 'Student Name : '.$candidate_name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), 62);
                $pdf->MultiCell(22, $height, 'Programme : ', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY(143.5, 62);
                $pdf->MultiCell(55, $height, $PROGRAMME, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->ln();

                $pdf->SetXY(12, $pdf->getY());
                $pdf->MultiCell(110, 8, 'Father’s Name : '.$Fathers_Name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(77, 8, 'Year of Admission : '.$batch, 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->ln();

                $pdf->SetXY(12, $pdf->getY());
                $pdf->MultiCell(190, 0, 'Examination : '.$examination, 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->ln();
                ...
                $pdf->MultiCell(186, 8, 'SEMESTER – ' .$semester, 1, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY(12, $subjectY);
                $pdf->MultiCell(35, 11, 'Course Code', 'LRTB', "C", 0, 0, '', '', true, 0, true);

                $pdf->setCellPaddings( $left = '', $top = '3.8', $right = '', $bottom = '');
                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY($pdf->getX(), $subjectY);
                $pdf->MultiCell(102, 11, 'Course Title', 'LRTB', "C", 0, 0, '', '', true, 0, true);

                $pdf->setCellPaddings( $left = '', $top = '3.8', $right = '', $bottom = '');
                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY($pdf->getX(), $subjectY);
                $pdf->MultiCell(15, 11, 'Credits', 'LRTB', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY($pdf->getX(), $subjectY);
                $pdf->MultiCell(17, 11, 'Credit Points', 'LRTB', "C", 0, 0, '', '', true, 0, true);
                $pdf->ln();
                ...
                for ($i = 1; $i < $length; $i++) {
                    ...
                    $lines = $pdf->MultiCell(102, $subHegight, $courseTitle, 'LR', 'L', 0, 0, '', '', true, 0, false,true, 0);
                    ...
                    $pdf->SetXY(12, $subjectY);
                    $pdf->MultiCell(35, $subHegight, $courseCode, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetXY($pdf->getX(), $subjectY);
                    $pdf->MultiCell(102, $subHegight, $courseTitle, $border, "L", 0, 0, '', '', true, 0, true);

                    $pdf->SetXY($pdf->getX(), $subjectY);
                    $pdf->MultiCell(15, $subHegight, $courseCreditHours, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetXY($pdf->getX(), $subjectY);
                    $pdf->MultiCell(17, $subHegight, $courseGradePoint, $border, "C", 0, 0, '', '', true, 0, true);

                    $pdf->SetXY($pdf->getX(), $subjectY);
                    $pdf->MultiCell(17, $subHegight, $creditPoint, $border, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetXY(12, $y);
                $pdf->MultiCell(137, 0, 'TOTAL', 1, "R", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $y);
                $pdf->MultiCell(15, 0, $TOTAL_CREDITS_CREDIT_HOURS, 1, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $y);
                $pdf->MultiCell(17, 0, '', 1, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $y);
                $pdf->MultiCell(17, 0, $TOTAL_CREDITS_CREDIT_POINTS, 1, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 11, '', false); 
                $pdf->SetXY(12, $y);
                $pdf->MultiCell(93, 0, 'SGPA : '.$SGPA, 1, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $y);
                $pdf->MultiCell(93, 0, 'CGPA : '.$CGPA, 1, "L", 0, 0, '', '', true, 0, true);
        ...
        //removed zip code 
        if($previewPdf!=1){
            $this->updateCardNo('BestiuEGradeCard',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'BestiuEGradeCard'.date("Ymdhms")).'.pdf';
       ...
       if($previewPdf!=1){
            ...
            $template_name="BestiuEGradeCard";
