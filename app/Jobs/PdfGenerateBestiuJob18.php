...
class PdfGenerateBestiuJob18
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $gbi = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Georgia-Bold.ttf', 'TrueTypeUnicode', '', 96);
        $fp = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Fertigo-Pro.ttf', 'TrueTypeUnicode', '', 96);
        $oef = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\old-english-five.ttf', 'TrueTypeUnicode', '', 96);
        $oefI = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\OldEnglish_I.ttf', 'TrueTypeUnicode', '', 96);
		$arialMTblack = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\ARIBL0.ttf', 'TrueTypeUnicode', '', 96);
		$timesbI = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\times new roman italic_I.ttf', 'TrueTypeUnicode', '', 96);
        $timesb = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Times-New-Roman-Bold.ttf', 'TrueTypeUnicode', '', 96);
        
        $preview_serial_no=1;
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('BestiuPDC');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="0946 Best Innovation University Degree Certificate A4_BG.jpg"; // bestiu_pdc_bg, bestiu_pdc_bg_data
                $low_res_bg="0946 Best Innovation University Degree Certificate A4_BG.jpg";
                $pdfBig->AddPage();
                ...
                $serialNo = trim($studentData[0]);
                $unique_id = trim($studentData[1]);
                $id_no = trim($studentData[2]);
                $candidate_name = trim($studentData[3]);
                $father_name = trim($studentData[4]);
                $degree = trim($studentData[5]); 
                $Specialization = trim($studentData[6]);  
                $topic = trim($studentData[7]);     
                $completion_date = trim($studentData[8]);
                // $average = trim($studentData[6]);
                // $percentage = trim($studentData[7]);
                // $division = trim($studentData[8]);
                $dated = trim($studentData[9]);
                
                $PHOTO = trim($studentData[10]);
                $AadharNumber = trim($studentData[11]);
                $Child_Relationship = trim($studentData[12]);
                $father_prefix = trim($studentData[13]);
                // $Gender_Specific2 = trim($studentData[14]);
                // $Specialization = trim($studentData[15]);
                ...
                $pdfBig->SetXY(11, 11);
                // $pdfBig->Cell(0, 0, 'Serial No.: '.$serialNo, 0, false, 'L');
                $pdfBig->SetXY(160, 11);
                $pdfBig->Cell(0, 0, 'ID No. '.$id_no, 0, false, 'L');

                if($AadharNumber) {
                    $pdfBig->SetFont($arial, '', 10, '', false); 
                    $pdfBig->SetTextColor(0, 0, 0);    
                    $pdfBig->SetXY(147, 45);
                    $pdfBig->Cell(0, 0, 'AADHAR NO. '.$AadharNumber, 0, false, 'L');
                }

                $pdfBig->SetFont($arialMTblack, '', 18, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, 55);
                $pdfBig->MultiCell(186, 0, $candidate_name, 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->SetFont($arial, '', 17, '', false); 
                $pdfBig->setCellHeightRatio(0.9);
                $pdfBig->SetXY(12, 66);
                $pdfBig->MultiCell(186, 0, $Child_Relationship.' '.$father_prefix. ' '.$father_name,  0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->SetFont($oef, '', 15, '', false); 
                $pdfBig->SetTextColor(60, 60, 60); 
                $pdfBig->setCellHeightRatio(1.6);
                $pdfBig->SetXY(12, 87);
                $pdfBig->MultiCell(186, 0, 'having Fulfilled the Academic Requirements '.$completion_date.' has this day been admitted by the Executive Council to the Degree of', 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetFont($gbi, '', 27, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);   
                $pdfBig->SetXY(12, 124);
                $pdfBig->MultiCell(186, 0, $degree , 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetFont($fp, '', 22, '', false); 
                $pdfBig->SetXY(12, 144);
                $pdfBig->MultiCell(186, 0, $Specialization , 0, 'C', 0, 0, '', '', true, 0, true);
                
                // store current object
                $pdfBig->startTransaction();
                $pdfBig->SetFont($fp, '', 22, '', false); 
                // get the number of lines
                $lines = $pdfBig->MultiCell(186, 0, $Specialization, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                // $y = 162;
                // $pdfBig->SetFont($oef, '', 13, '', false); 
                // $pdfBig->SetXY(10, $y);   
                // $pdfBig->Cell(190, 0, 'of this University on '.$completion_date.'.', 0, false, 'C');

                $pdfBig->SetFont($timesbI, 'I', 16, '', false); 
                $pdfBig->SetTextColor(60, 60, 60);    
                $pdfBig->setCellHeightRatio(1.1);
                $pdfBig->SetXY(12, $y);   
                // $pdfBig->Cell(190, 0, 'Topic: '.$topic.'.', 0, false, 'C');
                $pdfBig->MultiCell(186, 0, 'Topic: '.$topic, 0, 'C', 0, 0, '', '', true, 0, true);


                $y = $y + 48;       
                $pdfBig->SetFont($oef, '', 15, '', false); 
                $pdfBig->SetTextColor(80, 80, 80);    
                $pdfBig->SetFontStretching(85); 	
                $pdfBig->SetXY(12, $y); //174 
                $pdfBig->MultiCell(186, 0, 'Given under the seal of the University', 0, 'C', 0, 0, '', '', true, 0, true);
               	$pdfBig->SetFontStretching(100); // default normal width	
                
                // $y = $y +10;      
                // $pdfBig->SetXY(10, $y); //184
                // $pdfBig->Cell(190, 0, 'and percentage of marks is '.$percentage.'.', 0, false, 'C');			
                
                // $y = $y +11;
                // $pdfBig->SetXY(10, $y); //195
                // $pdfBig->MultiCell(190, 0, $Gender_Specific2.' has been placed in <span style="font-family:'.$arialMTblack.';">'.$division.'</span> Division.', 0, 'C', 0, 0, '', '', true, 0, true);	
                
                // $y = $y +13;
                // $pdfBig->SetXY(45.5, $y);  //208
                // $pdfBig->Cell(0, 0, 'The degree will be conferred on him/her at the', 0, false, 'L');
                
                // $y = $y +10;
                // $pdfBig->SetXY(67, $y); // 218 
                // $pdfBig->Cell(0, 0, 'next convocation or thereafter.', 0, false, 'L');
                
                
                $pdfBig->SetFont($arial, '', 10, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);    
                $pdfBig->SetXY(12, 275);
                $pdfBig->Cell(0, 0, 'Dated: '.$dated, 0, false, 'L');  
                // $pdfBig->SetFont($arial, '', 11, '', false); 
                // $pdfBig->SetXY(12, 260);
                // $pdfBig->Cell(0, 0, 'Controller of Examination', 0, false, 'L');  

                $pdfBig->SetFont($timesb, '', 14, '', false); 
                $pdfBig->SetXY(152, 260);
                ...
                $pdf->SetXY(11, 11);
                // $pdf->Cell(0, 0, 'Serial No.: '.$serialNo, 0, false, 'L');
                $pdf->SetXY(160, 11);
                $pdf->Cell(0, 0, 'ID No. '.$id_no, 0, false, 'L');

                if($AadharNumber) {
                    $pdf->SetFont($arial, '', 10, '', false); 
                    $pdf->SetTextColor(0, 0, 0);    
                    $pdf->SetXY(147, 45);
                    $pdf->Cell(0, 0, 'AADHAR NO. '.$AadharNumber, 0, false, 'L');
                }

                $pdf->SetFont($arialMTblack, '', 18, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, 55);
                $pdf->MultiCell(186, 0, $candidate_name, 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->SetFont($arial, '', 17, '', false); 
                $pdf->setCellHeightRatio(0.9);
                $pdf->SetXY(12, 66);
                $pdf->MultiCell(186, 0, $Child_Relationship.' '.$father_prefix. ' '.$father_name,  0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->SetFont($oef, '', 15, '', false); 
                $pdf->SetTextColor(60, 60, 60); 
                $pdf->setCellHeightRatio(1.6);
                $pdf->SetXY(12, 87);
                $pdf->MultiCell(186, 0, 'having Fulfilled the Academic Requirements '.$completion_date.' has this day been admitted by the Executive Council to the Degree of', 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetFont($gbi, '', 27, '', false); 
                $pdf->SetTextColor(0, 0, 0);   
                $pdf->SetXY(12, 124);
                $pdf->MultiCell(186, 0, $degree , 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetFont($fp, '', 22, '', false); 
                $pdf->SetXY(12, 144);
                $pdf->MultiCell(186, 0, $Specialization , 0, 'C', 0, 0, '', '', true, 0, true);
                
                // store current object
                $pdf->startTransaction();
                $pdf->SetFont($fp, '', 22, '', false); 
                // get the number of lines
                $lines = $pdf->MultiCell(186, 0, $Specialization, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                $pdf->SetFont($timesbI, 'I', 16, '', false); 
                $pdf->SetTextColor(60, 60, 60);    
                $pdf->setCellHeightRatio(1.1);
                $pdf->SetXY(12, $y);   
                // $pdf->Cell(190, 0, 'Topic: '.$topic.'.', 0, false, 'C');
                $pdf->MultiCell(186, 0, 'Topic: '.$topic, 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetXY(12, $y); //174 
                $pdf->MultiCell(186, 0, 'Given under the seal of the University', 0, 'C', 0, 0, '', '', true, 0, true);
                // $pdf->Cell(190, 0, 'Given under the seal of the University', 0, false, 'C');	
                $pdf->SetFontStretching(100);		
                
                $pdf->SetFont($arial, '', 10, '', false); 
                $pdf->SetTextColor(0, 0, 0);    
                $pdf->SetXY(12, 275);
                $pdf->Cell(0, 0, 'Dated: '.$dated, 0, false, 'L');  
                ...
                // Ghost image
                // $candidate_name_u = substr($candidate_name, strpos($candidate_name, ".") + 1);
                // $nameOrg=$candidate_name_u;
                // dd($candidate_name_u);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('BestiuPDC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'BestiuPDC'.date("Ymdhms")).'.pdf';
