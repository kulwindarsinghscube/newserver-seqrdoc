...
use Helper;
class PdfGenerateSdmMbbsIIIJob_3subjects
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$TA_Max3 = trim($studentData[31]);
			$TA_Min3 = trim($studentData[32]);
			$TA_OBT3 = trim($studentData[33]);
			$PV_MAX3 = trim($studentData[34]);
			$PV_MIN3 = trim($studentData[35]);
			$PV_OBT3 = trim($studentData[36]);
			$TOTAL_MAX3 = trim($studentData[37]);
			$TOTAL_MIN3 = trim($studentData[38]);
			$TOTAL_OBT3 = trim($studentData[39]);
			$REMARK3 = trim($studentData[40]);
			
			$Total_UE_MAX = trim($studentData[41]);
			$Total_UE_MIN = trim($studentData[42]);
			$Total_UE_OBT = trim($studentData[43]);
			$Total_IA_MAX = trim($studentData[44]);
			$Total_IA_MIN = trim($studentData[45]);
			$Total_IA_OBT = trim($studentData[46]);			
			$Total_UEIA_MAX = trim($studentData[47]);
			$Total_UEIA_MIN = trim($studentData[48]);
			$Total_UEIA_OBT = trim($studentData[49]);
			$Grand_Total_In_Words = trim($studentData[50]);
			$Percentage = trim($studentData[51]);
			$Result = strtoupper(trim($studentData[52]));
			
			//INTERNAL ASSESSMENT(IA)
			$IA_SL_no1 = strtoupper(trim($studentData[53]));
			$IA_Subject_no1 = strtoupper(trim($studentData[54]));
			$IA_T_Max1 = strtoupper(trim($studentData[55]));
			$IA_T_Min1 = strtoupper(trim($studentData[56]));
			$IA_T_OBT1 = strtoupper(trim($studentData[57]));
			$IA_P_Max1 = strtoupper(trim($studentData[58]));
			$IA_P_Min1 = strtoupper(trim($studentData[59]));
			$IA_P_OBT1 = strtoupper(trim($studentData[60]));
			$IA_TO_MAX1 = strtoupper(trim($studentData[61]));
			$IA_TO_MIN1 = strtoupper(trim($studentData[62]));
			$IA_TO_OBT1 = strtoupper(trim($studentData[63]));
			
			$IA_SL_no2 = strtoupper(trim($studentData[64]));
			$IA_Subject_no2 = strtoupper(trim($studentData[65]));
			$IA_T_Max2 = strtoupper(trim($studentData[66]));
			$IA_T_Min2 = strtoupper(trim($studentData[67]));
			$IA_T_OBT2 = strtoupper(trim($studentData[68]));
			$IA_P_Max2 = strtoupper(trim($studentData[69]));
			$IA_P_Min2 = strtoupper(trim($studentData[70]));
			$IA_P_OBT2 = strtoupper(trim($studentData[71]));
			$IA_TO_MAX2 = strtoupper(trim($studentData[72]));
			$IA_TO_MIN2 = strtoupper(trim($studentData[73]));
			$IA_TO_OBT2 = strtoupper(trim($studentData[74]));
			
						
			$ELIGIBILITY_FOR_UE = strtoupper(trim($studentData[75]));
			$GRAND_TOTAL_MAX = strtoupper(trim($studentData[76]));
			$GRAND_TOTAL_MIN = strtoupper(trim($studentData[77]));
			$GRAND_TOTAL_OBT = strtoupper(trim($studentData[78]));			
			$Date = trim($studentData[79]);
			$batch_of_qr = trim($studentData[81]);
			$aadhar_number_qr = trim($studentData[82]);
			$DOB = trim($studentData[83]);			
			$course = strtoupper(trim($studentData[84]));            
            
			$QR_Output="Batch Of: $batch_of_qr\nName of the Student: $candidate_name\nAadhar Number: $aadhar_number_qr\nCourse: $course\nYear: $year\nDate Of Birth: $DOB";            
			$footer_note="SDM College of MedicalSciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India";
			...
			$pdfBig->MultiCell(9.5, 10, '2', 'LRB', 'C', 0, 0);
			$pdfBig->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
            ...
            $pdf->MultiCell(9.5, 10, '2', 'LRB', 'C', 0, 0);
            $pdf->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
			...
			$pdfBig->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
			$pdfBig->MultiCell(44.5, 5, $Subject3, 'B', 'L', 0, 0);					
			$pdfBig->MultiCell(9, 5, $T_Max3, 'LRB', 'C', 0, 0);
			$pdfBig->MultiCell(9, 5, $T_Min3, 'LRB', 'C', 0, 0);
			$pdfBig->MultiCell(9, 5, $T_OBT3, 'LRB', 'C', 0, 0);	
			$pdfBig->setCellPaddings( $left = '', $top = 0, $right = '', $bottom = '');	

		    $subj_y=$pdf->GetY()+5;
            $pdf->SetXY(24.5, $subj_y);
            $pdf->setCellPaddings( $left = '', $top = 1, $right = '', $bottom = '');
            $pdf->MultiCell(44.5, 5, $Subject3, 'B', 'L', 0, 0);                 
            $pdf->MultiCell(9, 5, $T_Max3, 'LRB', 'C', 0, 0);
            $pdf->MultiCell(9, 5, $T_Min3, 'LRB', 'C', 0, 0);
            $pdf->MultiCell(9, 5, $T_OBT3, 'LRB', 'C', 0, 0);    
            $pdf->setCellPaddings( $left = '', $top = 0, $right = '', $bottom = ''); 		
            ...
            $pdfBig->MultiCell(18, 10,  $ELIGIBILITY_FOR_UE, 'LR', 'C', 0, 0);
            ...
            $pdf->MultiCell(18, 10,  $ELIGIBILITY_FOR_UE, 'LR', 'C', 0, 0);
			...
			*/
            ...
            $pdfBig->image($COE,$COE_x,$COE_y,$COE_Width,$COE_Height,"",'','L',true);
            $pdfBig->setPageMark();
			$pdf->image($COE,$COE_x,$COE_y,$COE_Width,$COE_Height,"",'','L',true);
            $pdf->setPageMark();
