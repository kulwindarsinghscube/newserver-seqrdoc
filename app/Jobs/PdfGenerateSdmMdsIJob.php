...
class PdfGenerateSdmMdsIJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->MultiCell(180, 49, '', 'LRTB', 'C', 0, 0);	//Entire Table
			$pdfBig->SetXY($left_str_x, 119.5);	
			$pdfBig->MultiCell(180, 18, '', 'T', 'C', 0, 0);	//Paper 
			$pdfBig->SetXY($left_str_x, 125);	
			$pdfBig->MultiCell(180, 6, '', 'TB', 'C', 0, 0); //Grand Total
			$pdfBig->SetXY(26.5, 102);	
			$pdfBig->MultiCell(46, 23, '', 'LR', 'C', 0, 0);  //Subject
			$pdfBig->SetXY(123, 102);	
			$pdfBig->MultiCell(112, 49, '', 'L', 'C', 0, 0); //middle vline
			
			$pdfBig->SetXY($left_str_x, 138);	
			$pdfBig->MultiCell(180, 6, '', 'TB', 'C', 0, 0); //Grand Total Line
			
			$pdfBig->SetXY(172, 102);	
			$pdfBig->MultiCell(1, 29, '', 'L', 'C', 0, 0); //Remark
			...
			$pdfBig->SetXY(72, 106);
			$pdfBig->MultiCell(50, 5, 'UNIVERSITY EXAMINATION', 0, 'C', 0, 0);
			$pdfBig->SetXY(120, 106);
			$pdfBig->MultiCell(54, 5, 'TOTAL', 0, 'C', 0, 0);			
			$pdfBig->SetXY(172, 109);
			$pdfBig->MultiCell(23, 5, 'REMARK', 0, 'C', 0, 0);
			
			$pdfBig->SetXY(72.5, 114);
			$pdfBig->MultiCell(17, 11, '', 'RT', 'C', 0, 0);
			$pdfBig->MultiCell(17, 11, '', 'RT', 'C', 0, 0);
			$pdfBig->MultiCell(17, 40, '', 'T', 'C', 0, 0);
			$pdfBig->SetXY(121, 114);
			$pdfBig->MultiCell(18, 17, '', 'RT', 'C', 0, 0);
			$pdfBig->MultiCell(17, 17, '', 'RT', 'C', 0, 0);
			...
			$pdf->MultiCell(180, 49, '', 'LRTB', 'C', 0, 0);	//Entire Table
			$pdf->SetXY($left_str_x, 119.5);	
			$pdf->MultiCell(180, 18, '', 'T', 'C', 0, 0);	//Paper 
			$pdf->SetXY($left_str_x, 125);	
			$pdf->MultiCell(180, 6, '', 'TB', 'C', 0, 0); //Grand Total
			$pdf->SetXY(26.5, 102);	
			$pdf->MultiCell(46, 23, '', 'LR', 'C', 0, 0);  //Subject
			$pdf->SetXY(123, 102);	
			$pdf->MultiCell(112, 49, '', 'L', 'C', 0, 0); //middle vline
			
			$pdf->SetXY($left_str_x, 138);	
			$pdf->MultiCell(180, 6, '', 'TB', 'C', 0, 0); //Grand Total Line
			
			$pdf->SetXY(172, 102);	
			$pdf->MultiCell(1, 29, '', 'L', 'C', 0, 0); //Remark
			...
			$pdf->SetXY(72, 106);
			$pdf->MultiCell(50, 5, 'UNIVERSITY EXAMINATION', 0, 'C', 0, 0);
			$pdf->SetXY(120, 106);
			$pdf->MultiCell(54, 5, 'TOTAL', 0, 'C', 0, 0);			
			$pdf->SetXY(172, 109);
			$pdf->MultiCell(23, 5, 'REMARK', 0, 'C', 0, 0);
			
			$pdf->SetXY(72.5, 114);
			$pdf->MultiCell(17, 11, '', 'RT', 'C', 0, 0);
			$pdf->MultiCell(17, 11, '', 'RT', 'C', 0, 0);
			$pdf->MultiCell(17, 40, '', 'T', 'C', 0, 0);
			$pdf->SetXY(121, 114);
			$pdf->MultiCell(18, 17, '', 'RT', 'C', 0, 0);
			$pdf->MultiCell(17, 17, '', 'RT', 'C', 0, 0);
			...
			$T_Max1 = trim($studentData[11]);			
			$T_Min1 = trim($studentData[12]);
			$T_Sec1 = trim($studentData[13]);			
			$Remark1 = strtoupper(trim($studentData[14]));			
			$Grand_Total_Max = trim($studentData[15]);			
			$Grand_Total_Min = trim($studentData[16]);
			$Grand_Total_Sec = trim($studentData[17]);
			$Grand_Total_In_Words = trim($studentData[18]);
			$Percentage = trim($studentData[19]);
			$Result = strtoupper(trim($studentData[20]));
			$Date = trim($studentData[21]);
			$batch_of_qr = trim($studentData[23]);
			$aadhar_number_qr = trim($studentData[24]);
			$DOB = trim($studentData[25]);			
			$course_qr = strtoupper(trim($studentData[26]));			
			...
			$pdfBig->Cell(188, 0, "PART I", 0, false, 'C'); 
			...
			*/
			...
			$pdf->Cell(188, 0, "PART I", 0, false, 'C'); 
