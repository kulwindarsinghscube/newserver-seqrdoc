...
use Carbon\Carbon;
use Helper;
class PdfGenerateMvsrCertificateJob7
{
...
// dd($studentDataOrg);
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
				...
				$high_res_bg="mvsr_cgc_bg.jpg"; // bestiu_pdc_bg, TranscriptData.jpg
				$low_res_bg="mvsr_cgc_bg.jpg";
				$pdfBig->AddPage();
				...
				$pdfBig->Cell(0, 0, 'Course/Branch', 0, false, 'L');
				...
				foreach($subjects as $term => $term_array){
					...
					if ($term == "I SEMESTER" || $term == "III SEMESTER" || $term == "V SEMESTER" || $term == "VII SEMESTER" ) {
						...
						$pdfBig->MultiCell(88.5, 5, $term .' (SGPA : '.$term_array[0][7].')', 'TB', 'C', 0, 0, '', '', true, 0, true);
        ...
        $pdf->Cell(0, 0, 'Course/Branch', 0, false, 'L');
        $pdf->SetXY(145, 55.5);    
        $pdf->Cell(0, 0, ': ', 0, false, 'L');
        $pdf->SetFont($arialb, '', 7, '', false);
        $pdf->SetXY(147, 55.5);    
