...
class PdfGenerateNnpJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $lucidaCalligraphy = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\LucidaUnicodeCalligraphy.ttf', 'TrueTypeUnicode', '', 96);

        $lucidaCalligraphyBold = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\LucidaUnicodeCalligraphyBold.ttf', 'TrueTypeUnicode', '', 96);
        // Palatino Linotype family
        $palatino       = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\palatinolinotype_roman.ttf',       'TrueTypeUnicode', '', 96);
        $palatinoItalic = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\palatinolinotype_italic.ttf',     'TrueTypeUnicode', '', 96);
        $palatinoBold   = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\palatinolinotype_bold.ttf',       'TrueTypeUnicode', '', 96);
        $palatinoBoldItalic = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\palatinolinotype_bolditalic.ttf', 'TrueTypeUnicode', '', 96);
        $Calibri = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\palatinolinotype_bolditalic.ttf', 'TrueTypeUnicode', '', 96);
        $calibri  = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\Calibri.ttf', 'TrueTypeUnicode', '', 96);
        $calibrib = TCPDF_FONTS::addTTFfont(public_path() . '\\' . $subdomain[0] . '\backend\canvas\fonts\Calibri Bold.ttf', 'TrueTypeUnicode', '', 96);  // or CalibriBold.ttf
        ...
        //$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails = $this->getNextCardNo('Nnp');
        ...
        if ($studentDataOrg && !empty($studentDataOrg)) {
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg = "CSCACS_BG.jpeg"; // bestiu_bg, bestiu_bg_data
                $low_res_bg = "CSCACS_BG.jpeg";
                $pdfBig->AddPage();
                ...
                $ghostImgArr = array();
                $pdf = new TCPDF('L', 'mm', array('210', '297'), true, 'UTF-8', false); // 420.116 296.926
                ...
                // $serial_no      = trim($studentData[0]); // REF NO
                // $unique_id      = trim($studentData[1]); // SEAT NO
                // $candidate_name  = trim($studentData[2]); // STUDENT NAME
                // $father_name     = trim($studentData[3]); // ??? not in Excel
                // $degree          = trim($studentData[3]); // COURSE NAME
                // $completion_date = trim($studentData[4]); // COMPLETION DATE
                // $average         = trim($studentData[5]); // GRADE
                // $date            = trim($studentData[6]); // CGPA
                // $place           = trim($studentData[7]); // ??? not in Excel
                // $PHOTO           = trim($studentData[8]); // PHOTO
                // Extract data from Excel row
                $unique_serial_no   = trim($studentData[0] ?? '');
                $certificate_no     = trim($studentData[1] ?? '');
                $candidate_name     = trim($studentData[2] ?? '');
                $security_line     = trim($studentData[12] ?? '');
         
                $microlinestr = strtoupper($security_line);
              

                //kamini chnages
                $pdfBig->SetFont($lucidaCalligraphy, '', 25);
                $nameWidth = $pdfBig->GetStringWidth($candidate_name);
                $microlinestr = preg_replace('/\s+/', '', $microlinestr);
                $nameWidth=$nameWidth+1;
                $pdfBig->SetFont($arial, '', 5);
                $microWidth = $pdfBig->GetStringWidth($microlinestr);
                $repeatCount = ceil($nameWidth / $microWidth) ;
                $microRepeated = str_repeat($microlinestr, $repeatCount);
                $finalMicro = '';
                for ($i = 0; $i < strlen($microRepeated); $i++) {
                    $test = $finalMicro . $microRepeated[$i];
                    if ($pdfBig->GetStringWidth($test) > $nameWidth) {
                        break;
                    }
                    $finalMicro = $test;
                }

                $pdfBig->SetTextColor(150,150,150);
                $pdfBig->SetFontStretching(100);

                $yPositions = [94, 97, 100, 103, 106, 109];

                foreach ($yPositions as $y) {
                    $pdfBig->SetXY(10, $y);
                    // $pdfBig->Cell($nameWidth, 0, $finalMicro, 0, 0, 'C');
                    $pdfBig->MultiCell(0, 0,$finalMicro, 0, 'C',false,1, '', '', true,false,true,0, 'M',false );
                }
                $pdfBig->SetFontStretching(100);
                $pdfBig->SetTextColor(0,0,0);
                $pdfBig->SetFont($lucidaCalligraphy, '', 25);
                $pdfBig->SetXY(10, 98);
                $pdfBig->MultiCell(0, 0,$candidate_name, 0, 'C',false,1, '', '', true,false,true,0, 'M',false );
                ...
                // ─── Main qualification = Subject (col 3) + Subject_2 (col 4) ──────────────
                $subject     = trim(preg_replace('/\s+/', ' ', $studentData[3] ?? ''));
                $subject_2   = trim(preg_replace('/\s+/', ' ', $studentData[4] ?? ''));

                $main_qualification = $subject;

                if ($subject_2 !== '' && $subject_2 !== $subject) {
                    // Merge cleanly - example: "Information And Communication Technology" + "Operation Level 4"
                    $main_qualification = rtrim($subject, ' ') . ' ' . ltrim($subject_2, ' ');
                    $main_qualification = preg_replace('/\s+/', ' ', trim($main_qualification));
                }

                // Defensive cleanup (prevent name/date leak)
                if (
                    stripos($main_qualification, $candidate_name) !== false ||
                    preg_match('/physcial|Examination Series|August 2025/i', $main_qualification)
                ) {
                    $main_qualification = $subject;
                }

                // Clean name - remove extra spaces (OCR often duplicates)
                $candidate_name = preg_replace('/\s+/', ' ', $candidate_name);
                $candidate_name = trim($candidate_name);

                // Exam series (usually last relevant column)
                $exam_series_text = trim($studentData[14] ?? '');

                // ─── Collect competences ────────────────────────────────────────────────
                $raw_comps = [];
                for ($i = 4; $i <= 10; $i++) {  // Start from Subject → competence5/6
                    if (!isset($studentData[$i]) || !is_string($studentData[$i])) continue;

                    $val = trim($studentData[$i]);
                    if ($val === '') continue;

                    // ─── Quality filters (no blacklisted words needed) ───
                    $len = strlen($val);
                    $spaces = substr_count($val, ' ');
                    $all_upper = (strtoupper($val) === $val);

                    // Skip junk patterns:
                    // - Too short
                    // - All uppercase headers
                    // - Looks like repeated name/date/exam text
                    // - Contains obvious garbage fragments
                    if (
                        $len < 10 ||
                        ($all_upper && $len < 50) ||
                        stripos($val, $candidate_name) !== false ||     // repeated name
                        preg_match('/physcial|ethics and practices|august 2025/i', $val) ||
                        preg_match('/^For the August|Examination Series$/i', $val) ||
                        preg_match('/^[A-Z0-9\/ ]{5,15}$/', $val)       // short codes
                    ) {
                        continue;
                    }
                ...
                // Remove exact duplicates + normalize spacing
                $competences = [];
                foreach ($raw_comps as $c) {
                    $clean = preg_replace('/\s+/', ' ', trim($c));
                    if ($clean && !in_array($clean, $competences)) {
                        $competences[] = $clean;
                    }
                }
                ...
                $pdfBig->SetMargins(10, 15, 10);
                $pdfBig->SetAutoPageBreak(false);

                // ─── Certificate Number (top right) ───
                $pdfBig->SetFont($arial, '', 11);
                $pdfBig->SetXY(120, 13);
                $pdfBig->writeHTML(
                    '<b>Certificate Number:</b> ' . htmlspecialchars($certificate_no),
                    true,
                    false,
                    false,
                    false,
                ...
                // for ($i = 0; $i < 6; $i++) {
                //     $pdfBig->SetXY($x_big, $y_big);
                //     $pdfBig->Cell($width_big, 0, $microlinestr, 0, false, 'C');
                //     $y_big += $lineHeight_big;
                // }
                ...
                // // ─── INSTITUTION TITLE ───
                // $pdfBig->SetFont($arialb, 'B', 18);  // assume $arialb is bold Arial
                // $pdfBig->SetXY(10, 35);
                // $pdfBig->Cell(190, 8, 'THE NYERI NATIONAL POLYTECHNIC', 0, 1, 'C');

                // ─── PARTIAL QUALIFICATION ───
                $pdfBig->SetFont($calibri, '', 20);
                $pdfBig->SetXY(10, 70);
                $pdfBig->Cell(190, 8, 'PARTIAL QUALIFICATION', 0, 1, 'C');

                // ─── This is to certify that ───
                $pdfBig->SetFont($arial, '', 18);
                $pdfBig->SetXY(10, 84);
                $pdfBig->writeHTMLCell(190, 0, '', '', 'This is to certify that', 0, 1, false, true, 'C');

                // ─── Candidate Name (big, bold+italic) ───
                // Use the Old English Five font for candidate name
                ...
                $pdfBig->SetFont($arial, '', 18);
                $pdfBig->SetXY(10, 115);
                $pdfBig->writeHTMLCell(190, 0, '', '', 'has demonstrated competence in', 0, 1, false, true, 'C');

                // ─── Competences (dynamic list) ───
                $y = 125;
                $pdfBig->SetFont($arial, '', 16);

                foreach ($competences as $comp) {
                    $pdfBig->SetXY(10, $y);
                    $pdfBig->writeHTMLCell(190, 0, '', '', htmlspecialchars($comp), 0, 1, false, true, 'C');
                    $y += 7.5;  // Slightly more generous spacing
                }

                // ─── being part of ───
                $pdfBig->SetFont($arial, '', 15);
                $pdfBig->SetXY(10, $y + 7);
                $pdfBig->writeHTMLCell(190, 0, '', '', 'being part of', 0, 1, false, true, 'C');

                // 2. Main qualification – red, bold, larger, centered
                // ─── Main qualification (RED, bold, centered, wrapped) ───
                $pdfBig->SetFont($palatinoBold, 'B', 25);

                // Set main qualification color (#e01515)
                $pdfBig->SetTextColor(255, 0, 0);

                $pdfBig->SetXY(30, $y + 18);
                $pdfBig->MultiCell(
                    150,
                    9,
                    htmlspecialchars($main_qualification),
                    0,
                    'C',
                    false,
                    1,
                ...
                // ─── Examination Series ───
                $pdfBig->SetFont($palatino, '', 16);
                $pdfBig->SetXY(10, $y + 41);
                $pdfBig->writeHTMLCell(190, 0, '', '', $exam_series_text, 0, 1, false, true, 'C');

                // ─── Signature block (adjust $sig_y based on content height) ───
                // $sig_y = max(235, $y + 65);  // push down if many competences

                // $pdfBig->SetFont($arial, '', 13);
                // $pdfBig->SetXY(20, $sig_y);
                // $pdfBig->Cell(80, 8, 'Chairman', 0, 0, 'C');

                // $pdfBig->SetXY(110, $sig_y);
                // $pdfBig->Cell(80, 8, 'Chief Principal', 0, 1, 'C');

                // $pdfBig->SetXY(10, $sig_y + 12);
                // $pdfBig->Cell(190, 6, 'The Nyeri National Polytechnic Governing Council', 0, 1, 'C');

                // // Optional footer note
                // $pdfBig->SetFont($arial, 'I', 10);
                // $pdfBig->SetXY(10, $sig_y + 25);
                // $pdfBig->Cell(190, 6, 'This certificate has been issued without any alterations whatsoever.', 0, 1, 'C');
                ...
                //kamini chnages
                $pdf->SetFont($lucidaCalligraphy, '', 25);
                $nameWidth = $pdf->GetStringWidth($candidate_name);
                $microlinestr = preg_replace('/\s+/', '', $microlinestr);
                $nameWidth=$nameWidth+1;
                $pdf->SetFont($arial, '', 5);
                $microWidth = $pdf->GetStringWidth($microlinestr);
                $repeatCount = ceil($nameWidth / $microWidth) ;
                $microRepeated = str_repeat($microlinestr, $repeatCount);
                $finalMicro = '';
                for ($i = 0; $i < strlen($microRepeated); $i++) {
                ...
                foreach ($yPositions as $y) {
                    $pdf->SetXY(10, $y);
                    // $pdf->Cell($nameWidth, 0, $finalMicro, 0, 0, 'C');
                    $pdf->MultiCell(0, 0,$finalMicro, 0, 'C',false,1, '', '', true,false,true,0, 'M',false );
                }
                $pdf->SetFontStretching(100);
                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($lucidaCalligraphy, '', 25);
                $pdf->SetXY(10, 98);
                $pdf->MultiCell(0, 0,$candidate_name, 0, 'C',false,1, '', '', true,false,true,0, 'M',false );
                ...
                // for ($i = 0; $i < 6; $i++) {
                //     $pdf->SetXY($x_big, $y_big);
                //     $pdf->Cell($width_big, 0, $microlinestr, 0, false, 'C');
                //     $y_big += $lineHeight_big;
                // }
                ...
                // // ─── INSTITUTION TITLE ───
                // $pdf->SetFont($arialb, 'B', 18);  // assume $arialb is bold Arial
                // $pdf->SetXY(10, 35);
                // $pdf->Cell(190, 8, 'THE NYERI NATIONAL POLYTECHNIC', 0, 1, 'C');

                // ─── PARTIAL QUALIFICATION ───
                $pdf->SetFont($calibri, '', 20);
                $pdf->SetXY(10, 70);
                $pdf->Cell(190, 8, 'PARTIAL QUALIFICATION', 0, 1, 'C');

                // ─── This is to certify that ───
                $pdf->SetFont($arial, '', 18);
                $pdf->SetXY(10, 84);
                $pdf->writeHTMLCell(190, 0, '', '', 'This is to certify that', 0, 1, false, true, 'C');
                ...
                $pdf->SetFont($arial, '', 18);
                $pdf->SetXY(10, 115);
                $pdf->writeHTMLCell(190, 0, '', '', 'has demonstrated competence in', 0, 1, false, true, 'C');
                ...
                foreach ($competences as $comp) {
                    $pdf->SetXY(10, $y);
                    $pdf->writeHTMLCell(190, 0, '', '', htmlspecialchars($comp), 0, 1, false, true, 'C');
                    $y += 7.5;  // Slightly more generous spacing
                }
                ...
                $pdf->SetXY(10, $y + 7);
                $pdf->writeHTMLCell(190, 0, '', '', 'being part of', 0, 1, false, true, 'C');

                // 2. Main qualification – red, bold, larger, centered
                // ─── Main qualification (RED, bold, centered, wrapped) ───
                $pdf->SetFont($palatinoBold, 'B', 25);
                ...
                // ─── Examination Series ───
                $pdf->SetFont($palatino, '', 16);
                $pdf->SetXY(10, $y + 41);
                $pdf->writeHTMLCell(190, 0, '', '', $exam_series_text, 0, 1, false, true, 'C');
                ...
                //  left side
                // $sign1 = public_path() . '\\' . $subdomain[0] . '\backend\canvas\images\KNGA_LOGO_EXTR.png';
                // $sign1_x = 95;
                // $sign1_y = 235;
                // $sign1_Width = 31.75;
                // $sign1_Height = 9.79;
                ...
                /* 🔹 REMOVE / FROM EXCEL SERIAL NO */
                if (strpos($serial_no, '/') !== false) {
                    $serial_no = str_replace('/', '', trim($serial_no));
                }

                $upload_sign1_org = $sign1;
                $pathInfo = pathinfo($sign1);

                $sign1 = public_path() . '\\' . $subdomain[0] . '\backend\\templates\\100\\'
                    . $pathInfo['filename'] . "_{$serial_no}." . $pathInfo['extension'];
                ...
                $microlinestr = strtoupper($security_line);
                $microlinestr2 = strtoupper($security_line);




                $str = $microlinestr; //strtoupper($studentData['name'] . $studentData['usn'] . $studentData['course'] . $studentData['course_month']);
                ...
                $pdf->StartTransform();
                $pdf->SetXY(10, 94);
                $pdf->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
                $pdf->SetXY(10, 97);
                $pdf->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
                $pdf->SetXY(10, 100);
                $pdf->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
        ...
        if ($previewPdf != 1) {
            $this->updateCardNo('Nnp', $card_serial_no - $cardDetails->starting_serial_no, $card_serial_no);
        }
        $msg = '';

        $file_name =  str_replace("/", "_", 'Nnp' . date("Ymdhms")) . '.pdf';
