...
class PDFBothGenerateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $back_bg_template_img_generate = $pdf_data['back_bg_template_img_generate'];
        $back_bg_template_width_generate = $pdf_data['back_bg_template_width_generate'];
        $back_bg_template_height_generate = $pdf_data['back_bg_template_height_generate'];
        $back_bg_template_img = $pdf_data['back_bg_template_img'];
        $back_bg_template_width = $pdf_data['back_bg_template_width'];
        $back_bg_template_height = $pdf_data['back_bg_template_height'];
        ...
        if($subdomain[0]=="minoshacloud" || ($subdomain[0]=="sales"&&($FID['template_id']=="7"||$FID['template_id']=="8"))){
        ...
        for($excel_row = $excel_row_num; $excel_row <= $highestRow; $excel_row++)
        {
            ...
            if(!$isPreview) {
                ...
                if($FID['is_back_template'] == 1) {
                    $countback =  count($FID_BACK['mapped_name']);
                }
            } else {
                $count = count($FID['field_position']);
                if($FID['is_back_template'] == 1) {
                    $countback =  count($FID_BACK['field_position']);
                }
            }
                            ...
                            if(!$isPreview) {
                                ...
                                if($FID['infinite_height'][$extra_fields] != 1){
                                    ...
                                    if ($FID['visible'][$extra_fields] == 0) {
                                        ...
                                        if(($extra_fields==86&&isset($FID['data'][86])&&$template_name=="GalgotiyaUniversity")||($extra_fields==111&&isset($FID['data'][111])&&$template_name=="GalgotiyaUniversity-copy")||($subdomain[0]=="cu" && ($FID['template_id']==1 ||$FID['template_id']==2 ||$FID['template_id']==3 ||$FID['template_id']==4) && $extra_fields==9 && isset($FID['data'][9]))){
                                       ...
                                       foreach ($strArr as $character) {
                                            ...
                                            $i++;
                                             if($subdomain[0]=="cu" && ($FID['template_id']==1 ||$FID['template_id']==2 ||$FID['template_id']==3 ||$FID['template_id']==4) && $extra_fields==9 && isset($FID['data'][9])){
                                                $x=$x+3.5+$j; 
                                            }else{
                                                $x=$x+2.2+$j; 
                                            }
                    ...
                    case 'QR Code':
                        ...
                        if($subdomain[0]=="demo") {
                            ...
                            if($fullString!="{{QR Code}}"){
                                ...
                                //     $codeContents= GibberishAES::enc($codeContents, \Config::get('constant.EStamp_Salt'));
                                // }
                                //$codeData = $fullString."\n\n".$codeContents;
                            ...
                            if(!empty($EncryptedString)){
                                ...
                                if($subdomain[0]=="demo"&&($FID['template_id']==697||$FID['template_id']==698||$FID['is_block_chain']==1)){
                                    ...
                                    if($subdomain[0]=="nidantechnologies"&&$FID['template_id']==12){
                                        ...
                                        if(!empty($fullString)&&$fullString!="{{QR Code}}") {
                                            $codeData = $fullString."\n\n".$conStriCode;    
                                        } else {
                            ...
                            if($FID['is_encrypted_qr'][$extra_fields] == 1) {
                                $codeData= GibberishAES::enc($codeContents, \Config::get('constant.EStamp_Salt'));
                            }elseif($subdomain[0]=="nidantechnologies"&&$FID['template_id']==12){
                                $codeData = $codeData;
                            } else {
                                $codeData = $codeContents;
                            }
                    ...
                    case 'Yellow Patch':
                        ...
                        if($FID['visible'][$extra_fields] == 0){
                            ...
                            for($i = 0; $i < $len; $i++) {
                                ...
                                $yellow_chr_path = public_path().'\\backend\canvas\yellowpatchimages\\'.$value.'.png';

                                    
                                list($width, $height) = getimagesize($yellow_chr_path);
                                $yellowPatchWidth=$width*0.05 ;
                                
                                $upload_image_org = $yellow_chr_path;
                                $pathInfo = pathinfo($upload_image_org);
                                $yellow_chr_path_temp = $pathInfo['dirname'].'/temp/'.$subdomain[0].'_'.$pathInfo['filename'].'_'.$excel_row.'.'.$pathInfo['extension'];
                                
                                \File::copy($upload_image_org,$yellow_chr_path_temp);
                                
                                if($FID['visible_varification'][$extra_fields] == 0){
                            
                                    $pdf->Image($yellow_chr_path_temp, $yellowPatchx, $yellowPatchy, $yellowPatchWidth,  $yellowPatchHeight, '', '', '', false, 600);
                                }
                                
                                if($FID['visible'][$extra_fields] == 0){
                                
                                    $pdfBig->Image($yellow_chr_path_temp, $yellowPatchx, $yellowPatchy, $yellowPatchWidth,  $yellowPatchHeight, '', '', '', false, 600);
                                }

                                
                                
                                $yellowPatchx=$yellowPatchx+$yellowPatchWidth;
                                unlink($yellow_chr_path_temp);
                            }
            ...
            if($FID['is_back_template'] == 1) {
                ...
                if($back_bg_template_img_generate != ''){
                    $template_img_generate = str_replace(' ', '+', $back_bg_template_img_generate);
                    $pdf->Image($template_img_generate, 0, 0, $back_bg_template_width_generate, $back_bg_template_height_generate, "JPG", '', 'R', true);
                }

                $pdfBig->AddPage();

                if($back_bg_template_img != ''){
                
                    $back_template_img = str_replace(' ', '_', $back_bg_template_img);    
                    $ext = pathinfo($back_bg_template_img, PATHINFO_EXTENSION);
                    ...
                    $pdfBig->Image($back_template_img, 0, 0, $back_bg_template_width, $back_bg_template_height, $image_extension, '', 'R', true);
                }
                ...
                for($extra_fields = 0; $extra_fields < $countback; $extra_fields++)
                {
                    if(!$isPreview) 
                    {
                        if($FID_BACK['security_type'][$extra_fields] == "Static Microtext Border" || $FID_BACK['security_type'][$extra_fields] == "Static Text")
                        {
                            $str = $FID_BACK['sample_text'][$extra_fields];
                        }
                        else{
                            //code for check map excel or database 
                            // get string from excel columns or database selected columns value
                            if ($FID_BACK['is_mapped'][0] == 'excel') {
                                if(isset($rowData[0][$FID_BACK['mapped_excel_col'][$extra_fields]])){
                                    $str = $rowData[0][$FID_BACK['mapped_excel_col'][$extra_fields]];
                                }
                                else{
                                    $str = '';
                                }
                            }else{
                                if(isset($rowData[$FID_BACK['mapped_name'][$extra_fields]]))
                                    $str = $rowData[$FID_BACK['mapped_name'][$extra_fields]];
                                else
                                    $str = 'Not mapped';
                            }
                        }
                        ...
                        if($FID_BACK['font_style'][$extra_fields] != ''){
                            $bold = $FID_BACK['font_style'][$extra_fields];
                        }

                        $angle = $FID_BACK['angle'][$extra_fields];
                        if(isset($FID_BACK['font_id'][$extra_fields]))
                        {

                            $font = $FID_BACK['font_id'][$extra_fields];
                        }
                        if(isset($FID_BACK['sample_image'][$extra_fields])){
                            $sample_image = $FID_BACK['sample_image'][$extra_fields];
                        }
                    }

                    if(isset($FID_BACK['line_gap'][$extra_fields])){

                        $line_gap =  $FID_BACK['line_gap'][$extra_fields];
                    }
                    $style = $FID_BACK['font_style'][$extra_fields];

                    if(isset($FID_BACK['length'][$extra_fields])){

                        $length =  $FID_BACK['length'][$extra_fields];
                    }
                        
                    $font_colorHex = $FID_BACK['font_color'][$extra_fields];
                    ...
                    $security_type = $FID_BACK['security_type'][$extra_fields];
                    ...
                    $text_align = $FID_BACK['text_justification'][$extra_fields];
                    $width = $FID_BACK['width'][$extra_fields];
                    if(isset($FID_BACK['uv_percentage'][$extra_fields])){
                        $uv_percentage =  $FID_BACK['uv_percentage'][$extra_fields];
                    }
                    ...
                    $x = $FID_BACK['x_pos'][$extra_fields];
                    $y = $FID_BACK['y_pos'][$extra_fields];
                    $height = $FID_BACK['height'][$extra_fields];
                    $font_size = $FID_BACK['font_size'][$extra_fields];
                    if(empty($font_size)){
                        $font_size = 0;
                    }
                    if(isset($FID_BACK['font_color_extra'][$extra_fields])){

                        $font_color_extra = $FID_BACK['font_color_extra'][$extra_fields];
                    }
                    $strposDouble  = substr($str, 0,1);
                    $strposSingle  = substr($str, 0,1);
                    ...
                    switch  ($security_type) {
                        ...
                        case 'Normal':
                                ...
                                if(!$isPreview) {
                                    ...
                                    if($FID_BACK['infinite_height'][$extra_fields] != 1){
                                        
                                        if ($FID_BACK['visible'][$extra_fields] == 0) {


                                            
                                            $pdfBig->SetAlpha(1);

                                            if(($extra_fields==86&&isset($FID_BACK['data'][86])&&$template_name=="GalgotiyaUniversity")||($extra_fields==111&&isset($FID_BACK['data'][111])&&$template_name=="GalgotiyaUniversity-copy")||($subdomain[0]=="cu" && ($FID_BACK['template_id']==1 ||$FID_BACK['template_id']==2 ||$FID_BACK['template_id']==3 ||$FID_BACK['template_id']==4) && $extra_fields==9 && isset($FID_BACK['data'][9]))){
                                        $strArr = str_split($str);
                                        $x_org=$x;
                                        $y_org=$y;
                                        $font_size_org=$font_size;
                                        ...
                                        foreach ($strArr as $character) {
                                                ...
                                                $pdfBig->Cell($width, 0, $character, 0, $ln=0,  $text_align, 0, '', 0, false, 'B', 'B');
                                                $i++;
                                                if($subdomain[0]=="cu" && ($FID_BACK['template_id']==1 ||$FID_BACK['template_id']==2 ||$FID_BACK['template_id']==3 ||$FID_BACK['template_id']==4) && $extra_fields==9 && isset($FID_BACK['data'][9])){
                                                    $x=$x+3.5+$j; 
                                                }else{
                                                    $x=$x+2.2+$j; 
                                                }
                                        ...
                                        if ($FID_BACK['visible_varification'][$extra_fields] == 0) {
                                            if(($extra_fields==86&&isset($FID_BACK['data'][86])&&$template_name=="GalgotiyaUniversity")||($extra_fields==111&&isset($FID_BACK['data'][111])&&$template_name=="GalgotiyaUniversity-copy")||($subdomain[0]=="cu" && ($FID_BACK['template_id']==1 ||$FID_BACK['template_id']==2 ||$FID_BACK['template_id']==3 ||$FID_BACK['template_id']==4) && $extra_fields==9 && isset($FID_BACK['data'][9]))){
                                        $strArr = str_split($str);
                        ...
                        case 'QR Code':
                            ...
                            if($subdomain[0]=="demo") {
                                $fullString=$FID_BACK['combo_qr_text'][$extra_fields];
                                
                                // if($subdomain[0]=="demo") {
                                    $EncryptedString=$FID_BACK['encrypted_qr_text'][$extra_fields]; 
                                // }
                                ...
                                if($fullString!="{{QR Code}}"){
                                    if (preg_match_all('#\{\{(.*?)\}\}#', $fullString, $matches)) {
                                        ...
                                        foreach ($qr_fields as $field) {
                                            ...
                                            $columnIndex = array_search($field, $FID_BACK['name']);
                                            $columnName=$FID_BACK['mapped_name'][$columnIndex];
                                            $columnIndexRow = array_search($columnName, $firstRow[0]);
                                    
                                            if ($FID_BACK['is_mapped'][$columnIndex] == 'excel') {
                                                if(isset($rowData[0][$columnIndexRow])){
                                                    $strComboQr = $rowData[0][$columnIndexRow];
                                                }
                                                else{
                                    ...
                                    $QRName = strtoupper(md5($str . $dt));
                                    $codeContents = strtoupper(md5($str . $dt));
                                    // if($FID_BACK['is_encrypted_qr'][$extra_fields] == 1) {
                                    //     $codeContents= GibberishAES::enc($codeContents, \Config::get('constant.EStamp_Salt'));
                                    // }
                                    //$codeData = $fullString."\n\n".$codeContents;
                                    //Blockchain Integration
                                    if($subdomain[0]=="demo"&&($FID_BACK['template_id']==697||$FID_BACK['template_id']==698||$FID_BACK['is_block_chain']==1)){
                                        $codeData=$fullString."\n\n".CoreHelper::getBCVerificationUrl(encrypt($codeContents))."\n\n".$codeContents;
                                    }else{
                                        if($subdomain[0]=="nidantechnologies"&&$FID_BACK['template_id']==12){
                                            $certName = preg_replace('/[^a-zA-Z0-9]/', '_', $serial_no).".pdf";
                                            $url = 'http://'.$subdomain[0].'.'.$subdomain[1].'.com/'; 
                                            $codeData =$url.'/'.$subdomain[0].'/backend/pdf_file/'.$certName;
                                        }else{
                                ...
                                if($FID_BACK['is_encrypted_qr'][$extra_fields] == 1) {
                                    $codeData= GibberishAES::enc($codeContents, \Config::get('constant.EStamp_Salt'));
                                }elseif($subdomain[0]=="nidantechnologies"&&$FID_BACK['template_id']==12){
                                    $codeData = $codeData;
                                } else {
                                    $codeData = $codeContents;
                                }
                            ...
                            if(!$isPreview){
                                ...
                                if($FID_BACK['include_image'][$extra_fields] == 1){
                                    $logopath = $FID_BACK['sample_image'][$extra_fields];
                        ...
                        case 'Qr Code':
                                ...
                                if(!$isPreview){
                                    if($FID_BACK['include_image'][$extra_fields] == 1){
                                        $field_image = explode('qr/',$FID_BACK['sample_image'][$extra_fields]);
                                        if($get_file_aws_local_flag->file_aws_local == '1'){
                                            $logopath = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.canvas').'/qr/'.$field_image[1];
                                        }
                                        else{
                                    ...
                                    QrCode::format('png')->size(200)->generate($codeContentsData,$pngAbsoluteFilePath);
                                    $QR = imagecreatefrompng($pngAbsoluteFilePath);
                                    $include_image = $FID_BACK['include_image'][$extra_fields];
                        ...
                        case 'Micro line':
                            ...
                            if(!$isPreview) {
                                ...
                                if($FID_BACK['visible_varification'][$extra_fields] == 0){
                                    ...
                                    $pdf->SetXY($x, $y);
                                    $pdf->Rotate($angle);
                                    $is_repeat =  $FID_BACK['is_repeat'][$extra_fields];
                                    if($is_repeat == 1){
                        ...
                        case 'Micro Text Border':
                            ...
                            */
                                ...
                                if(!$isPreview) {
                                    ...
                                    //if($subdomain[0]=="demo"){
                                        // $explode_default = explode(',',$FID_BACK['field_sample_text_width'][0]);
                                        //$explode_vertical = explode(',',$FID_BACK['field_sample_text_vertical_width'][0]);
                                        //$explode_horizontal = explode(',',$FID_BACK['field_sample_text_horizontal_width'][0]);
                                        //  $length = $explode_default[$draw_key];
                                        $vertical_length = $height * (25.4/100);
                                        $horizontal_length = $width *  (25.4/100);
                        ...
                        case 'Security line':
                                ...
                                //Updated by mandar
                                if($FID_BACK['is_font_case'][$extra_fields]==1){
                                    $str=strtoupper($str);
                                ...
                                if(!$isPreview) {
                                    ...
                                    if($FID_BACK['visible_varification'][$extra_fields] == 0){
                                        ...
                                        $pdf->SetTextColor($r,$g,$b);
                                        $pdf->SetAlpha($FID_BACK['text_opacity'][$extra_fields]);
                                        $pdf->SetFont($fonts_array[$extra_fields], $bold, $font_size, '', false);
                                        $pdf->StartTransform();
                                        $pdf->SetXY($x-2, $y);
                                        $pdf->Rotate($angle);
                                    ...
                                    if($FID_BACK['visible'][$extra_fields] == 0){
                                        ...
                                        $pdfBig->SetTextColor($r,$g,$b);
                                        $pdfBig->SetAlpha($FID_BACK['text_opacity'][$extra_fields]);
                                        $pdfBig->SetFont($fonts_array[$extra_fields], $bold, $font_size, '', false);
                                        $pdfBig->StartTransform();
                                        $pdfBig->SetXY($x-2, $y);
                                        $pdfBig->Rotate($angle);
                        ...
                        case 'Dynamic Image':
                            ...
                            */
                                ...
                                if(!$isPreview){
                                    ...
                                    $uv_image = $FID_BACK['is_uv_image'][$extra_fields];
                                    if($uv_image == 1){

                                        if($get_file_aws_local_flag->file_aws_local == '1'){
                                    
                                            $location = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$str;
                                            
                                        }
                                        else{
                                            
                                            $location = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$str;
                                    ...
                                    // echo $location;
                                    // echo "<br>";
                                        $uv_location = public_path()."/backend/canvas/dummy_images/".$FID_BACK['template_name']."/".$file_name.'_uv.'.$ext;
                                        ...
                                        if($ext == 'png'){
                                            ...
                                            $im = imagecreatefromjpeg($location);
                                                                imagefilter($im, IMG_FILTER_GRAYSCALE);
                                                                imagefilter($im, IMG_FILTER_NEGATE);
                                                                if($subdomain[0]=="minoshacloud" || ($subdomain[0]=="sales"&&($FID_BACK['template_id']=="7"||$FID_BACK['template_id']=="8"))){
                                                                    imagefilter($im, IMG_FILTER_COLORIZE, 255, 255, 0);
                                                                }else{
                                                                imagefilter($im, IMG_FILTER_COLORIZE, 255, 255, 0);
                                                                }
                                        ...
                                        if($get_file_aws_local_flag->file_aws_local == '1'){
                                                ...
                                                $aws_uv_image_path = '/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id']."/".$file_name.'_uv.'.$ext;
                                            

                                            $aws_default_storage = \Storage::disk('s3')->put($aws_uv_image_path, file_get_contents($uv_location), 'public');
                                            $aws_default_filename = \Storage::disk('s3')->url($file_name.'_uv.'.$ext);
                                    
                                                $get_aws_uv_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$file_name.'_uv.'.$ext;
                                                ...
                                                $aws_uv_image_path = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id']."/".$file_name.'_uv.'.$ext;
                                        ...
                                        if($FID_BACK['grey_scale'][$extra_fields] == 1){
                                            if($FID_BACK['is_uv_image'][$extra_fields] != 1){

                                                if($FID_BACK['visible_varification'][$extra_fields] == 0){
                                                    if($get_file_aws_local_flag->file_aws_local == '1'){
                                            
                                                            $pdf->image(\Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str, $x, $y,$width / 3,$height / 3, "", '', 'L', true, 3600);
                                                    
                                                    }
                                                    else{
                                            
                                                            $pdf->image(public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str, $x, $y,$width / 3,$height / 3, "", '', 'L', true, 3600);
                                            ...
                                            if($get_file_aws_local_flag->file_aws_local == '1'){
                                                    ...
                                                    $upload_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str;
                                                
                                            }
                                            else{
                                        
                                                    $upload_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str;
                                            ...
                                            if($FID_BACK['is_transparent_image'][$extra_fields] == 1){
                                                $upload_image_org = $upload_image;
                                                $pathInfo = pathinfo($upload_image);
                                                $upload_image = $pathInfo['dirname'].'/'.$pathInfo['filename'].'_'.$excel_row.'.'.$pathInfo['extension'];
                                                \File::copy($upload_image_org,$upload_image);
                                            ...
                                            if($get_file_aws_local_flag->file_aws_local == '1'){
                                                    ...
                                                    $simple_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str;
                                                    $grey_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/grey_'.$str;
                                            
                                            }
                                            else{
                                    
                                                    $simple_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/'.$str;
                                                    $grey_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID_BACK['template_id'].'/grey_'.$str;
                        ...
                        case 'Static Image':
                            // same as Dynamic Image
                                ...
                                $extension = explode(".", $FID_BACK['sample_image'][$extra_fields]);
                                $ext = strtoupper($extension[1]);
                                ...
                                if ($uv_image == 1) {
                                    ...
                                    if($get_file_aws_local_flag->file_aws_local == '1'){
                                            ...
                                            $location = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$sample_image;
                                        
                                    }
                                    else{
                                    
                                            $location = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$sample_image;
                                    ...
                                    if($get_file_aws_local_flag->file_aws_local == '1'){     
                                            ...
                                            $uvImagePath = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$file_name.'_uv.'.$extension;
                                        
                                    }
                                    else{
                                    
                                            $uvImagePath = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$file_name.'_uv.'.$extension;
                                    ...
                                    /*if($subdomain[0]=="test"){
                                        ...
                                        if($FID_BACK['is_uv_image'][$extra_fields] != 1){
                                            if($FID_BACK['visible_varification'][$extra_fields] == 0){
                                                if($get_file_aws_local_flag->file_aws_local == '1'){      
                                                        ...
                                                        $pdf->image(\Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$FID_BACK['sample_image'][$extra_fields], $x, $y,$width / 3.34,$height / 3.34, "", '', 'L', true, 3600);
                                                    
                                                }
                                                else{
                                                
                                                        $pdf->image(public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$FID_BACK['sample_image'][$extra_fields], $x, $y,$width / 3.34,$height / 3.34, "", '', 'L', true, 3600);
                                        ...
                                        if($get_file_aws_local_flag->file_aws_local == '1'){ 
                                                ...
                                                $upload_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$FID_BACK['sample_image'][$extra_fields];
                                            
                                        }
                                        else{
                                        
                                            
                                            $upload_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$FID_BACK['sample_image'][$extra_fields];
                                        }
                                        ...
                                        if($FID_BACK['is_transparent_image'][$extra_fields] == 1){
                                            ...
                                            if($FID_BACK['visible'][$extra_fields] == 0){
                                                ...
                                                $testArr=array($excel_row,$x,$y,$FID_BACK['sample_image'][$extra_fields]);
                                                $pdfBig->image($upload_image, $x, $y,$width / 3.34,$height / 3.34, "", '', 'L', true, 3600);
                                            }
                                        ...
                                        if($get_file_aws_local_flag->file_aws_local == '1'){
                                                ...
                                                $simple_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$sample_image;
                                            $grey_image = \Config::get('constant.amazone_path').$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/grey_'.$sample_image;
                                        
                                        }
                                        else{
                                    
                                                $simple_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/'.$sample_image;
                                                $grey_image = public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID_BACK['template_id'].'/grey_'.$sample_image;
                    ...
                    /**Block Chain Meta Data Fields**/
                    if($subdomain[0]=="demo"&&$FID_BACK['is_meta_data'][$extra_fields] == 1){
                            
                            $fullString=$FID_BACK['meta_data_value'][$extra_fields];
                            if (preg_match_all('#\{\{(.*?)\}\}#', $fullString, $matches)) {
                            ...
                            /* print_r($columnIndexRow);
                                ...
                                if ($FID_BACK['is_mapped'][$columnIndex] == 'excel') {
                                    ...
                                    if(isset($FID_BACK['sample_text'][$columnIndex])&&!empty($FID_BACK['sample_text'][$columnIndex])){
                                        
                                        $strValueMetaData = $FID_BACK['sample_text'][$columnIndex];
                                    }
                                    else{
                                        $strValueMetaData = '';
                                    }
                        ...
                        $metaDataArr[]=array("label"=>$FID_BACK['meta_data_label'][$extra_fields],"value"=>$fullString);
                    }
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$get_file_aws_local_flag,$storagePath,$excelRow=[],$FID,$metaDataArr)
    {
        ...
        if($subdomain[0]=='notest'){
           ...
           // $pyscript = \Config::get('constant.pathCompressScript'); 
            ...
            //Compress pdf file  //Enable compression code for all instances at 25-05-2023 15:44 by Mandar
            $compressionInstances = \Config::get('constant.compressionInstances');
        
            if(in_array($subdomain[0], $compressionInstances)) {    
                $source=\Config::get('constant.directoryPathBackward')."\\backend\\temp_pdf_file\\".$certName;//$file1
                $output=\Config::get('constant.directoryPathBackward')."\\backend\\pdf_file\\".$certName;
               CoreHelper::compressPdfFile($source,$output);
            }else{
        ...
        if($subdomain[0]=="demo"){
            ...
            //Blockchain Integration
            if($FID['is_block_chain']==1&&($template_id==697||$template_id==698||$template_id==718||$template_id==719)){
                ...
                if(!empty($excelRow)){
                    ...
                    // rohit added code
                    $mintData['blockchain_mode'] = $this->pdf_data['blockchain_check_option'];
                    // rohit added code
                    $response=CoreHelper::mintPDF($mintData);
                    
                    if($response['status']==200){
                        if(isset($response['ipfsHash'])){
                            $resultu = StudentTable::where('serial_no',''.$serial_no)->where('key',''.$key)->where('status',1)->update(['bc_txn_hash'=>$response['txnHash'],'bc_ipfs_hash'=>$response['ipfsHash'],'bc_generation_type'=>$this->pdf_data['blockchain_check_option']]);
                        }else{
                            $resultu = StudentTable::where('serial_no',''.$serial_no)->where('key',''.$key)->where('status',1)->update(['bc_txn_hash'=>$response['txnHash'],'bc_generation_type'=>$this->pdf_data['blockchain_check_option']]);
                        }
                    }
