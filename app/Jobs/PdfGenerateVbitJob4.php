...
class PdfGenerateVbitJob4
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if(!empty($pdf_data['loader_token'])&&!empty($loader_data) && isset($loader_data['generatedCertificates'])){
       ...
       // End Update code for batchwise genration
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('vbitUGM');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->MultiCell(88.7, 6.5, 'Semester Grade Point Average (SGPA): '.$SGPA.'&nbsp;&nbsp;&nbsp;&nbsp;','LRTB', 'L', 0, 0, '', '', true, 0, true);
            $pdfBig->MultiCell(89.4, 6.5, 'Cumulative Grade Point Average (CGPA): '.$CGPA.'&nbsp;&nbsp;&nbsp;&nbsp;', 'LRTB', 'L', 0, 0, '', '', true, 0, true);
   ...
   //          $pdf->setPageMark();	
       ...
       if($previewPdf!=1){
        $this->updateCardNo('vbitUGM',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'vbitUGM'.date("Ymdhms")).'.pdf';
