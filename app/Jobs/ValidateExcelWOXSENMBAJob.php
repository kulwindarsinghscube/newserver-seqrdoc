...
class ValidateExcelWOXSENMBAJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        //dd($rowData);
                ...
                if($columnsCount1==9){
                    $columnArr=array('Student_ID', 'PGID','NAME','Name of the Program','Place','Date','Serial No','ABC ID','COA ID');
                ...
                //  echo "</br>";
                    ...
                    $profile_path_jpg = public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.trim($value1[0]).'.jpg';
                    $profile_path_jpeg = public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.trim($value1[0]).'.jpeg';
                    $profile_path_png = public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.trim($value1[0]).'.png';

                    if (!file_exists($profile_path_jpg)) {
                        if(!file_exists($profile_path_png)){
                          if(!file_exists($profile_path_jpeg)){
							array_push($profErrArr, $value1[0]);
						  }
                        } 
                    }
               ...
               // print_r($blobArr);
                ...
                if(count($mismatchArr)>0){
                 ...
                 if(count($profErrArr)>0){
                    return response()->json(['success'=>false,'type' => 'error','message' => 'Sheet1 : Missing images : '.implode(',', $profErrArr)]);
                }
