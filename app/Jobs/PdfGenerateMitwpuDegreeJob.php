...
class PdfGenerateMitwpuDegreeJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('MitwpuDegree');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                // $cgpa_krutidev = trim($studentData[15]);
                ...
                $cgpa = trim($studentData[13]);
                $cgpa_hindi = trim($studentData[14]);
                $cgpa_krutidev = trim($studentData[15]);
                $cgpa_krutidev = htmlspecialchars($cgpa_krutidev);
                ...
                $specialization = trim($studentData[20]);
                $specialization_krutidev = trim($studentData[21]);
                $specialization_krutidev = htmlspecialchars($specialization_krutidev);

                
                $certificate_id = trim($studentData[22]);
                $completion_date = trim($studentData[23]);
                $completion_date_krutidev = trim($studentData[24]);
                ...
                // $issue_date = trim($studentData[25]);
                // $issue_date_krutidev = trim($studentData[26]);

                $serialNo = trim($studentData[27]);
                ...
                $pdfBig->SetXY(15, 51);
                $pdfBig->MultiCell(180, 0, "laLFkkid v/;{k rFkk dk;Zdkjh v/;{k] iz'kkldh; vf/kdkjh ,oa ".$faculty_name_krutidev." dh flQkfj'k dks vuqeksfnr djrs gq,];g izekf.kr djrs gSa fd", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdfBig->MultiCell(180, 0, $full_name_krutidev .' <span style="font-size:16;">'.$mother_name_krutidev.'</span>', 0, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
                ...
                if(stripos($course_name_krutidev, 'fMIyksek') !== FALSE){
                    $pdfBig->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd  mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);

                } else {
                    if($specialization_krutidev) {
                        $pdfBig->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd ".$specialization_krutidev." Lis'kykbts'ku esa mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                    } else {
                        $pdfBig->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                    }
                ...
                // $pdfBig->SetFont($KRDEV100, '', 16.02, '', false);
                // $pdfBig->SetXY(15, 105);
                // $pdfBig->MultiCell(182, 0, $faculty_name_krutidev." Lis'kykbts'ku esa mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA", 0, 'C', 0, 0, '', '', true, 0, true);


                // $pdfBig->SetTextColor(50,50,154);
                // $pdfBig->SetFont($KRDEV100, '', 16.02, '', false);
                // $pdfBig->SetXY(15, 113);
                // $pdfBig->MultiCell(180, 0, "bls ekU;rk nsrs gq, vkt 04 uoacj 2023 ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetTextColor(255,0,0);
                $pdfBig->SetFont($timesb, '', 19.98, '', false); // Font Size - 19.98
                $pdfBig->SetXY(10, 158);
                // $pdfBig->MultiCell(190, 0, $full_name. "<span style='color:black;'> Mother's Name</span> ".$mother_name , 0, 'C', 0, 0, '', '', true, 0, true);


                $pdfBig->MultiCell(190, 0, trim(ucwords(strtolower($full_name))).'<span style="color:#32329a;font-family:'.$MTCORSVA.';font-size:16.02px; ">, Mother\'s Name</span> '.trim(ucwords(strtolower($mother_name))), 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdfBig->MultiCell(190, 0, trim(ucwords(strtolower($full_name))).' <span style="font-size:16;display: block; height: 50px;padding-bottom:10px;">'.  trim(ucwords(strtolower($mother_name))).'</span>', 0, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');

                // $pdfBig->MultiCell(0, 0, trim(ucwords(strtolower($full_name))).' <span style="font-size:16;display: block; height: 50px;padding-bottom:10px;">'.  trim(ucwords(strtolower($mother_name))).'</span>', 0, 'C', false, 1, "", "", true, 0, false, true, 0, 'T', false);

                // $pdfBig->Cell(190, 0, trim(ucwords(strtolower($full_name))).' <span style="font-size:16;display: block; height: 50px;padding-bottom:10px;">'.  trim(ucwords(strtolower($mother_name))).'</span>', 0, $ln=0, 'C', 0, '', 0, false, 'C', 'C');
                ...
                if(stripos($course_name, 'Diploma') !== FALSE){
                    ...
                    $pdfBig->MultiCell(190, 0, " with ".$cgpa." CGPA secured 
                    ...
                    if($specialization) {
                        ...
                        $pdfBig->MultiCell(190, 0, $specilaText." with ".$cgpa." CGPA secured 
                ...
                $pdf->MultiCell(180, 0, "laLFkkid v/;{k rFkk dk;Zdkjh v/;{k] iz'kkldh; vf/kdkjh ,oa ".$faculty_name_krutidev." dh flQkfj'k dks vuqeksfnr djrs gq,];g izekf.kr djrs gSa fd", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdf->MultiCell(180, 0, $full_name_krutidev .' <span style="font-size:16;">'.$mother_name_krutidev.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                if(stripos($course_name_krutidev, 'fMIyksek') !== FALSE){
                    $pdf->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd  mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);

                } else {

                    if($specialization_krutidev) {
                        $pdf->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd ".$specialization_krutidev." Lis'kykbts'ku esa mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                    } else {
                        $pdf->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ds vuqlkj ;g izekf.kr fd;k tkrk gS fd mUgksausa ".$cgpa_krutidev." lhthih, izkIr fd;k gSA bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                    }
                ...
                if(stripos($course_name, 'Diploma') !== FALSE){
                    ...
                    $pdf->MultiCell(190, 0, " with ".$cgpa." CGPA secured 
                            in the examination held in ".$completion_date_new." 
                            <br>In recognition we have hereunder placed our names and
                            <br>the seal of the University on this ".$issue_formatted_date.".", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $NewPRN = str_replace('DC_'.$year.'_', '', $PRN);

                $pdfBig->SetTextColor(50,50,154);
                $pdfBig->SetFont($calibrib, '', 10.98, '', false);
                $pdfBig->SetXY(147, 259);
                $pdfBig->MultiCell(60, 0, 'PRN No. - '.$NewPRN, 0, 'L', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetXY(147, 259);
                $pdf->MultiCell(60, 0, 'PRN No. - '.$NewPRN, 0, 'L', 0, 0, '', '', true, 0, true);
                ...
                $codeContents = $full_name.','.$faculty_name.','.$course_name.','.$cgpa.','.$completion_date.','.$NewPRN;
        ...
        //if($previewPdf!=1){
            // $this->updateCardNo('MitwpuDegree',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        //}
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MitwpuDegree'.date("Ymdhms")).'.pdf';
