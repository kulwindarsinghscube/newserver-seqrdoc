    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        for($excel_row = $excel_row_num; $excel_row <= $highestRow; $excel_row++)
        {
            ...
            if((!$isPreview&&(!isset($pdf_data['generation_from'])||(isset($pdf_data['generation_from'])&&$pdf_data['generation_from']!='API')))||(!$isPreview&&(isset($pdf_data['generation_from'])&&$pdf_data['generation_from']=='API'&&$pdf_data['generation_type']=='Live'))) {
                ...
                $student_table_id = $this->addCertificate($serial_no, $certName, $dt,$FID['template_id'],$admin_id,$get_file_aws_local_flag);
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$get_file_aws_local_flag)
    {
        ...
        if($get_file_aws_local_flag->file_aws_local == '1'){
            if(isset($systemConfig['sandboxing']) && $systemConfig['sandboxing'] == 1){
                ...
                $aws_qr = \Storage::disk('s3')->put('/'.$subdomain[0].'/backend/pdf_file/sandbox/'.$certName, file_get_contents($file2), 'public');
            ...
            if(isset($systemConfig['sandboxing']) && $systemConfig['sandboxing'] == 1){
                $aws_qr = \File::copy($file2,public_path().'/'.$subdomain[0].'/backend/pdf_file/sandbox/'.$certName);
            }else{
