...
class ValidateExcelChanakyaTranscriptJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $columnsCount2=count($rowData2[0]);
                ...
                {
                    if($columnsCount1==9 &&$columnsCount2==8)
                    {
                        // $columnArr=array('Register ID','student Name','Terms','Course_Code','Subject_Title','Credits','Grade','Grade Point ','CGP');
                        $columnArr=array('Unique_Id','student Name','Programme','Register No','Terms','CGPA Secured','Total credit of all semesters','serial_no','transcript_title');
                        // dd( $columnArr);
                        ...
                        $columnArr2=array('Register No','Terms','Course_Name','Code','Credit','Grade','GP','CP');
                        ...
                        if(count($mismatchColArr)>0){
                           ...
                           // --- Add Register No mismatch validation here ---
                $sheet1RegisterNos = array_column(array_slice($rowData1, 1), 3); // index 3 => Register No
                $sheet2RegisterNos = array_column(array_slice($rowData2, 1), 0); // index 0 => Register No

                $diffRegisterNos = array_diff($sheet1RegisterNos, $sheet2RegisterNos);
                if (count($diffRegisterNos) > 0) {
                    return response()->json([
                        'success' => false,
                        'type' => 'error',
                        'message' => 'Register No mismatch: The following Register No(s) in Sheet1 do not exist in Sheet2: '.implode(',', $diffRegisterNos)
                    ]);
                }
                    
                    
                    }elseif($columnsCount1!=7){
