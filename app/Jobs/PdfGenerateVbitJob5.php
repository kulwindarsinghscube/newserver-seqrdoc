    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $ghostImgArr = array();
            ...
            if($generated_documents == 0){
                ...
                $pdfBig = new TCPDF('P', 'mm', array('210', '297'), true, 'UTF-8', false);    $pdfBig->SetAuthor('TCPDF');
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('vbitPVC');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="ProvisionalCertificateForUgAndPg_BG.jpg"; // ProvisionalCertificateForUgAndPg_BG, pvc
			$low_res_bg="ProvisionalCertificateForUgAndPg_BG.jpg";
			$pdfBig->AddPage();
			...
			$PC_No = trim($studentData[3]);
			$HT_No = trim($studentData[4]);        
			$Serial_No = trim($studentData[5]);
			$Degree = trim($studentData[6]);
			$Year = trim($studentData[7]);
			$Placed_in = trim($studentData[8]);
			//$Image = trim($studentData[9]);
			$Memo_No = trim($studentData[10]);
			$Place = trim($studentData[11]);
			$Date = trim($studentData[12]);
			$Certificate = trim($studentData[13]);
            ...
            if(!$studentDataExist) {
                $CertificateStr = '';
            }  else if( $studentDataExist &&  !empty($Certificate) ) {
                $CertificateStr = $Certificate;
            } else if($studentDataExist &&  empty($Certificate) ) {
                $CertificateStr = 'DUPLICATE';
            } else {
                $CertificateStr = 'DUPLICATE';
            }
            ...
            $pdf->SetXY(92, 38);
			$pdf->writeHTML('<u>'.$CertificateStr.'</u>', true, 0, true, 0);
            ...
            $pdf->Cell(0, 0, "PC No. : ", 0, false, 'L');
            $pdf->SetXY($title1_x, $left_title3_y);
            $pdf->Cell(0, 0, "HT No. : ", 0, false, 'L');            
            ...
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetXY($left_str_x+13, $left_title1_y);
            $pdf->Cell(0, 0, 'JNTUH, HYDERABAD', 0, false, 'L');
            $pdf->SetXY($left_str_x+3.5, $left_title2_y);
            $pdf->Cell(125, 0, $PC_No, 0, false, 'L');
            $pdf->SetXY($left_str_x+3.5, $left_title3_y);
            $pdf->Cell(0, 0, $HT_No, 0, false, 'L');            
            $pdf->SetXY(142.5, $left_title1_y);
            $pdf->Cell(0, 0, 'P6', 0, false, 'L');
            $pdf->SetXY(134, $left_title2_y);
            $pdf->Cell(0, 0, $Serial_No, 0, false, 'L');
			
			$pdf->SetFont($times, '', 15, '', false);
            $pdf->SetXY(11, 123);
			$pdf->MultiCell(70, 0, 'This is to certify that Mr./Ms.',0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($times, '', 13, '', false);
            $pdf->SetXY(75, 123.3);
			$pdf->MultiCell(121, 0, $candidate_name,'B', 'L', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(11, 137);
			$pdf->MultiCell(45, 0, 'Son / Daughter of',0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($times, '', 13, '', false);
            $pdf->SetXY(51, 137.3);
			$pdf->MultiCell(145, 0, $Parents_Name,'B', 'L', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(11, 151);
			$pdf->MultiCell(30, 0, 'Passed',0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($times, '', 13, '', false);
            $pdf->SetXY(27, 151.3);
			$pdf->MultiCell(170, 0, $Degree,'B', 'L', 0, 0, '', '', true, 0, true);
			/*$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(169, 151);
			$pdf->MultiCell(30, 0, 'degree',0, 'L', 0, 0, '', '', true, 0, true);*/
			
			$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(11, 166);
			$pdf->MultiCell(0, 0, 'degree examination of this college (Autonomous) affiliated to JNTUH, Hyderabad,',0, 'L', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(11, 179.5);
			$pdf->MultiCell(0, 0, 'held in',0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($times, '', 13, '', false);
            $pdf->SetXY(29, 179.5);
			$pdf->MultiCell(82, 0, $Year,'B', 'C', 0, 0, '', '', true, 0, true);
			$pdf->SetFont($times, '', 15, '', false);
			$pdf->SetXY(114, 179.5);
			$pdf->MultiCell(0, 0, 'and that he / she was placed in',0, 'L', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($times, '', 13, '', false);
            $pdf->SetXY(19, 194);
			$pdf->MultiCell(175, 0, $Placed_in,'B', 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetFont($times, '', 15, '', false);
            $pdf->SetXY(18, 208);
			$pdf->MultiCell(175, 0, 'He / She has satisfied all the requirements for the award of the degree.',0, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetXY(92, 38);
			$pdfBig->writeHTML('<u>'.$CertificateStr.'</u>', true, 0, true, 0);
            ...
            $pdfBig->Cell(0, 0, "PC No. : ", 0, false, 'L');
            $pdfBig->SetXY($title1_x, $left_title3_y);
            $pdfBig->Cell(0, 0, "HT No. : ", 0, false, 'L');            
            ...
            $pdfBig->SetTextColor(0, 0, 0);
            $pdfBig->SetXY($left_str_x+13, $left_title1_y);
            $pdfBig->Cell(0, 0, 'JNTUH, HYDERABAD', 0, false, 'L');
            $pdfBig->SetXY($left_str_x+3.5, $left_title2_y);
            $pdfBig->Cell(125, 0, $PC_No, 0, false, 'L');
            $pdfBig->SetXY($left_str_x+3.5, $left_title3_y);
            $pdfBig->Cell(0, 0, $HT_No, 0, false, 'L');            
            $pdfBig->SetXY(142.5, $left_title1_y);
            $pdfBig->Cell(0, 0, 'P6', 0, false, 'L');
            $pdfBig->SetXY(134, $left_title2_y);
            $pdfBig->Cell(0, 0, $Serial_No, 0, false, 'L');
			
			$pdfBig->SetFont($times, '', 15, '', false);
            $pdfBig->SetXY(11, 123);
			$pdfBig->MultiCell(70, 0, 'This is to certify that Mr./Ms.',0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($times, '', 13, '', false);
            $pdfBig->SetXY(75, 123.3);
			$pdfBig->MultiCell(121, 0, $candidate_name,'B', 'L', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(11, 137);
			$pdfBig->MultiCell(45, 0, 'Son / Daughter of',0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($times, '', 13, '', false);
            $pdfBig->SetXY(51, 137.3);
			$pdfBig->MultiCell(145, 0, $Parents_Name,'B', 'L', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(11, 151);
			$pdfBig->MultiCell(30, 0, 'Passed',0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($times, '', 13, '', false);
            $pdfBig->SetXY(27, 151.3);
			$pdfBig->MultiCell(170, 0, $Degree,'B', 'L', 0, 0, '', '', true, 0, true);
			/*$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(169, 151);
			$pdfBig->MultiCell(30, 0, 'degree',0, 'L', 0, 0, '', '', true, 0, true);*/
			
			$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(11, 166);
			$pdfBig->MultiCell(0, 0, 'degree examination of this college (Autonomous) affiliated to JNTUH, Hyderabad,',0, 'L', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(11, 179.5);
			$pdfBig->MultiCell(0, 0, 'held in',0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($times, '', 13, '', false);
            $pdfBig->SetXY(29, 179.5);
			$pdfBig->MultiCell(82, 0, $Year,'B', 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->SetFont($times, '', 15, '', false);
			$pdfBig->SetXY(114, 179.5);
			$pdfBig->MultiCell(0, 0, 'and that he / she was placed in',0, 'L', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetFont($times, '', 13, '', false);
            $pdfBig->SetXY(13, 194);
			$pdfBig->MultiCell(183, 0, $Placed_in,'B', 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetFont($times, '', 15, '', false);
            $pdfBig->SetXY(18, 208);
			$pdfBig->MultiCell(175, 0, 'He / She has satisfied all the requirements for the award of the degree.',0, 'C', 0, 0, '', '', true, 0, true);
            ...
            $codeContents = '';
            $codeContents .= $candidate_name.", ".$HT_No.", ".$Serial_No;
            // $codeContents .="\n";
            $codeContents .="\n\n".strtoupper(md5($str));
       ...
       if($previewPdf!=1){
        $this->updateCardNo('vbitPVC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'vbitPVC'.date("Ymdhms")).'.pdf';
