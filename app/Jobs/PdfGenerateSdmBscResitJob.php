...
use Helper;
class PdfGenerateSdmBscResitJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('sdmBSCMedicalLabTech');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->SetXY(74.8, 106);	
			$pdfBig->MultiCell(99, 0, '', 'T', 'C', 0, 0);	//UIT HR Line
			$pdfBig->SetXY($left_str_x, 110);	
			$pdfBig->MultiCell(180, 0, '', 'T', 'C', 0, 0);	//SSUITR HR Line
			
			$pdfBig->SetXY(24.5, 97);	
			$pdfBig->MultiCell(50, 13, '', 'LR', 'C', 0, 0);  //Subject
			$pdfBig->MultiCell(27.4, 13, '', 'LR', 'C', 0, 0); //UE
			$pdfBig->MultiCell(27.1, 13, '', 'R', 'C', 0, 0); //IA
            $pdfBig->MultiCell(17.9, 13, '', 'R', 'C', 0, 0); //viva
			...
			$pdfBig->SetXY(25, 102);
			$pdfBig->MultiCell(50, 5, 'SUBJECT', 0, 'C', 0, 0);
			$pdfBig->SetXY(75, 98);
			$pdfBig->MultiCell(27, 5, 'UNIVERSITY EXAMINATION (UE)', 0, 'C', 0, 0);
			$pdfBig->SetXY(100, 98);
			$pdfBig->MultiCell(27, 5, 'INTERNAL ASSESSMENT (IA)', 0, 'C', 0, 0);
            $pdfBig->SetXY(125, 98);
			$pdfBig->MultiCell(27, 5, 'VIVA', 0, 'C', 0, 0);
			$pdfBig->SetXY(150, 100);
			$pdfBig->MultiCell(27, 5, 'TOTAL', 0, 'C', 0, 0);
			$pdfBig->SetXY(177, 102);
			$pdfBig->MultiCell(18, 5, 'REMARKS', 0, 'C', 0, 0);
            ...
            $pdf->SetXY(74.8, 106);	
            $pdf->MultiCell(99, 0, '', 'T', 'C', 0, 0);	//UIT HR Line
            $pdf->SetXY($left_str_x, 110);	
            $pdf->MultiCell(180, 0, '', 'T', 'C', 0, 0);	//SSUITR HR Line

            $pdf->SetXY(24.5, 97);	
            $pdf->MultiCell(50, 13, '', 'LR', 'C', 0, 0);  //Subject
            $pdf->MultiCell(27.4, 13, '', 'LR', 'C', 0, 0); //UE
            $pdf->MultiCell(27.1, 13, '', 'R', 'C', 0, 0); //IA
            $pdf->MultiCell(17.9, 13, '', 'R', 'C', 0, 0); //viva
            ...
            $pdf->SetXY(25, 102);
            $pdf->MultiCell(50, 5, 'SUBJECT', 0, 'C', 0, 0);
            $pdf->SetXY(75, 98);
            $pdf->MultiCell(27, 5, 'UNIVERSITY EXAMINATION (UE)', 0, 'C', 0, 0);
            $pdf->SetXY(100, 98);
            $pdf->MultiCell(27, 5, 'INTERNAL ASSESSMENT (IA)', 0, 'C', 0, 0);
            $pdf->SetXY(125, 98);
            $pdf->MultiCell(27, 5, 'VIVA', 0, 'C', 0, 0);
            $pdf->SetXY(150, 100);
            $pdf->MultiCell(27, 5, 'TOTAL', 0, 'C', 0, 0);
            $pdf->SetXY(177, 102);
            $pdf->MultiCell(18, 5, 'REMARKS', 0, 'C', 0, 0);
			...
			$pdfBig->MultiCell(9, 0, 'MIN', 0, 'C', 0, 0);
			$pdfBig->MultiCell(9, 0, 'SEC', 0, 'C', 0, 0);
            $pdfBig->MultiCell(9, 0, 'MAX', 0, 'C', 0, 0); //VA
			$pdfBig->MultiCell(9, 0, 'SEC', 0, 'C', 0, 0);
            ...
            $pdf->MultiCell(9, 0, 'MIN', 0, 'C', 0, 0);
            $pdf->MultiCell(9, 0, 'SEC', 0, 'C', 0, 0);
            $pdf->MultiCell(9, 0, 'MAX', 0, 'C', 0, 0); //VA
            $pdf->MultiCell(9, 0, 'SEC', 0, 'C', 0, 0);
			...
			foreach ($subjectsArr as $subjectDatas){
				...
				$UE_Max = trim($subjectDatas[2]);	
				$UE_Min = trim($subjectDatas[3]);	
				$UE_Sec = trim($subjectDatas[4]);
				$IA_Max = trim($subjectDatas[5]);	
				$IA_Min = trim($subjectDatas[6]);	
				$IA_Sec = trim($subjectDatas[7]);
                $VA_Max = trim($subjectDatas[8]);
                $VA_Sec = trim($subjectDatas[9]);
				$T_Max = trim($subjectDatas[10]);	
				$T_Min = trim($subjectDatas[11]);	
				$T_Sec = trim($subjectDatas[12]);	
				$Remark = strtoupper(trim($subjectDatas[13]));	
				/*start pdfbig line*/
				...
				$lines = $pdfBig->MultiCell(50, 0, $Subject_Name, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
				$lines = $pdf->MultiCell(50, 0, $Subject_Name, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                $pdfBig->MultiCell(50, $th_height, $Subject_Name, 1, 'L', 0, 0);					
                $pdfBig->SetXY(74.5, $subj_y);					
                $pdfBig->MultiCell(9.5, $th_height_half, $UE_Max, 1, 'C', 0, 0);
                ...
                $pdfBig->MultiCell(9, $th_height_half, $VA_Max, 1, 'C', 0, 0);
                $pdfBig->MultiCell(9, $th_height_half, $VA_Sec, 1, 'C', 0, 0);
                ...
                $pdfBig->MultiCell(21, $th_height_half, $Remark, 1, 'C', 0, 0);

                $pdf->SetFont($MyriadProRegular, '', 10, '', false);	
                $pdf->SetXY($left_str_x, $subj_y);
                $pdf->MultiCell(9.5, $th_height, $Sr_no, 1, 'C', 0, 0);
                $pdf->MultiCell(50, $th_height, $Subject_Name, 1, 'L', 0, 0);					
                $pdf->SetXY(74.5, $subj_y);					
                $pdf->MultiCell(9.5, $th_height_half, $UE_Max, 1, 'C', 0, 0);
                ...
                $pdf->MultiCell(9, $th_height_half, $VA_Max, 1, 'C', 0, 0);
                $pdf->MultiCell(9, $th_height_half, $VA_Sec, 1, 'C', 0, 0);
                ...
                $pdf->MultiCell(21, $th_height_half, $Remark, 1, 'C', 0, 0);
			...
			$pdfBig->SetXY($left_str_x, $subj_y);	
			$pdfBig->MultiCell(132.1, 5, 'GRAND TOTAL:', 'LRB', 'R', 0, 0); 
			$pdfBig->SetFont($MyriadProRegular, '', 10, '', false);
			$pdfBig->MultiCell(9, 5, $Grand_Total_Max, 1, 'C', 0, 0); 
			$pdfBig->MultiCell(9, 5, $Grand_Total_Min, 1, 'C', 0, 0); 
			$pdfBig->MultiCell(9, 5, $Grand_Total_Sec, 1, 'C', 0, 0); 
			$pdfBig->MultiCell(21, 5, '', 1, 'C', 0, 0); 
			$pdfBig->SetFont($MyriadProBold, 'B', 10, '', false);
			$pdfBig->SetXY($left_str_x, $subj_y+5);	
			$pdfBig->MultiCell(132.1, 5, 'GRAND TOTAL IN WORDS:', 'LR', 'R', 0, 0); 
			$pdfBig->MultiCell(48, 5, $Grand_Total_In_Words, 'R', 'L', 0, 0);
			$pdfBig->SetXY($left_str_x, $subj_y+10);	
			// $pdfBig->MultiCell(132.1, 5, 'PERCENTAGE:', 'LR', 'R', 0, 0); 
			// $pdfBig->MultiCell(48, 5, $Percentage, 'R', 'L', 0, 0);
			// $pdfBig->SetXY($left_str_x, $subj_y+15);	
			$pdfBig->MultiCell(132.1, 5, 'RESULT:', 'LRB', 'R', 0, 0); 
			$pdfBig->MultiCell(48, 5, $Result, 'LRB', 'L', 0, 0);
						

            $pdf->SetFont($MyriadProBold, 'B', 10, '', false);
            $pdf->SetXY($left_str_x, $subj_y);	
            $pdf->MultiCell(132.1, 5, 'GRAND TOTAL:', 'LRB', 'R', 0, 0); 
            $pdf->SetFont($MyriadProRegular, '', 10, '', false);
            $pdf->MultiCell(9, 5, $Grand_Total_Max, 1, 'C', 0, 0); 
            $pdf->MultiCell(9, 5, $Grand_Total_Min, 1, 'C', 0, 0); 
            $pdf->MultiCell(9, 5, $Grand_Total_Sec, 1, 'C', 0, 0); 
            $pdf->MultiCell(21, 5, '', 1, 'C', 0, 0); 
            $pdf->SetFont($MyriadProBold, 'B', 10, '', false);
            $pdf->SetXY($left_str_x, $subj_y+5);	
            $pdf->MultiCell(132.1, 5, 'GRAND TOTAL IN WORDS:', 'LR', 'R', 0, 0); 
            $pdf->MultiCell(48, 5, $Grand_Total_In_Words, 'R', 'L', 0, 0);
            $pdf->SetXY($left_str_x, $subj_y+10);	
            // $pdf->MultiCell(132.1, 5, 'PERCENTAGE:', 'LR', 'R', 0, 0); 
            // $pdf->MultiCell(48, 5, $Percentage, 'R', 'L', 0, 0);
            // $pdf->SetXY($left_str_x, $subj_y+15);	
            $pdf->MultiCell(132.1, 5, 'RESULT:', 'LRB', 'R', 0, 0); 
            $pdf->MultiCell(48, 5, $Result, 'LRB', 'L', 0, 0);
            
            // $notes="Min. Marks for Eligibility to appear for University Examination: <br>
            //     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;5% in each Theory and Internal Assessment separately.<br><br>
            //     Min. Marks to Pass:  University Examination: <br>
            //     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;40% marks in theory and 40% marks in practical separately to pass in each subject<br>
            //     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;50% aggregate including Theory+ IA + viva voce<br>
            //     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;50% aggregate including Practical+ IA in each subject.";		
            $notes="Min. Marks for Eligibility to appear for University Examination:<br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;35% in each Theory and Practical Internal Assessment separately.<br><br>
                Min. Marks to Pass:  University Examination: <br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;40% marks in theory paper and 40% marks in practical separately to pass in each subject<br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;50% aggregate including Theory (UE) + IA + viva voce (UE)<br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;50% aggregate including Practical (UE) + IA in each subject.";	
            ...
            if($specilisation == 'B.Sc. IN ANAESTHESIOLOGY TECHNOLOGY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory to pass in each subject
                //     50% aggregate including Theory (UE) + IA + viva voce
                    
                $notes="Min. Marks for Eligibility to appear for University Examination:  <br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;35% in each Theory and Internal Assessment separately.<br><br>
                    Min. Marks to Pass:  University Examination: <br>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;40% marks in theory to pass in each subject<br>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 50% aggregate including Theory (UE) + IA + viva voce";                       
                    
            }elseif($specilisation == 'B.Sc. IN OPTOMETRY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each subject Theory Internal Assessment 
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper to pass in each subject
                //     50% aggregate including Theory+ IA 
                    
                $notes="Min. Marks for Eligibility to appear for University Examination: <br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;35% in each subject Theory Internal Assessment<br><br>
                Min. Marks to Pass:  University Examination: <br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;40% marks in theory paper to pass in each subject<br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 50% aggregate including Theory+ IA"; 

            }elseif($specilisation == 'B.Sc. IN MEDICAL LABORATORY TECHNOLOGY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical separately to pass in each subject
                //     50% aggregate including Theory (UE) + IA + viva voce (UE)
                //     50% aggregate including Practical (UE) + IA in each subject.
                    
                $notes="Min. Marks for Eligibility to appear for University Examination:<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                35% in each Theory and Practical Internal Assessment separately.<br><br>
                Min. Marks to Pass:  University Examination: <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                40% marks in theory paper and 40% marks in practical separately to pass in each subject
                <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 
                50% aggregate including Theory (UE) + IA + viva voce (UE)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                50% aggregate including Practical (UE) + IA in each subject."; 
            }elseif($specilisation == 'B.Sc. IN RENAL DIALYSIS TECHNOLOGY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each subject Theory Internal Assessment 
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper to pass in each subject
                ...
                $notes="Min. Marks for Eligibility to appear for University Examination:<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                35% in each subject Theory Internal Assessment<br><br>
                Min. Marks to Pass:  University Examination: <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                40% marks in theory paper to pass in each subject
                <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 
                50% aggregate including Theory+ IA"; 

            }elseif($specilisation == 'B.Sc. IN MEDICAL IMAGING TECHNOLOGY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory and 40% marks in practical separately
                //     50% aggregate including Theory+ IA 
                //     50% aggregate including Practical+ IA 
                ...
                40% marks in theory and 40% marks in practical separately
                <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 
                50% aggregate including Theory+ IA <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                50% aggregate including Practical+ IA"; 

            }elseif($specilisation == 'B.Sc. IN ANAESTHESIOLOGY TECHNOLOGY' &&  $year == 'FIRST YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory to pass in each subject
            ...
            }elseif($specilisation == 'B.Sc. IN OPTOMETRY' &&  $year == 'THIRD YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical UE separately 
                //     50% aggregate including Theory+ IA 
                //     50% aggregate including Practical+ IA 
                ...
                50% aggregate including Theory+ IA <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                50% aggregate including Practical+ IA"; 
            }elseif($specilisation == 'B.Sc. IN MEDICAL LABORATORY TECHNOLOGY' &&  $year == 'THIRD YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical UE separately to pass in each subject
                //     50% aggregate including Theory+ IA + viva voce
                //     50% aggregate including Practical+ IA in each subject.
                ...
                40% marks in theory paper and 40% marks in practical UE separately to pass in each subject
                <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 
                50% aggregate including Theory+ IA + viva voce <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;
                50% aggregate including Practical+ IA in each subject."; 
            }elseif($specilisation == 'B.Sc. IN RENAL DIALYSIS TECHNOLOGY' &&  $year == 'THIRD YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical UE separately
            ...
            }elseif($specilisation == 'B.Sc. IN MEDICAL IMAGING TECHNOLOGY' &&  $year == 'THIRD YEAR' ){
                    // Min. Marks for Eligibility to appear for University Examination: 
                    // 35% in each Theory and Practical Internal Assessment separately.
                    // Min. Marks to Pass:  University Examination: 
                    // 40% marks in theory paper and 40% marks in practical UE separately 
            ...
            }elseif($specilisation == 'B.Sc. IN OPTOMETRY' &&  $year == 'SECOND YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical (UE) separately 
            ...
            }elseif($specilisation == 'B.Sc. IN ANAESTHESIOLOGY TECHNOLOGY' &&  $year == 'SECOND YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each subject Theory Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper to pass in each subject
                //     50% aggregate including Theory+ IA + viva voce 
                    
                $notes="Min. Marks for Eligibility to appear for University Examination:  <br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;35% in each subject Theory Internal Assessment separately.<br><br>
                    Min. Marks to Pass:  University Examination: <br>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp;40% marks in theory paper to pass in each subject<br>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size:18;'>•</span>&nbsp;&nbsp;&nbsp; 50% aggregate including Theory+ IA + viva voce";  

            }elseif($specilisation == 'B.Sc. IN MEDICAL LABORATORY TECHNOLOGY' &&  $year == 'SECOND YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical separately 
                //     50% aggregate including Theory (UE) + IA + viva voce (UE)
                //     50% aggregate including Practical (UE) + IA in each subject.
            ...
            }elseif($specilisation == 'B.Sc. IN RENAL DIALYSIS TECHNOLOGY' &&  $year == 'SECOND YEAR' ){
            ...
            }elseif($specilisation == 'B.Sc. IN MEDICAL IMAGING TECHNOLOGY' &&  $year == 'SECOND YEAR' ){
                // Min. Marks for Eligibility to appear for University Examination: 
                //     35% in each Theory and Practical Internal Assessment separately.
                //     Min. Marks to Pass:  University Examination: 
                //     40% marks in theory paper and 40% marks in practical UE separately 
			...
			$pdfBig->SetFont($MyriadProRegular, '', 6, '', false);
			$pdfBig->SetXY(51.5, 283);
			$pdfBig->MultiCell(92, 0, "SDM College of Medical Sciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India",0, 'C', 0, 0);
            ...
            $pdf->SetFont($MyriadProRegular, '', 6, '', false);
            $pdf->SetXY(51.5, 283);
            $pdf->MultiCell(92, 0, "SDM College of Medical Sciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India",0, 'C', 0, 0);
			/******End pdfBig ******/
            ...
            $COE_x = 150;
            $COE_y = 247;
            $COE_Width = 37.041666667; //39.6875;
            $COE_Height = 25.135416667; //28.045833333;
