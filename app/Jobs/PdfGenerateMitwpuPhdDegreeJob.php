...
class PdfGenerateMitwpuPhdDegreeJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('MitwpuPhd');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                // $topic_krutidev = trim($studentData[15]);
                // $full_name = trim($studentData[1]);
                // $course_name = trim($studentData[10]);
                // $topic = trim($studentData[13]);
                ...
                $topic = trim($studentData[13]);
                $topic_hindi = trim($studentData[14]);
                $topic_krutidev = trim($studentData[15]);
                $topic_krutidev = htmlspecialchars($topic_krutidev);
                ...
                $specialization_krutidev = trim($studentData[20]);
                ...
                $pdfBig->SetTextColor(50,50,154);
                $pdfBig->SetFont($KRDEV100, '', 16.02, '', false);
                $pdfBig->SetXY(15, $pdfBig->GetY()+1);

                $topic_krutidev_text = '';
                if($topic_krutidev && $year == '2025') {
                    $topic_krutidev_text = "'kh\"kZd ß".trim($topic_krutidev).'Þ';
                }
                $pdfBig->MultiCell(182, 0, "mUgksaus ".$completion_date_krutidev." esa fofu;eksa ds rgr bl mikf/k dks çkIr djus ds fy, fu/kkZfjr vko';drkvksa
                    dks iwjk fd;k gS vkSj lQyrkiwoZd fFkfll ".$topic_krutidev_text." dk leFkZu fd;k gS
                    bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds
                ...
                $pdfBig->SetTextColor(50,50,154);
                $pdfBig->SetFont($MTCORSVA, '', 16.02, '', false);
                $pdfBig->SetXY(10, 165.5);
                $pdfBig->MultiCell(190, 0, "the Degree of", 0, 'C', 0, 0, '', '', true, 0, true);


                $pdfBig->SetTextColor(255,0,0);
                $pdfBig->SetFont($timesb, '', 18, '', false);
                $pdfBig->SetXY(10, 171.5);
                ...
                $topic_text = '';
                if($topic && $year == '2025') {
                    $topic_text = ' titled "'.$topic.'"';
                }

                $pdfBig->MultiCell(190, 0, 'has fulfilled the requirements for the degree as prescribed under the regulations and successfully defended the thesis'.$topic_text.' in '.$completion_date_new.'.
                ...
                $pdf->MultiCell(182, 0, "mUgksaus ".$completion_date_krutidev." esa fofu;eksa ds rgr bl mikf/k dks çkIr djus ds fy, fu/kkZfjr vko';drkvksa
                    dks iwjk fd;k gS vkSj lQyrkiwoZd fFkfll ".$topic_krutidev_text." dk leFkZu fd;k gS
                    bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds
                    vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);

                // $pdf->MultiCell(182, 0, "mUgksaus ".$completion_date_krutidev." esa fofu;eksa ds rgr bl mikf/k dks çkIr djus ds fy, fu/kkZfjr vko';drkvksa
                    // dks iwjk fd;k gS vkSj lQyrkiwoZd fFkfll 'kh\"kZd ß".$topic_krutidev."Þ dk leFkZu fd;k gS
                    // bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds
                    // vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdf->MultiCell(190, 0, trim(ucfirst($full_name)).' <span style="font-size:16;display: flex; align-items: center;">'.  trim(ucfirst($mother_name)).'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdf->MultiCell(190, 0, 'has fulfilled the requirements for the degree as prescribed under the regulations and successfully defended the thesis'.$topic_text.' in '.$completion_date_new.'.
                            <br>In recognition we have hereunder placed our names and
                            <br>the seal of the University on this '.$issue_formatted_date.'.', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdf->MultiCell(190, 0, 'has fulfilled the requirements for the degree as prescribed under the regulations and successfully defended the thesis  titled "'.$topic.'" in '.$completion_date_new.'.
                //             <br>In recognition we have hereunder placed our names and
                //             <br>the seal of the University on this '.$issue_formatted_date.'.', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->MultiCell(60, 0, 'PRN No. - '.$PRN, 0, 'L', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($calibrib, '', 10.98, '', false);
                $pdf->SetXY(147, 259);
                $pdf->MultiCell(60, 0, 'PRN No. - '.$PRN, 0, 'L', 0, 0, '', '', true, 0, true);
                ...
                $codeContents = $full_name.','.$faculty_name.','.$course_name.','.$completion_date_new.','.$PRN;
                    
                $codeContents = mb_convert_encoding($codeContents, 'UTF-8', 'auto');
                // if($subdomain[0] =='demo') {
                    $codeContents .="\n\n";
                    // $codeData =CoreHelper::getBCVerificationUrl(encrypt(strtoupper(md5($str))));
                ...
                if($previewPdf!=1){
                    ...
                    $mintData['metadata4']=["label"=> "Topic", "value"=> $topic];
        ...
        // if($previewPdf!=1){
		    // $this->updateCardNo('MitwpuPhd',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        // }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MitwpuPhd'.date("Ymdhms")).'.pdf';
...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$mintData)
    {
        ...
        if($systemConfig['sandboxing'] == 1){
            ...
            $result = StudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>$certName,'template_id'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id,'template_type'=>2,'certificate_type'=>'PHD Degree Certificate' ,'bc_txn_hash'=>$bc_txn_hash,'bc_ipfs_hash'=>$bc_ipfs_hash,'pinata_ipfs_hash'=>$pinata_ipfs_hash, 'bc_file_hash'=>$bc_file_hash]);
