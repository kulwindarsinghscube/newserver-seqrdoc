...
class ValidateExcelWOXSENTranscript10Job
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $dropdown_template_id=$pdf_data['dropdown_template_id'];
              ...
              $rowData1[0] = array_filter($rowData1[0]);
                ...
                if($columnsCount1==35&&$columnsCount2==6){

                    $columnArr=array("Unique_ID","Student_Name","Mother_Name","Father_Name","Program","Academic_Year","PGID","Admission_ID","TC-1","T1_GPA","TC-2","T2_GPA","TC-3","T3_GPA","TC-4","T4_GPA","TC-5","T5_GPA","TC-6","T6_GPA","TC-7","T7_GPA","TC-8","T8_GPA","TC-9","T9_GPA","TC-10","T10_GPA","Total_Credits_Earned","CGPA","Serial_No","Major1","Major2","Major/ Minor","Month and Year");
                ...
                foreach ($rowData1 as $key1 => $value1) {
                    ...
                    $profile_path_jpeg = public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.trim($value1[7]).'.jpeg';
                    $profile_path_png = public_path().'\\'.$subdomain[0].'\backend\templates\100\\'.trim($value1[7]).'.png';

                    //$profile_path_jpg = public_path()."/".$subdomain[0]."/backend/templates/100/".trim($value1[5]).'.jpg';
                    ...
                    if (!file_exists($profile_path_jpg)) {
                        if(!file_exists($profile_path_png)){
							if(!file_exists($profile_path_jpeg)){
								array_push($profErrArr, $value1[7]);
							}
                        } 
                    }
