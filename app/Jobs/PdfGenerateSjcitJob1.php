...
class PdfGenerateSjcitJob1 
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('SJCIT');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) 
            {
				...
				$high_res_bg="SJC_ Grade Card_BG.jpg"; // bestiu_pdc_bg, TranscriptData
				$low_res_bg="SJC_ Grade Card_BG.jpg";
				$pdfBig->AddPage();
                ...
                $sgpa=trim($studentData[11]);
             
                $credits_reg=trim($studentData[7]);
                $credits_earn=trim($studentData[8]);
                $cumulative=trim($studentData[9]);
                $ecg=trim($studentData[10]);
                $photo=trim($studentData[15]);                      			
				$Name=trim($studentData[1]);
                $Medium_of_Instruction=trim($studentData[13]);
                $Date=$this->convertExcelDate($studentData[14]);
                $Month_Year =trim($studentData[5]);
                $Appar_Id =trim($studentData[6]);
                // dd($Date);
                ...
                $pdfBig->SetXY(0, 47);
                $pdfBig->MultiCell(0, 8, $course_name, 0, 'C', 0, 1);

                // $pdfBig->SetFont($arial, '', 10, '', false); 
                // $pdfBig->SetX($left_pos);
                // $pdfBig->MultiCell(0, 5, 'Name of the Student      : '.$STUDENT_NAME, 0, 'L', 0, 1);
                // $y = $pdfBig->GetY();
                // $pdfBig->SetX($left_pos);
                // $pdfBig->MultiCell(0, 5, 'Father\'s/Mother\'s Name : '.$parent_Name, 0, 'L', 0, 1);

                // $pdfBig->SetX($left_pos);
                // $pdfBig->MultiCell(0, 5, 'USN                               : '.$Roll_NO, 0, 'L', 0, 1);

                 
                // $pdfBig->SetXY(122,  $y);
                // $pdfBig->MultiCell(0, 5, 'Month & Year  : '.$Month_Year , 0, 'L', 0, 1);
                // $pdfBig->SetX(122);
                // $pdfBig->MultiCell(0, 5, 'Appar Id          : '. $Appar_Id, 0, 'L', 0, 1);
                ...
                $labelWidth = 38;   // Fixed width for labels
                $valueWidth = 60;   // Fixed width for values
                $lineHeight = 5;
                // Row 1
                $pdfBig->SetX($left_pos);
                $pdfBig->Cell($labelWidth, $lineHeight, 'Name of the Student', 0, 0);
                $pdfBig->Cell(3, $lineHeight, ':', 0, 0);
                $pdfBig->Cell(0, $lineHeight, $STUDENT_NAME, 0, 1); 
                // Row 2
                $pdfBig->SetX($left_pos);
                $pdfBig->Cell($labelWidth, $lineHeight, 'Father\'s/Mother\'s Name', 0, 0);
                $pdfBig->Cell(3, $lineHeight, ':', 0, 0);
                $pdfBig->Cell($valueWidth, $lineHeight, $parent_Name, 0, 0);

                $pdfBig->SetX($right_pos);
                $pdfBig->Cell(23, $lineHeight, 'Month & Year', 0, 0);
                $pdfBig->Cell(3, $lineHeight, ':', 0, 0);
                $pdfBig->Cell(40, $lineHeight, $Month_Year, 0, 1); 
                // Row 3
                $pdfBig->SetX($left_pos);
                $pdfBig->Cell($labelWidth, $lineHeight, 'USN', 0, 0);
                $pdfBig->Cell(3, $lineHeight, ':', 0, 0);
                $pdfBig->Cell($valueWidth, $lineHeight, $Roll_NO, 0, 0);

                $pdfBig->SetX($right_pos);
                $pdfBig->Cell(23, $lineHeight, 'Appar Id', 0, 0);
                $pdfBig->Cell(3, $lineHeight, ':', 0, 0);
                $pdfBig->Cell(40, $lineHeight, $Appar_Id, 0, 1);


            $y = $pdfBig->GetY();

            $pdfBig->SetFont($arial, '', 10, '', false); 
            $pdfBig->setCellPaddings('', 2, '', '');

            $pdfBig->SetXY($left_pos, $y);	
            $pdfBig->MultiCell(10, 11.5, 'Sl. No', 'LT', 'C', 0, 0);
            $pdfBig->MultiCell(24, 11.5, 'Course Code', 'LT', 'C', 0, 0);
            $pdfBig->MultiCell(77, 11.5, 'Title of the Course Registered', 'LBT', 'C', 0, 0);
            $pdfBig->MultiCell(18, 11.5, 'Credits Assigned', 'TL', 'C', 0, 0);
            $pdfBig->MultiCell(19, 11.5, 'Credits Earned(C)', 'TL', 'C', 0, 0);
            $pdfBig->MultiCell(17, 11.5, 'Grade Point(G)', 'LT', 'C', 0, 0);

            // ⬇️ IMPORTANT: last cell ln = 1
            $pdfBig->MultiCell(16, 11.5, 'Letter Grade', 'LRT', 'C', 0, 1);

            $y_table_end = $pdfBig->GetY();

                $pdfBig->SetXY($left_pos,  $y_table_end);
                $pdfBig->MultiCell(181, 0, '', 'T', 'C', 0, 0);
                $pdfBig->SetXY($left_pos, $y_table_end );
                $pdfBig->MultiCell(10, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(24, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(77, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(18, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(19, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(17, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(16, 110, '', 'LRB', 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->Ln();
                $tableY=$pdfBig->GetY();
                $title_y= $y_table_end;
                $dataY =  $title_y;
                ...
                foreach (array_reverse($subjects, true) as $term => $term_array) {
                    ...
                    $pdfBig->SetFont($arial, '', 10, '', false); 
                        ...
                        $pdfBig->MultiCell(77, $subHegight, trim($row[3]), 0, 'L', 0, 0, '', '', true);
                        $pdfBig->MultiCell(18, $subHegight, trim($row[4]), 0, 'C', 0, 0, '', '', true);
                        $pdfBig->MultiCell(19, $subHegight, trim($row[5]), 0, 'C', 0, 0, '', '', true);
                        $pdfBig->MultiCell(16, $subHegight, trim($row[6]), 0, 'C', 0, 0, '', '', true);
                        $pdfBig->MultiCell(17, $subHegight, trim($row[7]), 0, 'C', 0, 0, '', '', true);
                        $pdfBig->Ln();
                ...
                // $pdfBig->MultiCell(25, 13, 'Credits\nEarned', 'LT', 'C', 0, 0, '', '', true, 0, true);
                // $pdfBig->MultiCell(25, 13, "Credits\nEarned", 'LT', 'C', 0, 0, '', '', true, 0, true);
                ...
                //    $barcode_y=$pdfBig->GetY();
                    ...
                    $pdfBig->SetTextColor(0, 0, 0);
                    $pdfBig->SetFont($arial, '', 10, '', false);
                    $pdfBig->SetXY(16, $pdfBig->GetY()+5);
                    $pdfBig->MultiCell(0,8,'Medium of instruction : ' . $Medium_of_Instruction, 0, 'L',  false, 1);
                    $pdfBig->SetXY(16, $pdfBig->GetY());
                    $pdfBig->MultiCell( 0,8,'Date : ' . $Date, 0,'L',false, 1);
           
                    // Continue with next content
                    $pdfBig->SetFont($arialb, '', 10, '', false);
                    // $pdfBig->SetXY(89, 265);
                    $ghostimage_y = $pdfBig->GetY(); 
// dd($ghostimage_y );
				//End pdfBig 
                $left_pos=14;
                // $left_pos_two=112;
                ...
                // $pdf->SetXY(0, 47);
                // $pdf->MultiCell(0, 8, $course_name, 0, 'C', 0, 1);

                // $pdf->SetFont($arial, '', 10, '', false); 
                // $pdf->SetX($left_pos);
                // $pdf->MultiCell(0, 5, 'Name of the Student      : '.$STUDENT_NAME, 0, 'L', 0, 1);
                // $y = $pdf->GetY();
                // $pdf->SetX($left_pos);
                // $pdf->MultiCell(0, 5, 'Father\'s/Mother\'s Name : '.$parent_Name, 0, 'L', 0, 1);

                // $pdf->SetX($left_pos);
                // $pdf->MultiCell(0, 5, 'USN                                : '.$Roll_NO, 0, 'L', 0, 1);

                 
                // $pdf->SetXY(122,  $y);
                // $pdf->MultiCell(0, 5, 'Month & Year  : '.$Month_Year , 0, 'L', 0, 1);
                // $pdf->SetX(122);
                // $pdf->MultiCell(0, 5, 'Appar Id           : '. $Appar_Id, 0, 'L', 0, 1);
                $right_pos = 112;
                ...
                // Row 1
                $pdf->SetX($left_pos);
                $pdf->Cell($labelWidth, $lineHeight, 'Name of the Student', 0, 0);
                $pdf->Cell(3, $lineHeight, ':', 0, 0);
                $pdf->Cell(0, $lineHeight, $STUDENT_NAME, 0, 1); 
                // Row 2
                $pdf->SetX($left_pos);
                $pdf->Cell($labelWidth, $lineHeight, 'Father\'s/Mother\'s Name', 0, 0);
                $pdf->Cell(3, $lineHeight, ':', 0, 0);
                $pdf->Cell($valueWidth, $lineHeight, $parent_Name, 0, 0);

                $pdf->SetX($right_pos);
                $pdf->Cell(23, $lineHeight, 'Month & Year', 0, 0);
                $pdf->Cell(3, $lineHeight, ':', 0, 0);
                $pdf->Cell(40, $lineHeight, $Month_Year, 0, 1); 
                // Row 3
                $pdf->SetX($left_pos);
                $pdf->Cell($labelWidth, $lineHeight, 'USN', 0, 0);
                $pdf->Cell(3, $lineHeight, ':', 0, 0);
                $pdf->Cell($valueWidth, $lineHeight, $Roll_NO, 0, 0);

                $pdf->SetX($right_pos);
                $pdf->Cell(23, $lineHeight, 'Appar Id', 0, 0);
                $pdf->Cell(3, $lineHeight, ':', 0, 0);
                $pdf->Cell(40, $lineHeight, $Appar_Id, 0, 1);
            ...
            $pdf->MultiCell(10, 11.5, 'Sl. No', 'LT', 'C', 0, 0);
            $pdf->MultiCell(24, 11.5, 'Course Code', 'LT', 'C', 0, 0);
            $pdf->MultiCell(77, 11.5, 'Title of the Course Registered', 'LBT', 'C', 0, 0);
            $pdf->MultiCell(18, 11.5, 'Credits Assigned', 'TL', 'C', 0, 0);
            $pdf->MultiCell(19, 11.5, 'Credits Earned(C)', 'TL', 'C', 0, 0);
            $pdf->MultiCell(17, 11.5, 'Grade Point(G)', 'LT', 'C', 0, 0);

            // ⬇️ IMPORTANT: last cell ln = 1
            $pdf->MultiCell(16, 11.5, 'Letter Grade', 'LRT', 'C', 0, 1);

            $y_table_end = $pdf->GetY();

                $pdf->SetXY($left_pos,  $y_table_end);
                $pdf->MultiCell(181, 0, '', 'T', 'C', 0, 0);
                $pdf->SetXY($left_pos, $y_table_end );
                $pdf->MultiCell(10, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(24, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(77, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(18, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(19, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(17, 110, '', 'LB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(16, 110, '', 'LRB', 'C', 0, 0, '', '', true, 0, true);
                $pdf->Ln();
                $tableY=$pdf->GetY();
                $title_y= $y_table_end;
                $dataY =  $title_y;
                ...
                foreach (array_reverse($subjects, true) as $term => $term_array) {
                    ...
                    $pdf->SetFont($arial, '', 10, '', false); 
                        ...
                        $pdf->MultiCell(77, $subHegight, trim($row[3]), 0, 'L', 0, 0, '', '', true);
                        $pdf->MultiCell(18, $subHegight, trim($row[4]), 0, 'C', 0, 0, '', '', true);
                        $pdf->MultiCell(19, $subHegight, trim($row[5]), 0, 'C', 0, 0, '', '', true);
                        $pdf->MultiCell(16, $subHegight, trim($row[6]), 0, 'C', 0, 0, '', '', true);
                        $pdf->MultiCell(17, $subHegight, trim($row[7]), 0, 'C', 0, 0, '', '', true);
                        $pdf->Ln();
                ...
                $pdf->SetXY($x, $ghost_y);
                   ...
                   $barcode_y=$pdf->GetY();
                    ...
                    $pdf->MultiCell(0,8,'Medium of instruction : ' . $Medium_of_Instruction, 0, 'L',  false, 1);
                    $pdf->SetXY(16, $pdf->GetY());
                    $pdf->MultiCell( 0,8,'Date : ' . $Date, 0,'L',false, 1);

                    // Continue with next content
                    $pdf->SetFont($arialb, '', 10, '', false);
                    // $pdf->SetXY(89, 265);
                    $ghostimage_y = $pdf->GetY(); 
			    //Start pdf
                ...
                // === Step 1: Load Student Image ===
                    $template_id = 100;
                    $Photo = trim($studentData[15]); // e.g., '20WU0201083'
        ...
        $file_name =  str_replace("/", "_",'SJCIT'.date("Ymdhms")).'.pdf';
