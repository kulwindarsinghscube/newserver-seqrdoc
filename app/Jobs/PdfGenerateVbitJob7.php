...
class PdfGenerateVbitJob7
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="id_7.jpg"; // id_7, CMM memo grade ug
			$low_res_bg="id_7.jpg";
			$pdfBig->AddPage();
            ...
            //start pdf
            /*Left Side*/
            $pdf->SetFont($arialNarrowB, '', 6.5, '', false);
			$pdf->SetXY(11.5, 78);	
			...
			$pdf->MultiCell(61, 5, 'SUBJECT TITLE', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '0.5', $top = '0', $right = '', $bottom = '');
			$pdf->MultiCell(8.5, 5, 'GRADE POINT', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '0.2', $top = '1.4', $right = '', $bottom = '');
			$pdf->MultiCell(9.2, 5, 'GRADE', 0, 'C', 0, 0, '', '', true, 0, true);
            $pdf->setCellPaddings( $left = '0', $top = '1.4', $right = '', $bottom = '');
			$pdf->MultiCell(9.5, 5, 'CREDITS', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			            
			$pdf->SetXY(12, 77);	
			$pdf->MultiCell(4, 8, '', 'LRTB', 'C', 0, 0);
			$pdf->MultiCell(62, 8, '', 'LRTB', 'C', 0, 0);
			$pdf->MultiCell(9, 8, '', 'LRTB', 'C', 0, 0);
			$pdf->MultiCell(8, 8, '', 'LRTB', 'C', 0, 0);
			$pdf->MultiCell(9.7, 8, '', 'LRTB', 'C', 0, 0);				
			
			$pdf->SetXY(12, 85.2);	
			$pdf->MultiCell(92.8, 0, 'I-I SEMESTER', 0, 'C', 0, 0);
			$pdf->SetXY(12, 85.2);	
			$pdf->MultiCell(185.6, 0, '', 0, 'C', 0, 0);
			...
			for ($lx = 1; $lx <= 4; $lx++) {
				$pdf->SetXY(12, $startY);   
				$pdf->MultiCell(4, 35, '', 'LRTB', 'C', 0, 0);
				$pdf->MultiCell(62, 35, '', 'LRTB', 'L', 0, 0);
				$pdf->MultiCell(9, 35, '', 'LRTB', 'C', 0, 0);
				$pdf->MultiCell(8, 35, '', 'LRTB', 'C', 0, 0);
				$pdf->MultiCell(9.7, 35, '', 'LRTB', 'C', 0, 0); 
				$startY+=38.5;	
            }			
			//2nd Sem            
            $startSY=88.8;
			...
			//start pdfBig
            /*Left Side*/
			$pdfBig->SetFont($arialNarrowB, '', 6.5, '', false);
			...
			$pdfBig->MultiCell(61, 5, 'SUBJECT TITLE', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '0.5', $top = '0', $right = '', $bottom = '');
			$pdfBig->MultiCell(8.5, 5, 'GRADE POINT', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '0.2', $top = '1.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(9.2, 5, 'GRADE', 0, 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->setCellPaddings( $left = '0', $top = '1.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(9.5, 5, 'CREDITS', 0, 'C', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
			            
			$pdfBig->SetXY(12, 77);	
			$pdfBig->MultiCell(4, 8, '', 'LRTB', 'C', 0, 0);
			$pdfBig->MultiCell(62, 8, '', 'LRTB', 'C', 0, 0);
			$pdfBig->MultiCell(9, 8, '', 'LRTB', 'C', 0, 0);
			$pdfBig->MultiCell(8, 8, '', 'LRTB', 'C', 0, 0);
			$pdfBig->MultiCell(9.7, 8, '', 'LRTB', 'C', 0, 0);			
			
			$pdfBig->SetXY(12, 85.2);	
			$pdfBig->MultiCell(92.8, 0, 'I-I SEMESTER', 0, 'C', 0, 0);
			$pdfBig->SetXY(12, 85.2);	
			$pdfBig->MultiCell(185.6, 0, '', 0, 'C', 0, 0);
			...
			for ($lx = 1; $lx <= 4; $lx++) {
				...
				$pdfBig->MultiCell(4, 35, '', 'LRTB', 'C', 0, 0);
				$pdfBig->MultiCell(62, 35, '', 'LRTB', 'L', 0, 0);
				$pdfBig->MultiCell(9, 35, '', 'LRTB', 'C', 0, 0);
				$pdfBig->MultiCell(8, 35, '', 'LRTB', 'C', 0, 0);
				$pdfBig->MultiCell(9.7, 35, '', 'LRTB', 'C', 0, 0); 
				$startY+=38.5;	
            }			
			//2nd Sem            
            $startSY=88.8;
			...
			$Class_Awarded = trim($studentData[10]);			
			$Serial_No = trim($studentData[11]);
			$CGPA = trim($studentData[12]);
			$Credits_Registered = trim($studentData[13]);
			$Credits_Secured = trim($studentData[14]);
			$Place = trim($studentData[15]);
			$Date = trim($studentData[16]);
			//$photo = $studentData[17];
			...
			foreach ($array_sem1 as $semData){
                ...
                if(strlen($SubjectTitle)>55){
	                ...
	                $pdf->MultiCell(62, 11, $SubjectTitle,0, 'L', 0, 0);
	                $pdf->SetFont($arialNarrow, '', $subj_font_size, '', false);
                }else{
	                $pdf->SetXY(17, $sem1_y);
	                $pdf->MultiCell(62, 11, $SubjectTitle,0, 'L', 0, 0);
                ...
                $pdf->SetXY(78, $sem1_y);
                $pdf->MultiCell(9, 11, $GradePoints, 0, 'C', 0, 0);     
                ...
                if(strlen($SubjectTitle)>55){
	                ...
	                $pdfBig->MultiCell(62, 11, $SubjectTitle,0, 'L', 0, 0);
	                $pdfBig->SetFont($arialNarrow, '', $subj_font_size, '', false);
                }else{
	                $pdfBig->SetXY(17, $sem1_y);
	                $pdfBig->MultiCell(62, 11, $SubjectTitle,0, 'L', 0, 0);
                }
                $pdfBig->SetXY(78, $sem1_y);
                $pdfBig->MultiCell(9, 11, $GradePoints, 0, 'C', 0, 0);     
			...
			$pdf->MultiCell(29, 11, $CGPA.'&nbsp;&nbsp;', 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetXY(10, 251.5);
            $pdfBig->Cell(189.5, 0, $arrayEnrollment1, 0, false, 'L');
