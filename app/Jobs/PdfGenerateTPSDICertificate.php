...
class PdfGenerateTPSDICertificate
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       if(!empty($loader_data) && isset($loader_data['generatedCertificates'])){
        ...
        if($generated_documents == 0){
            ...
            $pdfBig = new TCPDF('L', 'mm', array('210', '297'), true, 'UTF-8', false);
        ...
        $PlayfairDisplayRegular = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\PlayfairDisplay-Regular.TTF', 'TrueTypeUnicode', '', 96);
        $PlayfairDisplayBold = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\PlayfairDisplay-Bold.TTF', 'TrueTypeUnicode', '', 96);
        $PlayfairDisplayItalic = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\PlayfairDisplay-Italic.TTF', 'TrueTypeUnicode', '', 96);
        $PlayfairDisplayBoldItalic = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\PlayfairDisplay-BoldItalic.TTF', 'TrueTypeUnicode', '', 96);
        $edLavoniaRegular = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\ed-lavonia-regular.TTF', 'TrueTypeUnicode', '', 96);
        ...
        $card_serial_no = 0;
        $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\Tata_Power_Certificate_new.jpg'; 
        $style1D = array(
            'position' => '',
            'align' => 'C',
            'stretch' => false,
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) 
            {
                ...
                $program = 'Discom Officials';
                $course_name = trim($studentData[2]);
                
                $scheme = 'PM Surya Ghar Muft Bijli Yojana';
                $conduct_by = 'The Tata Power Co.Ltd';
                
                $location = trim($studentData[3]);
                ...
                $pdfBig->SetFont($PlayfairDisplayRegular, '', 16, '', false); 
                $pdfBig->SetXY(0, 79);
                $pdfBig->MultiCell(297, 10, 'This Certificate is awarded to', 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->ln();
                $pdfBig->SetFont($edLavoniaRegular, '', 40, '', false); 
                $pdfBig->SetXY(0, $pdfBig->getY());
                $pdfBig->MultiCell(297, 10,$name, 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->ln();
                $pdfBig->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdfBig->SetXY(0, $pdfBig->getY());
                $pdfBig->MultiCell(297, 10, 'on successfully completing a training program for <span style="font-family:'.$PlayfairDisplayBoldItalic.';">'.$program.'</span> on', 0, "C", 0, 0, '', '', true, 0, true);
                
                $pdfBig->ln();
                $pdfBig->SetFont($PlayfairDisplayBoldItalic, '', 17, '', false); 
                $pdfBig->SetXY(0, $pdfBig->getY());
                $pdfBig->MultiCell(297, 10, $course_name, 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->ln();
                $pdfBig->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdfBig->SetXY(0, $pdfBig->getY());
                $pdfBig->MultiCell(297, 10,'Under <span style="font-family:'.$PlayfairDisplayBoldItalic.';">'.$scheme.'</span> Conducted by', 0, "C", 0, 0, '', '', true, 0, true);


                $pdfBig->ln();
                $pdfBig->SetFont($PlayfairDisplayBoldItalic, '', 17, '', false); 
                $pdfBig->SetXY(0, $pdfBig->getY());
                $pdfBig->MultiCell(297, 12, $conduct_by, 0, "C", 0, 0, '', '', true, 0, true);


                $pdfBig->ln();
                $pdfBig->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdfBig->SetXY(35, $pdfBig->getY());
                $pdfBig->MultiCell(10, 0, 'at', 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(92, 0, $location, 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(15, 0, 'from', 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(45, 0, $from_date, 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(10, 0, 'to', 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(45, 0, $to_date, 0, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($PlayfairDisplayRegular, '', 16, '', false); 
                $pdf->SetXY(0, 79);
                $pdf->MultiCell(297, 10, 'This Certificate is awarded to', 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->ln();
                $pdf->SetFont($edLavoniaRegular, '', 40, '', false); 
                $pdf->SetXY(0, $pdf->getY());
                $pdf->MultiCell(297, 10,$name, 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->ln();
                $pdf->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdf->SetXY(0, $pdf->getY());
                $pdf->MultiCell(297, 10, 'on successfully completing a training program for <span style="font-family:'.$PlayfairDisplayBoldItalic.';">'.$program.'</span> on', 0, "C", 0, 0, '', '', true, 0, true);
                
                $pdf->ln();
                $pdf->SetFont($PlayfairDisplayBoldItalic, '', 17, '', false); 
                $pdf->SetXY(0, $pdf->getY());
                $pdf->MultiCell(297, 10, $course_name, 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->ln();
                $pdf->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdf->SetXY(0, $pdf->getY());
                $pdf->MultiCell(297, 10,'Under <span style="font-family:'.$PlayfairDisplayBoldItalic.';">'.$scheme.'</span> Conducted by', 0, "C", 0, 0, '', '', true, 0, true);


                $pdf->ln();
                $pdf->SetFont($PlayfairDisplayBoldItalic, '', 17, '', false); 
                $pdf->SetXY(0, $pdf->getY());
                $pdf->MultiCell(297, 12, $conduct_by, 0, "C", 0, 0, '', '', true, 0, true);


                $pdf->ln();
                $pdf->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdf->SetXY(35, $pdf->getY());
                $pdf->MultiCell(10, 0, 'at', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(92, 0, $location, 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(15, 0, 'from', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(45, 0, $from_date, 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($PlayfairDisplayItalic, '', 17, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(10, 0, 'to', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($PlayfairDisplayBold, '', 17, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(45, 0, $to_date, 0, "C", 0, 0, '', '', true, 0, true);
                ...
                // Head Sign
                $sign1 = public_path().'\\'.$subdomain[0].'\backend\canvas\images\head_signature.png';
                $sign1_x = 80;
                $sign1_y = 163;
                $sign1_Width = 25;
                $sign1_Height = 18;
                ...
                // head_assessment signature
                $sign2 = public_path().'\\'.$subdomain[0].'\backend\canvas\images\head_assessment.png';
                $sign2_x = 195;
                $sign2_y = 169;
                $sign2_Width = 25;
                $sign2_Height = 10;
                ...
                if($previewPdf!=1){
                    ...
                    $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'TPSDI_CERTIFICATE',$admin_id,$card_serial_no);
        ...
        //}
        $msg = '';
        $file_name =  str_replace("/", "_",'TPSDI_CERTIFICATE'.date("Ymdhms")).'.pdf';
        ...
        if($previewPdf!=1){
            ...
            $template_name="TPSDI_CERTIFICATE";
