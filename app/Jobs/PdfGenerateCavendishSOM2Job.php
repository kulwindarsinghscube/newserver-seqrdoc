...
class PdfGenerateCavendishSOM2Job
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $log_serial_no = 1;
        //$cardDetails=$this->getNextCardNo('CAVENDISH-SOM-2');
        ...
        $excelFolderPath = public_path().'\\'.$subdomain[0].'\backend\upload_excel\som_2_year\excel';
        ...
        foreach ($files as $file) 
        {
            ...
            $records = $this->getArrayFromSOM2Excel($sheet1);
            ...
            $imagePath = $subdomain[0].'\backend\upload_excel\som_2_year\photo/'.$fileDetail['filename'].'.PNG';
            $pdfBig->SetXY(177, 12);
            ...
            // $pdfBig->SetFont($Arial, '', 9, '', false);
            $pdfBig->SetXY(41, 42);
            $pdfBig->Cell(67, 0, $certificateName, 0, false, 'L');

            // $pdf->SetFont($Arial, '', 9, '', false);
            $pdf->SetXY(41, 42);
            $pdf->Cell(67, 0, $certificateName, 0, false, 'L');
            ...
            $pdfBig->SetXY(132, 49);
            $pdfBig->Cell(67, 0, $certificateSchool, 0, false, 'L');


            $pdf->SetXY(132, 49);
            $pdf->Cell(67, 0, $certificateSchool, 0, false, 'L');
            ...
            // end SEMESTER ONE table

            // start SEMESTER TWO table
            $tableX = 21;
            $tableY = $foundationY;
            $pdfBig->SetFont($Arial, 'B', 7, '', false);
            $pdfBig->SetXY($tableX, $tableY);
            ...
            $pdfBig->MultiCell($pageWidth, 4, 'SEMESTER TWO', '', 'C', 1, 0);

            $pdf->SetFont($Arial, 'B', 7, '', false);
            $pdf->SetXY($tableX, $tableY);
            $pdf->SetTextColor(255,255,255);
            $pdf->SetFillColor(19,57,106);
            $pdf->MultiCell($pageWidth, 4, 'SEMESTER TWO', '', 'C', 1, 0);
            ...
            // end SEMESTER TWO table

            
            $bottomSecY = $year1Y-0.5;
            // echo $year3Y+3.6;
            $pdfBig->setCellPaddings( $left = '0', $top = '0.5', $right = '', $bottom = '');
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'CAVENDISH-SOM-2',$admin_id,$card_serial_no);
            ...
            // \File::move($file, public_path().'\\'.$subdomain[0].'\\backend\\upload_excel\\som_2_year\\process_excel\\'.$fileDetail['basename']);

        }

        //if($previewPdf!=1){
            //$this->updateCardNo('CAVENDISH-SOM-2',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        //}
        if($previewPdf!=1){
            $this->updateCardNo('CAVENDISH-SOM-2',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        $file_name =  str_replace("/", "_",'CAVENDISH-SOM-2'.date("Ymdhms")).'.pdf';
    ...
    public function getArrayFromSOM2Excel($sheet1){
