...
class pdfGenerateJntuJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('JNTUGradeCard');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->MultiCell(65, $main_info_height, '<span style="font-family:'.$times.';">'.$branch.'</span>', 0 ,"L", 0, 0, '', '', true, 0, true);
			...
			$pdfBig->SetFont($times, '', $content_font_size, '', false);
			$pdfBig->Cell(2, 0, ':', 0, false, 'R');
			$pdfBig->MultiCell(65,  $main_info_height, '<span style="font-family:'.$times.';">'.$candidate_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
            
            
			$pdfBig->SetFont($timesb, '', $content_font_size, '', false);
            $pdfBig->SetXY($x_axis_main_info_second, $pdfBig->getY());
            $pdfBig->MultiCell(32,  $main_info_height, "HALL TICKET NO", 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->MultiCell(80,  $main_info_height, '<span style="font-family:'.$times.';">: '.$hall_ticket.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			...
			$pdf->MultiCell(65, $main_info_height, '<span style="font-family:'.$times.';">'.$branch.'</span>', 0 ,"L", 0, 0, '', '', true, 0, true);
			...
			$pdf->SetFont($times, '', $content_font_size, '', false);
			$pdf->Cell(2, 0, ':', 0, false, 'R');
			$pdf->MultiCell(65,  $main_info_height, '<span style="font-family:'.$times.';">'.$candidate_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
   ...
   //          \File::copy($upload_sign1_org,$sign1); 
       ...
       if($previewPdf!=1){
        $this->updateCardNo('JNTUGradeCard',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'JNTUGradeCard'.date("Ymdhms")).'.pdf';
        // $file_name = 'test.pdf';
