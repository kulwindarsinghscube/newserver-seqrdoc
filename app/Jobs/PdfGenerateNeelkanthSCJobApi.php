        ...
        foreach ($studentDataOrg as $studentData) {
			...
			if($previewPdf==1){
				...
				if($previewWithoutBg!=1){
					...
					$pdfBig->Image($template_img_generate, 0, 0, '210', '187', "JPG", '', 'R', true);
				}
			...
			$pdf = new TCPDF('L', 'mm', array('210', '187'), true, 'UTF-8', false);
			...
			if($previewPdf!=1){
			     $pdf->Image($template_img_generate, 0, 0, '210', '187', "JPG", '', 'R', true);
			}
			//$pdf->setPageMark();
			$pdf->setPageMark();
			//$pdfBig->setPageMark();
