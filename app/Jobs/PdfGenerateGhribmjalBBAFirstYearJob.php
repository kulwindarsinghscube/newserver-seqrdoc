...
use Helper;
class PdfGenerateGhribmjalBBAFirstYearJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		$preview_serial_no=1;
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('BBA');
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $programme = $studentData[2];
            $monthYearExamination = $studentData[3];
            $sem = $studentData[4];
            $prn = $studentData[5];
            $seatNo = $studentData[6];
            $examRegistrasionCredits = $studentData[47];
            $examCredits = $studentData[48];
            $earnGradePoint1 = $studentData[49];
            $sgpa = $studentData[50];
            $cumulativeCreditsEarned = $studentData[51];
            $earnGradePoint2 = $studentData[52];
            $cgpa = $studentData[53];
            $place = $studentData[54];
            $date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($studentData[55])->format('d-m-Y');
            $photo = $studentData[56];
            ...
            // $signWidth = 50;
            // $signHeight = 50;
            // $pdf->image($sign_image_org,$signx,$signy,$signWidth,$signHeight,"",'','C',true,3600);
            ...
            $pdfBig->setXY(18, 40);
            $pdfBig->MultiCell(149, 20, "", "LRBT", 'L', 0, 1);
            ...
            $pdfBig->setXY(44, 40);
            $pdfBig->MultiCell(123, 0, $studentName, '', 'L', 0, 1);

            $pdfBig->SetTextColor(2555,2555,0);
            $pdfBig->setXY(19, 36);
            $pdfBig->MultiCell(149, 0, $studentName, '', 'L', 0, 1);
            ...
            $pdfBig->setXY(115, 55);
            $pdfBig->MultiCell(18, 0, "Seat No.: ", '', 'L', 0, 1);
            $pdfBig->setXY(130, 55);
            $pdfBig->MultiCell(123, 0, $seatNo, '', 'L', 0, 1);
            ...
            $pdfBig->setXY(18, 65);
            $pdfBig->MultiCell(174, 140, "", 'LRBT', 'L', 0, 1);
            ...
            $pdfBig->setCellPaddings( $left = '', $top = 3, $right = '', $bottom = '');
            $pdfBig->MultiCell(35, 12, "Exam Registration Credits", 'RB', 'C', 0, 1);
            $pdfBig->setXY(53, 217.5);
            $pdfBig->MultiCell(18, 12, "Earn Credits", 'RB', 'C', 0, 1);
            $pdfBig->setXY(71, 217.5);
            $pdfBig->MultiCell(25, 12, "Earn Grade Points", 'RB', 'C', 0, 1);
            $pdfBig->setXY(96, 217.5);
            $pdfBig->setCellPaddings( $left = '', $top = 4, $right = '', $bottom = '');
            $pdfBig->MultiCell(16, 12, "SGPA", 'RB', 'C', 0, 1);
            $pdfBig->setXY(112, 217.5);
            $pdfBig->setCellPaddings( $left = '', $top = 3, $right = '', $bottom = '');
            $pdfBig->MultiCell(39, 12, "Cumulative Credits Earned", 'RB', 'C', 0, 1);
            ...
            $pdfBig->setXY(18, 228);
            $pdfBig->MultiCell(35, 12, $examRegistrasionCredits, 'RB', 'C', 0, 1);
            $pdfBig->setXY(53, 228);
            $pdfBig->MultiCell(18, 12, $examCredits, 'RB', 'C', 0, 1);
            $pdfBig->setXY(71, 228);
            $pdfBig->MultiCell(25, 12, $earnGradePoint1, 'RB', 'C', 0, 1);
            $pdfBig->setXY(96, 228);
            $pdfBig->MultiCell(16, 12, $sgpa, 'RB', 'C', 0, 1);
            $pdfBig->setXY(112, 228);
            $pdfBig->MultiCell(39, 12, $cumulativeCreditsEarned, 'RB', 'C', 0, 1);
            ...
            $pdf->setXY(18, 40);
            $pdf->MultiCell(149, 20, "", "LRBT", 'L', 0, 1);
            ...
            $pdf->setXY(115, 55);
            $pdf->MultiCell(18, 0, "Seat No.: ", '', 'L', 0, 1);
            $pdf->setXY(130, 55);
            $pdf->MultiCell(123, 0, $seatNo, '', 'L', 0, 1);
            ...
            $pdf->setXY(18, 65);
            $pdf->MultiCell(174, 140, "", 'LRBT', 'L', 0, 1);
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'BBA',$admin_id,$card_serial_no);
       ...
       if($previewPdf!=1){
        $this->updateCardNo('BBA',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'BBA'.date("Ymdhms")).'.pdf';
