...
class PdfGenerateBestiuJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('BestiuC');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="BestInnovationUniversityDegreeCertificate.jpg"; // bestiu_bg, bestiu_bg_data
                $low_res_bg="BestInnovationUniversityDegreeCertificate.jpg";
                $pdfBig->AddPage();
                ...
                $ghostImgArr = array();
                $pdf = new TCPDF('L', 'mm', array('420', '297'), true, 'UTF-8', false); // 420.116 296.926
                ...
                $completion_date = trim($studentData[5]);
                $average = trim($studentData[6]);
                $date = trim($studentData[7]);
                $place = trim($studentData[8]);
                $PHOTO = trim($studentData[9]);
                $AadharNumber = trim($studentData[10]);
                ...
                //Start pdfBig
                $pdfBig->SetFont($oef, '', 18, '', false);
                $pdfBig->SetTextColor(183, 23, 43);
                //$pdfBig->SetXY(170, 60);
                //$pdfBig->Cell(135, 0, 'Degree Certificate', 0, false, 'C');
                $pdfBig->SetXY(12, 61);
                $pdfBig->Cell(395, 0, 'Degree Certificate', 0, false, 'C');

                $pdfBig->SetFont($arial, '', 15, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);    
                //$pdfBig->SetXY(17, 17);
                //$pdfBig->Cell(106, 0, 'Serial No.: '.$serial_no, 0, false, 'L');	
                $pdfBig->SetXY(297, 17);
                $pdfBig->Cell(107, 0, 'ID No.: '.$unique_id, 0, false, 'R');


                if($AadharNumber) {
                    $pdfBig->SetFont($arial, '', 15, '', false); 
                    $pdfBig->SetTextColor(0, 0, 0);    
                    $pdfBig->SetXY(297, 60.5);
                    $pdfBig->Cell(107, 0, 'Aadhar No.: '.$AadharNumber, 0, false, 'R');
                }


                $pdfBig->SetFont($oef, '', 21, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, 72);
                $pdfBig->Cell(395, 0, 'On the recommendation of the Academic Council and the', 0, false, 'C');
                $pdfBig->SetXY(12, 84);
                $pdfBig->Cell(395, 0, 'Board of Studies Bharatiya Engineering, Science and Technology', 0, false, 'C');
                $pdfBig->SetXY(12, 97);
                $pdfBig->Cell(395, 0, 'Innovation University hereby confers upon', 0, false, 'C');
                $pdfBig->SetFont($arialMTblack, '', 32, '', false);
                $pdfBig->SetTextColor(61, 61, 157);        
                $pdfBig->SetXY(12, 114);
                $pdfBig->Cell(395, 0, $candidate_name, 0, false, 'C');
                $pdfBig->SetFont($oef, '', 21, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, 135);
                //$pdfBig->Cell(395, 0, 'Son/Daughter of Shri./Smt. '.$father_name, 0, false, 'C'); 
                $pdfBig->MultiCell(395, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->SetXY(12, 152.5);
                $pdfBig->Cell(395, 0, 'the degree of', 0, false, 'C');
                $pdfBig->SetFont($oef, '', 32, '', false); 
                $pdfBig->SetTextColor(61, 61, 157);        
                $pdfBig->SetXY(12, 165);
                // $pdfBig->Cell(395, 0, $degree, 0, false, 'C');
                $pdfBig->MultiCell(395,  0,$degree, 0, "C", 0, 0, '', '', true, 0, true);

                $pdfBig->ln();
                
                $pdfBig->SetFont($oef, '', 21, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, $pdfBig->getY()+2.5); //199.5
                $pdfBig->Cell(395, 12, 'having successfully completed on '.$completion_date.',', 0, false, 'C');
                
                $pdfBig->ln();
                /*$pdfBig->Cell(237, 0, 'having successfully completed on '.$completion_date, 0, false, 'R');
                $pdfBig->SetXY(249, 199.5);
                $pdfBig->Cell(53, 0, $completion_date, 'B', false, 'C');*/          
                $pdfBig->SetXY(12, $pdfBig->getY()); //212
                $pdfBig->Cell(395, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average.'.', 0, false, 'C');
                /*$pdfBig->Cell(326, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average, 0, false, 'R');
                $pdfBig->SetXY(338, 212);
                $pdfBig->Cell(47, 0, $average, 'B', false, 'C');*/  
                $pdfBig->SetFont($arial, '', 14, '', false); 
                $pdfBig->SetXY(17, 272.5);
                $pdfBig->Cell(0, 0, 'Place: '.$place, 0, false, 'L');
                $pdfBig->SetXY(17, 278);
                $pdfBig->Cell(0, 0, 'Date:', 0, false, 'L');
                $pdfBig->SetXY(32, 278);
                $pdfBig->Cell(0, 0, $date, 0, false, 'L');

                 $pdfBig->SetFont($arialb, 'B', 21, '', false);
                $pdfBig->SetXY(50, 250);
                $pdfBig->Cell(0, 0, 'Chancellor', 0, false, 'L');

                $pdfBig->SetXY(150, 250);
                $pdfBig->Cell(0, 0, 'Vice Chancellor', 0, false, 'L');

                  $pdfBig->SetXY(265, 250);
                $pdfBig->Cell(0, 0, 'Registrar', 0, false, 'L');
                
                //End pdfBig 
                
                //start pdf
                $pdf->SetFont($oef, '', 18, '', false);
                $pdf->SetTextColor(183, 23, 43);
                //$pdf->SetXY(170, 60);
                //$pdf->Cell(135, 0, 'Degree Certificate', 0, false, 'C');
                $pdf->SetXY(12, 61);
                $pdf->Cell(395, 0, 'Degree Certificate', 0, false, 'C');

                $pdf->SetTextColor(0, 0, 0);    
                //$pdf->SetXY(17, 17);
                //$pdf->Cell(106, 0, 'Serial No.: '.$serial_no, 0, false, 'L');	
                $pdf->SetXY(297, 17);
                $pdf->Cell(107, 0, 'ID No.: '.$unique_id, 0, false, 'R');
                $pdf->SetFont($oef, '', 21, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, 72);
                $pdf->Cell(395, 0, 'On the recommendation of the Academic Council and the', 0, false, 'C');
                $pdf->SetXY(12, 84);
                $pdf->Cell(395, 0, 'Board of Studies Bharatiya Engineering, Science and Technology', 0, false, 'C');
                $pdf->SetXY(12, 97);
                $pdf->Cell(395, 0, 'Innovation University hereby confers upon', 0, false, 'C');
                $pdf->SetFont($arialMTblack, '', 32, '', false); 
                $pdf->SetTextColor(61, 61, 157);        
                $pdf->SetXY(12, 114);
                $pdf->Cell(395, 0, $candidate_name, 0, false, 'C');
                $pdf->SetFont($oef, '', 21, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, 135);
                //$pdf->Cell(395, 0, 'Son/Daughter of Shri./Smt. '.$father_name, 0, false, 'C');
                $pdf->MultiCell(395, 0, 'Son/Daughter of Shri./Smt. <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->SetXY(12, 152.5);
                $pdf->Cell(395, 0, 'the degree of', 0, false, 'C');
                
                $pdf->SetFont($oef, '', 32, '', false); 
                $pdf->SetTextColor(61, 61, 157);        
                $pdf->SetXY(12, 165);
                // $pdf->Cell(395, 0, $degree, 0, false, 'C');
                $pdf->MultiCell(395,  0,$degree, 0, "C", 0, 0, '', '', true, 0, true);

                $pdf->ln();
                
                $pdf->SetFont($oef, '', 21, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, $pdf->getY()+2.5); //199.5
                $pdf->Cell(395, 12, 'having successfully completed on '.$completion_date.',', 0, false, 'C');
                
                $pdf->ln();
                /*$pdf->Cell(237, 0, 'having successfully completed on '.$completion_date, 0, false, 'R');
                $pdf->SetXY(249, 199.5);
                $pdf->Cell(53, 0, $completion_date, 'B', false, 'C');*/          
                $pdf->SetXY(12, $pdf->getY()); //212
                $pdf->Cell(395, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average.'.', 0, false, 'C');
                /*$pdf->Cell(326, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average, 0, false, 'R');
                $pdf->SetXY(338, 212);
                $pdf->Cell(47, 0, $average, 'B', false, 'C');*/  
                $pdf->SetFont($arial, '', 14, '', false); 
                $pdf->SetXY(17, 272.5);
                $pdf->Cell(0, 0, 'Place: '.$place, 0, false, 'L');
                $pdf->SetXY(17, 278);
                ...
                $pdf->Cell(0, 0, $date, 0, false, 'L');
                  ...
                  $pdf->SetFont($arialb, 'B', 21, '', false);
                $pdf->SetXY(50, 250);
                $pdf->Cell(0, 0, 'Chancellor', 0, false, 'L');
                ...
                // $pdf->SetFont($oef, '', 21, '', false); 
                // $pdf->SetTextColor(0, 0, 0);        
                // $pdf->SetXY(12, 185); //199.5
                // $pdf->Cell(395, 0, 'having successfully completed on '.$completion_date.',', 0, false, 'C');
                // /*$pdf->Cell(237, 0, 'having successfully completed on '.$completion_date, 0, false, 'R');
                // $pdf->SetXY(249, 199.5);
                // $pdf->Cell(53, 0, $completion_date, 'B', false, 'C');*/			
                // $pdf->SetXY(12, 197); //212
                // $pdf->Cell(395, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average.'.', 0, false, 'C');
                // /*$pdf->Cell(326, 0, 'all the requirements for the same with an Overall Grade Point Average of '.$average, 0, false, 'R');
                // $pdf->SetXY(338, 212);
                // $pdf->Cell(47, 0, $average, 'B', false, 'C');*/	
                ...
                //$pdf->StartTransform();
                $pdf->SetXY(369, 95);        
                $pdf->Cell(26, 0, $microlinestr, 0, false, 'C');     
                
                $pdfBig->SetFont($arialb, '', 2, '', false);
                $pdfBig->SetTextColor(0, 0, 0);
                //$pdfBig->StartTransform();
                $pdfBig->SetXY(369, 95);        
                $pdfBig->Cell(26, 0, $microlinestr, 0, false, 'C'); 
       ...
       if($previewPdf!=1){
        $this->updateCardNo('BestiuC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'BestiuC'.date("Ymdhms")).'.pdf';
