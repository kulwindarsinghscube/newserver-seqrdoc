         ...
         /* print_r($mintData);
           ...
           // exit;
            if($response['status']==200){
                $resultu = StudentTable::where('serial_no',''.$serial_no)->where('key',''.$key)->where('status',1)->update(['bc_txn_hash'=>$response['txnHash'],'bc_ipfs_hash'=>$response['ipfsHash'], 'pinata_ipfs_hash'=>$response['pinataIpfsHash'] ]);
            }
