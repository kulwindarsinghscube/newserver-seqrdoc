...
class PreviewPDFGenerateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        //$pdfBig->SetProtection(array('modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble'), null, 'mypwd', 1);
               ...
               /* if($subdomain[0]=='raisoni'){
                ...
                switch  ($security_type) {
                    ...
                    case 'Micro line':
                            ...
                            //print_r($FID['font_size'][$extra_fields]);
                           ...
                           // exit;
                            ...
                            $pdfBig->SetFont($fonts_array[$extra_fields], $bold,$FID['font_size'][$extra_fields], '', false);//1.2
                    ...
                    case 'Micro Text Border':
                        ...
                        */
                        if($FID['field_extra_visible'][$extra_fields - 2] == 0){
                           ...
                           // $horizontal_length=95.51;
                        /* print_r($explode_horizontal);
                          print_r($explode_vertical);
                          print_r($explode_default);
                         */
                         /* 
                        ...
                        if($FID['field_extra_visible'][$extra_fields - 2] == 0){
                            ...
                            if ($print_color == 'CMYK') {
                               ...
                               // $imageSource = public_path().'/backend/canvas/ghost_images/VOID FINAL FOR BROTHER.SVG';
                                 $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_1.svg';
                               // $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_3.png';
                                $pdfBig->ImageSVG($file=$imageSource, $x, $y, 50, 10, $link='', $align='', $palign='', $border=0, $fitonpage=false);
                               // $pdfBig->Image($imageSource, $x, $y, 50, 10, "PNG", '', 'L', true, 3600);

                                 $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_2.svg';
                               // $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_3.png';
                                $pdfBig->ImageSVG($file=$imageSource, $x, $y+30, 50, 10, $link='', $align='', $palign='', $border=0, $fitonpage=false);

                                 $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_3.svg';
                               // $imageSource = public_path().'/backend/canvas/ghost_images/Copy R & D_3.png';
                                $pdfBig->ImageSVG($file=$imageSource, $x, $y+60, 50, 10, $link='', $align='', $palign='', $border=0, $fitonpage=false);
                            }
                        }
                    break;
    ...
    function microLine($text, $w, $fontSize)
{
    $microlineEnrollment = strtoupper($text);
    $microlineEnrollment = preg_replace('/\s+/', '', $microlineEnrollment);
    $textArrayEnrollment = imagettfbbox($fontSize, 0, public_path().'/backend/canvas/fonts/Arial.TTF', $microlineEnrollment);
