...
class PdfGenerateSaiuGrade3Job
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('LawAndRegGradeCard');
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $TGPA1 = sprintf('%0.2f', trim($studentData[62]));
            $CreditsEarned1 =trim($studentData[63]);
            $TGPA2 = sprintf('%0.2f', trim($studentData[64]));
            $CreditsEarned2 = trim($studentData[65]);
            $TGPA3 = sprintf('%0.2f', trim($studentData[66]));
            $CreditsEarned3 = trim($studentData[67]);
            $TotalCreditsEarned = trim($studentData[68]);
            $CGPA = sprintf('%0.2f', trim($studentData[69]))."/10";
            //$Result = trim($studentData[70]);
            $DeanSignature = trim($studentData[70]);
            $COESignature = trim($studentData[71]);
            ...
            $pdfBig->SetXY(15, 59);
            $pdfBig->MultiCell(29, 6, 'Name', 0, "L", 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetFont($arial, '',10, '', false);
            $pdfBig->SetXY(140, 67);
            $pdfBig->MultiCell(56, 8, $doi, 0, "L", 0, 0, '', '', true, 0, true);
           ...
           $pdfBig->setCellPaddings( $left = '1', $top = '1.5', $right = '', $bottom = '');
            ...
            $pdf->SetXY(15, 59);
            $pdf->MultiCell(29, 6, 'Name', 0, "L", 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($arial, '',10, '', false);
            $pdf->SetXY(140, 67);
            $pdf->MultiCell(56, 8, $doi, 0, "L", 0, 0, '', '', true, 0, true);
   ...
   //              ->generate($codeContents, $qr_code_path);
        ...
        if($previewPdf!=1){
            $this->updateCardNo('LawAndRegGradeCard',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'SaiuLawRegReport'.date("Ymdhms")).'.pdf';
        ...
        if($previewPdf!=1){
            ...
            $template_name="SaiuLawRegReport";
