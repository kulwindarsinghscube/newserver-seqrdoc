...
class PdfGenerateMachakosApiJob 
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        // Log::info('PDF Data: ', $pdf_data);

        // Extract other fields from JSON data
        $businessRegistrationNumber = $pdf_data['Business Registration Number'] ?? null;
        $businessName               = $pdf_data['Business Name'] ?? null;
        $businessAddress            = $pdf_data['Business Address'] ?? null;
        $permitNumber               = $pdf_data['Permit Number']?? null;
        $year                       = $pdf_data['Year']?? null;
        $permitType                 = $pdf_data['Permit Type']?? null;
        $businessActivity           = $pdf_data['Business Activity']?? null;
        $AmountInFigures            = $pdf_data['Amount in Figures']?? null;
        $amountInWords              = $pdf_data['Amount in Words']?? null;
        $validityPeriod             = $pdf_data['Validity Period']?? null;
        $dateIssued                 = $pdf_data['Date Issued']?? null;
        $expiryDate                 = $pdf_data['Expiry Date']?? null;
        $vat                        = $pdf_data['VAT']?? null;
        $pin                        = $pdf_data['PIN']?? null;
    ...
    // $admin_id = $admin->toArray();
        
    // $user = Admin::find(1); // Find the user you want to issue a token to
    ...
    // $token = $user->createToken('Personal Access Token')->plainTextToken;
        ...
        //$pdf->SetTitle('Secure License PDF');
        $pdfBig->SetSubject('');
        ...
        $pdfBig->setPageMark();
        $pdfBig->SetFont('helvetica', '', 12);

        // Add content to the PDF
        $html = '<h1 style="text-align: center;">COUNTY GOVERNMENT OF MACHAKOS</h1>';
        if ($businessName) {
            $html .= '<p><strong>Business Name:</strong> ' . htmlspecialchars($businessName) . '</p>';
        } else {
            $html .= '<p><strong>Business Name:</strong> ' . htmlspecialchars($businessName) . '</p>';
        }
        if ($businessAddress) {
            $html .= '<p><strong>Business Address:</strong> ' . htmlspecialchars($businessAddress) . '</p>';
        } else {
            $html .= '<p><strong>Business Address:</strong> ' . htmlspecialchars($businessAddress) . '</p>';
        }
        if ($businessRegistrationNumber) {
            $html .= '<p><strong>Business Registration Number:</strong> ' . htmlspecialchars($businessRegistrationNumber) . '</p>';
        } else {
            $html .= '<p><strong>Business Registration Number:</strong> ' . htmlspecialchars($businessRegistrationNumber) . '</p>';
        }
        if ($permitNumber) {
            $html .= '<p><strong>Permit Number:</strong> ' . htmlspecialchars($permitNumber) . '</p>';
        } else {
            $html .= '<p><strong>Permit Number:</strong> ' . htmlspecialchars($permitNumber) . '</p>';
        }
        if ($year) {
        ...
        if ($permitType) {
            $html .= '<p><strong>Permit Type:</strong> ' . htmlspecialchars($permitType) . '</p>';
        } else {
            $html .= '<p><strong>Permit Type:</strong> ' . htmlspecialchars($permitType) . '</p>';
        }
        if ($businessActivity) {
            $html .= '<p><strong>Business Activity:</strong> ' . htmlspecialchars($businessActivity) . '</p>';
        } else {
            $html .= '<p><strong>Business Activity:</strong> ' . htmlspecialchars($businessActivity) . '</p>';
        }
        if ($AmountInFigures) {
            $html .= '<p><strong>Amount in Figures:</strong> ' . htmlspecialchars($AmountInFigures) . '</p>';
        } else {
            $html .= '<p><strong>Amount in Figures:</strong> ' . htmlspecialchars($AmountInFigures) . '</p>';
        }
        if ($amountInWords) {
            $html .= '<p><strong>Amount in Words:</strong> ' . htmlspecialchars($amountInWords) . '</p>';
        } else {
            $html .= '<p><strong>Amount in Words:</strong> ' . htmlspecialchars($amountInWords) . '</p>';
        }
        if ($validityPeriod) {
            $html .= '<p><strong>Validity Period:</strong> ' . htmlspecialchars($validityPeriod) . '</p>';
        } else {
            $html .= '<p><strong>Validity Period:</strong> ' . htmlspecialchars($validityPeriod) . '</p>';
        }
        if ($dateIssued) {
            $html .= '<p><strong>Date Issued:</strong> ' . htmlspecialchars($dateIssued) . '</p>';
        } else {
            $html .= '<p><strong>Date Issued:</strong> ' . htmlspecialchars($dateIssued) . '</p>';
        }
        if ($expiryDate) {
            $html .= '<p><strong>Expiry Date:</strong> ' . htmlspecialchars($expiryDate) . '</p>';
        } else {
            $html .= '<p><strong>Expiry Date:</strong> ' . htmlspecialchars($expiryDate) . '</p>';
        }
        if ($vat) {
            $html .= '<p><strong>VAT:</strong> ' . htmlspecialchars($vat) . '</p>';
        } else {
            $html .= '<p><strong>VAT:</strong> ' . htmlspecialchars($vat) . '</p>';
        }
        if ($pin) {
            $html .= '<p><strong>PIN:</strong> ' . htmlspecialchars($pin) . '</p>';
        } else {
            $html .= '<p><strong>PIN:</strong> ' . htmlspecialchars($pin) . '</p>';
        }
        $pdfBig->writeHTML($html, true, false, true, false, '');        
        ...
        //qr code    
        $dt = date("_ymdHis");
        $str=$businessRegistrationNumber;
        ...
        $GUID=$serial_no=$businessRegistrationNumber;
        ...
        $fileVerificationPath=$myPath . DIRECTORY_SEPARATOR . $certName;
            ...
            $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'license',$admin_id);
       ...
       $auth_site_id=317;
        ...
        License::create([
            ...
            'pdf_file' => $file_name,
            'json_data' => json_encode($pdf_data),
