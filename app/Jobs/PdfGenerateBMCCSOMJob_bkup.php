...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$blob)
    {
        ...
        if($systemConfig['sandboxing'] == 1){
            // $resultu = SbStudentTable::where('serial_no','T-'.$serial_no)->update(['status'=>'0']) new record
...
public function addPrintDetails($username, $print_datetime, $printer_name, $printer_count, $print_serial_no, $sr_no,$template_name,$admin_id,$card_serial_no)
    {
        ...
        if($systemConfig['sandboxing'] == 1){
            $result = SBPrintingDetail::create(['username'=>$username,'print_datetime'=>$print_datetime,'printer_name'=>$printer_name,ial_no,'sr_no'=>'T-'.$sr_no,'template_name'=>$template_name,'card_serial_no'=>$card_serial_no,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id,'publish'=>1]);
        }else{
