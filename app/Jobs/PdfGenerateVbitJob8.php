    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // End Update code for batchwise genration
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="id_7.jpg"; // VBIT_consolidated_BG, CMM memo grade ug
			$low_res_bg="id_7.jpg"; // VBIT_consolidated_BG, CMM memo grade ug
			$pdfBig->AddPage();
			...
			$pdfBig->MultiCell(6, 5, 'Sr. No', 0, 'L', 0, 0, '', '', true, 0, true);
			...
			$pdfBig->MultiCell(9.5, 5, 'GRADE POINT', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->setCellPaddings( $left = '0.8', $top = '1.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(9.5, 5, 'GRADE', 0, 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->setCellPaddings( $left = '0', $top = '1.4', $right = '', $bottom = '');
			$pdfBig->MultiCell(10.5, 5, 'CREDITS', 0, 'C', 0, 0, '', '', true, 0, true);
			...
			$pdf->MultiCell(6, 5, 'Sr. No', 0, 'L', 0, 0, '', '', true, 0, true);
			...
			$pdf->MultiCell(9.5, 5, 'GRADE POINT', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->setCellPaddings( $left = '0.8', $top = '1.4', $right = '', $bottom = '');
			$pdf->MultiCell(9.5, 5, 'GRADE', 0, 'C', 0, 0, '', '', true, 0, true);
            $pdf->setCellPaddings( $left = '0', $top = '1.4', $right = '', $bottom = '');
			$pdf->MultiCell(10.5, 5, 'CREDITS', 0, 'C', 0, 0, '', '', true, 0, true);
            ...
            $Memo_No = trim($studentData[4]);
            $Degree = trim($studentData[5]);
            $Branch = trim($studentData[6]);
            $Gender = trim($studentData[7]);
            $hall_ticket_no = trim($studentData[8]);
            $College_Code = trim($studentData[9]);
            $Serial_No = trim($studentData[10]);
            $HonorsMinors = trim($studentData[11]);
            $Credits_Secured = trim($studentData[12]);
            $Class_Awarded =  trim($studentData[13]); //Secured_Total_Credits
            $Credits_Registered_in_Program =  trim($studentData[14]);
            $Credits_Secured_in_Program =  trim($studentData[15]);
            $Credits_Registered_in_HM =  trim($studentData[16]);
            $Credits_Secured_in_HM =  trim($studentData[17]);
            $Place = trim($studentData[18]);
			$Date = trim($studentData[19]);
            $Marksheet = trim($studentData[21]);
			...
			$degree_numlines_pdf = $pdf->getNumLines($Degree, 143);
			$degree_numlines_pdfBig = $pdfBig->getNumLines($Degree, 143);
			$photo = $studentData[20];
            

            $MarksheetStr = '';

            $studentDataExist = StudentTable::where('serial_no',$unique_id)->first();
            
            if(!$studentDataExist) {
                $MarksheetStr = '';
            }  else if( $studentDataExist &&  !empty($Marksheet) ) {
                $MarksheetStr = $Marksheet;
            } else if($studentDataExist &&  empty($Marksheet) ) {
                $MarksheetStr = 'DUPLICATE';
            } else {
                $MarksheetStr = 'DUPLICATE';
            }
			...
			$pdfBig->writeHTML('<u>'.$MarksheetStr.'</u>', true, 0, true, 0);
			...
			$pdfBig->SetXY($title1_x, $left_title4_y+0.5);
			$pdfBig->Cell(0, 0, "MEMO No. : ", 0, false, 'L');  
			$pdfBig->SetXY($title1_x, $left_title5_y+0.5);
            $pdfBig->Cell(0, 0, "DEGREE : ", 0, false, 'L'); 
			if($degree_numlines_pdfBig>1){
				$pdfBig->SetXY($title1_x, $left_title6_y+1);
			}else{
				$pdfBig->SetXY($title1_x, $left_title6_y+0.5);
			}
            ...
            $pdfBig->SetFont($arial, '', $str_font_size, '', false); 
			$pdfBig->SetXY($left_str_x+16, $left_title2_y);
            $pdfBig->Cell(75, 0, $parent_name, 0, false, 'L');
            $pdfBig->SetXY($left_str_x+37, $left_title3_y);			
            $pdfBig->MultiCell(90, 0, $Month_Year_of_Exam, 0, 'L', 0, 0);
            $pdfBig->SetXY($left_str_x+7, $left_title4_y+0.5);						
            $pdfBig->MultiCell(0, 10, $Memo_No, 0, 'L', 0, 0);
            if($degree_numlines_pdfBig>1){
				$pdfBig->SetXY($left_str_x+4.5, $left_title5_y-1.5);
			}else{
				$pdfBig->SetXY($left_str_x+4.5, $left_title5_y+0.5);
			}	
			$pdfBig->MultiCell(143, 10, $Degree, 0, 'L', 0, 0, '', '', true, 0, true);			
            if($degree_numlines_pdfBig>1){
				$pdfBig->SetXY($left_str_x+4.5, $left_title6_y+1);
			}else{
				$pdfBig->SetXY($left_str_x+4.5, $left_title6_y+0.5);
			}
			$pdfBig->MultiCell(112, 7, $Branch, 0, 'L', 0, 0);
            
            $pdfBig->SetXY(126.5, $left_title1_y);
            $pdfBig->Cell(0, 0, $Gender, 0, false, 'L');
            $pdfBig->SetXY(138, $left_title2_y);
            $pdfBig->MultiCell(0, 0, $hall_ticket_no, 0, 'L', 0, 0);			
			$pdfBig->SetXY(137, $left_title3_y);
            $pdfBig->Cell(0, 0, $College_Code, 0, false, 'L');	
			$pdfBig->SetXY(130, $left_title4_y);
            ...
            $pdf->SetXY(91, 33.5);
			$pdf->writeHTML('<u>'.$MarksheetStr.'</u>', true, 0, true, 0);
            ...
            $pdf->SetFont($arial, '', $str_font_size, '', false); 
			$pdf->SetXY($left_str_x+16, $left_title2_y);
            $pdf->Cell(75, 0, $parent_name, 0, false, 'L');
            $pdf->SetXY($left_str_x+37, $left_title3_y);			
            $pdf->MultiCell(90, 0, $Month_Year_of_Exam, 0, 'L', 0, 0);
            $pdf->SetXY($left_str_x+7, $left_title4_y+0.5);						
            $pdf->MultiCell(0, 10, $Memo_No, 0, 'L', 0, 0);
            if($degree_numlines_pdf>1){
				$pdf->SetXY($left_str_x+4.5, $left_title5_y-1.5);
			}else{
				$pdf->SetXY($left_str_x+4.5, $left_title5_y+0.5);
			}	
			$pdf->MultiCell(143, 10, $Degree, 0, 'L', 0, 0, '', '', true, 0, true);			
            if($degree_numlines_pdf>1){
				$pdf->SetXY($left_str_x+4.5, $left_title6_y+1);
			}else{
				$pdf->SetXY($left_str_x+4.5, $left_title6_y+0.5);
			}
			$pdf->MultiCell(112, 7, $Branch, 0, 'L', 0, 0);
            
            $pdf->SetXY(126.5, $left_title1_y);
            $pdf->Cell(0, 0, $Gender, 0, false, 'L');
            $pdf->SetXY(138, $left_title2_y);
            $pdf->MultiCell(0, 0, $hall_ticket_no, 0, 'L', 0, 0);			
			$pdf->SetXY(137, $left_title3_y);
            $pdf->Cell(0, 0, $College_Code, 0, false, 'L');	
			$pdf->SetXY(130, $left_title4_y);
			...
			$sem1_y=89; 
            $pdfBig->SetXY(22, 99);
            $pdfBig->SetFont($arialb, '', 18, '', false); 
            $pdfBig->MultiCell(168, 0, 'DIRECT ADMISSION INTO II-YEAR UNDER<br>LATERAL ENTRY SCHEME', 0, 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->SetFont($arialNarrow, '', $subj_font_size, '', false);
            $pdf->SetXY(22, 99);
            $pdf->SetFont($arialb, '', 18, '', false); 
            $pdf->MultiCell(168, 0, 'DIRECT ADMISSION INTO II-YEAR UNDER<br>LATERAL ENTRY SCHEME', 0, 'C', 0, 0, '', '', true, 0, true);
			...
			$sem8_y=197.6; 
            $sem8_count=11-count($array_sem8);
			...
			$pdfBig->MultiCell(185.6, 0, strtoupper($HonorsMinors), 0, 'C', 0, 0);  
            $pdfBig->SetFont($arialNarrowB, '', 7.5, '', false);          
			...
			$pdf->SetXY(12, $sem8_y);	
            $pdf->SetFont($arialNarrowB, '', 7.5, '', false);
			$pdf->MultiCell(185.6, 0, strtoupper($HonorsMinors), 0, 'C', 0, 0);  
            $pdf->SetFont($arialNarrowB, '', 7.5, '', false);          
            ...
            $sem3_2_y=$sem8_y+4.9; 
            $sem3_2_count=3-count($array_sem3_2HM);
            ...
            $sem4_2_y=$sem3_2_y+3; 
            $sem4_2_count=3-count($array_sem4_2HM);          
			...
			$pdfBig->SetTextColor(0, 0, 0); 
            $pdfBig->SetFont($arialNarrowB, '', 7, '', false);
            $pdfBig->SetXY(12, $sem4_2_y+0.5);	
			$pdfBig->MultiCell(0, 0, '1. CGPA Secured : ', 0, 'L', 0, 0);
            $pdfBig->SetFont($arialNarrow, '', 7, '', false);
            $pdfBig->SetXY(31, $sem4_2_y+0.5);	
			$pdfBig->MultiCell(0, 0, $Credits_Secured.'&nbsp;&nbsp;', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdfBig->SetXY(74, $sem4_2_y+0.5);	
			$pdfBig->SetFont($arialNarrowB, '', 7, '', false);
			$pdfBig->MultiCell(0, 0, '2. Class Awarded :',0, 'L', 0, 0);
			$pdfBig->SetXY(93, $sem4_2_y+0.5);	
			$pdfBig->SetFont($arialNarrow, '', 7, '', false);
			$pdfBig->MultiCell(0, 11, $Class_Awarded.'&nbsp;&nbsp;', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($arialNarrow, '', 7, '', false);
            $pdfBig->SetXY(12, $sem4_2_y+3.6);	
			$pdfBig->writeHTML('<b>Number of Credits Registered in B.Tech. Program :</b> '.$Credits_Registered_in_Program, true, 0, true, 0);
			$pdfBig->SetXY(12, $sem4_2_y+6.5);	
			$pdfBig->writeHTML('<b>Number of Credits Secured in B.Tech. Program :</b> '.$Credits_Secured_in_Program, true, 0, true, 0);

            $pdfBig->SetFont($arialNarrow, '', 7, '', false);
            $pdfBig->SetXY(74, $sem4_2_y+3.6);	
			$pdfBig->writeHTML('<b>Number of Credits Registered in '.$resultFor.' Program :</b> '.$Credits_Registered_in_HM, true, 0, true, 0);
			$pdfBig->SetXY(74, $sem4_2_y+6.5);	
			$pdfBig->writeHTML('<b>Number of Credits Secured in '.$resultFor.' Program :</b> '.$Credits_Secured_in_HM, true, 0, true, 0);
			
			$pdfBig->SetXY(12,$sem4_2_y+9.5);	
			$pdfBig->SetFont($arialb, '', 7, '', false);
			$pdfBig->MultiCell(0, 0, 'PLACE :',0, 'L', 0, 0);
			$pdfBig->SetXY(23,$sem4_2_y+9.5);	
			$pdfBig->SetFont($arial, '', 7, '', false);
			$pdfBig->MultiCell(0, 0, $Place,0, 'L', 0, 0);

			$pdfBig->SetXY(12, $sem4_2_y+13);
			$pdfBig->SetFont($arialb, '', 7, '', false);	
			$pdfBig->MultiCell(0, 0, 'DATE :',0, 'L', 0, 0);
			$pdfBig->SetXY(21, $sem4_2_y+13);
			$pdfBig->SetFont($arial, '', 7, '', false);
			$pdfBig->MultiCell(0, 0, $Date,0, 'L', 0, 0);

            $pdfBig->SetFont($arialNarrowB, '', 5.5, '', false);
			$pdfBig->SetXY(12, $sem4_2_y+16);
			$pdfBig->MultiCell(0, 0, 'a)&nbsp;&nbsp;(^ Mandatory course passed for award of degree)<br />b)&nbsp;&nbsp;(* courses registered but not counted for calculation of aggregate)', 0, 'L', 0, 0, '', '', true, 0, true);
			...
			$pdf->SetTextColor(0, 0, 0); 
            $pdf->SetFont($arialNarrowB, '', 7, '', false);
            $pdf->SetXY(12, $sem4_2_y+0.5);	
			$pdf->MultiCell(0, 0, '1. CGPA Secured : ', 0, 'L', 0, 0);
            $pdf->SetFont($arialNarrow, '', 7, '', false);
            $pdf->SetXY(31, $sem4_2_y+0.5);	
			$pdf->MultiCell(0, 0, $Credits_Secured.'&nbsp;&nbsp;', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdf->SetXY(74, $sem4_2_y+0.5);	
			$pdf->SetFont($arialNarrowB, '', 7, '', false);
			$pdf->MultiCell(0, 0, '2. Class Awarded :',0, 'L', 0, 0);
			$pdf->SetXY(93, $sem4_2_y+0.5);	
			$pdf->SetFont($arialNarrow, '', 7, '', false);
			$pdf->MultiCell(0, 11, $Class_Awarded.'&nbsp;&nbsp;', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($arialNarrow, '', 7, '', false);
            $pdf->SetXY(12, $sem4_2_y+3.6);	
			$pdf->writeHTML('<b>Number of Credits Registered in B.Tech. Program :</b> '.$Credits_Registered_in_Program, true, 0, true, 0);
			$pdf->SetXY(12, $sem4_2_y+6.5);	
			$pdf->writeHTML('<b>Number of Credits Secured in B.Tech. Program :</b> '.$Credits_Secured_in_Program, true, 0, true, 0);

            $pdf->SetFont($arialNarrow, '', 7, '', false);
            $pdf->SetXY(74, $sem4_2_y+3.6);	
			$pdf->writeHTML('<b>Number of Credits Registered in '.$resultFor.' Program :</b> '.$Credits_Registered_in_HM, true, 0, true, 0);
			$pdf->SetXY(74, $sem4_2_y+6.5);	
			$pdf->writeHTML('<b>Number of Credits Secured in '.$resultFor.' Program :</b> '.$Credits_Secured_in_HM, true, 0, true, 0);
			
			$pdf->SetXY(12,$sem4_2_y+9.5);	
			$pdf->SetFont($arialb, '', 7, '', false);
			$pdf->MultiCell(0, 0, 'PLACE :',0, 'L', 0, 0);
			$pdf->SetXY(23,$sem4_2_y+9.5);	
			$pdf->SetFont($arial, '', 7, '', false);
			$pdf->MultiCell(0, 0, $Place,0, 'L', 0, 0);

			$pdf->SetXY(12, $sem4_2_y+13);
			$pdf->SetFont($arialb, '', 7, '', false);	
			$pdf->MultiCell(0, 0, 'DATE :',0, 'L', 0, 0);
			$pdf->SetXY(21, $sem4_2_y+13);
			$pdf->SetFont($arial, '', 7, '', false);
			$pdf->MultiCell(0, 0, $Date,0, 'L', 0, 0);

            $pdf->SetFont($arialNarrowB, '', 5.5, '', false);
			$pdf->SetXY(12, $sem4_2_y+16);
			$pdf->MultiCell(0, 0, 'a)&nbsp;&nbsp;(^ Mandatory course passed for award of degree)<br />b)&nbsp;&nbsp;(* courses registered but not counted for calculation of aggregate)', 0, 'L', 0, 0, '', '', true, 0, true);
            //end pdf
			...
			$codeContents = '';
            $codeContents .= $candidate_name.", ".$hall_ticket_no.", ".$Serial_No;
            // $codeContents .="\n";
            $codeContents .="\n\n".strtoupper(md5($str));
            ...
            $upload_COE = $Principal;
            $pathInfo = pathinfo($Principal);
            $Principal = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_$studentData[0].".$pathInfo['extension'];
            \File::copy($upload_COE,$Principal);
            $Principal_x = 167;
            $Principal_y = 262.5;
            ...
            // $pdf->image($Principal,$Principal_x,$Principal_y,$Principal_Width,$Principal_Height,"",'','L',true,3600);
            // $pdf->setPageMark();		
