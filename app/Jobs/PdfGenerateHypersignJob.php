...
class PdfGenerateHypersignJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('HypersignCerticate');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="SCUBE_University_SSSL_UNIVERSITY_Template.jpg"; // 
			$low_res_bg="SCUBE_University_SSSL_UNIVERSITY_Template.jpg";
			$pdfBig->AddPage();
			...
			$program = strtoupper(trim($studentData[4]));
            ...
            $pdfBig->SetFont($arialNarrowB, '', 9, '', false);
			$pdfBig->SetXY(26, 10.5);
			$pdfBig->MultiCell(40, '', $enrollment_no, 0, "L", 0, 0, '', '', true, 0, true);
            

            $pdfBig->SetFont($arialNarrowB, '', 9, '', false);
			$pdfBig->SetXY(160, 10.5);
			$pdfBig->MultiCell(30, '', $serial_no, 0, "R", 0, 0, '', '', true, 0, true);



            $pdfBig->SetFont($arialNarrowB, '', 20, '', false);
			$pdfBig->SetXY(10, 84);
			$pdfBig->MultiCell(190, '', $candidate_name, 0, "C", 0, 0, '', '', true, 0, true);



            $pdfBig->SetFont($arialNarrowB, '', 20, '', false);
			$pdfBig->SetXY(10, 98);
			$pdfBig->MultiCell(190, '', $program, 0, "C", 0, 0, '', '', true, 0, true);


            $pdfBig->SetFont($arialNarrowB, '', 20, '', false);
			$pdfBig->SetXY(10, 113);
			$pdfBig->MultiCell(190, '', $year, 0, "C", 0, 0, '', '', true, 0, true);
            ...
            $encryptedData = '';
            $hypersignUrl = "https://".$domain."/hypersign/verify/".$encryptedData;
			$codeContents = strtoupper(md5($str));

            $encryptedString = strtoupper(md5($str));
            $codeContents = $hypersignUrl;
            $codeContents .= "\n\n".$encryptedString;
			...
			if($previewPdf!=1){
				...
				$this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'HypersignCerticate',$admin_id,$card_serial_no);
        ...
        if($previewPdf!=1){
			$this->updateCardNo('HypersignCerticate',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'HypersignCerticate'.date("Ymdhms")).'.pdf';        
		// $file_name = 'test.pdf';
        ...
        if($previewPdf!=1){
            ...
            $template_name="HypersignCerticate";
