...
class PdfGenerateSdmMdsIIJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$pdfBig->MultiCell(17, 36, '', 'RT', 'C', 0, 0);
			$pdfBig->MultiCell(17, 36, '', 'RT', 'C', 0, 0);
			$pdfBig->MultiCell(17, 40, '', 'T', 'C', 0, 0);
			$pdfBig->SetXY(121, 114);
			$pdfBig->MultiCell(18, 42, '', 'RT', 'C', 0, 0);
			...
			$pdfBig->SetXY(62, 182);//191
			$pdfBig->MultiCell(36, 18, '', 'L', 'C', 0, 0);
			$pdfBig->SetXY(103, 182);//191
			$pdfBig->MultiCell(48.5, 18, '', 'LR', 'C', 0, 0); //2 & 3 verticle line
			
			
			$pdfBig->SetFont($MyriadProRegular, '', 11, '', false);
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(18, 185.5);
			$pdfBig->MultiCell(40, 18, '% Marks (First Attempt)', 0, 'L', 0, 0);
			$pdfBig->SetXY(72, 185.5);
			$pdfBig->MultiCell(40, 18, '75% & More', 0, 'L', 0, 0);
			$pdfBig->SetXY(112, 183);
			$pdfBig->MultiCell(30, 18, '65% & More but Less than 75%', 0, 'C', 0, 0);
			$pdfBig->SetXY(158, 183);
			$pdfBig->MultiCell(30, 18, '50% & More but Less than 65%', 0, 'C', 0, 0);
			
			
			$pdfBig->SetXY($left_str_x, 194);//203
			$pdfBig->MultiCell(42, 5, 'Class', 0, 'C', 0, 0);
			$pdfBig->MultiCell(47, 5, 'Distinction', 0, 'C', 0, 0);
			$pdfBig->MultiCell(48, 5, 'First Class', 0, 'C', 0, 0);
			$pdfBig->MultiCell(41, 5, 'Second Class', 0, 'C', 0, 0);
			...
			$pdf->MultiCell(180, 18, '', 'TB', 'C', 0, 0);	//Paper 
			$pdf->SetXY($left_str_x, 150);	
			$pdf->MultiCell(180, 6, '', 'TB', 'C', 0, 0); //Grand Total
			$pdf->SetXY(26.5, 102);	
			$pdf->MultiCell(46, 48, '', 'LR', 'C', 0, 0);  //Subject
			$pdf->SetXY(123, 108);	
			...
			$pdf->SetXY(72.5, 114);
			$pdf->MultiCell(17, 36, '', 'RT', 'C', 0, 0);
			$pdf->MultiCell(17, 36, '', 'RT', 'C', 0, 0);
			$pdf->MultiCell(17, 40, '', 'T', 'C', 0, 0);
			$pdf->SetXY(121, 114);
			$pdf->MultiCell(18, 42, '', 'RT', 'C', 0, 0);
			...
			$pdf->SetXY(62, 182);//191
			$pdf->MultiCell(36, 18, '', 'L', 'C', 0, 0);
			$pdf->SetXY(103, 182);//191
			$pdf->MultiCell(48.5, 18, '', 'LR', 'C', 0, 0); //2 & 3 verticle line
			
			
			$pdf->SetFont($MyriadProRegular, '', 11, '', false);
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetXY(18, 185.5);
			$pdf->MultiCell(40, 18, '% Marks (First Attempt)', 0, 'L', 0, 0);
			$pdf->SetXY(72, 185.5);
			$pdf->MultiCell(40, 18, '75% & More', 0, 'L', 0, 0);
			$pdf->SetXY(112, 183);
			...
			$pdf->MultiCell(42, 5, 'Class', 0, 'C', 0, 0);
			$pdf->MultiCell(47, 5, 'Distinction', 0, 'C', 0, 0);
			$pdf->MultiCell(48, 5, 'First Class', 0, 'C', 0, 0);
			$pdf->MultiCell(41, 5, 'Second Class', 0, 'C', 0, 0);
			...
			$T_Max1 = trim($studentData[31]);			
			$T_Min1 = trim($studentData[32]);
			$T_Sec1 = trim($studentData[33]);
			$T_Max2 = trim($studentData[34]);
			$T_Min2 = trim($studentData[35]);
			$T_Sec2 = trim($studentData[36]);
			$Remark1 = strtoupper(trim($studentData[37]));
			$Remark2 = strtoupper(trim($studentData[38]));
			$Grand_Total_Max = trim($studentData[39]);			
			$Grand_Total_Min = trim($studentData[40]);
			$Grand_Total_Sec = trim($studentData[41]);
			$Grand_Total_In_Words = trim($studentData[42]);
			$Percentage = trim($studentData[43]);
			$Result = strtoupper(trim($studentData[44]));
			$Date = trim($studentData[45]);

			$batch_of_qr = trim($studentData[47]);
			$aadhar_number_qr = trim($studentData[48]);
			$DOB = trim($studentData[49]);			
			$course_qr = strtoupper(trim($studentData[50]));			
			...
			$pdfBig->Cell(188, 0, "MASTER OF DENTAL SURGERY", 0, false, 'C');
			...
			$pdfBig->MultiCell(92, 0, "SDM College of Dental Sciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India",0, 'C', 0, 0);
			...
			*/
			...
			$pdf->Cell(188, 0, "MASTER OF DENTAL SURGERY", 0, false, 'C');
			...
			$pdf->SetXY(51.5, 283);
			$pdf->MultiCell(92, 0, "SDM College of Dental Sciences and Hospital, Sattur, Dharwad – 580009, Karnataka, India",0, 'C', 0, 0);
