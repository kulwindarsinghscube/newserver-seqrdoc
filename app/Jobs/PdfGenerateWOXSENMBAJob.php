...
class PdfGenerateWOXSENMBAJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // dd($printer_name);
        ...
        $log_serial_no = 1;
        //$cardDetails=$this->getNextCardNo('WOXSEN-MBA'); not needed for this
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            if(trim($studentData[8])) {

                
                $pdfBig->SetFont($Arial, '', 10, '', false);
                $pdfBig->SetXY($x, $y);
                $pdfBig->Cell(0, 0, "COA ID", 0, false, 'L');

                $pdf->SetFont($Arial, '', 10, '', false);
                $pdf->SetXY($x, $y);
                $pdf->Cell(0, 0, "COA ID", 0, false, 'L');
                ...
                $pdfBig->SetXY($x, $y1);
                // $pdfBig->Cell(0, 0, ": <b>". $studentData[7]."</b>", 0, false, 'L');
                $pdfBig->MultiCell(0, 0, ": <b>".$studentData[8]."</b>", 0, 'L', 0, 0, '', '', true, 0, true);

                $pdf->SetFont($Arial, '', 12, '', false);
                $pdf->SetXY($x, $y1);

                $pdf->MultiCell(0, 0, ": <b>".$studentData[8]."</b>", 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            if(trim($studentData[7])) {
                
                //EN ABC ID
                $x = 14.5;
                $pdfBig->SetFont($Arial, '', 10, '', false);
                $pdfBig->SetXY($x, $y);
                $pdfBig->Cell(0, 0, "ABC ID", 0, false, 'L');

                $pdf->SetFont($Arial, '', 10, '', false);
                $pdf->SetXY($x, $y);
                $pdf->Cell(0, 0, "ABC ID", 0, false, 'L');
                ...
                $pdfBig->SetXY($x, $y1);
                // $pdfBig->Cell(0, 0, ": <b>". $studentData[7]."</b>", 0, false, 'L');
                $pdfBig->MultiCell(0, 0, ": <b>".$studentData[7]."</b>", 0, 'L', 0, 0, '', '', true, 0, true);

                $pdf->SetFont($Arial, '', 12, '', false);
                $pdf->SetXY($x, $y1);

                $pdf->MultiCell(0, 0, ": <b>".$studentData[7]."</b>", 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetFont($Arial, '', 10, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, "Student ID. :", 0, false, 'L');

            $pdf->SetFont($Arial, '', 10, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, "Student ID. :", 0, false, 'L');
            ...
            $pdfBig->SetXY($x, $y1);
            $pdfBig->Cell(0, 0, $studentData[0], 0, false, 'L');

            $pdf->SetFont($ArialB, '', 12, '', false);
            $pdf->SetXY($x, $y1);
            $pdf->Cell(0, 0, $studentData[0], 0, false, 'L');
            ...
            $codeContents = "";
            $codeContents .= $studentData[2];
            $codeContents .="\n";
            $codeContents .= $studentData[5];
            $codeContents .="\n";
            
            $codeContents .= $studentData[0];
            $codeContents .="\n\n".strtoupper(md5($str));
           ...
           //  EFMD Image
            $efmd = public_path().'\\'.$subdomain[0].'\backend\canvas\images\EFMD_Accreditated_MBA.png';
            $efmd_x = 144;
            $efmd_y = 18;
            $efmd_Width = 55;
            $efmd_Height = 28;
            $pdfBig->image($efmd,$efmd_x,$efmd_y,$efmd_Width,$efmd_Height,"",'','L',true,3600);
            $pdfBig->setPageMark();
            $pdf->image($efmd,$efmd_x,$efmd_y,$efmd_Width,$efmd_Height,"",'','L',true,3600);
            $pdf->setPageMark();
            ...
            $pdfBig->SetFont($Arial, 0, 7, '', false);
            $pdfBig->SetXY($x, $y);
        ...
        $x = 10;
        $y = 126;   
        $pdfBig->SetFont($timesNewRomanI, '', 20, '', false);
        $pdfBig->SetXY($x, $y);
        $pdfBig->Cell(0, 0, "The Academic Council of Woxsen University", 0, false, 'C');
        
        $pdf->SetFont($timesNewRomanI, '', 20, '', false);
        $pdf->SetXY($x, $y);
        ...
        if($Totalchar > 25){
            ...
            $pdfBig->SetFont($timesNewRomanB, '',  $char_font_size, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, $studentData[2], 0, false, 'C');

            $pdf->SetFont($timesNewRomanB, '',  $char_font_size, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, $studentData[2], 0, false, 'C');
        }else{
            $pdfBig->SetFont($timesNewRomanB, '', 29, '', false);
            $pdfBig->SetXY($x, $y);
            $pdfBig->Cell(0, 0, $studentData[2], 0, false, 'C');

            $pdf->SetFont($timesNewRomanB, '', 29, '', false);
            $pdf->SetXY($x, $y);
            $pdf->Cell(0, 0, $studentData[2], 0, false, 'C');
        }
        ...
        $pdfBig->SetXY($x, $y);
        $pdfBig->MultiCell(190, 20, $studentData[3], 0, 'C', 0, 0, '', '', true, 0, true);

        $pdf->SetFont($timesNewRomanB, '', 25, '', false);
        $pdf->SetXY($x, $y);
        $pdf->MultiCell(190, 20, $studentData[3], 0, 'C', 0, 0, '', '', true, 0, true);
        ...
        $nameOrg = '';
        for($i=0; $i < 7; $i++){
          $nameOrg .= str_replace("<br>"," ","$studentData[3]");
        } 
        $str = $nameOrg;
        $str = strtoupper(preg_replace('/\s+/', '', $str)); //added by Mandar
        $partyName=$str;
        $microFontSize=2;
        $microlinestr = $this->microLine($partyName, 720, $microFontSize, $subdomain[0]);
        $pdfBig->SetFont($Arial, '', $microFontSize, '', false);
        $pdfBig->SetTextColor(0, 0, 0);
        $pdfBig->SetXY($x, $y);
        $pdfBig->Cell(190, 0, $microlinestr, 0, 1, 'L', 0, '', 4);
        //$pdfBig->Cell(290, 0, $microlinestr, 1, false, 'L');

        $pdf->SetFont($Arial, '', 2, '', false);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetXY($x, $y);
        $pdf->Cell(190, 0, $microlinestr, 0, 1, 'L', 0, '', 4);
        //$pdf->Cell(290, 0, $microlinestr, 0, false, 'L');

        //left size bottom cornor
        $x = 21.5;
        //$y = 323.5;
        $y = 264; 
        $nameOrg='Woxsen University Woxsen University Woxsen University Woxsen University Woxsen University';
        $str = $nameOrg;
        $str = strtoupper(preg_replace('/\s+/', '', $str)); //added by Mandar
        $microlinestr=$str;
        $pdfBig->SetFont($Arial, '', 2, '', false);
		...
		$Dean_x = 30;
		$Dean_y = 249;
		$Dean_Width = 26.458333333;
		$Dean_Height = 14.816666667;


        
        // echo $studentData[3];
        // echo "<br>";
        if((strpos(trim($studentData[3]), "Bachelor of Architecture") !== false) || (trim($studentData[3])=="Bachelor of Architecture")) {

            $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOAP_Sign.png';

        }elseif((strpos(trim($studentData[3]), "Bachelor of Technology") !== false) || (trim($studentData[3])=="Bachelor of Technology") ){

            $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOT_Sign.png';

        }elseif((strpos(trim($studentData[3]), "Bachelor of Design") !== false) || (trim($studentData[3])=="Bachelor of Design")){

            $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOAD_Sign.png';

        }elseif((strpos(trim($studentData[3]), "Master of Business Administration") !== false) || (trim($studentData[3])=="Master of Business Administration") || (strpos(trim($studentData[3]), "Bachelor of Commerce") !== false) || (trim($studentData[3])=="Bachelor of Commerce") || (strpos(trim($studentData[3]), "Bachelor of Business Administration") !== false) || (trim($studentData[3])=="Bachelor of Business Administration") || (strpos(trim($studentData[3]), "Bachelor of Commerce (Honours)") !== false) || (trim($studentData[3])=="Bachelor of Commerce (Honours)")){

            $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOB_Sign.png';
        ...
        // if(   trim($studentData[3])=="Bachelor of Architecture") {

        //     $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOAP_Sign.png';

        // }elseif(trim($studentData[3])=="Bachelor of Technology"||trim($studentData[3])=="Bachelor of Technology"){

        //     $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOT_Sign.png';

        // }elseif(trim($studentData[3])=="Bachelor of Design"||trim($studentData[3])=="Bachelor of Design"){

        //     $Dean = public_path().'\\'.$subdomain[0].'\backend\canvas\images\SOAD_Sign.png';

        // }elseif(trim($studentData[3])=="Master of Business Administration" || trim($studentData[3])=="Bachelor of Commerce"|| trim($studentData[3])=="Bachelor of Business Administration" || trim($studentData[3])=="Master of Business Administration" || trim($studentData[3])=="Bachelor of Business Administration"|| trim($studentData[3])=="Bachelor of Commerce (Honours)"){
		...
		$VC_x = 155;
		$VC_y = 253;
		$VC_Width = 26.458333333;
		$VC_Height = 9.7895833333;
        ...
        //$y = 324; 
        $y = 264;  
        $pdfBig->SetFont($timesNewRomanB, '', 14, '', false);
        $pdfBig->SetXY($x, $y);
        $pdfBig->Cell(0, 0, "Dean", 0, false, 'L');

        $pdf->SetFont($timesNewRomanB, '', 14, '', false);
        $pdf->SetXY($x, $y);
        ...
        //$y = 330;
        $y = 269;    
        $pdfBig->SetFont($timesNewRoman, '', 14, '', false);
        $pdfBig->SetXY($x, $y);
        ...
        // $pdfBig->SetXY($x, $y);
        // $pdfBig->Cell(0, 0, $studentData[4].", ". $studentData[5], 0, false, 'C');

        // $pdf->SetFont($timesNewRoman, '', 12, '', false);
        // $pdf->SetXY($x, $y);
        // $pdf->Cell(0, 0, $studentData[4].", ". $studentData[5], 0, false, 'C');
        ...
        //Indiviual Student ROW from SHEET 1
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'WOXSEN-MBA',$admin_id,$card_serial_no);
    ...
    public function microLine($text, $w, $fontSize, $subdomain)
    {
        $microlineEnrollment = strtoupper($text);
        $microlineEnrollment = preg_replace('/\s+/', '', $microlineEnrollment);
        $textArrayEnrollment = imagettfbbox($fontSize, 0, public_path().'/'.$subdomain.'/backend/canvas/fonts/Arial.TTF', $microlineEnrollment);
