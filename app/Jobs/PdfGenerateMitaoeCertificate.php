...
class PdfGenerateMitaoeCertificate
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       if(!empty($loader_data) && isset($loader_data['generatedCertificates'])){
        ...
        $card_serial_no = '';
        $template_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\MIT_ACADEMY_OF_ENGINEERING_Grade_Card_BG.jpg'; 
        $style1D = array(
            'position' => '',
            'align' => 'C',
            'stretch' => false,
       ...
       // $signature_path = public_path().'\\'.$subdomain[0].'\backend\canvas\images\Vice Chancellor.png';
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) 
            {
                ...
                $RROLL = trim($studentData[7]);
                $CNAME = trim($studentData[8]);
                $GENDER = trim($studentData[9]);
                $DOB = trim($studentData[10]);
                $FNAME = trim($studentData[11]);
                $MNAME = trim($studentData[12]);
                $PHOTO = trim($studentData[13]);
                $MRKS_REC_STATUS = trim($studentData[14]);
                $RESULT = trim($studentData[15]);
                $YEAR = trim($studentData[16]);
                $CSV_MONTH = trim($studentData[17]);
                $MONTH = trim($studentData[18]);
                $PERCENT = trim($studentData[19]);
                $DOI = trim($studentData[20]);
                $CERT_NO = trim($studentData[21]);
                $SEM = trim($studentData[22]);
                $TOT = trim($studentData[23]);
                $TOT_MIN = trim($studentData[24]);
                $TOT_MRKS = trim($studentData[25]);
                $TOT_TH_MAX = trim($studentData[26]);
                $TOT_TH_MIN = trim($studentData[27]);
                $TOT_TH_MRKS = trim($studentData[28]);
                $TOT_PR_MAX = trim($studentData[29]);
                $TOT_PR_MIN = trim($studentData[30]);
                $TOT_PR_MRKS = trim($studentData[31]);
                $TOT_CE_MAX = trim($studentData[32]);
                $TOT_CE_MIN = trim($studentData[33]);
                $TOT_CE_MRKS = trim($studentData[34]);
                $TOT_VV_MAX = trim($studentData[35]);
                $TOT_VV_MIN = trim($studentData[36]);
                $TOT_VV_MRKS = trim($studentData[37]);
                $TOT_CREDIT = trim($studentData[38]);
                $TOT_CREDIT_POINTS = trim($studentData[39]);
                $TOT_GRADE_POINTS = trim($studentData[40]);
                $PREV_TOT_MRKS = trim($studentData[41]);
                $GRAND_TOT_MAX = trim($studentData[42]);
                $GRAND_TOT_MIN = trim($studentData[43]);
                $GRAND_TOT_MRKS = trim($studentData[44]);
                $GRAND_TOT_CREDIT = trim($studentData[45]);
                $CGPA = trim($studentData[46]);
                $REMARKS = trim($studentData[47]);
                $SGPA = trim($studentData[48]);
                $ABC_ACCOUNT_ID = trim($studentData[49]);
                $TERM_TYPE = trim($studentData[50]);
                $TOT_GRADE = trim($studentData[51]);
                
                $AADHAAR_NAME = trim($studentData[52]);
                $ADMISSION_YEAR = trim($studentData[53]);
                ...
                $pdfBig->MultiCell(40, 7, 'DATE : '.date('d F Y'), 0, "L", 0, 0, '', '', true, 0, true);
                ...
                // rgba(126,57,49,255)
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 15.2, '', false); 
                $pdf->SetXY(10, 39);
                $pdf->MultiCell(190, 10, 'GRADE CARD', 0, "C", 0, 0, '', '', true, 0, true);
                ...
                // Set border color to red
                $pdf->SetDrawColor(126,57,49); // RGB Red
                ...
                //////////////////////
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 49);
                $pdf->MultiCell(55, 0, 'STATEMENT OF GRADES FOR', 0, "L", 0, 0, '', '', true, 0, true);

                $headingStr = $ADMISSION_YEAR.' '.$ACADEMIC_COURSE_ID.' EXAMINATION '.$CSV_MONTH.' '.$YEAR ;
                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 49);
                $pdf->MultiCell(100, 0, $headingStr, 0, "L", 0, 0, '', '', true, 0, true);

                //////////////////////
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 55);
                $pdf->MultiCell(37, 0, 'SEAT NO', 0, "L", 0, 0, '', '', true, 0, true);


                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 55);
                $pdf->MultiCell(56, 0, $RROLL, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), 55);
                $pdf->MultiCell(32, 0, 'PERM REG. NO.', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 55);
                $pdf->MultiCell(26, 0, $REGN_NO, 0, "L", 0, 0, '', '', true, 0, true);

                //////////////////////
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 61);
                $pdf->MultiCell(37, 0, 'NAME OF STUDENT', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 61);
                $pdf->MultiCell(111, 0, $CNAME, 0, "L", 0, 0, '', '', true, 0, true);

                //////////////////////
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 67);
                $pdf->MultiCell(37, 0, 'MOTHER NAME', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 67);
                $pdf->MultiCell(111, 0, $MNAME, 0, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 73);
                $pdf->MultiCell(37, 0, 'PROGRAMME', 0, "L", 0, 0, '', '', true, 0, true);


                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 73);
                $pdf->MultiCell(19, 0, $ACADEMIC_COURSE_ID, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), 73);
                $pdf->MultiCell(16, 0, 'YEAR', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 73);
                $pdf->MultiCell(25, 0, $ADMISSION_YEAR, 0, "L", 0, 0, '', '', true, 0, true);


                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), 73);
                $pdf->MultiCell(32, 0, 'ACADEMIC YEAR', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 73);
                $pdf->MultiCell(20, 0, $SESSION, 0, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(21, 79);
                $pdf->MultiCell(37, 0, 'BRANCH', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), 79);
                $pdf->MultiCell(111, 0, $STREAM, 0, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(18, $tableY);
                $pdf->MultiCell(30, 0, 'CODE', 0, "C", 0, 0, '', '', true, 0, true);

                // $pdf->SetTextColor(0,0,0);
                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->MultiCell(100, 0, 'COURSE NAME', 0, "C", 0, 0, '', '', true, 0, true);


                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->MultiCell(27, 0, 'CREDITS', 0, "C", 0, 0, '', '', true, 0, true);
                ...
                foreach($subjects as $sem => $sem_array) {
                    ...
                    $pdf->SetXY(20, $tableY);
                    $pdf->MultiCell(40, 0, 'SEMESTER:'.trim($sem), 0, "L", 0, 0, '', '', true, 0, true);
                    ...
                    foreach($sem_array as $key => $val){
                        ...
                        $pdf->SetXY(18, $tableY);
                        $pdf->MultiCell(28, 0, $val[3], 0, "C", 0, 0, '', '', true, 0, true);

                        // $pdf->SetTextColor(0,0,0);
                        $pdf->SetXY($pdf->getX(), $tableY);
                        $pdf->MultiCell(110, 0, $val[2], 0, "L", 0, 0, '', '', true, 0, true);


                        $pdf->SetXY($pdf->getX(), $tableY);
                        $pdf->MultiCell(22, 0, $val[20], 0, "C", 0, 0, '', '', true, 0, true);

                        $pdf->SetXY($pdf->getX(), $tableY);
                        $pdf->MultiCell(12, 0, $val[18], 0, "C", 0, 0, '', '', true, 0, true);
                        $sem_grade_point += $val[19];
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(18, $tableY);
                $pdf->MultiCell(89, 7, 'CURRENT SEMESTER PERFORMANCE', 'R', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->MultiCell(89, 7, 'CUMILATIVE PERFORMANCE', 0, "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->MultiCell(29, 7, 'CREDITS', 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(33, 7, 'GRADE POINTS', 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(29, 7, 'SGPA', 'TR', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(25, 7, 'CGPA', 'T', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(16, $pdf->getY() );
                $pdf->MultiCell(29, 7, $TOT_CREDIT, 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(33, 7, $sem_grade_point, 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(29, 7, number_format($SGPA,2), 'TR', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(29, 7, $TOT_CREDIT_POINTS, 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(33, 7, $TOT_GRADE_POINTS, 'TR', "C", 0, 0, '', '', true, 0, true);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY($pdf->getX(), $pdf->getY() );
                $pdf->MultiCell(25, 7, number_format($CGPA,2), 'T', "C", 0, 0, '', '', true, 0, true);
                ...
                $pdf->MultiCell(29, 0, 'PLACE : PUNE', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->ln();
                $pdf->SetTextColor(126,57,49);
                $pdf->SetFont($arialb, '', 9.6, '', false); 
                $pdf->SetXY(15, $pdf->getY());
                $pdf->MultiCell(40, 7, 'DATE : '.date('d F Y'), 0, "L", 0, 0, '', '', true, 0, true);

                //end pdf
                        
                        
                // $template_id= 100;
                // $Photo=$studentData[10];
                ...
                if($previewPdf!=1){
                    ...
                    $this->addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$studentData[0],$studentData[1],$studentData[2],$studentData[6]);
