...
use Carbon\Carbon;
use Helper;
class PdfGenerateBestiuEDocumentJob6_10s
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $subjectDataOrg=CoreHelper::getverticaldata(array_filter($subjectDataOrg));
		$subjectsArr = array();
        if($subjectDataOrg) {
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
				...
				$high_res_bg="bestiu_pdc_bg.jpg"; // bestiu_pdc_bg, GradeCard.jpg
                ...
                $examination=trim($studentData[3]);
                $PROGRAMME =trim($studentData[4]);
                $batch =trim($studentData[5]);
                $classAwarded =trim($studentData[6]);
                $ID_No=trim($studentData[7]);
                $DATE=trim($studentData[8]);
                $TOTAL_CREDITS_EARNED = trim($studentData[9]);
                $TOTAL_CREDIT_POINTS = trim($studentData[10]);
                $GRADE_POINT_AVERAGE_CGPA = trim($studentData[11]);
                $PERCENTAGE_OF_MARKS = trim($studentData[12]);
                $schoolName=trim($studentData[13]);

				$hall_ticket_no=trim($studentData[0]);
                ...
                $pdfBig->SetXY(12, 11);
                $pdfBig->MultiCell(190, 0, 'ID NO.: <b>'.$ID_No.'</b>', 0, "L", 0, 0, '', '', true, 0, true);

               
               $pdfBig->SetTextColor(0,0,0);
                $pdfBig->SetFont($arialb, '', 12, '', false); 
                $pdfBig->SetXY(10, 43);
                $pdfBig->MultiCell(190, 0, 'CONSOLIDATED GRADE SHEET/CREDIT SHEET', 0, "C", 0, 0, '', '', true, 0, true);


                $pdfBig->SetFont($arialb, '', 8, '', false); 
                $pdfBig->SetXY(12, 50);
                $pdfBig->MultiCell(27, 6, 'Student Name : ', 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(83, 6, $candidate_name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), 50);
                $pdfBig->MultiCell(21, 6, 'Programme : ', 0, "L", 0, 0, '', '', true, 0, true);
                // $pdfBig->SetXY($pdfBig->getX(), 56);
                $pdfBig->MultiCell(57, 6, $PROGRAMME, 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();

                $pdfBig->SetXY(12, $pdfBig->getY());
                $pdfBig->MultiCell(27, 6, 'Father’s Name : ', 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->MultiCell(83, 6, $Fathers_Name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(24, 6, 'Class Awarded :', 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->SetXY(144, $pdfBig->getY());
                $pdfBig->MultiCell(57, 6, $classAwarded, 0, "L", 0, 0, '', '', true, 0, true);
                $pdfBig->ln();

                $pdfBig->SetXY(12, $pdfBig->getY()-2);
                $pdfBig->MultiCell(110, 6, 'Year of Completion : '.$examination, 0, "L", 0, 0, '', '', true, 0, true);


                $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                $pdfBig->MultiCell(77, 6, 'Year of Admission : '.$batch, 0, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetXY(12, $tableY);    
               
                $pdfBig->MultiCell(15, 7, 'Course Code', 1, 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $tableY);
                $pdfBig->setCellPaddings( $left = '', $top = '2', $right = '', $bottom = '');
                
                $pdfBig->SetXY($pdfBig->getX(), $tableY);
                $pdfBig->MultiCell(45.6, 7, 'Course Title', 1, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetXY($pdfBig->getX(), $tableY);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdfBig->MultiCell(10.2, 7, 'Grade<br>Letter', 1, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetXY($pdfBig->getX(), $tableY);
                $pdfBig->setCellPaddings( $left = '', $top = '2', $right = '', $bottom = '');
                $pdfBig->MultiCell(11, 7, 'Credits', 1, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');

                $pdfBig->SetXY($pdfBig->getX(), $tableY);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdfBig->MultiCell(10.3, 7, 'Credit Points', 1, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                ...
                $totalHeight = 0;
                // echo "<br>Term: ".$term;
                if ($term == "I SEMESTER" || $term == "III SEMESTER" || $term == "V SEMESTER") {
                    if($term == "III SEMESTER"){
                      $leftCurrentY=$leftCurrentY+4;  
                    }else if($term == "V SEMESTER"){
                      $leftCurrentY=$leftCurrentY+4;  
                    }
                    $pdfBig->SetFont($arialb, '', $font_size_new, '', false);
                    $pdfBig->SetXY(12, $leftCurrentY);
                    $pdfBig->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdfBig->MultiCell(92, 3, $term , 'TB', 'C', 0, 0, '', '', true, 0, true);
                    // $pdfBig->Cell(88.4, 3.7, $term.' (SGPA : '.$SEM1_SGPA.')', 'B', false, 'C');
                    $pdfBig->SetFont($arial, '', $font_size_new, '', false);
                    $pdfBig->ln();

                    $semLeftSGPA = $term_array[0][7];

                    $subjectTableHeight = 2.8;
                    ...
                    foreach($term_array as $key => $val){
                        ...
                        $lines = $pdfBig->MultiCell(45.6, 0, trim($val[3]), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                        $courselines = $pdfBig->MultiCell(15, 0, trim($val[2]), 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                        $pdfBig=$pdfBig->rollbackTransaction(); // restore previous object
                        
                        // echo $lines;
                        // echo "<br>";
                        if ($lines == 3 && $courselines > 1) {
                            $subjectTableHeight = 8.4;
                        }elseif ($lines == 2 || $courselines > 1) {
                            $subjectTableHeight = 5.6;
                        } else {
                            $subjectTableHeight = 2.8;
                        }
                        ...
                        $pdfBig->SetFont($arial, '', $font_size_new, '', false);
                        $pdfBig->SetXY(12, $pdfBig->getY());    
                        $pdfBig->MultiCell(15, $subjectTableHeight, $val[2], '', 'C', 0, 0, '', '', true, 0, true);
                        $pdfBig->setCellPaddings( $left = '1', $top = '', $right = '', $bottom = '');
                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(45.6, $subjectTableHeight,trim($val[3]), '', 'L', 0, 0, '', '', true, 0, true);
                        
                        // $pdfBig->setCellPaddings( $left = '5.6', $top = '', $right = '', $bottom = '');
                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(10.2, $subjectTableHeight, $val[5], '', 'C', 0, 0, '', '', true, 0, true);
                        // $pdfBig->setCellPaddings( $left = '1', $top = '', $right = '', $bottom = '');
                        ...
                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(10.5, $subjectTableHeight, $val[6], '', 'C', 0, 0, '', '', true, 0, true);

                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(10.3, $subjectTableHeight, $val[4], '', 'C', 0, 0, '', '', true, 0, true);
                    ...
                    // $pdfBig->SetFont($arialb, '', 8, '', false); 
                    // $pdfBig->SetXY(12, $pdfBig->getY());
                    // $pdfBig->MultiCell(92, 3, 'SGPA : '.'9.67', 1, "L", 0, 0, '', '', true, 0, true);
                    // $pdfBig->ln();
                ...
                } else if ($term == "II SEMESTER" || $term == "IV SEMESTER" || $term == "VI SEMESTER") {
                    // $tableSecondStart
                    if($term == "IV SEMESTER"){
                      $rightCurrentY=$rightCurrentY+4;  
                    }else if($term == "VI SEMESTER"){
                      $rightCurrentY=$rightCurrentY+4;  
                    }
                    $pdfBig->SetFont($arialb, '', $font_size_new, '', false);
                    ...
                    foreach($term_array as $key => $val){
                        ...
                        $pdfBig->MultiCell(15, $subjectTableHeight, $val[2], '0', 'C', 0, 0, '', '', true, 0, true);

                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(45.6, $subjectTableHeight, trim($val[3]), '0', 'L', 0, 0, '', '', true, 0, true);
                        ...
                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(11, $subjectTableHeight, $val[6], '0', 'C', 0, 0, '', '', true, 0, true);

                        $pdfBig->SetXY($pdfBig->getX(), $pdfBig->getY());
                        $pdfBig->MultiCell(10.3, $subjectTableHeight, $val[4], '0', 'C', 0, 0, '', '', true, 0, true);
                    ...
                    // $pdfBig->SetFont($arialb, '', 8, '', false); 
                    // // if($term == "II SEMESTER"){
                    // //   $pdfBig->SetXY(106, $pdfBig->getY()+3 );
                    // // }else{
                    //  $pdfBig->SetXY(106, $pdfBig->getY());
                    // // }
                    // $pdfBig->MultiCell(92, 3, 'SGPA : '.'9.67', 1, "L", 0, 0, '', '', true, 0, true);
                ...
                if($term == "II SEMESTER" || $term == "IV SEMESTER" || $term == "VI SEMESTER") {
                ...
                if($term == "II SEMESTER" || $term == "IV SEMESTER" || $term == "VI SEMESTER") {
                    ...
                    // $leftCurrentY = $leftCurrentY-4;
                    $pdfBig->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdfBig->SetFont($arialb, '', $font_size_new+2, '', false); 
                    $pdfBig->SetXY(12, $leftCurrentY);
                    $pdfBig->MultiCell(92.1, 3, 'SGPA : '.$semLeftSGPA, 'TRLB', "L", 0, 0, '', '', true, 0, true);
                    ...
                    $pdfBig->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdfBig->SetFont($arialb, '', $font_size_new+2, '', false); 
                    $pdfBig->SetXY(106, $leftCurrentY);
                    $pdfBig->MultiCell(92.1, 3, 'SGPA : '.$semRightSGPA, 'TRLB', "L", 0, 0, '', '', true, 0, true);
                    $pdfBig->ln();
                }   
                ...
                $pdfBig->SetFont($arial, '', $font_size_new, '', false);
                $pdfBig->SetXY(12, $tableBorderStartY); 
                $pdfBig->MultiCell(15, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $tableBorderStartY);
                $pdfBig->MultiCell(45.6, $totalHeight, '', 'LR', 'L', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetXY($pdfBig->getX(), $tableBorderStartY);
                $pdfBig->MultiCell(10.2, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetXY($pdfBig->getX(), $tableBorderStartY);
                $pdfBig->MultiCell(11, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY($pdfBig->getX(), $tableBorderStartY);
                $pdfBig->MultiCell(10.3, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);
                // $tableBorderStartY = 0;
                // $totalHeight = 0;
                ...
                $bottomY = ($leftCurrentY>$rightCurrentY)?$leftCurrentY:$rightCurrentY;
                
                
                // $y_axis=226.5;
                $y_axis=$bottomY+3;
                $pdfBig->setCellPaddings( $left = '', $top = '', $right = '', $bottom = '2.5');
                $pdfBig->SetXY(12, $y_axis+3);  
                $pdfBig->MultiCell(186.1, 4, '', 'LRTB', 'C', 0, 0, '', '', true, 0, true);
                
               $pdfBig->SetFont($arialb, '', 6, '', false);
                // Vertical lines (adjust X coordinates for perfect alignment)
                $pdfBig->Line(51,  $y_axis + 3,  51,  $y_axis + 8);   // After TOTAL CREDITS EARNED
                $pdfBig->Line(90, $y_axis + 3, 90, $y_axis + 8);   // After TOTAL CREDIT POINTS
                $pdfBig->Line(154, $y_axis + 3, 154, $y_axis + 8);   // After CGPA
                $ytotal=$y_axis + 4;
                // Text
                $pdfBig->SetXY(15.5, $y_axis + 4);
                $pdfBig->MultiCell(0, 0, 'TOTAL CREDITS EARNED: ' . $TOTAL_CREDITS_EARNED, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY(54, $y_axis + 4);
                $pdfBig->MultiCell(0, 0, 'TOTAL CREDIT POINTS: ' . $TOTAL_CREDIT_POINTS, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY(92, $y_axis + 4);
                $pdfBig->MultiCell(0, 0, 'CUMULATIVE GRADE POINT AVERAGE (CGPA): ' . $GRADE_POINT_AVERAGE_CGPA, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdfBig->SetXY(160, $y_axis + 4);
                $pdfBig->MultiCell(0, 0, 'PERCENTAGE OF MARKS: ' . $PERCENTAGE_OF_MARKS, 0, 'L', 0, 0, '', '', true, 0, true);

            

                
                // $pdfBig->SetFont($arialb, '', 9, '', false); 
                // $pdfBig->SetXY(12, $ytotal+5);
                // $pdfBig->MultiCell(90, 0, 'Note: “P: Practical.”', 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY(12, 262);
                $pdfBig->MultiCell(90, 0, 'Date: '.$DATE, 0, "L", 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialb, '', 9, '', false); 
                $pdfBig->SetXY(144, 262);
                $pdfBig->MultiCell(90, 0, 'Controller of Examinations', 0, "L", 0, 0, '', '', true, 0, true);
                //End pdfBig   
                ...
                $pdf->SetXY(12, 11);
                $pdf->MultiCell(190, 0, 'ID NO.: <b>'.$ID_No.'</b>', 0, "L", 0, 0, '', '', true, 0, true);

               
               $pdf->SetTextColor(0,0,0);
                $pdf->SetFont($arialb, '', 12, '', false); 
                $pdf->SetXY(10, 43);
                $pdf->MultiCell(190, 0, 'CONSOLIDATED GRADE SHEET/CREDIT SHEET', 0, "C", 0, 0, '', '', true, 0, true);


                $pdf->SetFont($arialb, '', 8, '', false); 
                $pdf->SetXY(12, 50);
                $pdf->MultiCell(27, 6, 'Student Name : ', 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(83, 6, $candidate_name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), 50);
                $pdf->MultiCell(21, 6, 'Programme : ', 0, "L", 0, 0, '', '', true, 0, true);
                // $pdf->SetXY($pdf->getX(), 56);
                $pdf->MultiCell(57, 6, $PROGRAMME, 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->ln();

                $pdf->SetXY(12, $pdf->getY());
                $pdf->MultiCell(27, 6, 'Father’s Name : ', 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->MultiCell(83, 6, $Fathers_Name, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(24, 6, 'Class Awarded :', 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->SetXY(144, $pdf->getY());
                $pdf->MultiCell(57, 6, $classAwarded, 0, "L", 0, 0, '', '', true, 0, true);
                $pdf->ln();

                $pdf->SetXY(12, $pdf->getY()-2);
                $pdf->MultiCell(110, 6, 'Year of Completion : '.$examination, 0, "L", 0, 0, '', '', true, 0, true);


                $pdf->SetXY($pdf->getX(), $pdf->getY());
                $pdf->MultiCell(77, 6, 'Year of Admission : '.$batch, 0, "L", 0, 0, '', '', true, 0, true);
                ...
                $pdf->MultiCell(15, 7, 'Course Code', 1, 'C', 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->setCellPaddings( $left = '', $top = '2', $right = '', $bottom = '');
                
                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->MultiCell(45.6, 7, 'Course Title', 1, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdf->MultiCell(10.2, 7, 'Grade<br>Letter', 1, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->setCellPaddings( $left = '', $top = '2', $right = '', $bottom = '');
                $pdf->MultiCell(11, 7, 'Credits', 1, 'C', 0, 0, '', '', true, 0, true);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');

                $pdf->SetXY($pdf->getX(), $tableY);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                $pdf->MultiCell(10.3, 7, 'Credit Points', 1, 'C', 0, 0, '', '', true, 0, true);
                $pdf->setCellPaddings( $left = '', $top = '0', $right = '', $bottom = '');
                ...
                if ($term == "I SEMESTER" || $term == "III SEMESTER" || $term == "V SEMESTER") {
                    ...
                    $pdf->SetFont($arialb, '', $font_size_new, '', false);
                    $pdf->SetXY(12, $leftCurrentY);
                    $pdf->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdf->MultiCell(92, 3, $term , 'TB', 'C', 0, 0, '', '', true, 0, true);
                    // $pdf->Cell(88.4, 3.7, $term.' (SGPA : '.$SEM1_SGPA.')', 'B', false, 'C');
                    $pdf->SetFont($arial, '', $font_size_new, '', false);
                    $pdf->ln();
                    ...
                    foreach($term_array as $key => $val){
                        ...
                        $pdf->SetFont($arial, '', $font_size_new, '', false);  
                        // get the number of lines
                        $lines = $pdf->MultiCell(45.6, 0, trim($val[3]), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                        $courselines = $pdf->MultiCell(15, 0, trim($val[2]), 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                        ...
                        $pdf->SetFont($arial, '', $font_size_new, '', false);
                        $pdf->SetXY(12, $pdf->getY());    
                        $pdf->MultiCell(15, $subjectTableHeight, $val[2], '', 'C', 0, 0, '', '', true, 0, true);
                        $pdf->setCellPaddings( $left = '1', $top = '', $right = '', $bottom = '');
                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(45.6, $subjectTableHeight,trim($val[3]), '', 'L', 0, 0, '', '', true, 0, true);
                        
                        // $pdf->setCellPaddings( $left = '5.6', $top = '', $right = '', $bottom = '');
                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(10.2, $subjectTableHeight, $val[5], '', 'C', 0, 0, '', '', true, 0, true);
                        // $pdf->setCellPaddings( $left = '1', $top = '', $right = '', $bottom = '');
                        ...
                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(10.5, $subjectTableHeight, $val[6], '', 'C', 0, 0, '', '', true, 0, true);

                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(10.3, $subjectTableHeight, $val[4], '', 'C', 0, 0, '', '', true, 0, true);
                    ...
                    // $pdf->SetFont($arialb, '', 8, '', false); 
                    // $pdf->SetXY(12, $pdf->getY());
                    // $pdf->MultiCell(92, 3, 'SGPA : '.'9.67', 1, "L", 0, 0, '', '', true, 0, true);
                    // $pdf->ln();
                    ...
                    foreach($term_array as $key => $val){
                        ...
                        $pdf->MultiCell(15, $subjectTableHeight, $val[2], '0', 'C', 0, 0, '', '', true, 0, true);

                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(45.6, $subjectTableHeight, trim($val[3]), '0', 'L', 0, 0, '', '', true, 0, true);
                        ...
                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(11, $subjectTableHeight, $val[6], '0', 'C', 0, 0, '', '', true, 0, true);

                        $pdf->SetXY($pdf->getX(), $pdf->getY());
                        $pdf->MultiCell(10.3, $subjectTableHeight, $val[4], '0', 'C', 0, 0, '', '', true, 0, true);
                ...
                if($term == "II SEMESTER" || $term == "IV SEMESTER" || $term == "VI SEMESTER") {
                    ...
                    // $leftCurrentY = $leftCurrentY-4;
                    $pdf->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdf->SetFont($arialb, '', $font_size_new+2, '', false); 
                    $pdf->SetXY(12, $leftCurrentY);
                    $pdf->MultiCell(92.1, 3, 'SGPA : '.$semLeftSGPA, 'TRLB', "L", 0, 0, '', '', true, 0, true);
                    ...
                    $pdf->setCellPaddings( $left = '', $top = '0.4', $right = '', $bottom = '');
                    $pdf->SetFont($arialb, '', $font_size_new+2, '', false); 
                    $pdf->SetXY(106, $leftCurrentY);
                    $pdf->MultiCell(92.1, 3, 'SGPA : '.$semRightSGPA, 'TRLB', "L", 0, 0, '', '', true, 0, true);
                    $pdf->ln();
                }   
                ...
                $pdf->SetFont($arial, '', $font_size_new, '', false);
                $pdf->SetXY(12, $tableBorderStartY); 
                $pdf->MultiCell(15, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $tableBorderStartY);
                $pdf->MultiCell(45.6, $totalHeight, '', 'LR', 'L', 0, 0, '', '', true, 0, true);
                
                $pdf->SetXY($pdf->getX(), $tableBorderStartY);
                $pdf->MultiCell(10.2, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetXY($pdf->getX(), $tableBorderStartY);
                $pdf->MultiCell(11, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);

                $pdf->SetXY($pdf->getX(), $tableBorderStartY);
                $pdf->MultiCell(10.3, $totalHeight, '', 'LR', 'C', 0, 0, '', '', true, 0, true);
                // $tableBorderStartY = 0;
                // $totalHeight = 0;
                ...
                // $y_axis=226.5;
                $y_axis=$bottomY+3;
                $pdf->setCellPaddings( $left = '', $top = '', $right = '', $bottom = '2.5');
                $pdf->SetXY(12, $y_axis+3);  
                $pdf->MultiCell(186.1, 4, '', 'LRTB', 'C', 0, 0, '', '', true, 0, true);
                
               $pdf->SetFont($arialb, '', 6, '', false);
                // Vertical lines (adjust X coordinates for perfect alignment)
                $pdf->Line(51,  $y_axis + 3,  51,  $y_axis + 8);   // After TOTAL CREDITS EARNED
                $pdf->Line(90, $y_axis + 3, 90, $y_axis + 8);   // After TOTAL CREDIT POINTS
                $pdf->Line(154, $y_axis + 3, 154, $y_axis + 8);   // After CGPA
                $ytotal=$y_axis + 4;
                // Text
                $pdf->SetXY(15.5, $y_axis + 4);
                $pdf->MultiCell(0, 0, 'TOTAL CREDITS EARNED: ' . $TOTAL_CREDITS_EARNED, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdf->SetXY(54, $y_axis + 4);
                $pdf->MultiCell(0, 0, 'TOTAL CREDIT POINTS: ' . $TOTAL_CREDIT_POINTS, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdf->SetXY(92, $y_axis + 4);
                $pdf->MultiCell(0, 0, 'CUMULATIVE GRADE POINT AVERAGE (CGPA): ' . $GRADE_POINT_AVERAGE_CGPA, 0, 'L', 0, 0, '', '', true, 0, true);

                $pdf->SetXY(160, $y_axis + 4);
                $pdf->MultiCell(0, 0, 'PERCENTAGE OF MARKS: ' . $PERCENTAGE_OF_MARKS, 0, 'L', 0, 0, '', '', true, 0, true);

            

                
                // $pdf->SetFont($arialb, '', 9, '', false); 
                // $pdf->SetXY(12, $ytotal+5);
                // $pdf->MultiCell(90, 0, 'Note: “P: Practical.”', 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY(12, 262);
                $pdf->MultiCell(90, 0, 'Date: '.$DATE, 0, "L", 0, 0, '', '', true, 0, true);

                $pdf->SetFont($arialb, '', 9, '', false); 
                $pdf->SetXY(144, 262);
                $pdf->MultiCell(90, 0, 'Controller of Examinations', 0, "L", 0, 0, '', '', true, 0, true);				           
				//End pdf			
				
                $COE = public_path().'\\'.$subdomain[0].'\backend\canvas\images\bestiue_sign.png';
                $COE_x = 150;
                $COE_y =248;
                $COE_Width = 31.75;
                $COE_Height = 16;
                ...
                $COE = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']."_$imagName.".$pathInfo['extension'];
       ...
       if($previewPdf!=1){
			// $this->updateCardNo('BestiuETransc',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'BestiuETransc'.date("Ymdhms")).'.pdf';
