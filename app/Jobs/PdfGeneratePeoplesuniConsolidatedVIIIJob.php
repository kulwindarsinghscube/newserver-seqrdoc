...
class PdfGeneratePeoplesuniConsolidatedVIIIJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $ghostImgArr = array();
        $pdfBig = new TCPDF('P', 'mm', array('215.9', '355.6'), true, 'UTF-8', false);
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="PEOPLES_UNIVERSITY_GRADE_CARD_LETTER_BG.jpg"; // 
			$low_res_bg="PEOPLES_UNIVERSITY_GRADE_CARD_LETTER_BG.jpg";
			$pdfBig->AddPage();
			...
			if($previewPdf==1){
				if($previewWithoutBg!=1){
					$pdfBig->Image($template_img_generate, 0, 0,  '215.9', '355.6', "JPG", '', 'R', true);
				}
				$date_font_size = '11';
				$date_nox = 13;
				$date_noy = 35;
                ...
                // $pdfBig->Image($template_img_generate, 0, 0,  '210', '355.6', "JPG", '', 'R', true);
                $pdf->Image($template_img_generate, 0, 0,  '215.9', '355.6', "JPG", '', 'R', true);
            ...
            $final_cgpa = trim($studentData[43]);
            $final_result = trim($studentData[44]);
            $division = trim($studentData[45]);
            $place = trim($studentData[46]);
            $issue_date = trim($studentData[47]);
            ...
            $rightX3 = 145;
            $pdfBig->SetFont($timesb, '', 8, '', false);
            $pdfBig->SetXY($rightX1, 67);
            $pdfBig->MultiCell(44, $headerHeight, 'PROGRAM', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($timesb, '', 8, '', false);
            $pdfBig->SetXY($rightX2, 67);
            $pdfBig->MultiCell(4, $headerHeight, ':', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 8, '', false);
            $pdfBig->SetXY($rightX3, 67);
            ...
            foreach($subjects as $term => $term_array){
                ...
                if($term == 'I' || $term == 'III' || $term == 'V' || $term == 'VII') {
                    ...
                    if($term == 'III') {
                        ...
                        if($tableTermV > $tableTermVI) {
                            $tableYV1 =  $tableTermV;
                        }else if($tableTermV < $tableTermVI) {
                            $tableYV1 =  $tableTermVI;
                        }
                    }
                    $pdfBig->setCellPaddings($left = '', $top = '0', $right = '', $bottom = '');
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(20, $tableY);
                    $pdfBig->MultiCell(30, 3, 'SEMESTER-'.$term, 'LT', 'L', 0, 0, '', '', true, 0, true);
    
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(47, $tableY);
                    $pdfBig->MultiCell(31, 3, 'MYOP : '.trim($studentData[$IndividualDataIndex]), 'LT', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
    
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(77, $tableY);
                    $pdfBig->MultiCell(31, 3, 'STATUS : '.trim($studentData[$IndividualDataIndex]), 'LTR', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdfBig->Ln();
                ...
                } else if($term == 'II' || $term == 'IV' || $term == 'VI' || $term == 'VIII') {
                    ...
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(108, $tableYV1);
                    $pdfBig->MultiCell(31, 3, 'SEMESTER-'.$term, 'LT', 'L', 0, 0, '', '', true, 0, true);
    
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(135, $tableYV1);
                    $pdfBig->MultiCell(31, 3, 'MYOP : '.trim($studentData[$IndividualDataIndex]), 'LT', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
    
                    $pdfBig->SetFont($timesb, '', 7, '', false);
                    $pdfBig->SetXY(165, $tableYV1);
                    $pdfBig->MultiCell(31, 3, 'STATUS : '.trim($studentData[$IndividualDataIndex]), 'LTR', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdfBig->Ln();
    
                    $pdfBig->SetFont($times, '', 7, '', false);
                    $pdfBig->SetXY(108, $pdfBig->GetY());
                    $pdfBig->setCellHeightRatio(0.9);
                    $pdfBig->setCellPaddings( $left = '', $top = '1', $right = '', $bottom = '');
                    $pdfBig->MultiCell(13, 6, 'PAPER CODE', 'LTR', 'C', 0, 0, '', '', true, 0, true);
                    $pdfBig->setCellHeightRatio(1);
            ...
            $pdfBig->setCellPaddings($left = '', $top = '0.3', $right = '', $bottom = '');
            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY(118, 302);
            $pdfBig->MultiCell(38, 3, 'OVERALL', 1, 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->Ln();
            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(118, $pdfBig->GetY());
            $pdfBig->MultiCell(13, 3.5, 'CGPA', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(25, 3.5, trim($final_cgpa), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->Ln();
            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(118, $pdfBig->GetY());
            $pdfBig->MultiCell(13, 3.5, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(25, 3.5, trim($final_result), 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetFont($timesb, '', 7, '', false);  
            // get the number of lines
            $lines = $pdfBig->MultiCell(25, 0, trim($division), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            $pdfBig=$pdfBig->rollbackTransaction(); // restore previous object
            ...
            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(118, $pdfBig->GetY());
            $pdfBig->MultiCell(13,$height, 'DIVISION', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(25,$height, trim($division), 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(61.5, 308);
            $pdfBig->MultiCell(40, 0, 'CHECKED BY:', 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(155.5, 320);
            $pdfBig->MultiCell(40, 0, 'CONTROLLER OF EXAMINATIONS', 0, 'L', 0, 0, '', '', true, 0, true);



            $pdfBig->SetFont($timesb, '', 7, '', false);
            $pdfBig->SetXY(71, 331.5);
            $pdfBig->MultiCell(70, 0, 'ISSUE DATE : '.$issue_date, 0, 'L', 0, 0, '', '', true, 0, true);

            $pdfBig->SetXY(115, 331.5);
            $pdfBig->MultiCell(70, 0, 'PLACE : '.$place, 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            $rightX3 = 145;
            $pdf->SetFont($timesb, '', 8, '', false);
            $pdf->SetXY($rightX1, 67);
            $pdf->MultiCell(44, $headerHeight, 'PROGRAM', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($timesb, '', 8, '', false);
            $pdf->SetXY($rightX2, 67);
            $pdf->MultiCell(4, $headerHeight, ':', 0, 'L', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 8, '', false);
            $pdf->SetXY($rightX3, 67);
            ...
            foreach($subjects as $term => $term_array){
                ...
                if($term == 'I' || $term == 'III' || $term == 'V' || $term == 'VII') {
                    ...
                    $pdf->SetXY(20, $tableY);
                    $pdf->MultiCell(30, 3, 'SEMESTER-'.$term, 'LT', 'L', 0, 0, '', '', true, 0, true);
    
                    $pdf->SetFont($timesb, '', 7, '', false);
                    $pdf->SetXY(47, $tableY);
                    $pdf->MultiCell(31, 3, 'MYOP : '.trim($studentData[$IndividualDataIndex]), 'LT', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
    
                    $pdf->SetFont($timesb, '', 7, '', false);
                    $pdf->SetXY(77, $tableY);
                    $pdf->MultiCell(31, 3, 'STATUS : '.trim($studentData[$IndividualDataIndex]), 'LTR', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdf->Ln();
                    ...
                    $pdf->SetFont($timesb, '', 7, '', false);
                    $pdf->SetXY(108, $tableYV1);
                    $pdf->MultiCell(31, 3, 'SEMESTER-'.$term, 'LT', 'L', 0, 0, '', '', true, 0, true);
    
                    $pdf->SetFont($timesb, '', 7, '', false);
                    $pdf->SetXY(135, $tableYV1);
                    $pdf->MultiCell(31, 3, 'MYOP : '.trim($studentData[$IndividualDataIndex]), 'LT', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
    
                    $pdf->SetFont($timesb, '', 7, '', false);
                    $pdf->SetXY(165, $tableYV1);
                    $pdf->MultiCell(31, 3, 'STATUS : '.trim($studentData[$IndividualDataIndex]), 'LTR', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdf->Ln();
            ...
            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY(118, 302);
            $pdf->MultiCell(38, 3, 'OVERALL', 1, 'C', 0, 0, '', '', true, 0, true);
            $pdf->Ln();
            $pdf->SetFont($timesb, '', 7, '', false);
            $pdf->SetXY(118, $pdf->GetY());
            $pdf->MultiCell(13, 3.5, 'CGPA', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(25, 3.5, trim($final_cgpa), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->Ln();
            $pdf->SetFont($timesb, '', 7, '', false);
            $pdf->SetXY(118, $pdf->GetY());
            $pdf->MultiCell(13, 3.5, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(25, 3.5, trim($final_result), 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($timesb, '', 7, '', false);  
            // get the number of lines
            $lines = $pdf->MultiCell(25, 0, trim($division), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            $pdf=$pdf->rollbackTransaction(); // restore previous object
            ...
            $pdf->SetFont($timesb, '', 7, '', false);
            $pdf->SetXY(118, $pdf->GetY());
            $pdf->MultiCell(13,$height, 'DIVISION', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(25,$height, trim($division), 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($timesb, '', 7, '', false);
            $pdf->SetXY(61.5, 308);
            $pdf->MultiCell(40, 0, 'CHECKED BY:', 0, 'L', 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($timesb, '', 7, '', false);
            $pdf->SetXY(71, 331.5);
            $pdf->MultiCell(70, 0, 'ISSUE DATE : '.$issue_date, 0, 'L', 0, 0, '', '', true, 0, true);

            $pdf->SetXY(115, 331.5);
            $pdf->MultiCell(70, 0, 'PLACE : '.$place, 0, 'L', 0, 0, '', '', true, 0, true);
