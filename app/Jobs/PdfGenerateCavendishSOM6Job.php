...
class PdfGenerateCavendishSOM6Job
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $log_serial_no = 1;
        //$cardDetails=$this->getNextCardNo('CAVENDISH-SOM-6');
        ...
        $excelFolderPath = public_path().'\\'.$subdomain[0].'\backend\upload_excel\som_6_year\excel';
        ...
        foreach ($files as $file) 
        {
            ...
            $records = $this->getArrayFromSOM6Excel($sheet1);
            ...
            $imagePath = $subdomain[0].'\backend\upload_excel\som_6_year\photo/'.$fileDetail['filename'].'.PNG';
            $pdfBig->SetXY(177, 12);
            ...
            $pdfBig->SetFont($Arial, 'B', 7, '', false);
            $pdfBig->SetXY(175, $bY5);
            $pdfBig->SetTextColor(0,0,0);
            $pdfBig->MultiCell(25, 0, 'PAGE 1 OF 2', '', 'R', 0, 0);

            $pdf->SetFont($Arial, 'B', 7, '', false);
            $pdf->SetXY(175, $bY5);
            $pdf->SetTextColor(0,0,0);
            $pdf->MultiCell(25, 0, 'PAGE 1 OF 2', '', 'R', 0, 0);
            ...
            $pdfBig->SetFont($Arial, 'B', 7, '', false);
            $pdfBig->SetXY(175, $bY5+3);
            $pdfBig->SetTextColor(0,0,0);
            $pdfBig->MultiCell(25, 0, 'PAGE 2 OF 2', '', 'R', 0, 0);

            $pdf->SetFont($Arial, '', 7, '', false);
            $pdf->SetXY(175, $bY5+3);
            $pdf->SetTextColor(0,0,0);
            $pdf->MultiCell(25, 0, 'PAGE 2 OF 2', '', 'R', 0, 0);
            ...
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'CAVENDISH-SOM-6',$admin_id,$card_serial_no);
            ...
            // \File::move($file, public_path().'\\'.$subdomain[0].'\\backend\\upload_excel\\som_6_year\\process_excel\\'.$fileDetail['basename']);

        }

        //if($previewPdf!=1){
            //$this->updateCardNo('CAVENDISH-SOM-6',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        //}
        if($previewPdf!=1){
            $this->updateCardNo('CAVENDISH-SOM-6',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        $file_name =  str_replace("/", "_",'CAVENDISH-SOM-6'.date("Ymdhms")).'.pdf';
    ...
    public function getArrayFromSOM6Excel($sheet1){
