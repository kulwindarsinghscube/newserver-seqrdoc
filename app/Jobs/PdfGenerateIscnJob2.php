...
class PdfGenerateIscnJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('iscnMScMath');
        ...
        foreach ($studentDataOrg as $studentData) {         
			...
			$Marks_Obtained = trim($studentData[19]);			
       ...
       if($previewPdf!=1){
        $this->updateCardNo('iscnMSc',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'iscnMSc'.date("Ymdhms")).'.pdf';
