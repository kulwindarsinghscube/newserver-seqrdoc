...
class pdfGenerateJntuJob1
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $total_internal_marks = trim($studentData[91]);
            $total_external_marks = trim($studentData[92]);
            $total_marks_100 = trim($studentData[93]);
            $total_credits = trim($studentData[94]);

            $total_subject = trim($studentData[95]);
            $total_appered = trim($studentData[96]);
            $total_passed = trim($studentData[97]);
          
            $agreegate_in_word =  trim($studentData[98]);
            $sgpa =  trim($studentData[99]);
            $cgpa =  trim($studentData[100]);
            $date_of_issue =  trim($studentData[101]);
            ...
            $max_width = 32; // Maximum width in mm

            // Set the initial font size
            $font_size = 17; // You can set an initial size as needed
            $pdfBig->SetFont($times, '', $font_size); 
            // Measure the text width at the initial font size
            // $textWidth = $pdfBig->GetStringWidth(': '.$appar_id);

            // // Automatically adjust font size to fit the cell width
            // if ($textWidth > $max_width) {
            //     $scalingFactor = $max_width / $textWidth; // Calculate scaling factor
            ...
            $pdfBig->SetFont($times,'', $adjustedFontSize);

            // Output the text with the final font size
            // $pdf->Cell($max_width, 10, $text, 1, 1); 
            $old_y_1 = $pdfBig->getY();
            $pdfBig->SetXY($x_axis_main_info_second+32, $pdfBig->getY()-0.8);
			$pdfBig->MultiCell($max_width+3,  $main_info_height, '<span style="font-family:'.$times.';">:  '.$appar_id.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
           
            $pdfBig->SetXY(154.5, $y_axis_main_info-1);
            $pdfBig->MultiCell($max_width,  $main_info_height, "", 1, "L", 0, 0, '', '', true, 0, true);
            $pdfBig->ln();
            ...
            $lines = $pdfBig->MultiCell(65, 0, $branch, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            ...
            if($lines>1){
                ...
                $main_info_height = $main_info_height;
            }
            ...
            $pdfBig->SetFont($times, '', $content_font_size, '', false);
            $pdfBig->Cell(2, 0, ':', 0, false, 'R');

            $content_font_size = 10;
            $pdfBig->SetFont($times, '', $content_font_size, '', false); 
			$pdfBig->MultiCell(65,  $main_info_height, $branch, 0, "L", 0, 0, '', '', true, 0, true);
            ...
            $main_info_height = 7;
            $pdfBig->SetXY(154.5, $pdfBig->getY());
            $pdfBig->ln();

            $pdfBig->startTransaction();
            $pdfBig->SetFont($times, '', 10, '', false);  
            // get the number of lines
            $lines = $pdfBig->MultiCell(65, 0, $candidate_name, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            $pdfBig=$pdfBig->rollbackTransaction(); // restore previous object
            ...
            $content_font_size = 10;
            $pdfBig->SetFont($times, '', $content_font_size, '', false); 
			$pdfBig->MultiCell(65,  $main_info_height, $candidate_name, 0, "L", 0, 0, '', '', true, 0, true);
            
            $pdfBig->SetTextColor(255,255, 0);
            $old_y=$pdfBig->getY();
            $pdfBig->SetXY(55, $pdfBig->getY()+$main_info_height-4);
            ...
            $pdfBig->SetFont($times,'', $adjustedFontSize); 
            $content_font_size = 10;
            $pdfBig->SetFont($times, '', $adjustedFontSize, '', false); 
            $old_y_1 = $pdfBig->getY();
            $pdfBig->SetXY($x_axis_main_info_second+32, $pdfBig->getY()-0.5);
            $pdfBig->MultiCell($max_width+3,  $main_info_height, '<span style="font-family:'.$times.';">:  '.$hall_ticket.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
            $pdfBig->SetXY(154.5, $pdfBig->getY());
            $pdfBig->MultiCell($max_width,  7, "", 1, "L", 0, 0, '', '', true, 0, true); 


            $fatherY = $pdfBig->getY() +$main_info_height;
            $main_info_height = 7;

            $pdfBig->SetXY($x_axis_main_info, $fatherY);
            $content_font_size = 9;
            $pdfBig->SetFont($timesb, '', $content_font_size, '', false);
            ...
            $content_font_size = 9;
            $pdfBig->SetFont($timesb, '', $content_font_size, '', false);
            $pdfBig->MultiCell(41,  $main_info_height, "MOTHER'S NAME", 0, "L", 0, 0, '', '', true, 0, true);
            $content_font_size = 10;
            $pdfBig->SetFont($times, '', $content_font_size, '', false); 
            $pdfBig->MultiCell(125.5,  $main_info_height, '<span style="font-family:'.$times.';">: '.$mother_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
            $pdfBig->ln();
            ...
            $pdfBig->MultiCell(24, 12.5, 'SUBJECT <br> CODE', 1, 'C', 0, 0, '', '', true, 0, true);    
			
            $pdfBig->setCellPaddings($left = '', $top = '4', $right = '', $bottom = '');
            $pdfBig->SetXY($pdfBig->getX(), $table_heading_y_axis);
			$pdfBig->MultiCell(66, 12.5, 'SUBJECT TITLE', 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdfBig->SetXY($pdfBig->getX(), $table_heading_y_axis);
            $pdfBig->MultiCell(13, 12.5, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);    
			...
			for ($s=0; $s < 10; $s++) {
                ...
                $table_content_height = 7;
                $pdfBig->startTransaction();
                $pdfBig->SetFont($times, '', $content_font_size, '', false);
                // get the number of lines
                $lines = $pdfBig->MultiCell(66, $table_content_height, trim($SubjectTitle), 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
                ...
                // echo "<br>";
                if($lines == 2){
                    $table_content_height1 = 11;
                }else if($lines == 3){
                    $table_content_height1 = 16;
                }else{
                ...
                $pdfBig->MultiCell(24, $table_content_height1, $SubjectCode, "RL", 'C', 0, 0, '', '', true, 0, true);    
                if( trim($SubjectCode) == '23AHSS1' ||  trim($SubjectCode) == '23AHSS2' ) {
                    $isInternal = true;
                }
                $pdfBig->SetXY($pdfBig->getX(), $table_content_y);
                $pdfBig->MultiCell(66, $table_content_height1, trim($SubjectTitle), "RL", 'L', 0, 0, '', '', true, 0, true);
                $pdfBig->SetXY($pdfBig->getX(), $table_content_y);
                if($isInternal) {
                ...
                $pdfBig->SetXY($pdfBig->getX(), $table_content_y);
                $pdfBig->MultiCell(13, $table_content_height1, $Result, "RL", 'C', 0, 0, '', '', true, 0, true);  
                $pdfBig->SetXY($pdfBig->getX(), $table_content_y);
            ...
            $pdfBig->Cell(97, 0, "SUBJECTS REGISTERED:  $total_subject    APPEARED: $total_appered    PASSED: $total_passed", 'LTB', false, 'L');  
            
            $pdfBig->setFontStretching(100);

            $old_x=$pdfBig->getX();
            $pdfBig->SetXY(10, $bottomY+4);  
            $pdfBig->SetTextColor(255, 255, 0);
            $pdfBig->MultiCell(94, $table_content_height1, $total_subject, "", 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->SetTextColor(0, 0, 0); 
            $pdfBig->SetXY($old_x, $bottomY);

 
            $pdfBig->SetXY(76, $bottomY+4);  
            $pdfBig->SetTextColor(255, 255, 0);
            $pdfBig->MultiCell(16, $table_content_height1, $total_appered, "", 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->SetTextColor(0, 0, 0); 
            $pdfBig->SetXY($old_x, $bottomY);

            $pdfBig->SetXY(96, $bottomY+4);  
            $pdfBig->SetTextColor(255, 255, 0);
            $pdfBig->MultiCell(17.5, $table_content_height1, $total_passed, "", 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->SetTextColor(0, 0, 0); 
            $pdfBig->SetXY($old_x, $bottomY);
            ...
            if($isInternal) {
                ...
                $pdfBig->Cell(180, 7, "* Internal Evaluation only for 100M", 0, false, 'L');  
                $pdfBig->ln();
            }

            $qrCodeYDef = $pdfBig->getY() -7;
            $pdfBig->SetXY(12, $pdfBig->getY());        
            ...
            $old_x=$pdfBig->getX();
            $pdfBig->SetXY(103,$pdfBig->getY()+1);  
            $pdfBig->SetTextColor(255, 255, 0);
            ...
            $pdfBig->SetFont($times, '', $content_font_size, '', false);
            $pdfBig->SetXY(10, 283);   
            $pdfBig->Cell(110, 0, "Note:Any discrepancy must be reprensented within 15 days from the date of issue. ", 0, false, 'L');  
            $pdfBig->SetXY(120, 283);   
            $pdfBig->MultiCell(60, 0, 'MP:MALPRACTICE', 0, "C", 0, 0, '', '', true, 0, true);
            ...
            // Measure the text width at the initial font size
            // $textWidth = $pdf->GetStringWidth(': '.$appar_id);
            ...
            $adjustedFontSize =13 ;
            // Set the adjusted font size
            $pdf->SetFont($times,'', $adjustedFontSize);

            // Output the text with the final font size
            // $pdf->Cell($max_width, 10, $text, 1, 1); 
            $old_y_1 = $pdf->getY();
            $pdf->SetXY($x_axis_main_info_second+32, $pdf->getY()-0.8);
            $pdf->MultiCell($max_width+3,  $main_info_height, '<span style="font-family:'.$times.';">:  '.$appar_id.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
           
            $pdf->SetXY(154.5, $y_axis_main_info-1);
            $pdf->MultiCell($max_width,  $main_info_height, "", 1, "L", 0, 0, '', '', true, 0, true);
            $pdf->ln();
            ...
            // get the number of lines
            $lines = $pdf->MultiCell(65, 0, $branch, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            ...
            $pdf->SetFont($times, '', $content_font_size, '', false);
            $pdf->Cell(2, 0, ':', 0, false, 'R');

            $content_font_size = 10;
            $pdf->SetFont($times, '', $content_font_size, '', false); 
            $pdf->MultiCell(65,  $main_info_height, $branch, 0, "L", 0, 0, '', '', true, 0, true);
            ...
            $main_info_height = 7;
            $pdf->SetXY(154.5, $pdf->getY());
            $pdf->ln();

            $pdf->startTransaction();
            $pdf->SetFont($times, '', 10, '', false);  
            // get the number of lines
            $lines = $pdf->MultiCell(65, 0, $candidate_name, 0, 'L', 0, 0, '', '', true, 0, false,true, 0);
            $pdf=$pdf->rollbackTransaction(); // restore previous object
            ...
            $content_font_size = 10;
            $pdf->SetFont($times, '', $content_font_size, '', false); 
            $pdf->MultiCell(65,  $main_info_height, $candidate_name, 0, "L", 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetFont($times,'', $adjustedFontSize); 
            $content_font_size = 10;
            $pdf->SetFont($times, '', $adjustedFontSize, '', false); 
            $old_y_1 = $pdf->getY();
            $pdf->SetXY($x_axis_main_info_second+32, $pdf->getY()-0.5);
            $pdf->MultiCell($max_width+3,  $main_info_height, '<span style="font-family:'.$times.';">:  '.$hall_ticket.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
            $pdf->SetXY(154.5, $pdf->getY());
            $pdf->MultiCell($max_width,  7, "", 1, "L", 0, 0, '', '', true, 0, true); 


            $fatherY = $pdf->getY() +$main_info_height;
            $main_info_height = 7;

            $pdf->SetXY($x_axis_main_info, $fatherY);
            $content_font_size = 9;
            $pdf->SetFont($timesb, '', $content_font_size, '', false);
            ...
            $content_font_size = 9;
            $pdf->SetFont($timesb, '', $content_font_size, '', false);
            $pdf->MultiCell(41,  $main_info_height, "MOTHER'S NAME", 0, "L", 0, 0, '', '', true, 0, true);
            $content_font_size = 10;
            $pdf->SetFont($times, '', $content_font_size, '', false); 
            $pdf->MultiCell(125.5,  $main_info_height, '<span style="font-family:'.$times.';">: '.$mother_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
            $pdf->ln();
            ...
            $pdf->MultiCell(24, 12.5, 'SUBJECT <br> CODE', 1, 'C', 0, 0, '', '', true, 0, true);    
			
            $pdf->setCellPaddings($left = '', $top = '4', $right = '', $bottom = '');
            $pdf->SetXY($pdf->getX(), $table_heading_y_axis);
			$pdf->MultiCell(66, 12.5, 'SUBJECT TITLE', 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            $pdf->SetXY($pdf->getX(), $table_heading_y_axis);
            $pdf->MultiCell(13, 12.5, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);    
			...
			for ($s=0; $s < 11; $s++) {
                ...
                $pdf->SetFont($times, '', $content_font_size, '', false);
                // get the number of lines
                $lines = $pdf->MultiCell(66, $table_content_height, $SubjectTitle, 0, 'C', 0, 0, '', '', true, 0, false,true, 0);
                // restore previous object
                ...
                // echo "<br>";
                if($lines == 2){
                    $table_content_height1 = 10;
                }else if($lines == 3){
                    $table_content_height1 = 14;
                }else{
                    $table_content_height1 = $table_content_height;
                }
                ...
                $pdf->MultiCell(24, $table_content_height1, $SubjectCode, "RL", 'C', 0, 0, '', '', true, 0, true); 
                if( trim($SubjectCode) == '23AHSS1' ||  trim($SubjectCode) == '23AHSS2' ) {
                    $isInternal = true;
                }

                $pdf->SetXY($pdf->getX(), $table_content_y);
                $pdf->MultiCell(66, $table_content_height1, $SubjectTitle, "RL", 'L', 0, 0, '', '', true, 0, true);
                $pdf->SetXY($pdf->getX(), $table_content_y);
                if($isInternal) {
                ...
                $pdf->SetXY($pdf->getX(), $table_content_y);
                $pdf->MultiCell(13, $table_content_height1, $Result, "RL", 'C', 0, 0, '', '', true, 0, true);  
                $pdf->SetXY($pdf->getX(), $table_content_y);
            ...
            $pdf->Cell(97, 0, "SUBJECTS REGISTERED:  $total_subject    APPEARED: $total_appered    PASSED: $total_passed", 'LTB', false, 'L');  
            $pdf->setFontStretching(90);
            ...
            if($isInternal) {
                ...
                $pdf->Cell(180, 7, "* Internal Evaluation only for 100M", 0, false, 'L');  
                $pdf->ln();
            }
            ...
            $pdf->SetFont($times, '', $content_font_size, '', false); 
            $pdf->SetXY(10, 283);   
            $pdf->Cell(110, 0, "Note:Any discrepancy must be reprensented within 15 days from the date of issue. ", 0, false, 'L');  
            $pdf->SetXY(120, 283);   
            $pdf->MultiCell(60, 0, 'MP:MALPRACTICE', 0, "C", 0, 0, '', '', true, 0, true);

            $pdf->SetXY(180, 283);   
            $pdf->Cell(20, 0, "AB:ABSENT", 0, false, 'L'); 
