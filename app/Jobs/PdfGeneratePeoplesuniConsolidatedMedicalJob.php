...
class PdfGeneratePeoplesuniConsolidatedMedicalJob
{
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            foreach($subjects as $term => $term_array){
                ...
                if($term == 'I' || $term == 'III' || $term == 'V') {
                    ...
                    $pdfBig->SetFont($times, '', 7, '', false);
                    $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
                    $pdfBig->MultiCell(28, 3, 'CGPA :'.trim($studentData[$IndividualDataIndex]), 'TRB', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdfBig->Ln();
                    $tableY = $pdfBig->GetY();
            ...
            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY(153, 248);
            $pdfBig->MultiCell(40, 3, 'OVERALL', 1, 'C', 0, 0, '', '', true, 0, true);
            $pdfBig->Ln();
            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY(153, $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, 'CGPA', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, trim($studentData[35]), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->Ln();
            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY(153, $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, trim($studentData[36]), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->Ln();
            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY(153, $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, 'DIVISION', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdfBig->SetFont($times, '', 7, '', false);
            $pdfBig->SetXY($pdfBig->GetX(), $pdfBig->GetY());
            $pdfBig->MultiCell(20, 3, trim($studentData[37]), 1, 'C', 0, 0, '', '', true, 0, true);
            ...
            foreach($subjects as $term => $term_array){
                ...
                if($term == 'I' || $term == 'III' || $term == 'V') {
                    ...
                    $pdf->SetFont($times, '', 7, '', false);
                    $pdf->SetXY($pdf->GetX(), $pdf->GetY());
                    $pdf->MultiCell(28, 3, 'CGPA :'.trim($studentData[$IndividualDataIndex]), 'TRB', 'L', 0, 0, '', '', true, 0, true);
                    $IndividualDataIndex++; 
                    $pdf->Ln();
                    $tableY = $pdf->GetY();
            ...
            $pdf->MultiCell(20, 3, 'CGPA', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(20, 3, trim($studentData[35]), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->Ln();
            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY(153, $pdf->GetY());
            $pdf->MultiCell(20, 3, 'RESULT', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(20, 3, trim($studentData[36]), 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->Ln();
            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY(153, $pdf->GetY());
            $pdf->MultiCell(20, 3, 'DIVISION', 1, 'C', 0, 0, '', '', true, 0, true);

            $pdf->SetFont($times, '', 7, '', false);
            $pdf->SetXY($pdf->GetX(), $pdf->GetY());
            $pdf->MultiCell(20, 3, trim($studentData[37]), 1, 'C', 0, 0, '', '', true, 0, true);
            // pdf end
