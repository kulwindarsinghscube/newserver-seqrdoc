...
class PdfGenerateCSCACSJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('CsscaC');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="CSCACS_BG.jpg"; // bestiu_bg, bestiu_bg_data
            ...
            $PHOTO           = trim($studentData[8]); // PHOTO
                ...
                //Start pdfBig
                $pdfBig->SetFont($arialb, '', 27, '', false);
                $pdfBig->SetXY(12, 61);
                $pdfBig->Cell(395, 0, 'PASSING CERTIFICATE', 0, false, 'C');


                $pdfBig->SetXY(20, 63);
                $pdfBig->SetFont($arial, '', 15);
                $pdfBig->Cell(25, 0, 'REF.No: ', 0, 0, 'L');
                $pdfBig->SetFont($arial, 'B', 15);
                $pdfBig->Cell(0, 0, $serial_no, 0, 0, 'L');

                $pdfBig->SetXY(20, 74);
                $pdfBig->SetFont($arial, '', 15);
                $pdfBig->Cell(25, 0, 'Seat No: ', 0, 0, 'L');
                $pdfBig->SetFont($arial, 'B', 15);
                $pdfBig->Cell(0, 0, $unique_id, 0, 0, 'L');
                ...
                // }
                /* ===== Certificate line ===== */
                $pdfBig->SetFont($arial, '', 18, '', false);   // normal font
                $pdfBig->SetXY(0, 120);                        // ⬆ above candidate name
                $pdfBig->writeHTMLCell(420, 0, 0, 120, 'This is to certify that', 0, 0, false, true, 'C', true);

                /* ===== Candidate Name ===== */
                $pdfBig->SetFont($arialb, '', 20, '', false);
                $pdfBig->SetXY(12, 135);
                $pdfBig->writeHTMLCell(430, 0, 0, 135, $candidate_name, 0, 0, false, true, 'C', true);

                /* ===== Line 1: "has passed …" with bold degree ===== */
                $pdfBig->SetFont($arial, '', 21, '', false);
                $pdfBig->SetXY(0, 150);
                $htmlLine1 = 'has passed <b>' . $degree . '</b>';
                $pdfBig->writeHTMLCell(410, 0, 0, 150, $htmlLine1, 0, 0, false, true, 'C', true);

                /* ===== Line 2: Static text ===== */
                $pdfBig->SetFont($arial, '', 21, '', false);
                $pdfBig->SetXY(0, 165);

                $htmlLine = 'Examination conducted by <b>Chandrabhan Sharma College of Arts, Commerce & Science (Autonomous)</b>';

                $pdfBig->writeHTMLCell(420.5, 0, 0, 165, $htmlLine, 0, 0, false, true, 'C', true);


                /* ===== Line 3: Affiliation with bold dynamic values ===== */
                $pdfBig->SetFont($arial, '', 21, '', false);
                $pdfBig->SetXY(0, 180);
                $htmlLine1 = '<b>Affiliated to University of Mumbai</b> in the month of <b>' . $completion_date . '</b> and is placed in Grade <b>' . $average . '</b> with <b>' . $place . '</b> CGPA.';
                $pdfBig->writeHTMLCell(430, 0, 0, 180, $htmlLine1, 0, 0, false, true, 'C', true);



                /* ===== Degree ===== */
                $pdfBig->SetFont($arialb, '', 32, '', false);
                $pdfBig->SetTextColor(61, 61, 157);
                $pdfBig->SetXY(12, 210);   // ⬇ moved down
                
                $pdfBig->SetFont($arialb, '', 21, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                ...
                $y = 232;
                $pdfBig->SetFont($arial, '', 16, '', false);
                $pdfBig->SetTextColor(0, 0, 0);
                $pdfBig->SetXY(32, $y);
                $pdfBig->Cell(0, 0, 'Date:', 0, false, 'L');
                $pdfBig->SetFont($arialb, '', 16, '', false); // Bold font
                $pdfBig->SetXY(47, $y);
                $pdfBig->Cell(0, 0, $date, 0, false, 'L');

                

                $y = 245; // adjust this number only

                $pdfBig->SetXY(120, $y);
                $pdfBig->Cell(20, 0, 'Controller of Examinations', 0, false, 'L');

                $pdfBig->SetXY(270, $y - 1); // tiny offset if needed
                ...
                //start pdf
                 //Start pdf
                $pdf->SetFont($arialb, '', 27, '', false);
                $pdf->SetXY(12, 61);
                $pdf->Cell(395, 0, 'PASSING CERTIFICATE', 0, false, 'C');


                $pdf->SetXY(20, 63);
                $pdf->SetFont($arial, '', 15);
                $pdf->Cell(25, 0, 'REF.No: ', 0, 0, 'L');
                $pdf->SetFont($arial, 'B', 15);
                $pdf->Cell(0, 0, $serial_no, 0, 0, 'L');

                $pdf->SetXY(20, 74);
                $pdf->SetFont($arial, '', 15);
                $pdf->Cell(25, 0, 'Seat No: ', 0, 0, 'L');
                $pdf->SetFont($arial, 'B', 15);
                $pdf->Cell(0, 0, $unique_id, 0, 0, 'L');




                // if($AadharNumber) {
                //     $pdf->SetFont($arial, '', 15, '', false); 
                //     $pdf->SetTextColor(0, 0, 0);    
                //     $pdf->SetXY(297, 60.5);
                //     $pdf->Cell(107, 0, 'Aadhar No.: '.$AadharNumber, 0, false, 'R');
                // }
                /* ===== Certificate line ===== */
                $pdf->SetFont($arial, '', 18, '', false);   // normal font
                $pdf->SetXY(0, 120);                        // ⬆ above candidate name
                $pdf->writeHTMLCell(420, 0, 0, 120, 'This is to certify that', 0, 0, false, true, 'C', true);

                /* ===== Candidate Name ===== */
                $pdf->SetFont($arialb, '', 20, '', false);
                $pdf->SetXY(12, 135);
                $pdf->writeHTMLCell(430, 0, 0, 135, $candidate_name, 0, 0, false, true, 'C', true);

                /* ===== Line 1: "has passed …" with bold degree ===== */
                $pdf->SetFont($arial, '', 21, '', false);
                $pdf->SetXY(0, 150);
                $htmlLine1 = 'has passed <b>' . $degree . '</b>';
                $pdf->writeHTMLCell(410, 0, 0, 150, $htmlLine1, 0, 0, false, true, 'C', true);
                ...
                $pdf->writeHTMLCell(420.5, 0, 0, 165, $htmlLine, 0, 0, false, true, 'C', true);
                ...
                $pdf->SetXY(0, 180);
                $htmlLine1 = '<b>Affiliated to University of Mumbai</b> in the month of <b>' . $completion_date . '</b> and is placed in Grade <b>' . $average . '</b> with <b>' . $place . '</b> CGPA.';
                $pdf->writeHTMLCell(430, 0, 0, 180, $htmlLine1, 0, 0, false, true, 'C', true);



                /* ===== Degree ===== */
                $pdf->SetFont($arialb, '', 32, '', false);
                $pdf->SetTextColor(61, 61, 157);
                $pdf->SetXY(12, 210);   // ⬇ moved down
                ...
                $y = 232;
                $pdf->SetFont($arial, '', 16, '', false);
                $pdf->SetTextColor(0, 0, 0);
                $pdf->SetXY(32, $y);
                $pdf->Cell(0, 0, 'Date:', 0, false, 'L');
                $pdf->SetFont($arialb, '', 16, '', false); // Bold font
                $pdf->SetXY(47, $y);
                $pdf->Cell(0, 0, $date, 0, false, 'L');
                ...
                $pdf->SetXY(270, $y - 1); // tiny offset if needed
       ...
       if($previewPdf!=1){
        $this->updateCardNo('CsscaC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'CsscaC'.date("Ymdhms")).'.pdf';
