...
class TemplateBackJob
{
    ...
    public function handle(Request $request,CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        /**Back Side Canvas**/
        if($subdomain[0]=="demo"&&isset($_POST['is_back_template'])&&$_POST['is_back_template']==1){
            $save_template_data->is_back_template = $get_template_data['is_back_template'];
            $save_template_data->back_bg_template_id = (int)$get_template_data['back_bg_template_id'];
        }
        /**Back Side Canvas**/
        ...
        if(isset($get_template_data['template_id'])){
            ...
            FieldMasterBackTemplate::where('template_id',$id)->delete();
        }

        // Comment For Back Side QR And Barcode Field

        //save data in field master of qr code By Rohit
        if(isset($get_template_data['field_qr_x'])){
            $save_field_qr_data = new FieldMasterBackTemplate();
        ...
        if(isset($get_template_data['field_id_x'])){
            $save_field_barcode_data = new FieldMasterBackTemplate();
        ...
        // Comment For Back Side QR And Barcode Field By Rohit
        ...
        if(isset($get_template_data['field_extra_name'])){
            ...
            for ($i=0; $i <$count ; $i++) { 
                $save_extra_field_data = new FieldMasterBackTemplate();
                //set field position
                $pos = $i + 3;
                //for text
        ...
        if($subdomain[0]=="demo"&&isset($_POST['is_block_chain'])&&$_POST['is_block_chain']==1){
            ...
            // CoreHelper::rohitCheckContactAddress($save_template_data->id,'NORMALTEMPLATE');
        }
