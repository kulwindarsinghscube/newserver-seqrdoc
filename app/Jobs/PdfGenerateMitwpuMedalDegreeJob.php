...
class PdfGenerateMitwpuMedalDegreeJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('MitwpuMedal');
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $student_photo = trim($studentData[13]);
                $faculty_name = trim($studentData[14]);
                $faculty_name_hindi = trim($studentData[15]);
                $faculty_name_krutidev = trim($studentData[16]);
                $faculty_name_krutidev = htmlspecialchars($faculty_name_krutidev);

                $certificate_id = trim($studentData[17]);
                $completion_date = trim($studentData[18]);
                $completion_date_krutidev = trim($studentData[19]);
                $completion_date_krutidev = htmlspecialchars($completion_date_krutidev);

                
                // $issue_date = trim($studentData[20]);
                // $issue_date_krutidev = trim($studentData[21]);
                ...
                $medal_type = trim($studentData[22]);
                $medal_type_krutidev = trim($studentData[23]);
                $medal_type_krutidev = htmlspecialchars($medal_type_krutidev);

                $specialization = trim($studentData[24]);
                $specialization_hindi = trim($studentData[25]);
                $specialization_krutidev = trim($studentData[26]);
                ...
                // if(stripos($course_name_krutidev, 'fMIyksek') !== FALSE){
                //     $pdfBig->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ls ;g izekf.kr fd;k tkrk gS 
                //     fd mUgsa] ".$medal_type_krutidev." ind iznku fd;k tkrk gSA", 0, 'C', 0, 0, '', '', true, 0, true);
                // } else {
                    if($specialization_krutidev) {
                        $pdfBig->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ls ;g izekf.kr fd;k tkrk gS 
                        fd ".$specialization_krutidev." Lis'kykbts'ku esa mUgsa] ".$medal_type_krutidev." ind iznku fd;k tkrk gSA", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdfBig->SetFont($KRDEV100, '', 16.02, '', false);
                $pdfBig->SetXY(15, $pdfBig->GetY()+2);
                $pdfBig->MultiCell(182, 0, "bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // if(stripos($course_name, 'Diploma') !== FALSE){
                //     $pdfBig->MultiCell(190, 0, " with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                ...
                // } else {
                    if($specialization) {
                        $pdfBig->MultiCell(190, 0, $specilaText." with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                        <br>In recognition we have hereunder placed our names and
                        <br>the seal of the University on this ".$issue_formatted_date.".", 0, 'C', 0, 0, '', '', true, 0, true);
                    } else {
                        $pdfBig->MultiCell(190, 0, " with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                ...
                // if(stripos($course_name_krutidev, 'fMIyksek') !== FALSE){
                //     $pdf->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ls ;g izekf.kr fd;k tkrk gS 
                //     fd mUgsa] ".$medal_type_krutidev." ind iznku fd;k tkrk gSA", 0, 'C', 0, 0, '', '', true, 0, true);
                // } else {
                    if($specialization_krutidev) {
                        $pdf->MultiCell(182, 0, "dh mikf/k iznku dh tkrh gSA ".$completion_date_krutidev." esa gqbZ ijh{kk ls ;g izekf.kr fd;k tkrk gS 
                ...
                $pdf->SetFont($KRDEV100, '', 16.02, '', false);
                $pdf->SetXY(15, $pdf->GetY()+2);
                $pdf->MultiCell(182, 0, "bls ekU;rk nsrs gq, vkt ".$issue_date_krutidev." ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);


                // $pdf->SetTextColor(50,50,154);
                // $pdf->SetFont($KRDEV100, '', 16.02, '', false);
                // $pdf->SetXY(15, 113);
                // $pdf->MultiCell(180, 0, "bls ekU;rk nsrs gq, vkt 04 uoacj 2023 ds volj ij ge vius uke rFkk fo'ofo|ky; dh eksgj ds vf/kdkjkuqlkj bls laiUu djrs gSaA", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // if(stripos($course_name, 'Diploma') !== FALSE){
                    // $pdf->MultiCell(190, 0, " with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                    //     <br>In recognition we have hereunder placed our names and
                    //     <br>the seal of the University on this ".$issue_formatted_date.".", 0, 'C', 0, 0, '', '', true, 0, true);
                // } else {
                    if($specialization) {
                        $pdf->MultiCell(190, 0, $specilaText." with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                        <br>In recognition we have hereunder placed our names and
                        <br>the seal of the University on this ".$issue_formatted_date.".", 0, 'C', 0, 0, '', '', true, 0, true);
                    } else {
                        $pdf->MultiCell(190, 0, " with ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                ...
                // $pdf->MultiCell(190, 0, "In ".$specialization." With ".$medal_type." Medal secured in the examination held in ".$completion_date_new.".
                    // <br>In recognition we have hereunder placed our names and
                    // <br>the seal of the University on this ".$issue_formatted_date.".", 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $last_prn = str_replace('MEDAL_'.$year.'_', '', $PRN);
                ...
                $codeContents = $full_name.','.$faculty_name.','.$course_name.','.$medal_type.','.$completion_date_new.','.$PRN;
                ...
                if($previewPdf!=1){
                    ...
                    $mintData['metadata4']=["label"=> "Medal", "value"=> $medal_type];
        ...
        //if($previewPdf!=1){
            // $this->updateCardNo('MitwpuMedal',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        //}
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MitwpuMedal'.date("Ymdhms")).'.pdf';
...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id,$mintData)
    {
        ...
        if($systemConfig['sandboxing'] == 1){
            ...
            $result = StudentTable::create(['serial_no'=>$serial_no,'certificate_filename'=>$certName,'template_id'=>$template_id,'key'=>$key,'path'=>$urlRelativeFilePath,'created_at'=>$datetime,'created_by'=>$ses_id,'updated_at'=>$datetime,'updated_by'=>$ses_id,'status'=>$sts,'site_id'=>$auth_site_id,'template_type'=>2,'certificate_type'=>'Medal Degree Certificate' ,'bc_txn_hash'=>$bc_txn_hash,'bc_ipfs_hash'=>$bc_ipfs_hash,'pinata_ipfs_hash'=>$pinata_ipfs_hash,'bc_sc_id' => $bc_sc_id, 'bc_file_hash'=>$bc_file_hash]);
