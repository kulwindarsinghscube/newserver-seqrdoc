...
use Helper;
class PdfGenerateBestiuEDocumentJob8
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="bestiu_edc_bg.jpg"; // bestiu_pdc_bg, bestiu_pdc_bg_data
                $low_res_bg="bestiu_edc_bg.jpg";
                $pdfBig->AddPage();
                ...
                // $program_name = trim($studentData[3]);
                $degree = trim($studentData[3]);        
                $completion_date = trim($studentData[5]);
                $average = trim($studentData[4]);
                $word = trim($studentData[6]);
                ...
                $pdfBig->SetTextColor(55, 53, 157);       
                $pdfBig->SetXY(12, 79);
                $pdfBig->Cell(186, 0, 'Certificate of Completion', 0, false, 'C');
                $y=$pdfBig->getY();

                $pdfBig->SetFont($arialb, '', 13, '', false);
                $pdfBig->SetTextColor(0, 0, 0);   
                $pdfBig->SetXY(18, $y+30);         
                $pdfBig->MultiCell(176, 0, 'On the recommendation of the Academic Council and the Board of Studies' , 0, 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($oef, '', 13, '', false); 
                $pdfBig->SetXY(12, $y+45);			
                $pdfBig->Cell(186, 0, 'This is to certify that', 0, false, 'C');

                $pdfBig->SetFont($arialMTblack, '', 18, '', false);
                $pdfBig->SetXY(10, $y+55);
                $pdfBig->MultiCell(190, 0, $candidate_name , 0, 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($arialb, '', 13, '', false); 
                $pdfBig->SetXY(10, $y+70);
                $pdfBig->Cell(190, 0, $word, 0, false, 'C');

                $pdfBig->SetFont($oef, '', 18.5, '', false); 
                $pdfBig->SetTextColor(55, 53, 157);
                $pdfBig->SetXY(18, $y+85);
                $pdfBig->MultiCell(176, 0, $degree, 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetFont($arialb, '', 13, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);  
                $pdfBig->SetXY(12, $y+105);
                $pdfBig->MultiCell(186, 0, 'On '.$completion_date , 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdfBig->SetXY(10, $y+120); 
                $pdfBig->MultiCell(190, 0, 'With a Cumulative Grade Point Average of ', 0, 'C', 0, 0, '', '', true, 0, true);	

                $pdfBig->SetXY(10, $y+130); 
                $pdfBig->MultiCell(190, 0,$average.'/10', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetTextColor(55, 53, 157);       
                $pdf->SetXY(12, 79);
                $pdf->Cell(186, 0, 'Certificate of Completion', 0, false, 'C');
                $y=$pdf->getY();

                $pdf->SetFont($arialb, '', 13, '', false);
                $pdf->SetTextColor(0, 0, 0);   
                $pdf->SetXY(18, $y+30);         
                $pdf->MultiCell(176, 0, 'On the recommendation of the Academic Council and the Board of Studies' , 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $pdf->SetFont($arialb, '', 13, '', false); 
                $pdf->SetXY(10, $y+70);
                $pdf->Cell(190, 0, $word, 0, false, 'C');

                $pdf->SetFont($oef, '', 18.5, '', false); 
                $pdf->SetTextColor(55, 53, 157);
                $pdf->SetXY(18, $y+85);
                $pdf->MultiCell(176, 0, $degree, 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetFont($arialb, '', 13, '', false); 
                $pdf->SetTextColor(0, 0, 0);  
                $pdf->SetXY(12, $y+105);
                $pdf->MultiCell(186, 0, 'On '.$completion_date , 0, 'C', 0, 0, '', '', true, 0, true);
                
                $pdf->SetXY(10, $y+120); 
                $pdf->MultiCell(190, 0, 'With a Cumulative Grade Point Average of ', 0, 'C', 0, 0, '', '', true, 0, true);   

                $pdf->SetXY(10, $y+130); 
                $pdf->MultiCell(190, 0,$average.'/10', 0, 'C', 0, 0, '', '', true, 0, true); 			           
                //end pdf			
                
                // Ghost image
               // $candidate_name_u = substr($candidate_name, strpos($candidate_name, ".") + 1);
                $clean = preg_replace("/[^A-Za-z0-9]/", "", $candidate_name);//updated by Mandar
                $nameOrg=$clean;
       ...
       $msg = '';
        ...
        $file_name =  str_replace("/", "_",'BestiuDC'.date("Ymdhms")).'.pdf';
