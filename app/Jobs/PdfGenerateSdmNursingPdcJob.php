...
use Helper;
class PdfGenerateSdmNursingPdcJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $sitka = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\SitkaVF.ttf', 'TrueTypeUnicode', '', 96);
        $sitkab = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Sitka Small Bold.ttf', 'TrueTypeUnicode', '', 96);
        $diploma = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Diploma.ttf', 'TrueTypeUnicode', '', 96);
        $MonotypeCorsiva = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Monotype Corsiva.ttf', 'TrueTypeUnicode', '', 96);
		...
		//$card_serial_no="";
        $log_serial_no = 1;
        $cardDetails=$this->getNextCardNo('sdmPDC');
        ...
        foreach ($studentDataOrg as $studentData) {
			...
			$high_res_bg="SDM_University_BG.jpg"; // SDM_University_BG.jpg, SDM_PDC
			...
			$Registration_No = trim($studentData[0]);
			$candidate_name = trim($studentData[1]);
			$Programme_Name = trim($studentData[2]);
			$Examination_held_in = trim($studentData[3]);        
			$Degree = trim($studentData[4]);
			$PDC_No = trim($studentData[5]);
			$Date = trim($studentData[6]);
			$Student_Image = trim($studentData[7]);
			
			$batch_of_qr = trim($studentData[8]);
			$aadhar_number_qr = trim($studentData[9]);
			$course_qr = strtoupper(trim($studentData[10]));
			$year_qr = strtoupper(trim($studentData[11]));
			$DOB = trim($studentData[12]);
			...
			$pdfBig->SetTextColor(62, 64, 149);
			$pdfBig->SetFont($sitkab, '', 14, '', false);
            $pdfBig->SetXY(11.4, 51.5);
			$pdfBig->MultiCell(188, 0, 'SHRI DHARMASTHALA MANJUNATHESHWARA UNIVERSITY', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($sitka, '', 14, '', false);
            $pdfBig->SetXY(11.4, 59);
			$pdfBig->MultiCell(188, 0, 'Manjushree Building, Sattur, Dharwad-580009', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(158, 44, 44);
			$pdfBig->SetFont($diploma, '', 24, '', false);			
            $pdfBig->SetXY(11.4, 78);
			$pdfBig->MultiCell(188, 0, 'Provisional Degree Certificate', 0, 'C', 0, 0, '', '', true, 0, true);
            
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdfBig->SetXY(11.4, 98.5);
			$pdfBig->MultiCell(188, 0, 'This is to certify that', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(242, 47, 53);
			$pdfBig->SetFont($calibrib, '', 16, '', false);			
            $pdfBig->SetXY(11.4, 117);
			$pdfBig->MultiCell(188, 0, $candidate_name, 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdfBig->SetXY(10.9, 134);
			$pdfBig->MultiCell(188, 0, 'has passed', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(62, 64, 149);
			$pdfBig->SetFont($calibrib, '', 14, '', false);
            $pdfBig->SetXY(11.4, 151);
			$pdfBig->MultiCell(188, 0, $Programme_Name, 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdfBig->SetXY(11.4, 165.5);
			$pdfBig->MultiCell(188, 0, 'examination held in <span style="font-family:'.$calibri.';font-size:14;">'.$Examination_held_in.'</span> with Registration Number <span style="font-family:'.$calibri.';font-size:14;">'.$Registration_No.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
            
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdfBig->SetXY(11.2, 180.5);
			$pdfBig->MultiCell(188, 0, '(Valid till the Original Degree Certificate is issued)', 0, 'C', 0, 0, '', '', true, 0, true);
			/*
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdfBig->SetXY(11.2, 193.5);
			$pdfBig->MultiCell(188, 0, 'He/She is eligible for the award of the degree', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetTextColor(62, 64, 149);
			$pdfBig->SetFont($calibrib, '', 14, '', false);			
			$pdfBig->SetXY(11.2, 207);
			$pdfBig->MultiCell(188, 0, $Degree, 0, 'C', 0, 0, '', '', true, 0, true);
			*/
			...
			$pdfBig->SetXY(43, 215);
			$pdfBig->MultiCell(125.5, 7, 'Batch Of:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$batch_of_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(43, 220);
			$pdfBig->MultiCell(125.5, 7, 'Name of the Student:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$candidate_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(43, 225);
			$pdfBig->MultiCell(125.5, 7, 'Aadhar Number:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$aadhar_number_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(43, 230);
			$pdfBig->MultiCell(125.5, 7, 'Course:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$course_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(43, 235);
			$pdfBig->MultiCell(125.5, 7, 'Year:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$year_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(43, 240);
			$pdfBig->MultiCell(125.5, 7, 'Date Of Birth:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$DOB.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			*/		
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(135, 240);
			$pdfBig->SetFont($calibri, '', 12, '', false);
			$pdfBig->MultiCell(60, 0, 'PDC NO.: '.$PDC_No, 0, 'R', 0, 0, '', '', true, 0, true);
			
			$pdfBig->SetXY(11, 265);
			$pdfBig->SetFont($calibri, '', 11, '', false);	
			$pdfBig->MultiCell(0, 0, 'Date: <span style="color:#e83339;">'.$Date.'</span>', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdfBig->SetXY(11, 270.5);
			$pdfBig->SetFont($calibri, '', 11, '', false);
			$pdfBig->MultiCell(0, 0, 'Dharwad',0, 'L', 0, 0);
			
			$pdfBig->SetXY(144, 269.5);
			$pdfBig->SetFont($arialNarrow, '', 1, '', false);
			$pdfBig->MultiCell(47.5, 1, '', 'T', 'C', 0, 0);
			$pdfBig->SetFont($calibri, '', 12, '', false);
			$pdfBig->SetXY(135, 270);
			$pdfBig->MultiCell(0, 0, 'Controller of Examinations', 0, 'C', 0, 0);
			
			$pdfBig->SetFont($MyriadProRegular, '', 6, '', false);
			$pdfBig->SetXY(51.5, 283);
			//$pdfBig->MultiCell(92, 0, "SDM College of Dental Sciences and Hospital, Sattur, Dharwad–580009, Karnataka, India",0, 'L', 0, 0);			
			//End pdfBig 
			
            //Start pdf
			
			$pdf->SetTextColor(62, 64, 149);
			$pdf->SetFont($sitkab, '', 14, '', false);
            $pdf->SetXY(11.4, 51.5);
			$pdf->MultiCell(188, 0, 'SHRI DHARMASTHALA MANJUNATHESHWARA UNIVERSITY', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($sitka, '', 14, '', false);
            $pdf->SetXY(11.4, 59);
			$pdf->MultiCell(188, 0, 'Manjushree Building, Sattur, Dharwad-580009', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(158, 44, 44);
			$pdf->SetFont($diploma, '', 24, '', false);			
            $pdf->SetXY(11.4, 78);
			$pdf->MultiCell(188, 0, 'Provisional Degree Certificate', 0, 'C', 0, 0, '', '', true, 0, true);
            
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdf->SetXY(11.4, 98.5);
			$pdf->MultiCell(188, 0, 'This is to certify that', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(242, 47, 53);
			$pdf->SetFont($calibrib, '', 16, '', false);			
            $pdf->SetXY(11.4, 117);
			$pdf->MultiCell(188, 0, $candidate_name, 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdf->SetXY(10.9, 134);
			$pdf->MultiCell(188, 0, 'has passed', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(62, 64, 149);
			$pdf->SetFont($calibrib, '', 14, '', false);
            $pdf->SetXY(11.4, 151);
			$pdf->MultiCell(188, 0, $Programme_Name, 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdf->SetXY(11.4, 165.5);
			$pdf->MultiCell(188, 0, 'examination held in <span style="font-family:'.$calibri.';font-size:14;">'.$Examination_held_in.'</span> with Registration Number <span style="font-family:'.$calibri.';font-size:14;">'.$Registration_No.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
            
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdf->SetXY(11.2, 180.5);
			$pdf->MultiCell(188, 0, '(Valid till the Original Degree Certificate is issued)', 0, 'C', 0, 0, '', '', true, 0, true);
			/*
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MonotypeCorsiva, '', 18, '', false);			
			$pdf->SetXY(11.2, 193.5);
			$pdf->MultiCell(188, 0, 'He/She is eligible for the award of the degree', 0, 'C', 0, 0, '', '', true, 0, true);
			
			$pdf->SetTextColor(62, 64, 149);
			$pdf->SetFont($calibrib, '', 14, '', false);			
			$pdf->SetXY(11.2, 207);
			$pdf->MultiCell(188, 0, $Degree, 0, 'C', 0, 0, '', '', true, 0, true);
			*/
			/*Verification Data*/
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont($MyriadProItalic, 'I', 10, '', false);
			$pdf->SetXY(43, 215);
			$pdf->MultiCell(125.5, 7, 'Batch Of:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$batch_of_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdf->SetXY(43, 220);
			$pdf->MultiCell(125.5, 7, 'Name of the Student:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$candidate_name.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdf->SetXY(43, 225);
			$pdf->MultiCell(125.5, 7, 'Aadhar Number:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$aadhar_number_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdf->SetXY(43, 230);
			$pdf->MultiCell(125.5, 7, 'Course:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$course_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdf->SetXY(43, 235);
			$pdf->MultiCell(125.5, 7, 'Year:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$year_qr.'</span>', 0, "L", 0, 0, '', '', true, 0, true);
			$pdf->SetXY(43, 240);
			$pdf->MultiCell(125.5, 7, 'Date Of Birth:&nbsp;&nbsp;<span style="font-family:'.$calibrib.';">'.$DOB.'</span>', 0, "L", 0, 0, '', '', true, 0, true);			
			
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetXY(135, 240);
			$pdf->SetFont($calibri, '', 12, '', false);
			$pdf->MultiCell(60, 0, 'PDC NO.: '.$PDC_No, 0, 'R', 0, 0, '', '', true, 0, true);
			
			$pdf->SetXY(11, 265);
			$pdf->SetFont($calibri, '', 11, '', false);	
			$pdf->MultiCell(0, 0, 'Date: <span style="color:#e83339;">'.$Date.'</span>', 0, 'L', 0, 0, '', '', true, 0, true);
			$pdf->SetXY(11, 270.5);
			$pdf->SetFont($calibri, '', 11, '', false);
			$pdf->MultiCell(0, 0, 'Dharwad',0, 'L', 0, 0);
			...
			$pdf->SetXY(51.5, 283);
			//$pdf->MultiCell(92, 0, "SDM College of Dental Sciences and Hospital, Sattur, Dharwad–580009, Karnataka, India",0, 'L', 0, 0);			
			//End pdf			
			...
			$name = str_replace('.','',$nameOrg);
			$name = substr(str_replace(' ','',strtoupper($name)), 0, 6);
			...
			*/
			...
			$pdfBig->SetTextColor(0, 0, 0);
			$pdfBig->SetXY(174, 219);        
			$pdfBig->Cell(18.5, 0, $microlinestr, 0, false, 'C'); 
			
			$pdf->SetFont($arialb, '', 1.3, '', false);
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetXY(174, 219);        
			$pdf->Cell(18.5, 0, $microlinestr, 0, false, 'C'); 
       ...
       if($previewPdf!=1){
        $this->updateCardNo('sdmPDC',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }
       $msg = '';
        
        $file_name =  str_replace("/", "_",'sdmPDC'.date("Ymdhms")).'.pdf';
