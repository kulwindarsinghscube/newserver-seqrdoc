...
file_put_contents($local_server_file_path, file_get_contents($destinationURL));
    ...
    public function addCertificate($serial_no, $certName, $dt,$template_id,$admin_id)
    {
        ...
        /**** Check instance is enaled for AWS or Not If not then move if file exist to inactive folder****/
        // awsS3Instances
        $awsS3Instances = \Config::get('constant.awsS3Instances');
        if(!in_array($subdomain[0], $awsS3Instances)) {
            $file_existes = public_path().'/'.$subdomain[0].'/backend/pdf_file/'.$certName;
            if(file_exists($file_existes)){    
            
                if(!is_dir(public_path().'/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF')){
                    mkdir(public_path().'/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF');
                }
                $student = StudentTable::where('status',1)->where('serial_no',$serial_no)->value('id');
                $inactivePdf = public_path().'/'.$subdomain[0].'/backend/pdf_file/Inactive_PDF/'.$student.'_'.$certName;
        ...
        //Compress pdf file  //Enable compression code for all instances at 01/06/2023 13:06 by Mandar
        ...
        if(in_array($subdomain[0], $compressionInstances)) {    
            ...
            \File::copy($file2,$pdfActualPath);  
        }

         //Uploding pdf to AWS by checking domain
        $outputFile = 'public/'.$subdomain[0]."\\backend\\pdf_file\\".$certName;
        
        if(in_array($subdomain[0], $awsS3Instances)) {
            CoreHelper::awsUpload($pdfActualPath,$outputFile,$serial_no,$certName);
        }
        // Mandar changes 01/06/2023
