    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        if($studentDataOrg&&!empty($studentDataOrg)){
            foreach ($studentDataOrg as $studentData) {
                ...
                $high_res_bg="bestiu_pdc_bg.jpg"; // bestiu_pdc_bg, bestiu_pdc_bg_data
                ...
                $Gender_Specific1 = trim($studentData[13]);
                ...
                $pdfBig->SetXY(11, 12);
                $pdfBig->Cell(0, 0, 'ID No.: '.$unique_id, 0, false, 'L');


                if($AadharNumber) {
                    $pdfBig->SetFont($arial, '', 8, '', false); 
                    $pdfBig->SetTextColor(0, 0, 0);    
                    $pdfBig->SetXY(11, 37.5);
                    $pdfBig->Cell(0, 0, 'Aadhar No.: '.$AadharNumber, 0, false, 'L');
                }

                

                $pdfBig->SetFont($oef, '', 22, '', false); 
                $pdfBig->SetTextColor(0, 0, 0);        
                $pdfBig->SetXY(12, 51.5);
                $pdfBig->Cell(186, 0, 'Provisional Degree Certificate', 0, false, 'C');
                $pdfBig->SetFont($oef, '', 13, '', false); 
                $pdfBig->SetXY(12, 74);			
                $pdfBig->Cell(186, 0, 'This is to certify that', 0, false, 'C');
                $pdfBig->SetFont($arialMTblack, '', 18, '', false);
                $pdfBig->SetXY(10, 88);
                // $pdfBig->Cell(186, 0, $candidate_name, 0, false, 'C');
                $pdfBig->MultiCell(190, 0, $candidate_name , 0, 'C', 0, 0, '', '', true, 0, true);

                $pdfBig->SetFont($oef, '', 13, '', false); 
                $pdfBig->SetXY(12, 104.5);
                $pdfBig->MultiCell(186, 0, $Child_Relationship.' of <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                $pdfBig->SetXY(10, 115.5);
                $pdfBig->Cell(190, 0, 'has successfully completed the requirements of course work for the', 0, false, 'C');
                $pdfBig->SetFont($oef, '', 18.5, '', false); 
                $pdfBig->SetXY(12, 127);
                $pdfBig->MultiCell(186, 0, '<span style="font-size:13;">Degree of</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                $y = $y +12;           	
                $pdfBig->SetXY(10, $y); //174 
                $pdfBig->Cell(190, 0, $Gender_Specific1.' Overall Grade Point Average is '.$average.' out of 10.00', 0, false, 'C');			
                ...
                $pdf->SetXY(11, 12);
                $pdf->Cell(0, 0, 'ID No.: '.$unique_id, 0, false, 'L');
                ...
                $pdf->SetFont($oef, '', 22, '', false); 
                $pdf->SetTextColor(0, 0, 0);        
                $pdf->SetXY(12, 51.5);
                $pdf->Cell(186, 0, 'Provisional Degree Certificate', 0, false, 'C');
                $pdf->SetFont($oef, '', 13, '', false); 
                $pdf->SetXY(12, 74);         
                $pdf->Cell(186, 0, 'This is to certify that', 0, false, 'C');
                $pdf->SetFont($arialMTblack, '', 18, '', false);
                $pdf->SetXY(10, 88);
                // $pdf->Cell(190, 0, $candidate_name, 0, false, 'C');          
                $pdf->MultiCell(190, 0, $candidate_name , 0, 'C', 0, 0, '', '', true, 0, true);

                $pdf->SetFont($oef, '', 13, '', false); 
                $pdf->SetXY(12, 104.5);
                $pdf->MultiCell(186, 0, $Child_Relationship.' of <span style="font-family:'.$arialMTblack.';">'.$father_name.'</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                $pdf->SetXY(10, 115.5);
                $pdf->Cell(190, 0, 'has successfully completed the requirements of course work for the', 0, false, 'C');
                $pdf->SetFont($oef, '', 18.5, '', false); 
                $pdf->SetXY(12, 127);
                $pdf->MultiCell(186, 0, '<span style="font-size:13;">Degree of</span>', 0, 'C', 0, 0, '', '', true, 0, true);
                ...
                // $pdf->SetFont($oef, '', 13, '', false); 
                // $pdf->SetXY(10, 162);   
                // $pdf->Cell(190, 0, 'of this University on '.$completion_date.'.', 0, false, 'C');            
                // $pdf->SetXY(10, 174); 
                // $pdf->Cell(190, 0, $Gender_Specific1.' Overall Grade Point Average is '.$average.' out of 10.00', 0, false, 'C');            
                // $pdf->SetXY(10, 184); 
                // $pdf->Cell(190, 0, 'and percentage of marks is '.$percentage.'.', 0, false, 'C');            
                // $pdf->SetXY(10, 195); 
                // $pdf->MultiCell(190, 0, $Gender_Specific2.' has been placed in <span style="font-family:'.$arialMTblack.';">'.$division.'</span> Division.', 0, 'C', 0, 0, '', '', true, 0, true);   
                // $pdf->SetXY(45.5, 208); 
                // $pdf->Cell(0, 0, 'The degree will be conferred on him/her at the', 0, false, 'L');
                // $pdf->SetXY(67, 218); 
                // $pdf->Cell(0, 0, 'next convocation or thereafter.', 0, false, 'L');
                ...
                $candidate_name_u = preg_replace("/[^A-Za-z0-9]/", "", $candidate_name);
