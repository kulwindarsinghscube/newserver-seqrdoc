...
class DICPDFGenerateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        for($excel_row = $excel_row_num; $excel_row <= $highestRow; $excel_row++)
        {
            ...
            //          if($subdomain[0]=="vgujaipur"&&$FID['template_id']==22){
            //     print_r("MAndar");
            //     echo $template_img_generate;
            //     exit;
            // }
...
//                     }
                       ...
                       // $fullString = "{{QR Code}}";
                        ...
                        if(!empty($EncryptedString)){
                           ...
                           // $codeContents = strtoupper(md5($str . $dt));
                            ...
                            if(isset($FID['is_encrypted_qr'])&&$FID['is_encrypted_qr'][$extra_fields] == 1) {
                                $conStriCode= GibberishAES::enc($conStriCode, \Config::get('constant.EStamp_Salt'));
                            }
                        ...
                        if(isset($FID['is_encrypted_qr'])&&$FID['is_encrypted_qr'][$extra_fields] == 1) {
                            ...
                            if(!$isPreview){
                                ...
                                if($FID['include_image'][$extra_fields] == 1){
                                    ...
                                    }elseif(file_exists(public_path()."/".$subdomain[0]."/backend/canvas/dummy_images/".$logopath)){
                                        $logopath = public_path()."/".$subdomain[0]."/backend/canvas/dummy_images/".$logopath;
                                    }elseif(file_exists(public_path()."/".$subdomain[0]."/backend/templates/".$FID['template_id']."/".$logopath)){
                                        $logopath = public_path()."/".$subdomain[0]."/backend/templates/".$FID['template_id']."/".$logopath;
                                    }
                                }else{
                    ...
                    case 'Micro Text Border':
                        ...
                        */
                            ...
                            if(!$isPreview) {
                                ...
                                // if($subdomain[0]=="demo" || $subdomain[0]=="po"){
                            ...
                            if(!$isPreview){
                                ...
                                if($uv_image == 1){
                                    ...
                                    if($get_file_aws_local_flag->file_aws_local == '1'){
                                        ...
                                        $str = CoreHelper::getFileWithExtension($str, public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template').'/'.$FID['template_id'].'/'.$str);
                                   ...
                                   // echo "<br>";
                                    ...
                                    if($FID['grey_scale'][$extra_fields] == 1){
                                        ...
                                        if($get_file_aws_local_flag->file_aws_local == '1'){
                                                ...
                                                $str = CoreHelper::getFileWithExtension($str, public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID['template_id'].'/'.$str);
                                                $grey_str = CoreHelper::getFileWithExtension('grey_'.$str, public_path().'/'.$subdomain[0].'/'.\Config::get('constant.template')."/".$FID['template_id'].'/grey_'.$str);
                    ...
                    case 'Ghost Image':
                        ...
                        if($FID['visible'][$extra_fields] == 0){
                            ...
                            // $name = substr($str1, 0, $length);
                            
                            if($subdomain[0] == 'print24x7') {
                                $name = $str1;
                            } else {
                    ...
                    case 'UV Repeat Fullpage':
                        ...
                        if($FID['visible'][$extra_fields] == 0){
                            ...
                            if($subdomain[0] == 'nrb') {
                                if ($bg_template_width == '148'){
                                    $pdfWidth = 148;
                                } else {
                                    $pdfWidth = $bg_template_width;
                                }

                                if($bg_template_height== '210') {
                                    $pdfHeight = 210;
                                } else {
            ...
            if((!$isPreview&&(!isset($pdf_data['generation_from'])||(isset($pdf_data['generation_from'])&&$pdf_data['generation_from']!='API')))||(!$isPreview&&(isset($pdf_data['generation_from'])&&$pdf_data['generation_from']=='API'&&$pdf_data['generation_type']=='Live'))) {
                ...
                // dd($FID['template_id'],$rowData,$student_table_id);
                //functional users registration start
                //37,44,42,38,41,36=>live tpsdi
                //37,44,655,657,41,36=>demo tpsdi
                try {
                    if(!empty($rowData[$find]) && in_array($FID['template_id'], [37,44,42,38,41,36,48]) && $subdomain[0]=="tpsdi")
                        $this->tpsdiTemplateCheck($FID['template_id'],$rowData,$student_table_id);
                    $find++;
                } catch (Exception $e) {
                    dd($e);
                }
                //functional users registration end
                $dbName = 'seqr_demo';
    ...
    public function tpsdiTemplateCheck($template_id,$rowData,$student_table_id){
        // coded by sewak start
        try {
                $domain = \Request::getHost();
                $subdomain = explode('.', $domain);
                if($subdomain[0]=="tpsdi"){
                // $founddata=CertificateDataMapping::where('Unique_no',$rowData[0][0])->first();
                    $founduser = FunctionalUsers::query();
                    // if(empty($founddata)){  
                    //demo=>live template id 655=>42 657=>38 change this when deploy on live
                    if(in_array($template_id, [37,48])){
                    $row=array(
                      'Unique_no'=>$rowData[0][0],
                      'Watermark_student_data'=>$rowData[0][1],  
                      'Watermark_TPSDI'=>$rowData[0][2],  
                      'student_name'=>$rowData[0][3],  
                      'Enrollment_NO'=>$rowData[0][4],  
                      'Program_Title'=>$rowData[0][5],  
                      'On_Date'=>$rowData[0][6],
                      'Student_Image'=>$rowData[0][7],  
                      'Place'=>$rowData[0][8],  
                      'Result'=>$rowData[0][9],  
                    ...
                    if (!empty($rowData[0][10])) {
                        $founduser->where('Mobile_Number', $rowData[0][10]);
                    }

                    if (!empty($rowData[0][11])) {
                        $founduser->orWhere('Email_ID', $rowData[0][11]);
                    }
                    ...
                    $row=array(
                      ...
                      'Enrollment_NO'=>$rowData[0][4],  
                      'Program_Title'=>$rowData[0][5],  
                      'On_Date'=>$rowData[0][6],//Carbon::createFromFormat('d-m-Y', $rowData[0][6])->format('Y-m-d'), 
                      'Student_Image'=>$rowData[0][7],  
                      'Place'=>$rowData[0][8],  
                      'Mobile_Number'=>$rowData[0][9],  
                      'Email_ID'=>$rowData[0][10],  
                      'Card_Category'=>$rowData[0][11],  
                      'Hub_Name'=>$rowData[0][12],  
                      'student_tbl_id'=>$student_table_id,  
                    );
                    

                    if (!empty($rowData[0][9])) {
                        $founduser->where('Mobile_Number', $rowData[0][9]);
                    }

                    if (!empty($rowData[0][10])) {
                        $founduser->orWhere('Email_ID', $rowData[0][10]);
                    }

                    if (!empty($rowData[0][10]) && !empty($rowData[0][9])) {
                        $founduser->where('Mobile_Number', $rowData[0][9])->orWhere('Email_ID', $rowData[0][10]);
                    }
                    $user_data=array(
                      'Student_Name'=>$rowData[0][3],
                      'Mobile_Number'=>$rowData[0][9],  
                      'Email_ID'=>$rowData[0][10],  
                      'is_inter'=>(empty($rowData[0][9])&&!empty($rowData[0][10]))?'Y':'N'  
                      // 'student_tbl_id'=>$studentData->id,  
                    );
                 }
                 ...
                 if(in_array($template_id, [38])){//Certificate-TPSSL
                    $row=array(
                      ...
                      'Student_Image'=>$rowData[0][7],  
                      'Place'=>$rowData[0][8],  
                      'Dealer_Name'=>$rowData[0][9],  
                      'Cert_type'=>$rowData[0][10],  
                      'Mobile_Number'=>$rowData[0][11],  
                      'Email_ID'=>$rowData[0][12],  
                      'Card_Category'=>$rowData[0][13],  
                      'Hub_Name'=>$rowData[0][14],  
                      'student_tbl_id'=>$student_table_id,  
                    );
                    

                    if (!empty($rowData[0][11])) {
                        $founduser->where('Mobile_Number', $rowData[0][11]);
                    }

                    if (!empty($rowData[0][12])) {
                        $founduser->orWhere('Email_ID', $rowData[0][12]);
                    }

                    if (!empty($rowData[0][12]) && !empty($rowData[0][11])) {
                        $founduser->where('Mobile_Number', $rowData[0][11])->orWhere('Email_ID', $rowData[0][12]);
                    }
                    $user_data=array(
                      'Student_Name'=>$rowData[0][3],
                      'Mobile_Number'=>$rowData[0][11],  
                      'Email_ID'=>$rowData[0][12],  
                      'is_inter'=>(empty($rowData[0][11])&&!empty($rowData[0][12]))?'Y':'N'  
                      // 'student_tbl_id'=>$studentData->id,  
                    );
                 }
                 if(in_array($template_id, [41])){//Certificate - College Name
                    $row=array(
                      ...
                      'student_name'=>$rowData[0][3],  
                      'roll_no'=>$rowData[0][4],  
                      'Enrollment_NO'=>$rowData[0][5],  
                      'Program_Title'=>$rowData[0][6],  
                      'On_Date'=>$rowData[0][7],
                      'Student_Image'=>$rowData[0][8],  
                      'Place'=>$rowData[0][9],
                      'Mobile_Number'=>$rowData[0][10],  
                      'Email_ID'=>$rowData[0][11],  
                      'Card_Category'=>$rowData[0][12],  
                 ...
                 if(in_array($template_id, [36])){//Certificate - Technical Colleges
                    $row=array(
                      'Unique_no'=>$rowData[0][0],
                      'Watermark_Insitute_Name'=>$rowData[0][1],  
                      'Watermark_TPSDI'=>$rowData[0][2],  
                      'Institute_name'=>$rowData[0][3],  
                      'From_Date'=>$rowData[0][4],
                      'To_Date'=>$rowData[0][5],
                      'Mobile_Number'=>$rowData[0][6],  
                      'Email_ID'=>$rowData[0][7],  
                      'Card_Category'=>$rowData[0][8],  
                      'Hub_Name'=>$rowData[0][9],  
                      'student_tbl_id'=>$student_table_id,  
                    );
                    

                    if (!empty($rowData[0][6])) {
                        $founduser->where('Mobile_Number', $rowData[0][6]);
                    }

                    if (!empty($rowData[0][7])) {
                        $founduser->orWhere('Email_ID', $rowData[0][7]);
                    }

                    if (!empty($rowData[0][7]) && !empty($rowData[0][6])) {
                        $founduser->where('Mobile_Number', $rowData[0][6])->orWhere('Email_ID', $rowData[0][7]);
                    }
                    $user_data=array(
                      'Student_Name'=>$rowData[0][3],
                      'Mobile_Number'=>$rowData[0][6],  
                      'Email_ID'=>$rowData[0][7],  
                      'is_inter'=>(empty($rowData[0][6])&&!empty($rowData[0][7]))?'Y':'N'  
                      // 'student_tbl_id'=>$studentData->id,  
                    );
                 }
                 // dd($row);
         ...
         /* print_r($mintData);
           ...
           // exit;
            if($response['status']==200){
                $bc_file_hash=CoreHelper::generateFileHash($pdf_file);
                $resultu = StudentTable::where('serial_no',''.$serial_no)->where('key',''.$key)->where('status',1)->update(['bc_txn_hash'=>$response['txnHash'],'bc_ipfs_hash'=>$response['ipfsHash'], 'pinata_ipfs_hash'=>$response['pinataIpfsHash'], 'bc_file_hash'=>$bc_file_hash ]);
            }
