...
class PdfGenerateKsgCustomJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        $first_sheet=$pdf_data['studentDataOrg']; 
        // get first worksheet rows
       ...
       $arialb = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Arialb.TTF', 'TrueTypeUnicode', '', 96);
        ...
        //$oef = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\old-english-five.ttf', 'TrueTypeUnicode', '', 96);
         //$labrit= TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\LABRIT__.ttf', 'TrueTypeUnicode', '', 96);
        $LucidaCalligraphyItalic= TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\Lucida Calligraphy Italic.ttf', 'TrueTypeUnicode', '', 96);
       ...
       $bookantiqua=TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\bookantiqua.ttf', 'TrueTypeUnicode', '', 96);
       $bookantiqua_bold=TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\bookantiqua_bold.ttf', 'TrueTypeUnicode', '', 96);
         $palatino_bold=TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\palatino-linotype-bold.ttf', 'TrueTypeUnicode', '', 96);
		...
		if($studentDataOrg&&!empty($studentDataOrg)){
			foreach ($studentDataOrg as $studentData) {
                ...
                if ($previewPdf == 0) {
                    KsgBtatchRecordModel::updateOrCreate(
                        ...
                        [
                            'batch_id'      => $dropdown_template_id,
                            'usn'           => $studentData[0],
                            'name'          => $studentData[1],
                            'course'        => $studentData[2],
                            'course_2'        => $studentData[3],
                            'course_date'   => $studentData[4],
                            'course_month'  => $studentData[5],
                            'fees'          => $studentData[6],
                            'unique_id_no'  => $studentData[7],
                            'id_type'       => $studentData[8],
                            'credit'        => $studentData[9],
                            'status'        =>'Approved',
                            'approval_id'   =>1,
                            'created_by'    => 1,
                            'updated_at'    => now(),
				...
				$high_res_bg="KSG CERTIFICATE.jpg"; // bestiu_pdc_bg, TranscriptData.jpg
				$low_res_bg="KSG CERTIFICATE.jpg";
				$pdfBig->AddPage();
				...
				$pdf->setPageMark();
				//$pdfBig->setPageMark();
				$str=strtoupper($studentData[1].$studentData[0].$studentData[2].$studentData[5]);							
                ...
                $pdfBig->SetTextColor(200, 200, 200);
                $pdfBig->SetFontStretching(130); 
                $pdfBig->StartTransform();
                $pdfBig->SetXY(15, 82);
                $pdfBig->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
                $pdfBig->SetXY(15, 85);
                $pdfBig->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
                $pdfBig->SetXY(15, 88);
                $pdfBig->Cell(0, 0, $arrayEnrollment, 0, false, 'C');
                ...
                $pdfBig->SetTextColor(0,74,164);
            
                $x = 15;
                $y = 82.4;
                $pdfBig->SetFont($bookantiqua_bold, 'B', 32, '', false);
                $pdfBig->SetXY(15, $y);
                $pdfBig->Cell(0, 10, $studentData['1'], 0, false, 'C');
                $pdfBig->SetTextColor(255,0,0);
                $y = 126;
                $pdfBig->SetFont( $bookantiqua_bold, 'B', 28, '', false);
                // $pdfBig->SetXY($x+10, $y);
                // $pdfBig->Cell(0, 10, $studentData['course'], 0, false, 'C');

                    if (!empty($studentData[2]) && !empty($studentData[3])) {
                     
                        $pdfBig->SetXY($x+10, $y);
                        $pdfBig->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                        $pdfBig->SetXY($x+10, $pdfBig->getY()); 
                        $pdfBig->MultiCell(0, 6, $studentData[3], 0, 'C', false);

                    } elseif (!empty($studentData[2]) && !empty($studentData[9]) && empty($studentData[3])) {
                        $pdfBig->SetXY($x+10, $y);
                        $pdfBig->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                        $pdfBig->SetFont($bookantiqua_bold, 'B', 22, '', false);
                        $pdfBig->SetXY($x+10, $pdfBig->getY()); 
                        $pdfBig->MultiCell(0, 6, $studentData[9], 0, 'C', false);
                    }
                    else if(!empty($studentData[2])&& (empty($studentData[3])|| empty($studentData[9]))){
                        $pdfBig->SetXY($x+10, $y);
                            $pdfBig->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                    ...
                    // if(!empty($studentData[8])){
                    //     $pdfBig->SetFont( $bookantiqua_bold, 'B', 22, '', false);
                    //     $pdfBig->SetXY(0, $pdfBig->getY()); 
                    //     $pdfBig->MultiCell(0,6,$studentData[8],0, 'C', false );
                    // }
                

                $pdfBig->SetTextColor(0,0,0); 
                $pdfBig->SetFont($LucidaCalligraphyItalic, '', 16, '', false);
                $y= $pdfBig->getY();
                $pdfBig->SetXY(0, $y);       
            
                $pdfBig->MultiCell(0, 10,'on this &nbsp;&nbsp;<span style="color:#004AA4; font-size:22;"><font face="'.$bookantiqua_bold.'">'. $studentData[4].'</font></span> &nbsp;&nbsp;day of  &nbsp;&nbsp;<span style="color:#004AA4;font-size:22;"><font face="'.$bookantiqua_bold.'">'. $studentData[5].'</font></span>',0,'C', 0, 0, '', '', true, 
                0,  true,   true,  0, 'T', true);
                $y= $pdfBig->getY();
                $pdfBig->SetXY(0, $y+14); 
                $pdfBig->MultiCell(0, 11,'and is entered in official records',0,'C', 0, 0, '', '', true, 
                0,  true,   true,  0, 'T', true);


                $pdfBig->SetFont($palatino_bold, '', 10, '', false); 
                $pdfBig->SetXY(30, 185); 
                // $pdfBig->Write(10, 'DIRECTOR GENERAL');
                $pdfBig->Cell(60, 8, 'DIRECTOR GENERAL', 'T', false, 'C');
                $pdfBig->SetXY(210, 185); 
                // $pdfBig->Write(10, 'SENIOR DIRECTOR ACADEMIC AFFAIRS');
                $pdfBig->Cell(72, 8, 'SENIOR DIRECTOR ACADEMIC AFFAIRS', 'T', false, 'C');

                // $x = 160;
                // $y = 192;
                // $pdfBig->SetFont($ariali, '', 10, '', false);
                // $pdfBig->SetXY($x, $y);
                // $pdfBig->Cell(20, 10, $studentData[0], 0, false, 'C');
                // // if($previewPdf!=1){
                // // 	if($previewWithoutBg!=1){
                //          $pdfBig->SetX($x-35);
                //       $pdfBig->Cell(20, 10, 'Certificate NO:', 0, false, 'C'); 
                ...
                $pdf->SetTextColor(0,74,164);
            
                $x = 15;
                $y = 82.4;
                $pdf->SetFont($bookantiqua_bold, 'B', 32, '', false);
                $pdf->SetXY(15, $y);
                $pdf->Cell(0, 10, $studentData['1'], 0, false, 'C');
                $pdf->SetTextColor(255,0,0);
                $y = 126;
                $pdf->SetFont( $bookantiqua_bold, 'B', 28, '', false);
                // $pdf->SetXY($x+10, $y);
                // $pdf->Cell(0, 10, $studentData['course'], 0, false, 'C');

                    if (!empty($studentData[2]) && !empty($studentData[3])) {
                     
                        $pdf->SetXY($x+10, $y);
                        $pdf->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                        $pdf->SetXY($x+10, $pdf->getY()); 
                        $pdf->MultiCell(0, 6, $studentData[3], 0, 'C', false);

                    } elseif (!empty($studentData[2]) && !empty($studentData[9]) && empty($studentData[3])) {
                        $pdf->SetXY($x+10, $y);
                        $pdf->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                        $pdf->SetFont($bookantiqua_bold, 'B', 22, '', false);
                        $pdf->SetXY($x+10, $pdf->getY()); 
                        $pdf->MultiCell(0, 6, $studentData[9], 0, 'C', false);
                    }
                    else if(!empty($studentData[2])&& (empty($studentData[3])|| empty($studentData[9]))){
                        $pdf->SetXY($x+10, $y);
                            $pdf->MultiCell(0, 7, $studentData[2], 0, 'C', false);
                    ...
                    //     $pdf->MultiCell(0,6,$studentData[8],0, 'C', false );
                    // }
                

                $pdf->SetTextColor(0,0,0); 
                $pdf->SetFont($LucidaCalligraphyItalic, '', 16, '', false);
                $y= $pdf->getY();
                $pdf->SetXY(0, $y);       
            
                $pdf->MultiCell(0, 10,'on this &nbsp;&nbsp;<span style="color:#004AA4; font-size:22;"><font face="'.$bookantiqua_bold.'">'. $studentData[4].'</font></span> &nbsp;&nbsp;day of  &nbsp;&nbsp;<span style="color:#004AA4;font-size:22;"><font face="'.$bookantiqua_bold.'">'. $studentData[5].'</font></span>',0,'C', 0, 0, '', '', true, 
                0,  true,   true,  0, 'T', true);
                $y= $pdf->getY();
                $pdf->SetXY(0, $y+14); 
                $pdf->MultiCell(0, 11,'and is entered in official records',0,'C', 0, 0, '', '', true, 
                0,  true,   true,  0, 'T', true);


                $pdf->SetFont($palatino_bold, '', 10, '', false); 
                $pdf->SetXY(30, 185); 
                // $pdf->Write(10, 'DIRECTOR GENERAL');
                $pdf->Cell(60, 8, 'DIRECTOR GENERAL', 'T', false, 'C');
                $pdf->SetXY(210, 185); 
                // $pdf->Write(10, 'SENIOR DIRECTOR ACADEMIC AFFAIRS');
                $pdf->Cell(72, 8, 'SENIOR DIRECTOR ACADEMIC AFFAIRS', 'T', false, 'C');

                // $x = 160;
                // $y = 192;
                // $pdf->SetFont($ariali, '', 10, '', false);
                // $pdf->SetXY($x, $y);
                // $pdf->Cell(20, 10, $studentData[0], 0, false, 'C');
                // // if($previewPdf!=1){
                // //   if($previewWithoutBg!=1){
                //          $pdf->SetX($x-35);
                //       $pdf->Cell(20, 10, 'Certificate NO:', 0, false, 'C'); 
            ...
            $pdf->setPageMark();
                ...
                $str=$serial_no.$dt;
                // $codeContents = $str;
                // $qr_code_path = public_path().'\\'.$subdomain[0].'\backend\canvas\images\qr\/'.$serial_no.'.png';
           ...
           \PHPQRCode\QRcode::png($codeContents, $qr_code_path, $ecc, $pixel_Size, $frame_Size);
            ...
            //     ->generate($codeContents, $qr_code_path);
				...
				$usn=$studentData[0];
                $usnnew = str_replace(['/', '\\'], '_', $usn);
                $COE= public_path().'\\'.$subdomain[0].'\backend\canvas\images\DIRECTOR GENERAL SIGNATURE.png';
                $upload_COE = $COE;
                $pathInfo = pathinfo($COE);
                $COE = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']. "_$usnnew.".$pathInfo['extension'];
                ...
                $COE_Width = 31.75;
                $COE_Height = 14.1125;
                $pdfBig->image($COE,$COE_x,$COE_y,$COE_Width,$COE_Height,'png','','L',true,3600);
                $pdfBig->setPageMark();	
                $pdf->image($COE,$COE_x,$COE_y,$COE_Width,$COE_Height,'png','','L',true,3600);
                $pdf->setPageMark();

                unlink($COE);
                //sign2
                $COE =public_path().'\\'.$subdomain[0].'\backend\canvas\images\SENIOR DIRECTOR SIGNATURE.png';
                $upload_COE = $COE;
                $pathInfo = pathinfo($COE);
                $COE = public_path().'\\'.$subdomain[0].'\backend\\templates\\100\\'.$pathInfo['filename']. "_$usnnew.".$pathInfo['extension'];
                \File::copy($upload_COE,$COE);       
				...
				if($previewPdf!=1){
					...
					$this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'Ksg',$admin_id,$card_serial_no);
        ...
        $file_name =  str_replace("/", "_",'KSG'.date("Ymdhms")).'.pdf';
