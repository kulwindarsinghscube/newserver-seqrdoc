...
use Helper;
class PdfGenerateMakeroWristbandJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
		...
		$card_serial_no="";
        $log_serial_no = 1;
        // $cardDetails=$this->getNextCardNo('MakeroWristband');
        // $card_serial_no=$cardDetails->next_serial_no;
        $generated_documents=0;  //for custom loader

        // $studentDataOrg = $rowData1;
        $batchSize = 7; // Define how many records per batch
        $chunks = array_chunk($studentDataOrg, $batchSize); // Split the array into chunks of 6
 
        foreach ($chunks as $batch) { // Loop through each batch of 6 records
			...
			$high_res_bg="Wrist_Band_bg.jpg"; // Wrist_Band_bg.jpg
			$low_res_bg="Wrist_Band_bg.jpg";
			$pdfBig->AddPage();
			...
			$template_img_generate1 = public_path().'\\'.$subdomain[0].'\backend\canvas\bg_images\\Wrist_Band_bg_v1.jpg';   
            ...
            $qrX = 21;
            $logo_img_generate = public_path().'\\'.$subdomain[0].'\backend\canvas\images\\WristBandLogo.jpg';   
            foreach ($batch as $row) {

                $unique_id = trim($row[0]);
                $event_name = trim($row[1]);
                

                // Start pdfBig

                $pdfBig->StartTransform();  // Begin transformation
                $pdfBig->SetXY($qrX, 98);
                $pdfBig->Rotate(90); // Rotate 90 degrees around (X=50, Y=50)
                $pdfBig->Image($logo_img_generate, $qrX, 98, 35, 15, "JPG", '', '', true);
                $pdfBig->StopTransform();  // End transformation


                $pdfBig->SetFont($arial, '', 20, '', false); 
                $pdfBig->SetXY($staticX, 155);
                $pdfBig->StartTransform();
                $pdfBig->Rotate(90);
                $pdfBig->MultiCell(40, 5, 'SSSL', 0, "R", 0, 0, '', '', true, 0, true);
                $pdfBig->StopTransform();


                // Event Name
                $max_width = 85; // Maximum width in mm
                $min_font_size = 10; // Minimum font size
                // Adjust font size
                $font_size = 17;
                $pdfBig->SetFont($arial, '', $font_size);
                $textWidth = $pdfBig->GetStringWidth($event_name.' '.$unique_id);

                if ($textWidth > $max_width) {
                    $scalingFactor = $max_width / $textWidth;
                    $adjustedFontSize = floor($font_size * $scalingFactor);
                    $adjustedFontSize = $adjustedFontSize-1;
                } else {
                    $adjustedFontSize = $font_size;
                }

                $adjustedFontSize = max($adjustedFontSize, $min_font_size); // Prevent font from being too small
                $pdfBig->SetFont($arial, '', $adjustedFontSize);
                $pdfBig->SetXY($dynamicX, 220);
                $pdfBig->StartTransform();
                $pdfBig->Rotate(90);
                $pdfBig->MultiCell($max_width, 10, $event_name.' '.$unique_id, 0, "R", 0, 0, '', '', true, 0, true);
                $pdfBig->StopTransform();
                ...
                $dt = date("_ymdHis");
                $str=$GUID.$dt;
                $QR_Output = $unique_id.', '.$event_name;
                ...
                $x = $x +25.5;
                $staticX = $staticX +25.7;
                $dynamicX = $dynamicX +25.8;
                ...
                $pdf = new TCPDF('P', 'mm', array('60', '297'), true, 'UTF-8', false);
                ...
                if($previewPdf!=1){
                    $pdf->Image($template_img_generate1, 0, 0, '60', '297', "JPG", '', 'R', true);
                }
                $pdf->setPageMark();

                $pdf->StartTransform();  // Begin transformation
                $pdf->SetXY(21, 98);
                $pdf->Rotate(90); // Rotate 90 degrees around (X=50, Y=50)
                $pdf->Image($logo_img_generate, 21, 98, 35, 15, "JPG", '', '', true);
                $pdf->StopTransform();  // End transformation
                ...
                $pdf->StartTransform();
                $pdf->Rotate(90);
                $pdf->MultiCell(40, 5, 'SSSL', 0, "R", 0, 0, '', '', true, 0, true);
                $pdf->StopTransform();
                ...
                $textWidth = $pdf->GetStringWidth($event_name.' '.$unique_id);
                ...
                $pdf->StartTransform();
                $pdf->Rotate(90);
                $pdf->MultiCell($max_width, 10, $event_name.' '.$unique_id, 0, "R", 0, 0, '', '', true, 0, true);
                $pdf->StopTransform();
        ...
        if($previewPdf!=1){
            // $this->updateCardNo('MakeroWristband',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
        }
        $msg = '';
        
        $file_name =  str_replace("/", "_",'MakeroWristband'.date("Ymdhms")).'.pdf';
