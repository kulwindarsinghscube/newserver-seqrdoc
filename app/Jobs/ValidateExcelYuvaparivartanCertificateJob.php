...
class ValidateExcelYuvaparivartanCertificateJob
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
        ...
        // dd($columnsCount1);
                if($dropdown_template_id==1){
                    if($columnsCount1==11){
                        $columnArr=array('Certificate No','Issue Date','Name of the Candidate','S/o,D/o,W/o,Guardian Name ','NSDC Course Name','Name of the Sector','Grade','Center Location','Start Date ','End Date','Image');
                    ...
                    if($columnsCount1==9){
                        $columnArr=array('Certificate No','Issue Date','Name of the Candidate','S/o,D/o,W/o,Guardian Name ','NSDC Course Name','Name of the Sector','Center Location','Start Date ','End Date');
                        $mismatchColArr=array_diff($rowData1[0], $columnArr);
                        if(count($mismatchColArr)>0){
                            return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : '.implode(',', $mismatchColArr)]);
                        }
                    ...
                    if($columnsCount1==10){
                        $columnArr=array('Certificate No','Issue Date','Name of the Candidate','S/o,D/o,W/o,Guardian Name ','NSDC Course Name','Name of the Sector','Center Location','Start Date ','End Date','photo');
                        $mismatchColArr=array_diff($rowData1[0], $columnArr);
                        if(count($mismatchColArr)>0){
                            return response()->json(['success'=>false,'type' => 'error', 'message' => 'Sheet1/Json : Column names not matching as per requirement. Please check columns : '.implode(',', $mismatchColArr)]);
                        }
