...
class PdfGenerateMinoshaJob2
{
    ...
    public function handle(CheckUploadedFileOnAwsORLocalService $checkUploadedFileOnAwsOrLocal)
    {
       ...
       // $systemConfig = SystemConfig::select('sandboxing','printer_name')->where('site_id',$auth_site_id)->first();
        ...
        $pdfBig = new TCPDF('P', 'mm', array('210', '286'), true, 'UTF-8', false);
       ...
       // $pdfBig->SetOverprint(true, true, 0);
        // add spot colors
        $pdfBig->AddSpotColor('Special', 30, 100, 90, 10);        // For Invisible
        ...
        $ARIALlMONOSPACEDMTBOLD = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\ARIALlMONOSPACEDMTBOLD.ttf', 'TrueTypeUnicode', '', 96);
        ...
        $LUCIDACONSOLE = TCPDF_FONTS::addTTFfont(public_path().'\\'.$subdomain[0].'\backend\canvas\fonts\LUCIDACONSOLE.ttf', 'TrueTypeUnicode', '', 96);*/
        
        //$card_serial_no="";
		$preview_serial_no=1;
        $log_serial_no = 1;
        /*$cardDetails=$this->getNextCardNo('Minosha');
        if($previewPdf!=1){
        ...
        foreach ($studentDataOrg as $studentData) {
            ...
            $high_res_bg="TEST_SAMPLE_WITHOUT_SF_AND_VARIABLE.jpg";// 
           // $low_res_bg="uneb_bg_COE.jpg";
           // $low_res_bg_verification="uneb_bg_COE_Verification.jpg";
            $pdfBig->AddPage();
           ...
           // $pdfBig->setPageMark();
            ...
            $pdfBig->SetFont('Arial', '', 1.5, '', false);
            $pdfBig->setXY(171, 268);//258 //265
            $pdfBig->MultiCell(19, 0, trim($candidateName), '', 'C', 0, 1);
...
/*
            if($previewPdf!=1){
                ...
                $this->addPrintDetails($username, $print_datetime, $printer_name, $print_count, $print_serial_no, $serial_no,'Minosha',$admin_id,$card_serial_no);
       ...
       /*if($previewPdf!=1){
        $this->updateCardNo('Minosha',$card_serial_no-$cardDetails->starting_serial_no,$card_serial_no);
       }*/
       $msg = '';
        
        $file_name =  str_replace("/", "_",'Minosha'.date("Ymdhms")).'.pdf';
