...
<div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> Anant National University
        ...
        if($ftp_flag=="Not Connected"){ 
            echo "<h4 class='text-danger'>Failed to connect ANU server FTP ($ftpHost). Please Check. Otherwise PDF won't be saved to $ftpHost and not generating live PDF</h4>";
        } 
        ?>
        ...
        <div class="row">
            <div class="col-lg-7"> <br />
                <form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('anu-certificate.uploadfile')?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" id="template_id" name="template_id">
                                ...
                                <option value="4" data-tempid="4">Grade Card 2022 batch onwards</option>                    
                                <option value="2" data-tempid="2">A4 Transcript</option>                    
                                <option value="3" data-tempid="3">A3 Transcript</option> 
                                <option value="5" data-tempid="5">Award Certificate - Convocation</option> 
                                <option value="6" data-tempid="6">Final AFCA Certificate </option> 
                                <option value="7" data-tempid="7"> BTCC Blockchain Grade Card </option>                    
                            </select>
                        </div>
                    </div>
					<div class="form-group">
                        <label class="control-label col-sm-3" for="temp">Grade Points:</label>
                        <div class="col-sm-9">
                            <select class="form-control" id="points" name="points">
                                <option value="0">Select Point</option>
                                <option value="4">4</option>                    
                                <option value="10">10 point (For 2016 & 2017 batch)</option>                    
                                <option value="10_2">10 point (For 2022 onwards)</option>                    
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">                        
                        <div class="col-sm-offset-3 col-sm-9">
                            <?php 
                            if($ftp_flag=="Not Connected"){ 
                                echo "<h5 class='text-danger'><strong>Failed to connect ANU server ($ftpHost). Please Check.</strong></h5>";
                            }else{ 
                            ?> 						
            ...
            <div class="col-lg-5" style="padding: 83px 1em;"> 
                <?php
                    ...
                    $excel_url_1=$path.$subdomain[0]."/backend/sample_excel/GC Sample.xlsx";
                    $excel_url_2=$path.$subdomain[0]."/backend/sample_excel/anu_a4_transcript.xlsx";
                    $excel_url_3=$path.$subdomain[0]."/backend/sample_excel/anu_a3_transcript.xlsx";
                    $excel_url_4=$path.$subdomain[0]."/backend/sample_excel/GC_Sample_2022_onwards.xlsx";
                     $excel_url_6=$path.$subdomain[0]."/backend/sample_excel/Anu_Final_AFCA_Certificate.xlsx";
                    $excel_url_5=$path.$subdomain[0]."/backend/sample_excel/Anu_Award_convocation.xlsx";
                    $excel_url_7=$path.$subdomain[0]."/backend/sample_excel/anu_btcc_sample_excel.xlsx";
                ?>
...
//on select new template 
$('#template_id').on('change', function(){
        var tempId = $(this).val();

        if(tempId == '5' || tempId == '6' ){
            // hide Grade Points field
            $('#points').closest('.form-group').hide();
        } else {
            // show Grade Points field
            $('#points').closest('.form-group').show();
        }
    });
...
$('#btn_updfile').click(function (event) {
	...
	// else if(points == 0){
  else if ((template_id != 5 && template_id != 6 ) && points == 0){
 
		toastr["error"]("Please select grade points.");
		return false;
	}
	else{
		validateUpload();
...
function validateUpload() {
  ...
  $('#downloadZipLink').html('');
	...
	$.ajax({
		url:'<?= route('anu-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function olduploadfile(){
	...
	$.ajax({
		url:'<?= route('anu-certificate.uploadfile')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
      ...
      $('#downloadZipLink').html('');
        ...
        //if(resp.zip_link) {
          $('#downloadZipLink').html(resp.zip_link);
        //}
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/admin/anu/index.blade.php ENDPATH**/ ?>