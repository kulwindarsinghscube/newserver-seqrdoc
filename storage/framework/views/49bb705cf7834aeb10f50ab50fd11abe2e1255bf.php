			...
			<div class="cardpadding" style="background-color: #f0f0f0;">
				...
				<div class="row" style="    background-color: #fff; margin: 0px;">
					<div class="col-md-6" style="text-align: center;   padding: 15px;">
						<button type="button" class="btn btn-sm btn-block form-btn" style="border:1px solid #2196f3;background-color: #03a9f4 !important;color:#fff;max-width: 200px;    margin: auto; " id="scan-more-btn"> Scan one more QR</button>
					</div>
					<div class="col-md-6" style="text-align: center;   padding: 15px;">
				...
				<div class="row">
					<div class="col-md-12" >
						<h4 style="margin-top: 30px;     margin-bottom: 20px; text-align: center;    width: 100%; color: red;font-size: 15px;"><span style="color: brown;">Note :</span> Maximum 10 scans allowed for single request.</h4>
					</div>
				</div>
				<!-- <div class="ajaxResponse">
					Scan a QR code to begin
...
$('#scan-more-btn').click(function(){
	if(totalFilesScanned<10){
		...
		$('#scan-more-btn').hide();
		$('#responseMessage').html('<span style="color:orange;">You can scan only 10 QR codes for single request.</span>');
	}
});
...
$("#proceed-to-payment-btn").click(function(e){
		...
		formData.append("action", "saveRequest");
...
function ajaxCall(val,update='0'){
	...
	if(totalFilesScanned<10){
			...
			$.ajax({
		        ...
		        success: function(data) {
					...
					if(data.status == true){
						...
						qrCodes.push(data.key);
						var resMsg='<span style="color:green;">'+data.message+'</span>';
					}else{

						var resMsg='<span style="color:red;">'+data.message+'</span>';
					}
					...
					if(totalAmount!=0){
						...
						$('#proceed-to-payment-btn').html('View Certificate');
					}
		...
		$('#responseMessage').html('You have reached maximum scan limit.').show(100);
	}
...
<?php echo $__env->make('verify.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/verify/dashboard.blade.php ENDPATH**/ ?>