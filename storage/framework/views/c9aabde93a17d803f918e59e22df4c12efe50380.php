...
<div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i>  Chanakya University
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;"><?php echo e(Breadcrumbs::render('generatechanakyaucertificates')); ?></ol>
				</h1>
        ...
        <div class="row">
            ...
            <form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('chanakya-certificate.uploadfile')?>">
              ...
              <div class="form-group">
                  ...
                  <div class="col-sm-9">
                      <select class="form-control" id="template_id" name="template_id" required>
                          ...
                          <option value="1" data-tempid="1"> Convocation Certificate</option>
                          <option value="2" data-tempid="2">Transcript CMM</option>
                          <option value="3" data-tempid="3">Transcript CMM (new)</option>
						  <option value="4" data-tempid="4">PHD</option>  
						  <option value="5" data-tempid="5">MBA Trimester</option> 
						  <option value="6" data-tempid="6"> UG with 6 semesters</option> 
						  <option value="7" data-tempid="7">UG (Lateral Entry) </option> 
						           
                      </select>
                  </div>
              </div>

			  <div class="form-group" id="extraSelectDiv" style="display: none;">
                  <label class="control-label col-sm-3" for="temp">Semester:</label>
				  <div class="col-sm-9"  >
						<select class="form-control" id="semister_no" name="semister_no">
							<option value="0">Select Additional Option</option>
            ...
            <div class="col-lg-5" style="padding: 83px 1em;"> 
                <?php
                    ...
                    $excel_url_1=$path.$subdomain[0]."/backend/sample_excel/chanakya_convocation.xlsx";
                    $excel_url_2=$path.$subdomain[0]."/backend/sample_excel/chanakya_transcript.xlsx";
					$excel_url_4=$path.$subdomain[0]."/backend/sample_excel/Chanakya_PHD_Sample File.xlsx";
					$excel_url_3=$path.$subdomain[0]."/backend/sample_excel/chanakya_transcriptnew.xlsx";
					$excel_url_5=$path.$subdomain[0]."/backend/sample_excel/chanakya_MBA.xlsx";
					$excel_url_6=$path.$subdomain[0]."/backend/sample_excel/chnanakya_UG.xlsx";
					// Chanakya_PHD_Sample File
                ?>
				...
				<span style="display:none;" id="excel_url_7"><b>Please Click <a href="<?php echo e($excel_url_6); ?>" download>HERE</a> To Download Sample Excel</b></span>
    ...
    <div class="col-sm-12">
        <h5>Notes</h5>
        <blockquote>Ensure that the "Unique Id" in excel file is unique across all data.</blockquote>
        <blockquote>Name are case sensitive, column sequence insensitive, extra columns are ignored.</blockquote>
        ...
        <blockquote>Don't use Excel functions.</blockquote>
...
/*$('#template_id').change(function() {
            if ($(this).val() == '2') { // Check if 'Transcript CMM' is selected
                $('#extraSelectDiv').show(); // Show the new select dropdown
            } else {
                $('#extraSelectDiv').hide(); // Hide the new select dropdown
            }
  });*/
...
function validateUpload() {
	var template_id= $('#template_id').val();
	//var semister_no= $('#semister_no').val();
  ...
  var previewPdf = $('#previewPdfValue').val();
	...
	fd.append('semister_no',semister_no);
	$.ajax({
		url:'<?= route('chanakya-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function uploadfile(startRow = 2, endRow = 2, highestRow = null) {
    ...
    $.ajax({
        url: '<?= route('chanakya-certificate.uploadfile') ?>',
        type: "POST",
        dataType: "JSON",
        data: fd,
        processData: false,
            ...
            if (resp.success === false) {
                ...
                if (parseInt(resp.endRow) < parseInt(highestRowCheck)) {
                  ...
                  if(resp.link=="Will be generated soon!"){
                    ...
                    console.log('All rows processed successfully1.');
                  }
                } else {
                  $('#downloadLink').html(resp.link);
                 toastr["success"](resp.message);
                    console.log('All rows processed successfully2.');
                }
            }
        },
        error: function (resp) {
...
$('#template_id').change(function() {
    let tempid = $(this).find(':selected').data('tempid');
    if (tempid == "0") {
        for (let i = 1; i <= 7; i++) {  // 👈 updated to 3 to include the new one
            $("#excel_url_" + i).hide();
        }             
    } else {
        $("#excel_url_" + tempid).show();
        for (let i = 1; i <= 7; i++) {  // 👈 updated to 3
            if (i != tempid) {
                $("#excel_url_" + i).hide();
            }
        }        
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/admin/chanakya/index.blade.php ENDPATH**/ ?>