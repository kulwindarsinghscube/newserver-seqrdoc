...
$bg_file_directory='https://'.$domain.'/'.$subdomain[0].'/backend/canvas/bg_images/';
?>
	<div class="container">
		<div class="col-xs-12">
			<div class="clearfix">
                ...
                <form id="form_process" method='post' action='' enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                      ...
                      <!-- <label id="previewPdfCheckbox" style="position: absolute;margin: 3px 30px;color: #000000bd;">
                        <input type="checkbox"  name="previewWithoutBg" id="previewWithoutBg" style="height: 20px;width: 20px;vertical-align: bottom;" /> 
                        <span>Select this to preview without Background</span>
                      </label> -->
        ...
        <div class="modal fade" id="editTtitle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              ...
              <div class="modal-header">
                  ...
                  <?php if($subdomain[0]=="demo"||$subdomain[0]=="konkankrishi" || $subdomain[0] == 'abyssinia' || $subdomain[0] == 'alarduni'){?>    
        ...
        <div id="UpdateTextVerificationData" class="modal fade" role="dialog">
            <div class="modal-dialog" style="max-width: 650px;">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" id="process_close" data-dismiss="modal"><span
                            aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="process-title2">Text Verification Setting</h4>
                       
                    </div>
                    <div class="modal-body">
                        <!-- Form -->
                        <span style="font-size:1.5rem">Choose the fields to be displayed for verification.</span>
                        <form id="UpdateTextVerificationForm" method='post' action='' enctype="multipart/form-data">
                            <div id="dynamic-checkbox-container" class="row"></div>
                                <input type="hidden" name="id" id="text_verification_template_id">
                            <hr>
              ...
              <ul class="nav nav-pills" id="pills-filter">
                ...
                <li><a id="fail-pill"data-toggle="pill" href="#failed" class="failed"><i class="fa fa-fw fa-lg fa-times"></i> Inactive Template</a></li>						
							  <?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemaker')): ?>
							  <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemaker')): ?>	
...
<script type="text/javascript">
    ...
    var oTable = $('#example').DataTable({
      ...
      "aoColumns":[
            ...
            <?php if($subdomain[0]=="icat"){ ?>
    		{mData: "LabCount", bSortable: false, sWidth: "10%", sClass: "text-center"},
            <?php } ?>
            <?php if($subdomain[0]=="demo"||$subdomain[0]=="konkankrishi"){ ?>
            ...
            mRender: function(v, t, o) {
            	...
            	buttons = '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <span style="cursor:pointer"><i title="Edit Template" id="editData" data-id="'+o['id']+'" data-temp="'+o['template_name']+'" data-page="'+o['pdf_page']+'" data-file="'+o['file_name']+'" data-toggle="tooltip" class="editrow fa fa-edit fa-lg green"></i> </span> <?php endif; ?> <?php endif; ?>';

              buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.duplicatetemplate')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.duplicatetemplate')): ?> <span style="cursor:pointer" data-id="'+o['id']+'" id="dupData" class="duplicateData" ><i title="Copy Template" class="copyrow fa fa-copy fa-lg yellow"></i></span> <?php endif; ?> <?php endif; ?>';
                
              buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.processpdf')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.processpdf')): ?> <span style="cursor:pointer" id="processData" data-id="'+o['id']+'" data-temp="'+o['template_name']+'" data-pdf_page="'+o['pdf_page']+'" data-file="'+o['file_name']+'" data-template_name = "'+o['template_name']+'" data-flag=""><i title="Generate PDF"  class="pdfGenerate fa fa-file-pdf-o fa-lg blue"></i></span> <?php endif; ?> <?php endif; ?>';   
                
              if(o['publish']==1){ rcheck="checked"; title_str="Inactivate Template"; }else{ rcheck=""; title_str="Activate Template";}        
              buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> &nbsp;<span title="'+title_str+'" style="cursor:pointer"><input name="ai'+o['id']+'" value="'+o['id']+'" type="checkbox" '+rcheck+'></span> <?php endif; ?> <?php endif; ?>';
              <?php if($subdomain[0]=="icat"){ ?>
                var assign_url = "<?php echo e(route('pdf2pdf.AssignLab',':id')); ?>"
            	  assign_url = assign_url.replace(':id',o['id']);
                buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> &nbsp;<span title="Assign Lab" style="cursor:pointer"><a href="'+assign_url+'"><i class="pdfGenerate fa fa-code-fork fa-lg blue"></i></a></span> <?php endif; ?> <?php endif; ?>';
              <?php } ?>
              buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <span style="cursor:pointer"><i title="Edit Template Title" id="editTitle" data-id="'+o['id']+'" data-temp="'+o['template_name']+'" data-title="'+o['template_title']+'" data-isblockchain="'+o['is_block_chain']+'" data-toggle="tooltip" class="editrow fa fa-edit fa-lg"></i> </span> <?php endif; ?> <?php endif; ?>';
  			      <?php //if($subdomain[0]=="demo"){ ?>
              buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?> <span style="cursor:pointer"><i title="Assign Background" id="assignBG" data-id="'+o['id']+'" data-title="'+o['template_title']+'" data-pbg = "'+o['print_bg_file']+'" data-pbgs = "'+o['print_bg_status']+'" data-vbg = "'+o['verification_bg_file']+'" data-vbgs = "'+o['verification_bg_status']+'" data-toggle="tooltip" class="editrow fa fa-image fa-lg green"></i> </span> <?php endif; ?> <?php endif; ?>';
              <?php //} ?>
              ...
              if (o['verification_type'] == 1) {
                  buttons += '<?php if(App\Helpers\SitePermissionCheck::isPermitted('pdf2pdf.processpdf')): ?> <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.processpdf')): ?> <span style="cursor:pointer" id="UpdateTextVerificationData" data-id="' + o['id'] + '" data-temp="' + o['template_name'] + '" data-pdf_page="' + o['pdf_page'] + '" data-file="' + o['file_name'] + '" data-template_name="' + o['template_name'] + '" data-flag="invisible"><i title="Text Verification Settings"  class="pdfGenerate fa fa-gear fa-lg yellow"></i></span> <?php endif; ?> <?php endif; ?>';
              }
              return buttons;
   ...
   oTable.on('click', '#UpdateTextVerificationData', function (e) {
    e.preventDefault();
    id = $(this).data('id');
    // Define the URL for the AJAX request
    const apiUrl = '<?=route('getTextVerification')?>'; // Replace with your actual API endpoint

    // Get the checkbox container
    const checkboxContainer = $('#dynamic-checkbox-container');
    checkboxContainer.empty(); // Clear any existing content
        ...
        success: function (response) {
            ...
            if (response && Array.isArray(response)) {
                ...
                response.forEach((item, index) => {
                    const isChecked = item.is_status === 1 ? 'checked' : ''; // Check if is_status is 1
    

                    const checkboxHtml = `
                        <div class="col-md-12">
                            <div class="form-check form-check-lg"> <!-- Large checkbox -->
                                <input class="form-check-input custom-checkbox" style="height: 12px; width: 26px;" type="checkbox" id="checkbox-${index}" name="dynamicCheckbox[]" value="${item.name}" ${isChecked}>
                                <input  type="hidden"   name="dynamicCheckboxId[]" value="${item.id}"  >

                                <label class="form-check-label" for="checkbox-${index}" style="    font-weight: 200;font-size:1.2rem;text-transform: uppercase;">
                                   <b> ${item.name}  </b>
                                </label>
                            </div>
                        </div>
                ...
                $('#text_verification_template_id').val(id);
                // Show the modal
                $('#UpdateTextVerificationData').modal('show');
            } else {
  ...
  <?php if(App\Helpers\RolePermissionCheck::isPermitted('pdf2pdf.templatemakeredit')): ?>
  	//edit data
  	$('#UserEdit').click(function(e){
  		...
  		var update_url = "<?=route('pdf2pdf.templatemasterupdate',':id')?>";
      ...
      $.ajax({
          ...
          success: function(response){
              ...
              if(res.type == 'Success'){                    
                  ...
                  $('#preview').html(res.msg+"<br /><input type='button' class='btn btn-primary progress-status' data-file='"+res.filename+"' data-templateid='"+res.template_id+"' data-pdfpage='"+res.pdf_page+"' data-unids='"+res.unids+"' data-progressfile='"+res.progressfile+"' data-previewPdf='"+previewPdf+"' value='Proceed' id='btn_proceed'>");
                  $('#btn_upload').hide();
                  $('#progress-file').hide();
                  $('#message-file').hide();
                  $('#progress-file').html('');
  ...
  $(document.body).on('click', "#btn_proceed", function(){    
      ...
      $.ajax({
          ...
          type: 'post',
          data: {'_token':'<?= csrf_token()?>',file:file, template_id:template_id, pdf_page:pdf_page, unids:unids, progressfile:progress_file,previewPdf:previewPdf},
          beforeSend : function() {
              $('#preview').html('Processing...');    
          },
          success: function(response){                
  ...
  $(document.body).on('click', "#btn_bg", function(){ 
      ...
      $.ajax({
          url: "<?=route('pdf2pdf.templatebgupdate')?>",
          type: "POST",
  ...
  $(function () {
   ...
   $('#UpdateTextVerificationForm').on('submit', function (e) {
    e.preventDefault();

    // Check if at least one checkbox is checked
    if ($('input[name="dynamicCheckbox[]"]:checked').length === 0) {
        toastr.error('Please select at least one Text verification Option.'); 
    ...
    $.ajax({
        url: '<?= route('saveTextVerification') ?>', // Use the correct route for saving
        type: 'POST',
        data: formData,
        headers: {
        ...
        success: function (response) {
            ...
            toastr.success('Text verification updated successfully!');
            $('#UpdateTextVerificationData').modal('hide');
        },
        error: function (xhr, status, error) {
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/admin/pdf2pdf/templateMaster/index.blade.php ENDPATH**/ ?>