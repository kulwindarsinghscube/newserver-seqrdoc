...
<div class="container">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i>  Branch
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;"><?php echo e(Breadcrumbs::render('Branch')); ?></ol>
				</h1>
    ...
    <!-- panel panel-default -->
     <div>
        <?php if(App\Helpers\SitePermissionCheck::isPermitted('add-branch')): ?>
        <?php if(App\Helpers\RolePermissionCheck::isPermitted('add-branch')): ?> 
...
<script type="text/javascript">
 ...
 $(document).ready(function() {
            var table = $('#tableYajra').DataTable({
                ...
                serverSide: true,
                ajax: {
                url: "<?php echo e(route('ksg-branch.datatable')); ?>",
                method: 'POST',
                data: {
                        approve_flag: 0,
                    },
...
$('#BranchForm').on('submit', function(event) {
        ...
        var url = $('#batchId').val() ? "<?php echo e(route('update-branch')); ?>" : "<?php echo e(route('add-branch')); ?>";
        $.ajax({
            url: url,
            method: 'POST',
            data: $(this).serialize(),
    ...
    $('#tableYajra').on('click', '.editBranch', function() {
        ...
        $.ajax({
            url: "<?php echo e(route('edit-branch', '')); ?>/" + id,
            method: 'GET',
    ...
    $('body').on('click', '.DeleteBranch', function () {
     ...
     var url = "<?php echo e(url('delete-branch')); ?>/" + id;
     if(userConfirmed){
             $.ajax({
                 type: "get",
                 url: url,
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/admin/ksg/branch_index.blade.php ENDPATH**/ ?>