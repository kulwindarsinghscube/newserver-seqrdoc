...
<div class="container">
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> SAIU
        ...
        <div class="row">
            <div class="col-lg-7"> <br />
                <form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('saiu-certificate.uploadfile')?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" id="template_id" name="template_id">
                                ...
                                <option value="1" data-tempid="1">Grade Certificate</option>
                                <option value="2" data-tempid="2">Degree Certificate</option>
                                <option value="3" data-tempid="3">Law And Regulation</option>
                                <option value="4" data-tempid="4">Law And Regulation - Daksha</option>
                                <option value="5" data-tempid="5">Technology Law And Policy - Daksha</option>
                                <option value="6" data-tempid="6">B.A.LL.B. (Hons.)</option>
                                
                                <option value="7" data-tempid="7">Six Semester Transcript</option>
                                <option value="8" data-tempid="8">Eight Semester Transcript</option>
                                <option value="9" data-tempid="9">Single / Two Semester Transcript</option>
                    ...
                    <div class="form-group">
                        <label class="control-label col-sm-3" for="temp">BG Merging:</label>
                        <div class="col-sm-9">
                            <select class="form-control" id="merging_type" name="merging_type">
                                <option value="0" data-merge="0">Select Merging</option>
                                <option value="Pre" data-merge="Pre">Pre</option>                    
                                <option value="Pro" data-merge="Pro">Pro</option>                    
                            </select>
                        </div>
                    </div> 
                    <?php 
                    $results = DB::select( DB::raw("SELECT * FROM background_template_master where status='1' order by id desc")); 
                    ?>
                    <div class="form-group">
                        <label class="control-label col-sm-3" for="temp">BG Image:</label>
                        <div class="col-sm-9">
                          <select id="verification_bg" name="verification_bg" class="form-control">
                          <option value="0" data-file="0">Select BG</option>
                            <?php $__currentLoopData = $results; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $got): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($got->image_path); ?>" data-file="<?php echo e($got->image_path); ?>"><?php echo e($got->background_name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> 
                          </select>  
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-offset-3 col-sm-9"><img id="image_verification" src='' width="50%"></div>
                    </div>                    
            ...
            <div class="col-lg-5" style="padding: 83px 1em;"> 
                <?php
                    ...
                    $excel_url_1=$path.$subdomain[0]."/backend/sample_excel/grade_card.xlsx";
                    $excel_url_2=$path.$subdomain[0]."/backend/sample_excel/degree_certicate.xlsx";
                    $excel_url_3=$path.$subdomain[0]."/backend/sample_excel/law_and_regulation.xlsx";
                    $excel_url_4=$path.$subdomain[0]."/backend/sample_excel/law_and_regulation_daksha.xlsx";
                    $excel_url_5=$path.$subdomain[0]."/backend/sample_excel/technolgy_law_and_policy_daksha.xlsx";
                    $excel_url_6=$path.$subdomain[0]."/backend/sample_excel/Sample Excel_B.A.LL.B. (Hons.).xlsx";
                    //$excel_url_7=$path.$subdomain[0]."/backend/sample_excel/Sample Excel_Provisional Transcript.xlsx";
                    $excel_url_7=$path.$subdomain[0]."/backend/sample_excel/Six Semester Transcript.xlsx";
                    
                    // $excel_url_8=$path.$subdomain[0]."/backend/sample_excel/Sample Excel_Provisional 8 semester transcript.xlsx";
                    $excel_url_8=$path.$subdomain[0]."/backend/sample_excel/Eight Semester Transcript.xlsx";
                    // $excel_url_9=$path.$subdomain[0]."/backend/sample_excel/Sample Excel_Provisional Transcript_new.xlsx";
                    $excel_url_9=$path.$subdomain[0]."/backend/sample_excel/Single or Two Semester Transcript.xlsx";
...
$('#btn_updfile').click(function (event) {
  var template_id=$('#template_id').val();
    var merging_type=$('#merging_type').val();
    var verification_bg=$('#verification_bg').val();
  if(template_id == 0){
    //alert("Please select template.");
    toastr["error"]("Please select template.");
    return false;
  }
    else if(merging_type == 0){
    toastr["error"]("Please select BG Merging.");
    return false;
  }
    else if(verification_bg == 0){
    toastr["error"]("Please select BG Image.");
    return false;
  }
  else{
    validateUpload();
...
function validateUpload() {
  ...
  $.ajax({
    url:'<?= route('saiu-certificate.validateexcel')?>',
      type: "POST",
    dataType: "JSON",
    data:fd,
    processData: false,
...
function uploadfile(){
  ...
  fd.append('template_id',template_id);
  fd.append('merging_type',merging_type);  
  fd.append('verification_bg',verification_bg);
  fd.append('field_file',files);
  fd.append('_token',token);
  fd.append('loader_token',$('#loader_token').val()); //for custom loader
  $.ajax({
    url:'<?= route('saiu-certificate.uploadfile')?>',
      type: "POST",
    dataType: "JSON",
    data:fd,
    processData: false,
...
$('#verification_bg').change(function(){
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\vhosts\seqrdoc.com\httpdocs\demo\resources\views/admin/saiu/index.blade.php ENDPATH**/ ?>