$(window).load(function(){   
    $('#overlay').fadeOut(); // PAGE IS FULLY LOADED. FADE OUT OVERLAYING DIV.
 });
 ...
 if(action == 'edit'){
     ...
     var original_file = getParameterByName('original_file'); 
     //var pdf = new PDFAnnotate('pdf-container', file_name, 'edit', 'documents/'+template_name+'.json');
     // console.log(config.subdomain+'/documents/'+template_name+'.json');
     var pdf = new PDFAnnotate('pdf-container', '../'+config.subdomain+'/'+file_name, 'edit', '../'+config.subdomain+'/documents/'+template_name+'.json');
 
     $('#template_id').val(rid);
     $('input[name="original_file"]').val(original_file);
 ...
 function enableZoomIn(event) {
     event.preventDefault();
     pdf.enableZoomIn();
 }
 function enableZoomOut(event) {
     event.preventDefault();
     pdf.enableZoomOut();
 }
 
 function enableRectangle(event,objectType) { 
     event.preventDefault();
     ...
     pdf.enableRectangle(id,objectType);    
 }
 ...
 {
     var sampleText=document.getElementById('sample_text').value;
     pdf.changeSampleText(sampleText);    
 }
 ...
 function changePlacerType()
 {
     ...
     if(PlacerType == "UV Image"){
         img_name=$('#uv_image_path').val();
     }
     if(PlacerType == "Image" || PlacerType == "UV Image"){
     $.ajax({
         url: 'imgList.php',
         type: 'post',
         data: {img_name: img_name, placer_type:PlacerType},
    ...
    // alert(subdomain);
     ...
     for(i in pdf.rectObjs){
         ...
         if(nature == 'extractor'){
             ...
             placerData['sample_text'] = pdf.rectObjs[i].sample_text;            
             ...
             if(pdf.rectObjs[i].placer_type == 'Watermark Multi Lines' && pdf.rectObjs[i].lineHeight==''){
                 alert("Please select line height of Watermark Multi Lines Background.");
                 return false; 
             }
             ...
             sourceData['sample_text'] = pdf.rectObjs[i].sample_text;
            ...
            //  console.log(pdf.rectObjs[i].placer_type);
     ...
     if(count_qr > 0){
        // console.log(extractorNameGroup);
 ...
 */  
 ...
 setInputFilter(document.getElementById("rect-width"), function(value) {
   return /^-?\d*[.]?\d*$/.test(value); });
 setInputFilter(document.getElementById("rect-height"), function(value) {
   return /^-?\d*[.]?\d*$/.test(value); });  
 setInputFilter(document.getElementById("placer_font_size"), function(value) {
   return /^-?\d*[.]?\d*$/.test(value); });  
 setInputFilter(document.getElementById("RectBoxId"), function(value) {
   return /^-?\d*[.]?\d*$/.test(value); });    
 setInputFilter(document.getElementById("degree_angle"), function(value) {
   return /^-?\d*[.]?\d*$/.test(value); });
 setInputFilter(document.getElementById("line_height"), function(value) {
   return /^-?\d*$/.test(value); });   
 ...
 $(document).ready(function(){
     ...
     $('#btn_upload').click(function(){        
         ...
         var original_file=$('#original_file').val();
         ...
         fd.append('original_file',original_file);
         ...
         $.ajax({
             ...
             success: function(response){
                 ...
                 if(res.type == 'Success'){                    
                     ...
                     var sub_domain = host.split('.');
 
                     
                     var url = baseUrl+'/'+sub_domain[0]
                     console.log(url);
                     $('#preview').html("<a href='"+url+'/'+res.dlink+"' target='_blank' style='color: #0056b3;'>Download</a>");
                 }
     ...
     $(document.body).on('click', "#select_image", function(){    
        ...
        $.ajax({
            ...
            type: 'post',
             // data: {img_name: img_name, placer_type:PlacerType},
            data: {_token:config.csrf_token,img_name: img_name,template_id:template_id},
            success: function(response){ 
               $('#imgModal .modal-body').html(response);
               $('#imgModal').modal({backdrop: 'static', keyboard: false, show: true}); 
            }
     ...
     $('#RegForm').validate({        
         ...
         submitHandler: function (form,event) { 
            ...
            //  if (record_id=='0'){
            //      url_page='image_FormSave.php';
            //  }else{
            //      url_page='image_FormEdit.php';
            //  }
            template_id=$("#template_id").val();
     ...
     $(document.body).on('click', "#pdf_preview", function(){    
         ...
         if(template_id > 0)
         {
                 ...
                 $.ajax({
                     // url: 'pdfPreviewExcel.php',
                     url: config.routes.pdfPreview,
                     type: 'post',
                     // data: {template_id:template_id, pdf_page:pdf_page},
                     ...
                     success: function(response){
                         ...
                         if(res.type == 'Success'){                    
                                ...
                                var baseUrl = window.location.origin; 
                                    ...
                                    var url = baseUrl+'/'+sub_domain[0]
                                    console.log(url);
                                    $('#preview_wait').html("<a href='"+url+'/'+res.dlink+"' target='_blank' style='color: white;'>View</a>");
                            //  $('#preview_wait').html("<a href='"+res.dlink+"' target='_blank' style='color: white;'>View</a>");
                         }
                         else{
                           alert('Error');
                         }
