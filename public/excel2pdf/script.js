...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(res.type == 'Success'){                    
                    ...
                    $('#preview').html(res.msg+"<br /><input type='button' class='btn btn-primary' data-file='"+res.filename+"' data-templateid='"+res.template_id+"' data-pdfpage='"+res.pdf_page+"' data-unids='"+res.unids+"' data-progressfile='"+res.progressfile+"' data-previewPdf='"+previewPdf+"' data-blockchainPdf='"+blockchainPdf+"' value='Proceed' id='btn_proceed'>");
                    $('#btn_upload').hide();
                    $('#progress-file').hide();
                    $('#message-file').hide();
                    $('#progress-file').html('');
    ...
    $(document.body).on('click', "#btn_proceed", function(){    
        ...
        $.ajax({
            ...
            type: 'post',
            data: {_token:config.csrf_token,file:file, template_id:template_id, pdf_page:pdf_page, unids:unids, progressfile:progress_file,previewPdf:previewPdf,blockchainPdf:blockchainPdf},
            beforeSend : function() {
                $('#preview').html('Processing...');
            }, 
            success: function(response){
