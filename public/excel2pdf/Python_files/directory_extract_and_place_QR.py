...
#userpdflocation=directory+"multi_pages"
userpdflocation=rootDir+sys.argv[7]+'/'+sys.argv[2]
...
#    docheck = fitz.open(userpdflocation+"/"+pdffilename)
#    pagecounts +=docheck.pageCount
...
if connection2.is_connected():
    ...
    recordGenerated= currentValue + int(totalPDF)
...
if sys.argv[4]=='Fresh':
    get_pdffiles = []
    duplicate_pdffiles = []
    for pdf_file_name in pdffiles:
        doc = fitz.open(userpdflocation+"/"+pdf_file_name)
        for pname in doc:
            pageNo=pname.number
            for boks in boxes:
                if pageNo == boks["page_no"]:
                    if pageNo == 0 and boks['placer_type'] == 'QR Dynamic':
                        get_text=[]
                        if boks['source'] != '' and boks['source'] != 'Current DateTime':
                            ...
                            if connection.is_connected():
                                ...
                                if row_count > 0:    
                                    ...
                                    duplicate_pdffiles.append(pdf_file_name)                             
    ...
    if not get_pdffiles:
        ...
        get_duplicate_pdffiles_string = ','.join(duplicate_pdffiles)
        print(get_duplicate_pdffiles_string)
        exit()

page_counts=len(pdffiles)
data_folder=rootDir+sys.argv[7]+'/'+"uploads/data/multi/"+str(record_unique_id) #path to store original pdfs
...
for file_name in pdffiles:
    doc = fitz.open(userpdflocation+"/"+file_name)
    ...
    print(file_name)
    save_file.append(file_name) #save into DB table
    output_file = rootDir+sys.argv[7]+'/'+"processed_pdfs/"+file_name
    path_pdf_moved=inner_folder+"/" + file_name
    ...
    for i in doc:        
        ...
        #print(page_dict)
        dirName = rootDir+sys.argv[7]+'/'+"documents/" + str(sys.argv[1])        
        ...
        if watermark_check == "On Each Page":
            ...
            wr.fillTextbox(new_page_rect, wm_repeat_txt, fontsize=wm_placer_font_size, align=wm_placer_align)
        ...
        if qr_position_check == "On Each Page":            
            ...
            if verification_setbg=='Yes':
                for dpage in doc_new:
                    dpage.insertImage(page_rect, verification_bg_file,overlay=False)
            doc_new.save(pdf_folder+"/"+dtstring+".pdf", garbage=4, deflate=True)
           # doc_new.save(pdf_folder+"/"+dtstring+".pdf", garbage=4, deflate=True, owner_pw="owner", encryption=encrypt_meth, permissions=perm)
    ...
    if connection.is_connected():
        ...
        if qr_position_check == "On Each Page":
            val = (file_records_next_id, template_id, template_name, pdf_page, cnt, barcode_encode, dtstring, add_qr_edited_txt, userid, record_unique_id)
            certificate_filename=dtstring+'.pdf'
        else:
        ...
        if qr_position_check == "On Each Page":
            serial_no_cert=dtstring
            qr_path='qr/'+barcode_encode+'.png'
            val2 = (dtstring, certificate_filename, template_id, barcode_encode, qr_path, userid, userid, 1, 1, current_date_time,sys.argv[13],1)
        else: 
            serial_no_cert=dt_string
        ...
        cursor.execute("SELECT * FROM printing_details WHERE sr_no = '%s' " % (serial_no_cert))
        records = cursor.fetchall()  
        ...
        val3 = (sys.argv[14], current_date_time, printer_name, print_count, next_print_serial, serial_no_cert, template_name, current_date_time, userid, current_date_time,userid, 1, sys.argv[13], 1,student_table_id)
        ...
        if page_no==PageCount-1: 
            ...
            if qr_position_check == "On Each Page":
                page_sheet.append([dtstring,barcode_encode,add_qr_edited_txt])
                #print(dtstring,barcode_encode,qr_text)
            else:
                page_sheet.append([dt_string,barcode_en,qr_txt])
            ...
            count_pages=cnt * PageCount
            arr_content['percent'] = int(cnt/page_counts * 100)
            arr_content['message'] = "Generating "+str(cnt)+"/"+str(page_counts)+" PDF(s)"
            ...
            arr_content['pages_processed'] = str(count_pages)+" ("+str(PageCount)+" Pages x "+str(cnt)+" PDFs)"
    ...
    shutil.copyfile(userpdflocation+"/"+file_name, data_folder+"/"+file_name)
    
    
if connection.is_connected():
    save_files = ','.join(save_file)
    sql = "INSERT INTO file_records (template_id, template_name, pdf_page, total_records, pages_in_pdf, source_file, userid, record_unique_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
    val = (template_id, cnt-1, file_name)
    val = (template_id, template_name, pdf_page, cnt-1, PageCount, save_files, userid, record_unique_id)
    cursor.execute(sql, val)
    connection.commit()
    
    for x in save_file:
        print(userpdflocation+"/"+x)
        if os.path.isfile(userpdflocation+"/"+x):
            os.remove(userpdflocation+"/"+x)   
...
#print("documents/"+template_name+"/"+template_name+"-"+str(record_unique_id)+".zip")
#print(placer_font_name.flags)
#os.rmdir(userpdflocation)
#os.chmod(userpdflocation, stat.S_IRWXU| stat.S_IRWXG| stat.S_IRWXO)

if (userpdflocation.find('multi_pages/') != -1):      
    shutil.rmtree(userpdflocation, ignore_errors=True)
    #os.removedirs(userpdflocation)
"""
