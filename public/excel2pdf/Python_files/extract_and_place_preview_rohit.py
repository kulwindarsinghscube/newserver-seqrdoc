...
#sys.argv[16] = preview or generate
...
for i in doc:
   ...
   # print(dirName)
    for box in boxes:
        ...
        """ 
        if not os.path.exists(rootDir+sys.argv[7]+"/processed_pdfs/preview"):
                os.makedirs(rootDir+sys.argv[7]+"/processed_pdfs/preview") 
        ...
        dt_string_file_name = now.strftime("%Y%m%d%H%M%S")
        previewFile = rootDir+sys.argv[7]+"/processed_pdfs/preview/"+sys.argv[1]+"/"+ dt_string_file_name+".pdf"

        doc_new.save(previewFile, garbage=4, deflate=True)
        # pdf_file_path=rootDir+sys.argv[7]+"/processed_pdfs/preview/" + dt_string+".pdf"
        # shutil.move(rootDir+"/processed_pdfs/preview/" + dt_string+".pdf", pdf_file_path)
        """
        doc_new.save(pdf_folder+"/"+dt_string+".pdf", garbage=4, deflate=True)
        pdf_file_path=rootDir+sys.argv[7]+"/processed_pdfs/preview/" + dt_string+".pdf"
