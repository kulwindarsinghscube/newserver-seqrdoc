...
try:
    ...
    if connection.is_connected():
        ...
        cursor.execute("select ep_details, id, extractor_details, template_name, pdf_page, print_bg_file, print_bg_status, verification_bg_file, verification_bg_status, IFNULL(bc_contract_address, '') AS bc_contract_address, bc_document_description, bc_document_type,IFNULL(bc_contract_address_test, '') AS bc_contract_address_test from uploaded_pdfs where id = '%s'" % (sys.argv[1]))
        ...
        bc_contract_address_test=record[12]
...
if connection2.is_connected():
    ...
    cursor2.execute("select IFNULL(bc_wallet_address, '') AS bc_wallet_address,IFNULL(bc_wallet_address_test, '') AS bc_wallet_address_test from sites where site_url = '%s'" % (siteurl_param))
    rsSite = cursor2.fetchone()
    bc_wallet_address=rsSite[0]
    bc_wallet_address_test=rsSite[1]
...
for i in doc:
   ...
   # print(dirName)
    ...
    # sys.argv[19] == blockchain param  -- [0 -> test || 1 -> live]
    # Update Rohit 01/09/2023
    ...
    if use_count > 0 and bc_contract_address != '' and sys.argv[18] != '1':      
        ...
        if sys.argv[7]=='mpkv' or sys.argv[7]=='ksu' or sys.argv[7]=='konkankrishi' or (sys.argv[7]=='demo' and template_id==137):
            if(sys.argv[7]=='demo'):
                ...
                blockchain_response = requests.post('http://veraciousapis.herokuapp.com/v1/mint', data=mintData, files=files)
        else:
            if sys.argv[19]=='0':
                mintData["description"]=mintData["description"]
                mintData["walletID"] = bc_wallet_address
                # mintData["walletID"] = bc_wallet_address_test
                mintData["smartContractAddress"] = bc_contract_address
                # mintData["smartContractAddress"] = bc_contract_address_test
                mintData["uniqueHash"] = barcode_en
                mintData["pdf_file"] = "https://"+sys.argv[7]+".seqrdoclocal.com/"+sys.argv[7]+"/backend/pdf_file/" + dt_string+".pdf"
                blockchain_response = requests.post('http://mainnet-apis.herokuapp.com/v1/mainnet/mint', data=mintData, files=files)
                ...
                if blockchain_response.status_code == 200:
                    blockchain_message_i = 'success'
                else:
                    blockchain_message_i = 'failed'
                # Update Rohit 20/03/2024
                mint_json1 = blockchain_response.text
                if connection.is_connected():
                    sql = "INSERT INTO bc_api_tracker (api_name, request_method, request_url, request_parameters,response,status,client_ip) VALUES (%s,%s,%s,%s,%s,%s,%s)"
                    val = ("mintData", "POST", "http://mainnet-apis.herokuapp.com/v1/mainnet/mint", json.dumps( mintData ),mint_json1,blockchain_message_i,ip_address)
    ...
    # site_url='demo.seqrdoclocal.com'
    # if connection.is_connected():
    ...
    #     siteData = cursor.fetchone()
        ...
        if blockchain_response.status_code == 200:
            ...
            # Update Rohit 01/09/2023
            if sys.argv[18] == '1':
                sql2 = """INSERT INTO sb_student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`bc_generation_type`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            else:
                sql2 = """INSERT INTO student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`bc_generation_type`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            # Update Rohit 01/09/2023
            val2 = (dt_string, certificate_filename, template_id, barcode_en, qr_path, userid, userid, 1, 1, current_date_time,sys.argv[13],1,mint_json['txnHash'],mint_json['ipfsHash'],sys.argv[19])
        else:
