...
output_file = rootDir+sys.argv[7]+'/'+"processed_pdfs/preview/"
preview_folder = rootDir+sys.argv[7]+"/processed_pdfs/preview"

folder=output_file + template_name
...
# for i in doc:
    ...
    for box in boxes:
        ...
        # print(box['degree_angle'])
        placer_degree_angle = box['degree_angle']
        
        # print(placer_degree_angle)
        ...
        if placer_type == 'QR Default': 
            ...
            if(int(float(nxt_prt_width)) > int(box['width'])):
                # calculate_coordinates
                
                nxt_prt_width_v1 = int(float(pcoords[0])) + int(float(nxt_prt_width))
                nxt_prt_height_v1 = int(float(pcoords[1]))+ int(float(nxt_prt_height))
            ...
            # if box['source'] in keys:
            #     col_inx=keys.index(box['source'])
            # else:
            #     col_inx = None 

            # if col_inx is not None:
            ...
            if box['source'] in keys:
                ...
                col_inx = None
                print(f"Warning: '{box['source']}' not found in keys list.")

            if col_inx is not None:
                temp_img = str(sheet_obj.cell(row=cnt + 1, column=col_inx + 1).value)
                ImagedirName = os.path.join(rootDir, sys.argv[7], "backend/templates/excel2pdf", sys.argv[1])

                if temp_img and temp_img.lower() != 'none':  # Ensure `temp_img` is valid
                    img_path = os.path.join(ImagedirName, temp_img)

                    if os.path.exists(img_path):
                        try:
                            pix = fitz.Pixmap(img_path)
                            
                            # Ensure `prect` is properly defined before using it
                            if 'prect' in locals() and prect is not None:
                                page.insertImage(prect, img_path, overlay=True)
                            else:
                                print("Error: `prect` is not defined or invalid.")
                        except Exception as e:
                            print(f"Error processing image at '{img_path}': {e}")
                    else:
                        print(f"Error: Image file '{img_path}' does not exist.")
                else:
                    print(f"Warning: Invalid or missing image name '{temp_img}'.")
            else:
                print("Warning: Column index for box['source'] is None.")
...
sys.argv[2] = sys.argv[2].replace('.xlsx','')
doc.save(pdf_folder+"/"+sys.argv[2]+".pdf", garbage=4, deflate=True)

pdfPath = pdf_folder+"/"+sys.argv[2]+".pdf"
