...
function getFieldPanel($id=1, $ratio)
{
    ...
    if($id == 1){
		...
		/* Rohit Changes */
		str += '<tr id="metadataDivRohit">';
		str += '<td colspan="2" id="is_meta_label">Add Encrypted Text </td>';
	    str += '<td colspan="2"><input type="checkbox" name="is_encrypted_qr" id="is_encrypted_qr" value="'+$('#is_encrypted_qr').val()+'"></td>';
		str += '</tr>';
		var is_encrypted_qr = $('#is_encrypted_qr').val();

		if(is_encrypted_qr==1){
			//console.log('Metadata '+is_meta_data);
			var encrypted_qr_text_display="";
		}else{
			var encrypted_qr_text_display="display:none;";
		}
		str += '<tr id="metadataDetailsDivRohit" style="'+encrypted_qr_text_display+'">';
		str += '<td >Encrypted QR  Text</td>';
		str += '<td colspan="3"><textarea type="text" style="width:100%;" class="custom_text" id="encrypted_qr_text" name="encrypted_qr_text">GUID : {{QR Code}}</textarea></td>';
		str += '</tr>';
		/* End Rohit Changes */
