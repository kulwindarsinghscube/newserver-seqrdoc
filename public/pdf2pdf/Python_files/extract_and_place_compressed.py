...
def extract_infoV1(extractor_detail,search_value,extra_line_first='',extra_line_second='',page=''):
    for keyval in extractor_detail:
        if search_value.lower() == keyval['name'].lower():
            source_coord = keyval['coords']
            scoord = source_coord.split(",")
    ...
    for box in boxes:
        ...
        if placer_type == 'QR Default': 
            ...
            if barcode_content_flag==1:
                # if sys.argv[7] =='imcc':
                if sys.argv[7] == 'imcc' or sys.argv[7] == 'dypcoe':
                    temp=''
                    if box['qr_details']=='':
                        get_text=[]
                        get_text.append(page.getTextbox(srect).replace("\n", ""))
            ...
            if box['qr_details']=='':  
                ...
                for inv in qr_list:
                    str_val = inv.split("^")
                    str_val_count=len(str_val)
                    inv_txt = inv_txt + extract_infoV1(extractor_details,str_val[1],str(str_val[0]),str(str_val[2]),page)                          
            ...
            if otxt_string == 'current_date':
                nowv1 = datetime.datetime.now()
                formatted_date = nowv1.strftime("%d/%m/%Y")
                otxt_string = formatted_date
            wr.fillTextbox(prect, otxt_string, font=placer_font_name, fontsize=placer_font_size, align=placer_align)
