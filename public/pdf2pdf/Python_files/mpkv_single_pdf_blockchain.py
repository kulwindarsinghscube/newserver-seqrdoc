...
try:
    ...
    username = 'developer'
    password = 'developer'
    bc_wallet_address = '0xcbC1410a0fED332BaDEF2c734130927184f1Cb59' 
    ...
    if connection.is_connected():
        ...
        cursor.execute("select * from student_table where template_id='%s' AND `status`=1 AND publish=1 AND bc_txn_hash IS NULL" % (template_id))
        ...
        #cursor.execute("select * from student_table where serial_no ='MPKV000008113' AND template_id='%s' AND updated_at <='2024-01-30 16:40:21' AND `status`=1 AND publish=1 limit 1" % (template_id))
                ...
                if os.path.exists(rootDir+subdomain+'/backend/pdf_file/'+certificate_filename):
                    ...
                    for i in doc:
                        ...
                        if use_count > 0 and bc_contract_address != '':    
                            ...
                            mintData["description"]=mintData["description"] + " "+serial_no
                        ...
                        if status_code == 200:                            
                           #print(mint_json['txnHash'], mint_json['gasPrice'], mint_json['tokenID'], barcode_en, current_date_time)
                            sql = "UPDATE student_table SET bc_txn_hash = %s, bc_ipfs_hash = %s WHERE serial_no = %s AND status = %s"
                            val = (mint_json['txnHash'], mint_json['ipfsHash'], serial_no, '1')
