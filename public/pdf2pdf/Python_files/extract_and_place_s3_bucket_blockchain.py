...
def checkContract(wallet_address,template_id, limit=100):
    try:
        ...
        """
        cursor.execute(check_count_query, (template_id,))
        ...
        if current_contract is not None and (current_contract[0] < limit):
            ...
            if blockchain_response.get('status') == 200:
                ...
                """
                cursor.execute(deactivate_existing_contracts_query, (template_id,))
...
for i in doc:
   ...
   # print(dirName)
    ...
    if use_count > 0 and bc_contract_address != '' and sys.argv[18] != '1':      
        ...
        if sys.argv[7]=='mpkv' or sys.argv[7]=='ksu' or sys.argv[7]=='konkankrishi' or (sys.argv[7]=='demo' and (template_id==137 or template_id==162)) or (sys.argv[7]=='demo' and template_id==144) or (sys.argv[7]=='demo' and template_id==145):
            if(sys.argv[7]=='demo' and template_id!=162):
                mintData["walletID"] = "0xB509AF6532Af95eE59286A8235f2A290c26b5730" 
            elif(sys.argv[7]=='demo' and template_id==162):
                mintData["walletID"] = "0x2A13685119CBfa6B780c191b59eA97d6BD572f14" 
    ...
    #     siteData = cursor.fetchone()
        ...
        if blockchain_response.status_code == 200:
            ...
            # Update Rohit 01/09/2023
            if sys.argv[18] == '1':
                sql2 = """INSERT INTO sb_student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`pinata_ipfs_hash`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            else:
                sql2 = """INSERT INTO student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`pinata_ipfs_hash`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            # Update Rohit 01/09/2023
        ...
        if blockchain_response.status_code == 200:
            ...
            val = (mint_json['txnHash'],mint_json['ipfsHash'],mint_json['pinataIpfsHash'], dt_string, '1')
            # Update Rohit 01/09/2023
            cursor.execute(sql, val)
            # Update Rohit 01/09/2023
            if sys.argv[18] != '1':
