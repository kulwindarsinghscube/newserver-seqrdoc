...
try:
    ...
    if connection.is_connected():
        ...
        cursor.execute("select ep_details, id, extractor_details, template_name, pdf_page, print_bg_file, print_bg_status, verification_bg_file, verification_bg_status, template_title from uploaded_pdfs where id = '%s'" % (sys.argv[1]))
...
for i in doc:
   ...
   # print(dirName)
    for box in boxes:
        ...
        if placer_type == 'QR Default': 
            ...
            if connection.is_connected():
                ...
                next_print_serial_save='CD-'+str(next_printSN)
    ...
    subprocess.Popen(['C:/Program Files/gs/gs9.56.1/bin/gswin64c.exe', '-sDEVICE=pdfwrite', '-dCompatibilityLevel=1.4', '-dPDFSETTINGS=/screen', '-dNOPAUSE', '-dBATCH',  '-dQUIET', str(arg1), arg2], stdout=subprocess.PIPE)
    ...
    #     siteData = cursor.fetchone()
        ...
        sql4 = """INSERT INTO qr_contents (`logs_id`, `serial_no`, `key`, `path`, `filename`, `template_name`, `compress_status`, `status`, `publish`, `created_at`, `updated_at`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        val4 = (template_id,next_print_serial_save, barcode_en, qr_path, certificate_filename, template_title, 0, 1, 1, current_date_time,current_date_time)
...
bucket = "securedoc"
client = boto3.client('s3', aws_access_key_id='AKIAYVH7GDWGKCX5WVMQ', aws_secret_access_key='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', region_name='ap-south-1')
for source_pdffile in test_pdffiles:
    pdffilename = source_pdffile.rsplit('/', 1)[-1]
    source_pdffile = source_pdffile.replace('\\\\', '\\')
    destination_path="public/newserver/backend/pdf_file/"
    destination_file="public/newserver/backend/pdf_file/" + pdffilename
    transfer = S3Transfer(client)
    transfer.upload_file(source_pdffile, bucket, destination_file)
"""
for source_pdffile in test_pdffiles:
    source_pdffile = source_pdffile.replace('\\\\', '\\')
    if os.path.exists(source_pdffile):
        os.remove(source_pdffile)
"""
