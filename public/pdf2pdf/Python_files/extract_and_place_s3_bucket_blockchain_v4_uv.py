...
def update_student(student_id, bc_sc_id):
    try:
        ...
        print(f"An error occurred: update_student  {err}")
...
def CreateGhostImage(dirName, name, p_font_size, ghost_width, ghost_height):
    ...
    # This code is local
    # im.thumbnail((isize), Image.Resampling.LANCZOS)
    # This code is local
...
# def generate_file_hash(file_path, salt):
...
if connection2.is_connected():
    ...
    if rsSite:
        ...
        bc_wallet_address = ''
            ...
            # checkContractExist =checkContract(template_id,int(page_count))
...
for i in doc:
   ...
   # print(dirName)
    ...
    for box in boxes:
        ...
        if placer_type == 'QR Default': 
            ...
            if qr_show_flag==1:
                ...
                encryptedData=base64.b64encode(dt_string.encode())
                qr_txt="https://"+sys.argv[7]+".seqrdoc.com/document/" + encryptedData.decode()
                qr.add_data(qr_txt)
        ...
        if sys.argv[18] == '1':
            ...
            if(sys.argv[7] == 'demo'):
                # Construct the target folder and inactive PDF file path
                inactive_folder = os.path.join(rootDir, sys.argv[7], "backend/pdf_file", "Inactive_PDF")
                inactive_pdf_file_path = os.path.join(inactive_folder, f"{dt_string}.pdf")

                # Check if the file exists
                if os.path.exists(pdf_file_path):
                    # Check if the target folder exists, if not, create it
                    if not os.path.exists(inactive_folder):
                        os.makedirs(inactive_folder)
                    
                    # Move the file to the inactive folder
                    shutil.move(pdf_file_path, inactive_pdf_file_path)
                    print(f"File moved to {inactive_pdf_file_path}")
                else:
    ...
    # Only Enable for demo instance and specific template_id
    if(bc_wallet_address):
        # if (sys.argv[7] == 'demo' and template_id == 120) or (sys.argv[7] == 'demo' and template_id == 155):
    ...
    if use_count > 0 and bc_contract_address != '' and sys.argv[18] != '1':      
        ...
        # print('1')
            # print(mintData)    
            # if len(sys.argv) > 21 and sys.argv[20] and sys.argv[21]:
            #     mintData["walletID"] = sys.argv[20]
            #     mintData["smartContractAddress"] = sys.argv[21]
            #     print(f"walletID: {mintData['walletID']}, smartContractAddress: {mintData['smartContractAddress']}")
          
            # blockchain_url = 'https://mainnet-apis.herokuapp.com/v1/mainnet/mint'
        ...
        #blockchain_response = requests.post(blockchain_url, data=mintData, files=files)

        headers = {
            'accept': 'application/json',
            'x-api-key': '123456789'
        }
        # blockchain_response = requests.request("POST", blockchain_url, headers=headers, data=mintData, files=files)
        
        blockchain_response = requests.post(blockchain_url, headers=headers, data=mintData, files=files)

            # print('2')
        # else:
        #     blockchain_url = 'https://veraciousapis.herokuapp.com/v1/mint'
        #     blockchain_response = requests.post(blockchain_url, data=mintData, files=files)
        #     # print('4')
    else:
    ...
    if connection.is_connected():
        ...
        if row_count > 0: 
            ...
            if sys.argv[7] == 'demo':
                # Rename the copied file to include the student name
                inactive_folder = os.path.join(rootDir, sys.argv[7], "backend/pdf_file", "Inactive_PDF")
                inactive_pdf_file_path_rename = os.path.join(inactive_folder, f"{records[0][0]}_{dt_string}.pdf")
                copied_file_path = os.path.join(inactive_folder, f"{dt_string}.pdf")
                
                # Check if the target file already exists
                if os.path.exists(inactive_pdf_file_path_rename):
                    # print(f"File {inactive_pdf_file_path_rename} already exists. Generating a unique name.")
                    
                    # Generate a unique name by appending a counter or timestamp
                    base_name, ext = os.path.splitext(inactive_pdf_file_path_rename)
                    counter = 1
                    while os.path.exists(inactive_pdf_file_path_rename):
                        inactive_pdf_file_path_rename = f"{base_name}_{counter}{ext}"
                        counter += 1
                
                # Rename the file
                os.rename(copied_file_path, inactive_pdf_file_path_rename)
    ...
    #     siteData = cursor.fetchone()
        ...
        # salt = 'AJITNATH'
        # hash_result = generate_file_hash("https://"+sys.argv[7]+".seqrdoc.com/"+sys.argv[7]+"/backend/pdf_file/" + dt_string+".pdf", salt)
        salt = 'AJITNATH'
        ...
        if blockchain_response.status_code == 200:
            ...
            # ipfs_hash = mint_json.get('ipfsHash')
            # pinata_ipfs_hash = mint_json.get('pinataIpfsHash')
            # txn_hash = mint_json.get('txnHash')

            # val2 = (
            #     dt_string, certificate_filename, template_id, barcode_en, qr_path,
            #     userid, userid, 1, 1, current_date_time,
            #     sys.argv[13], 1,
            #     txn_hash, ipfs_hash, pinata_ipfs_hash
            # )
        ...
        if blockchain_response.status_code == 200:
            # bc_sc_id = sys.argv[19]
            # update_student(student_table_id, bc_sc_id)
            mint_json = json.loads(blockchain_response.text)
            ...
            # Update Rohit 01/09/2023
                ...
                response_time = round(time.time() - bc_start_time, 3)
                try:
                    ...
                    val2313 = (
                        ...
                        'success',
                        client_ip,
                        str(response_time),                # response_time (optional or calculate duration)
                        current_datetime_bc
                    )
