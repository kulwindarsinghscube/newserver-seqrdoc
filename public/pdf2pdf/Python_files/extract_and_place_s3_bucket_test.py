   ...
   # print(dirName)
    ...
    for box in boxes:
        ...
        if placer_type == 'QR Default': 
            ...
            get_text=[]
            get_text.append(page.getTextbox(srect).replace("\n", ""))
            otxt_string = get_text[0].strip().replace("/", "").replace(".", "")
            ...
            l=otxt_string.split(' ', 1) 
            PrintableChars=l[0] #extract first word
        ...
        if sys.argv[13] :
            URL = "https://"+sys.argv[7]+"."+sys.argv[10]+"/admin/store-file"
            PARAMS = {'pdf_path':pdf_file_path,'site_id':sys.argv[13]}
            response =requests.get(url = URL, params = PARAMS, timeout=None)        
            response.close()
        """
        ...
        if sys.argv[7]=='test':            
            s3_client = boto3.client(
                ...
                aws_access_key_id='AKIAYVH7GDWGCLDP3BO5',
                aws_secret_access_key='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'
            )
            #destination_file="public/"+sys.argv[7]+"/backend/pdf_file/" + dt_string+".pdf"
            #bucket = "securedoc"
            source_file=pdf_file_path
            #aws_response=s3_client.upload_file(source_file, bucket, destination_file)             
...
#         region_name='ap-south-1',
#         aws_access_key_id='AKIAYVH7GDWGCLDP3BO5',
#         aws_secret_access_key='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'
#     )    
#     bucket = "securedoc"
#     client = boto3.client('s3', aws_access_key_id='AKIAYVH7GDWGCLDP3BO5',
#                      aws_secret_access_key='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')
#     for source_pdffile in test_pdffiles:
#         pdffilename = source_pdffile.rsplit('/', 1)[-1]
#         source_pdffile = source_pdffile.replace('\\\\', '\\')
#         print("file to upload "+ source_pdffile)
#         destination_file="public/"+sys.argv[7]+"/backend/pdf_file/" + pdffilename
#         #aws_response=s3_client.upload_file(source_pdffile, bucket, destination_file)
#         transfer = S3Transfer(client)
#         transfer.upload_file(source_pdffile, bucket, "public/"+sys.argv[7]+"/backend/pdf_file/"+pdffilename)
#         response = client.put_object_acl(ACL='public-read', Bucket=bucket, Key="%s/%s" % ("public/"+sys.argv[7]+"/backend/pdf_file/", pdffilename))
#         print(pdffilename)
    #source_pdffiles_string = ','.join(test_pdffiles)        
    #print(source_pdffiles_string)
