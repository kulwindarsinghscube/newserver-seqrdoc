    ...
    try:
        ...
        if current_contract is not None and (current_contract[0] < limit):
            ...
            if blockchain_response.get('status') == 200:
                ...
                """
                cursor_bc.execute(deactivate_existing_contracts_query, (current_date_time,template_id,))
                ...
                # )
                current_date_timeV1=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S');
                insert_new_contract_query = """
                    INSERT INTO bc_smart_contracts 
                    (count, template_id, smart_contract_address, wallet_address, template_type, is_active, is_live,created_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s);
                    """
                cursor_bc.execute(insert_new_contract_query, (0, template_id, contract_address, bc_wallet_address, 1, 1, 1,current_date_timeV1))
                connection3.commit()
   ...
   # print(dirName)
    ...
    #     siteData = cursor.fetchone()
        ...
        if blockchain_response.status_code == 200:
            ...
            # Update Rohit 01/09/2023
            # Old code comment on 29-07-2025
            # if sys.argv[18] == '1':
            #     sql2 = """INSERT INTO sb_student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`pinata_ipfs_hash`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            # else:
            #     sql2 = """INSERT INTO student_table (`serial_no`, `certificate_filename`, `template_id`, `key`, `path`, `created_by`, `updated_by`,`status`, `publish`, `created_at`, `site_id`,`template_type`, `bc_txn_hash`,`bc_ipfs_hash`,`pinata_ipfs_hash`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            # # Update Rohit 01/09/2023
        ...
        if blockchain_response.status_code == 200:
            ...
            # Update Blockchain Rohit 17/06/2025
            if bc_sc_id:
                ...
                current_datetime_bcV1 = now.strftime('%Y-%m-%d %H:%M:%S')
                sql2308V1 = "UPDATE bc_smart_contracts SET count = count+1,updated_at = %s WHERE id = %s"
                val2308V1 = (current_datetime_bcV1,bc_sc_id,)
