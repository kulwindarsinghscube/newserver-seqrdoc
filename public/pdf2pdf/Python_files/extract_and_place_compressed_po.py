...
try:
    ...
    if connection.is_connected():
        ...
        cursor.execute("select ep_details, id, extractor_details, template_name, pdf_page, print_bg_file, print_bg_status, verification_bg_file, verification_bg_status,verification_type from uploaded_pdfs where id = '%s'" % (sys.argv[1]))
...
def extract_barcodeText(extractor_detail,search_value,extra_line_first='',extra_line_second='',page=''):
    for keyval in extractor_detail:
        if search_value.lower() == keyval['name'].lower():
            source_coord = keyval['coords']
            scoord = source_coord.split(",")
...
for i in doc:
    ...
    # print(dirName)
    # Added by rohit for verification in po 17-06-2025
    verificationData = []
    verification_type_flag = 0


    if verification_type == 1:
        i=0
        for box_verifiction in extractor_details:
            meta_value = ''
    
            if "text_verification" in box_verifiction:
                if box_verifiction['text_verification'] == 'use':
                    verification_type_flag = 1
            
            if verification_type_flag == 1:
                text_verification_id=box_verifiction['id']
                text_verification_lable=box_verifiction['text_verification_lable']
                text_verification_value=box_verifiction['text_verification_value'] 
                text_verification_value = text_verification_value.replace("{", "").replace("}", "")
                meta_value=''
                meta_list=list(filter(bool, text_verification_value.splitlines()))
                for mdv in meta_list:
                    str_val = mdv.split("^")
                    if i == 0:
                        meta_value = extract_plainText(extractor_details,str_val[1],str(str_val[0]),str(str_val[2]),page)
                    else:
                        meta_value = meta_value + extract_plainText(extractor_details,str_val[1],str(str_val[0]),str(str_val[2]),page)
                verificationData.append(json.dumps(dict(id=text_verification_id,label = text_verification_lable, value = meta_value)))
                i=i+1
    ...
    for box in boxes:
        ...
        if placer_type == 'QR Default': 
            ...
            sys.argv[1]
            if sys.argv[7] =='wcem' and sys.argv[1] =='1':
                qr_txt =  "https://wcem.edu.in/marksheet-verification" +"\n"+"\n"+barcode_en
                # qr_txt =  qr_txt + "\n"+ "https://wcem.edu.in/marksheet-verification" +"\n"+"\n"+barcode_en
                # qr_txt =  "https://wcem.edu.in/marksheet-verification"
            else:
                qr_txt =  qr_txt + "\n"+barcode_en
            #print(qr_txt)
                ...
                if sys.argv[7] =='imcc':
                    ...
                    if box['qr_details']=='':
                        ...
                        for inv in qr_list:
                            str_val = inv.split("^")
                            str_val_count=len(str_val)
                            temp = temp + extract_barcodeText(extractor_details,str_val[1],str(str_val[0]),str(str_val[2]),page)
                            next_print_serial=temp
                else:
                    get_text=[]
                    get_text.append(page.getTextbox(srect).replace("\n", ""))
                    temp = get_text[0].replace(" ", "").strip()
    ...
    #     siteData = cursor.fetchone()
        ...
        # Added by rohit for verification in po 17-06-2025
        if verification_type == 1 and verificationData:  
            # Convert the list to a JSON string
            verification_data_json = json.dumps(verificationData)
            
            # Encode the JSON string to bytes
            encoded_data = base64.b64encode(verification_data_json.encode('utf-8')).decode('utf-8')
            decoded_data = base64.b64decode(encoded_data)

            decoded_json = decoded_data.decode('utf-8') 
            print(json.loads(decoded_json))
            # Prepare the INSERT query
            query = """
                INSERT INTO text_verification_data (student_id, text)
                VALUES (%s,%s)
            """ 
            # Execute the query
            cursor.execute(query, (student_table_id, encoded_data ))
        ...
        next_print=record[0]+1
        if sys.argv[7] =='imcc':
            next_print_serial=next_print_serial
        else:
            next_print_serial=current_year+str(next_print)
