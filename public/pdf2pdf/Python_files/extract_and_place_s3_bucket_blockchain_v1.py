    ...
    try:
        ...
        if current_contract is not None and (current_contract[0] < limit):
            ...
            if blockchain_response.get('status') == 200:
                ...
                insert_new_contract_query = """
                    INSERT INTO bc_smart_contracts 
                    (count, template_id, smart_contract_address, wallet_address, template_type, is_active, ,created_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s);
                    """
                cursor_bc.execute(insert_new_contract_query, (0, template_id, contract_address, bc_wallet_address, 1, 1, 1,now_date_time))
                connection3.commit()
...
for i in doc:
   ...
   # print(dirName)
    ...
    if use_count > 0 and use_count < 6:
        for uc in range(use_count+1, 7):
    ...
    if use_count > 0 and bc_contract_address != '' and sys.argv[18] != '1':      
        ...
        if sys.argv[7]=='alarduni':
            mintData["description"]=mintData["description"] + " "+use_barcode_extracted
    ...
    #     siteData = cursor.fetchone()
        ...
        if blockchain_response.status_code == 200:
            ...
            # Update Rohit 01/09/2023
                ...
                if student:
                    ...
                    sql = """
                    INSERT INTO blockchain_other_data (student_table_id, vendor_identifier,bc_md_ipfs_hash)
                    VALUES (%s, %s, %s)
                    ON DUPLICATE KEY UPDATE vendor_identifier = VALUES(vendor_identifier)
                    """
                    cursor.execute(sql, (student_id, 1,mint_json.get('metadata_ipfs_hash')))
                    connection.commit()
            ...
            if bc_sc_id:
                ...
                val2308V1 = (now_date_time,bc_sc_id,)
        ...
        if sys.argv[18] == '1':
            ...
            sql3 = """INSERT INTO printing_details (`username`, `print_datetime`, `printer_name`, `print_count`, `print_serial_no`, `sr_no`, `template_name`,`created_at`, `created_by`, `updated_at`, `updated_by`,`status`, `site_id`, `publish`, `student_table_id`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        # Update Rohit 01/09/2023
        if (sys.argv[7] == 'alarduni') and (use_barcode_extracted != None):
            val3 = (sys.argv[14], current_date_time, printer_name, print_count, use_barcode_extracted, dt_string, template_name, current_date_time, userid, current_date_time,userid, 1, sys.argv[13], 1,student_table_id)
        else:
