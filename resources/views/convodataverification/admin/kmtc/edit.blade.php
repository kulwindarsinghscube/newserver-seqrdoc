...
@section('content')
                        ...
                        <div class="form-row row">
                            ...
                            <div class="form-group col-md-3">
                                <label for="secondary_email_id">Secondary Email ID:<small
                                        ...
                                        {!! @$student->studentAckLogs->se_status == 0
                                            ? '<i class="fa fa-times-circle" style="font-size: 14px;color:red"></i>'
                                            : '' !!}
                                        {!! @$student->studentAckLogs->se_status == 1
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="text" class="form-control" id="topic" name="topic"
                                    value="{{ old('topic', $student->topic) }}" required>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="form-row row">
                            <div class="col-md-12" style="max-width: 500px !important">
                                ...
                                <div class="row" style="text-align: center;">
                                    <div class="col-md-12">
                                        <button id="cropButton" type="button" class="btn btn-success" style="display: none; margin-bottom: 10px;margin-top: 10px;"><i class="fa fa-crop" style="font-size:16px;margin-right:2%"></i> Crop</button>
                                        <button id="cropButton_close" type="button" class="btn btn-danger" style="display: none; margin-bottom: 10px;margin-top: 10px;">Close</button>
                                    ...
                                    @elseif(@$student->collection_mode == 'Attending Convocation')
                                        ...
                                        <div class="form-group border p-3" style="    margin-top: 8%;">
                                            ...
                                            <input class="form-control" id="no_of_people_accompanied" name="no_of_people_accompanied" 
                                               min="0" type="number" value="{{ @$student->no_of_people_accompanied }}">
            ...
            @if (!empty($student->certificate_pdf))
                <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 certificate_div" style="margin-top: 1%;display:none">
                        <embed src="{{ $certificate_pdf }}?t={{ time() }}#toolbar=0&navpanes=0&scrollbar=0" type="application/pdf"
    ...
    <!-- Bootstrap 4 Modal -->
    <div class="modal fade" id="valueModal" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Selected Values</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row" id="table-container" style="padding: 7px;">
                                
                    </div>
                <input type="text" name="input_element" id="input_element" value="" hidden>
                <button class="btn btn-info" id="getValues">Submit</button>    
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        function english_text_change(from_element, to_hindi_element, to_krutidev_element) {
            ...
            var convertTo = 'hindi';
            let wordsArray = text.split(" ").filter(function(word) {
                return word.trim() !== ""; // Remove empty strings and spaces
            });
            var responsesArray = []; // Store responses here
            var promises = []; // Store AJAX promises
            $("#table-container").html('<div style="text-align:center;">Please Wait...</div>');
            wordsArray.forEach((word, index) => {
                let quill_url = "<?= route('convo_student.quillpad_Api') ?>" + "?inString=" + word + "&lang=" + convertTo;
                
                // Store each AJAX call as a promise
                let request = $.ajax({
                    url: quill_url,
                    type: 'GET',
                    dataType: 'html'
                }).done(function(response) {
                    responsesArray[index] = response; // Store response at the correct index
                });

                promises.push(request); // Add request to promises array
            });

            // Wait until all AJAX requests are completed
            $.when.apply($, promises).then(function() {
                let tableHtml = "<table class='table table-bordered' border='1'><thead><tr>";
                ...
                responsesArray.forEach(response => {
                    tableHtml += `<td>${response}</td>`;
                });

                tableHtml += "</tr></tbody></table>";

                // Append table to a container (modify `#table-container` with your actual div)
                $("#table-container").html(tableHtml);
                // hindi_text_change(to_hindi_element, to_krutidev_element);
            });
            $('#input_element').val(to_hindi_element+'-'+to_krutidev_element);
            ...
            //         $(to_hindi_element).val(response.value);//mystring
            //         hindi_text_change(to_hindi_element, to_krutidev_element);
        ...
        $(document).ready(function () {
            $("#getValues").click(function (e) {
                ...
                // Loop through each column based on the unique name attribute of radio buttons
                $("thead th").each(function () {
                    var columnName = $(this).text().trim(); // Get the column header text
                    // var selectedRadio = $("input[name='" + columnName + "']:checked").val();
                    var selectedRadio = $("input[name='" + columnName.replace(/'/g, "\\'") + "']:checked").val();
                    
                    if (selectedRadio) {
                        selectedValues.push(selectedRadio);
                    } 
                });
                if (selectedValues.length<$("#table-container thead th").length) {
                    alert("Please select an option for each column before proceeding.");
                    return; // Stop execution
                }
                // Display the selected values
                let selectors=$('#input_element').val().split("-");
                $(selectors[0]).val(selectedValues.join(" "));
                hindi_text_change(selectors[0], selectors[1]);
