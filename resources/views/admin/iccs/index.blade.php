...
<div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i>Generate ICCS
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('generateiccscertificates') }}</ol>
				</h1>
		...
		<form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('iccs-certificate.uploadfile')?>">
		    	...
		    	<div class="col-sm-offset-2 col-sm-10">
		    		<?php
	        			...
	        			$protocol = isset($_SERVER["HTTPS"]) ? 'https' : 'http';
     					...
     					$excel_url=$path.$subdomain[0]."/backend/sample_excel/ICCS Sample Excel.xlsx";
        			?>
...
function validateUpload() {
	...
	$.ajax({
		url:'<?= route('iccs-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function uploadfile(){
	...
	fd.append('_token',token);
	$.ajax({
		url:'<?= route('iccs-certificate.uploadfile')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
