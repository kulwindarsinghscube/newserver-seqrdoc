...
<script type="text/javascript">
	...
	oTable.on('click', '#infoData', function () {
		...
		var key = $(this).closest("tr").find('td:eq(3)').text();
		$.ajax({
                ...
                success: function(data) {
					$('#info').modal('show');
					resp = data.resp;
					var status = resp['0']['status'] ? 'Active' : 'Inactive';
					var path   = resp['0']['path'];
					// var src ="{{ asset('backend/canvas/images')}} "+'/'+resp['0']['path'];

					$('#info1').html(resp['0']['serial_no']);
					$('#info2').html(resp['0']['student_name']);
					$('#info4').html(status);
					// $('#info5').html("<img class='img-responsive img-thumbnail'");
					$('#info5').html('<img src="{{ asset("backend/canvas/images/qr") }}'+'/'+path+'" class= "img-responsive img-thumbnail" "/>');
				
					$('#info6').html(resp['0']['key']);
					$("a#info3").attr("href","{{asset('webapp/pdf')}}"+'/'+resp['0']['certificate_filename']);
					$('#info3').html(resp['0']['certificate_filename']);
                }
