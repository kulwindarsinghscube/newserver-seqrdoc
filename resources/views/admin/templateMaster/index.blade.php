	...
	var oTable = $('#example').DataTable({
		...
		{mData: 'id',
            ...
            mRender: function(v, t, o) {
                ...
                buttons += '@if(App\Helpers\RolePermissionCheck::isPermitted('template-master.copyTemplate.copy')) @if(App\Helpers\RolePermissionCheck::isPermitted('template-master.copyTemplate.copy')) <span style="cursor:pointer" onclick="copyTemplate('+o['id']+')" ><i title="Copy Template" class="copyrow fa fa-copy fa-lg yellow"></i></span> @endif @endif';
                ...
                buttons += '@if(App\Helpers\SitePermissionCheck::isPermitted('template-master.template-map.edit')) @if(App\Helpers\RolePermissionCheck::isPermitted('template-master.template-map.edit')) <a href="'+map_url+'"> <span style="cursor:pointer"><i title="Map Template" data-id="'+o['id']+'" class="maprow fa fa-map-marker fa-lg black"></i></span></a> @endif @endif';
				 ...
				 if (subdomain == "tpsdi") {
					buttons += '@if(App\Helpers\SitePermissionCheck::isPermitted('template-master.uplodafile')) @if(App\Helpers\RolePermissionCheck::isPermitted('template-master.uplodafile')) <span style="cursor:pointer"><i title="Generate Certificate With Data Mapping" data-id="'+o['id']+'" class="dicpdfGenerate fa fa-file-pdf-o fa-lg red"></i></span> @endif @endif';
				}
			...
			success:function(response){
				if(response == 'success'){
					...
					function(data){
						...
						{
							$temp = window.btoa($template_id);
							var url = '{{ route("dictemplate-map.index", ":template_id") }}';
							url = url.replace(':template_id', $template_id);
							window.location.href=url;
						}
						else
