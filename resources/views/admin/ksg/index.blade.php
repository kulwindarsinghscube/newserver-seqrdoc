...
<div class="container">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> Batch
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('Batch') }}</ol>
				</h1>
    ...
    <!-- panel panel-default -->
     <div>
          <!-- @if(App\Helpers\SitePermissionCheck::isPermitted('add-batch'))
          @if(App\Helpers\RolePermissionCheck::isPermitted('add-batch')) -->
          ...
          <!-- @if(App\Helpers\SitePermissionCheck::isPermitted('ksg.uploadfile'))
          @if(App\Helpers\RolePermissionCheck::isPermitted('ksg.uploadfile')) -->
          <a href="#" class="btn btn-primary" role="button" id="UploadExcel">Upload Excel </a>
          <!-- @endif
          @endif -->
     </div>
...
//edit data
    ...
    $.ajax({
        url: "{{ route('template-master.store') }}", 
        method: 'POST',
    ...
    $('#BatchForm').on('submit', function(event) {
        ...
        var url = $('#batchId').val() ? "{{ route('update-batch') }}" : "{{ route('add-batch') }}";
        $.ajax({
            url: url,
            method: 'POST',
            data: formData,
