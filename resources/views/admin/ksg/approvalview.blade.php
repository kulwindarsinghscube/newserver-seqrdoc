		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i>  Batches for Approval
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('Approval Page') }}</ol>
				</h1>
...
<script type="text/javascript">
  $(document).ready(function() {
            var table = $('#tableYajra').DataTable({
                ...
                serverSide: true,
                ajax: {
                url: "{{ route('ksg-approved-batch.datatable') }}",
                method: 'POST',
                data: {
                        approve_flag: 1 ,
    ...
    $(document).on('click', '.approveRejectBatch', function() {
        ...
        $("#updateRecordId").val(recordId);
         ...
         var url = "{{ url('edit-batch') }}/" + recordId;
           $.ajax({
            url: url,
            method: 'GET',
            data: $(this).serialize(),
        ...
        var updateRecordId=$('#updateRecordId').val();
                ...
                $.ajax({
                    // url: '{{ route("send.otp") }}',
                    // url: '{{ route("send.otp") }}' + '?name=' + encodeURIComponent(updateRecordName),
                    url :'{{ route("send.otp") }}' +
                    ...
                    data: {
                        ...
                        email: '{{ Auth::user()->email }}' // or any target email
                    },
        ...
        var url = "{{ route('update-batch-status') }}";
        $.ajax({
            url: url,
            method: 'POST',
            data: $(this).serialize(),
