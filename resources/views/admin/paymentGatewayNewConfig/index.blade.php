	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('pg_newconfig') }}</ol>
			...
			<div class="panel-body">
				<form  id="update_data">
					...
					<div class="form-group clearfix">
						@if(App\Helpers\SitePermissionCheck::isPermitted('pg_newconfig.update'))
						@if(App\Helpers\RolePermissionCheck::isPermitted('pg_newconfig.update'))
...
$(document).ready(function() {	
	...
	// show payment method on select input
     var ajaxURL="{{ URL::route('pg_newconfig.index') }}";
   ...
   // on change payment method call featch data
	 $("#opt_wl").change(function(event) {
	 	var pg_id=$(this).val();
        var show_path="{{ URL::route('pg_newconfig_fetch_dropdown_value.show') }}";
        var token="{{ csrf_token() }}";
	 	$.ajax({
             url:show_path,
   ...
   $("#update_save").click(function(event) {
      ...
      var update_path="{{ URL::route('pg_newconfig.update') }}";
      var token="{{ csrf_token() }}";
