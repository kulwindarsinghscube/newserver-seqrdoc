...
<div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> Generate Cavendish
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('generatecavendishcertificates') }}</ol>
				</h1>
        ...
        <div class="row">
            ...
            <form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('cavendish-certificate.uploadfile')?>">
              ...
              <div class="form-group">
                  ...
                  <div class="col-sm-9">
                      <select class="form-control" id="template_id" name="template_id">
                          ...
                          <option value="1" data-tempid="1">SOM</option>
                          <option value="4" data-tempid="4">SOM - MBCHB</option>

                          <option value="2" data-tempid="2">BIT</option>               
                          <option value="3" data-tempid="3">AESS</option>               
                      </select>
                  </div>
              </div>
              <div class="form-group">
            ...
            <div class="col-lg-5" style="padding: 83px 1em;"> 
                <?php
                    ...
                    $excel_url_1=$path.$subdomain[0]."/backend/sample_excel/cavendish_som.csv";
                    $excel_url_2=$path.$subdomain[0]."/backend/sample_excel/cavendish_bit.csv";
                    $excel_url_3=$path.$subdomain[0]."/backend/sample_excel/cavendish_aess.csv";
                ?>
...
function validateUpload() {
  ...
  var previewPdf = $('#previewPdfValue').val();
	...
	$.ajax({
		url:'<?= route('cavendish-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function uploadfile(){
  ...
  fd.append('template_id',template_id);
	...
	$.ajax({
		url:'<?= route('cavendish-certificate.uploadfile')?>',
	  type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
    ...
    var loaderid =setTimeout(function () {
        $.ajax({
            ...
            success: function (result) {
            	...
            	$('#pendingCertificates').text(0);
            	// $('#recordsToGenerate').text(result.recordsToGenerate);
            	$('#recordsToGenerate').text(1);
