	...
	function(data){
		...
		if(data.type == 'success'){
						...
						if(data.requestData.offer_letter!=''&&data.requestData.offer_letter!=null){
							$('#offerLetterView').html('<a href="<?= Config::get('constant.local_base_path').$subdomain[0]."/"?>'+data.requestData.offer_letter+'" target="_blank">View File</a>');
						}else{  
						...
						if(documentLength>0){
							for(var i=0;i<documentLength;i++){
								var document_path='<?= Config::get('constant.local_base_path').$subdomain[0]?>/'+data.requestDetails[i].document_path;
