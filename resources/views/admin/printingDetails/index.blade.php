...
<?php $domain = \Request::getHost();
 ...
 <div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa-fw fa-print"></i> <?php if($subdomain[0]=="sales"){ echo "Generation details"; }else{ echo "Printing details"; } ?>
				<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('printingdetail') }}</ol>
				<i class="fa fa-info-circle iconModalCss" title="User Manual" id="printingDetailsClick"></i>				
</h1>
			...
			<div class="col-xs-12">
				<table id="example" class="table table-hover" cellspacing="0" width="100%">
					<thead>
						<tr>
							<th>#</th>
							<th><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> sr. no.</th>
							<th>Certificate sr. no.</th>
							<th><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> count</th>
							<th>Printer name</th>
							<th><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> Datetime</th>
							<th>Username</th>
							<th>Action</th>
              <th></th>
						</tr>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><?php if($subdomain[0]=="sales"){ echo "Generation details"; }else{ echo "Printing details"; } ?> Information</h4>
      </div>
      <div class="modal-body" id="ajaxResponse">
				...
				<div class="row">
					<div class="col-xs-3"><label><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> Datetime</label></div>
					<div class="col-xs-1"><label>:</label></div>
				...
				<div class="row">
					<div class="col-xs-3"><label><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> count</label></div>
					<div class="col-xs-1"><label>:</label></div>
					<div class="col-xs-8"><span id="print_count_info"></span></div>
				</div>
				<div class="row">
					<div class="col-xs-3"><label><?php if($subdomain[0]=="sales"){ echo "Generation"; }else{ echo "Print"; } ?> serial no</label></div>
					<div class="col-xs-1"><label>:</label></div>
...
<script type="text/javascript">
   ...
   {
   	  var url="{{ URL::route('printing-detail.getdetail') }}";
   ...
   var oTable = $('#example').DataTable( {
     'dom':  "<'row'<'col-sm-3'i><'col-sm-4' p><'col-sm-1' ><'col-sm-4'f>>",
      "bProcessing": false,
        ...
        "sAjaxSource":"<?= URL::route('printing-detail.index',['status'=>1])?>",
    "aoColumns":[
          ...
          {mData: "username",bSortable:true},
          {
            ...
            mRender:function(v, t, o){
                    var act_html;
                     ...
                     act_html ='@if(App\Helpers\SitePermissionCheck::isPermitted('printing-detail.getdetail')) @if(App\Helpers\RolePermissionCheck::isPermitted('printing-detail.getdetail'))<a onclick="print_info('+v+')"><i class="fa fa-info-circle fa-lg blue"></i></a>@endif @endif';
  ...
  $('#success-pill').click(function(){
    ...
    var url="<?= URL::route('printing-detail.index',['status'=>1])?>";
    oTable.ajax.url(url);
    oTable.ajax.reload();
  ...
  $('#fail-pill').click(function(){
    ...
    var url="<?= URL::route('printing-detail.index',['status'=>0])?>";
    oTable.ajax.url(url);
    oTable.ajax.reload();
    $('.loader').removeClass('hidden');
  });
