	...
	<div id="wrapper">
				...
				<div class="dropdown-menu" aria-labelledby="pagesDropdown1">
					<table class="table table-dark">
						...
						<tbody>
							...
							@if($subdomain[0]=="demo" || $subdomain[0]=="print24x7")
							<tr>
								<td style="padding: 1.75rem" class="custom_table" id="yellow_patch"><i class="fa fa-sticky-note custom_icon"></i><br>Yellow Patch</td>
							</tr>
							@endif
						</tbody>
					</table>
		...
		<?= Form::open(['id'=>'template_frm','files'=>true])?>
								...
								<table class="table table-condensed">  
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<select class="custom_select"  name="back_bg_template_id" id="back_bg_template_id" class="form-control">

													<option value="0" <?php if(isset($TEMPLATE["back_bg_template_id"]) && $TEMPLATE["back_bg_template_id"] == 0) echo "selected"; ?>>Blank Template</option>
													<?php
					                                    $status = 1; 
					                                    if(isset($BGTEMPLATE))
					                                    {
					                                       foreach ($BGTEMPLATE as $key => $br_row) {
					                                          ...
					                                          if(isset($TEMPLATE["back_bg_template_id"]) && $TEMPLATE["back_bg_template_id"] == $br_row['id'])
										...
										<!--Blockchain-->
										<?php if($subdomain[0]=="demo" || $subdomain[0]=="print24x7"){ ?>
										<tr>
										...
										<tr>
											...
											<td>Template Back Side</td>
											<td>  
												<label class="switch">  
												<input type="checkbox" class="form-control" id="is_back_template_slider" value="<?php 
				                                    if(isset($TEMPLATE["is_back_template"]))
				                                     {
				                                       echo $TEMPLATE["is_back_template"];
				                                     ...
				                                     <?php
				                                     if(isset($TEMPLATE["is_back_template"])){
				                                       if($TEMPLATE["is_back_template"] == 0){
				                                       ?>
				                                        unchecked
				                                        <?php      
				                                          }else{
										...
										<?php if($subdomain[0]=="demo" || $subdomain[0]=="print24x7"){ ?>
											...
											<input type="hidden" id="is_back_template" name="is_back_template" value="<?php 
											if(isset($TEMPLATE["is_back_template"]))
											{
											echo $TEMPLATE["is_back_template"];
			...
			<div class="container">
				...
				<iframe id="myframe" src="{{URL::route('canvasmakerback_edit')}}" style="width :710px;"></iframe>
			</div>
		</div> 
	</div>
@stop
@section('script')
	...
	<?php if($subdomain[0]=="demo" || $subdomain[0]=="print24x7"){ ?>
		...
		<script type="text/javascript" src="{{asset('backend/canvas/back_blockchainjs/canvas.js')}}"></script>
		<script type="text/javascript" src="{{asset('backend/canvas/back_blockchainjs/templatestyle.js')}}"></script>
		<script type="text/javascript" src="{{asset('backend/canvas/back_blockchainjs/extraField.js')}}"></script>
	<?php }else{ ?>
	...
	<script type="text/javascript">
					...
					success:function(imageResponse){
						if(imageResponse.data[0].type == 'error'){
							...
							if(imageResponse.data[0].security_type == 'Qr Code'){
				        		...
				        		$('[name="field_extra_width[]"]')[i].setAttribute('value',imageResponse.data[0].width);
				        		$('[name="field_extra_height[]"]')[i].setAttribute('value',imageResponse.data[0].height);
				        		$('#field_extra_width').val(imageResponse.data[0].width)
				        		$('#field_extra_height').val(imageResponse.data[0].height)
								...
								if(imageResponse.data[0].template_name){
					        		...
					        		myframe.contentWindow.postFrameMessage('update',$id,'','',imageResponse.data[0].width,imageResponse.data[0].height,'',$security_type,'','','',field_sample_text,'','','','',imageResponse.data[0].filename,imageResponse.data[0].template_id,'','','','','','','',imageResponse.data[0].grey_scale,'',imageResponse.data[0].is_uv_image,imageResponse.data[0].is_transparent_image);
					        		// template name not available
					        	}else{
					        		var src = canvas_upload_path+'/templates/customImages/'+imageResponse.data[0].filename+''
					        		console.log(src);
					        		$('.uploaded_image').attr('src',src);
					        		myframe.contentWindow.postFrameMessage('update',$id,'','',imageResponse.data[0].width,imageResponse.data[0].height,'',$security_type,'','','',field_sample_text,'','','','',imageResponse.data[0].filename,'','','','','','','','',imageResponse.data[0].grey_scale,'',imageResponse.data[0].is_uv_image,imageResponse.data[0].is_transparent_image);
					        	}
				        	}
		...
		$('#btnSave, #btnSaveAndClose').click(function(e){
			...
			var url = '<?=route('template-master.template_back_store')?>';
			var type = 'POST'; 
	        ...
	        $('#template_frm').ajaxSubmit({
	            ...
	            dataType: 'json',
	           	beforeSubmit: function (formData, jqForm, options) {
					...
					$('#btnPreview').removeAttr('disabled');
					$('#btnCancel').removeAttr('disabled');
					// console.log(resObj.data.type);
					if(resObj.data.type == 'success') {
						toastr["success"](resObj.data.message);
						if($('#bSaveAndClose').val() == 1)
							...
							$('#template_id').val(resObj.data.last_id);
							//store id in session during create
							<?php
								if(!isset($TEMPLATE['id'])){
							?>
								sessionStorage.setItem('template_id',resObj.data.last_id)
							<?php
								}
							?>
						...
						toastr["error"](resObj.data.message);
					}
				},
		...
		$('#back_bg_template_id').change(function (event) {
			var myframe = document.getElementById('myframe');
			if($('#back_bg_template_id').val() != 0) {
				// get width height of selected background
				var url = '<?= route('templateMaster.bgtemplatechanges')?>',
				data = {id: $('#back_bg_template_id').val()};
				$.get(url, data, function (resp) {
					if(!checkFieldsinside(resp.width, resp.height))
					{
						...
						$('#back_bg_template_id').val(previousBgId);
						return false;
					}
					// set background
					myframe.contentWindow.setBackground($('#back_bg_template_id').val(), $('#template_width').val(), $('#template_height').val());
					// set iframe
		...
		function showHeightWidth() {
		    ...
		    if($('#back_bg_template_id').val() == 0){
				...
				// alert($('#back_bg_template_id').val());
				//console.log(url);
		      	data = {id: $('#back_bg_template_id').val()};
		      	$.get(url, data, function (resp) {
			    }, 'json');
		    }
		    previousBgId = $('#back_bg_template_id').val();
		}
		...
		$('#btnPreview').click(function(event){
			...
			*/
			...
			var url = '<?=route('previewbackstore')?>',
			data = $('#template_frm').serialize();
			
			$.post(url, data, function (resp) {
				if(window.newwinow)
				window.newwinow.close();
				window.newwinow = window.open('<?=route('template-master.backpreview.back_pdf')?>', 'mywindow','width=600,height=800');
			});
		})
		/**Blockchain**/
		...
		$('#is_back_template_slider').on('change',function(){
			...
			var val = $('#is_back_template_slider').val();
			$('#is_back_template').val(val);
		});
