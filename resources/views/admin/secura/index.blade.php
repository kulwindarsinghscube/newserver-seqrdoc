...
<div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> Molwa Documents
                            <ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;"><!--{{ Breadcrumbs::render('generateghribmjalcertificates') }}--></ol>
                        </h1>
    ...
    <div id="accordion">
        ...
        <!--<div class="card-header" id="headingOne">
          <h5 class="mb-0">
            <button class="btn btn-link" data-toggle="collapse" data-target="#collapseOne" aria-expanded="false" aria-controls="collapseOne" style="outline: none;width:100%;text-align:left !important;text-decoration:none;">
              Excel Example
            </button>
          </h5>
        </div>-->
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-8"> <br />
                        <form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('secura-certificate.uploadfile')?>">
                            <input type="hidden" name="func" id="func" value="uploadFile"> 
                            <input type="hidden" name="_token" value="{{csrf_token()}}"> 
                            <div class="form-group">                    
                            <div class="col-sm-3"></div>
                            <div class="col-sm-9"><mark>
                            This toggle is to either generate preview-only-pdf [default selected] or live generation.
                            </mark>
                            </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="template_id" name="template_id">
                                        ...
                                        <option value="1" data-tempid="1">Certificate</option>
                                        <option value="2" data-tempid="2">ID Card</option>               
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="col-lg-4" style="padding: 110px 1em;">                 
                        <?php
                            $domain = \Request::getHost();
                            $subdomain = explode('.', $domain);
                            $protocol = isset($_SERVER["HTTPS"]) ? 'https' : 'http';
                            $path = $protocol.'://'.$subdomain[0].'.'.$subdomain[1].'.com/';
                            $excel_url_1=$path.$subdomain[0]."/backend/sample_excel/SampleData.xlsx";
                            $excel_url_2=$path.$subdomain[0]."/backend/sample_excel/SampleData.xlsx";
                        ?>
                             
                        <span style="display:none;" id="excel_url_1"><b>Please Click <a href="{{$excel_url_1}}" download>HERE</a> To Download Sample Excel.</b><br />Remove data from photo column.</span>
                ...
                <div id="download_link">
                    ...
                    <div class="col-sm-12">
                        <h5>Notes:</h5>
                        <blockquote>Ensure that the Freedom Fighter Number in excel file is unique across all data.</blockquote>
                        <blockquote>Name are case sensitive, column sequence insensitive, extra columns are ignored.</blockquote>
                        <blockquote>If the unique Freedom Fighter Number is repeated or found duplicate in the excel data, then it will replace the already existing data (making them inactive), with the current excel data.</blockquote>
        ...
        <!--<div class="card-header" id="headingTwo">
          <h5 class="mb-0">
            <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo" style="outline: none;width:100%;text-align:left !important;text-decoration:none;">
              Import Data
            </button>
          </h5>
        </div>-->
        <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
          <div class="card-body" style="padding:20px;">
            <button type="button" class="btn btn-primary" id="btn_import">Import</button> <br /><br />
            <div id="progress"></div>
            <div id="show_msg"></div>
          </div>
        </div>
    ...
    <a href="{{ route('molwa-certificate.ImportPrint') }}">Import & Print</a> &nbsp;|&nbsp;
    <a href="{{ route('molwa-certificate.index') }}">Manage Records</a> &nbsp;|&nbsp;
    <a href="{{ route('molwa-certificate.ReImportPrint') }}">Re-Import & Print</a>
</div>
@stop
@section('script')
...
function validateUpload() {
	...
	$.ajax({
		url:'<?= route('secura-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function uploadfile(){
	...
	fd.append('_token',token);
	$.ajax({
		url:'<?= route('secura-certificate.uploadfile')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
$('#btn_import').click(function (event) {
    ...
    $.ajax({
		type:'GET',
		url : "<?=route('molwa-certificate.importData')?>",
		dataType:'json',
        data: {
        "_token": token
        },
