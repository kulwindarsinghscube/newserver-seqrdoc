	...
	<div class="container">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <form id="form_process" method='post' action='' enctype="multipart/form-data">
                  ...
                  <?php if($subdomain[0] == 'demo'||$subdomain[0] == 'newserver'|| $subdomain[0] == 'mmk' || $subdomain[0] == 'ksu') { ?> 
                  ...
                  <?php if($subdomain[0] == 'demo') { ?> 
                      <div class="form-group" id="is_encrypted_div" style="display: none;">
                          <label class="control-label col-sm-3" for="isEncrypted">Is Encrypted :</label>
                          <label class="switch">  
                              <input type="checkbox" class="form-control" id="isEncrypted" name="isEncrypted" value="1" checked="">
          
                              <span class="slider round"></span>
                          </label>
                          <label id="isEncryptedtoggleText" style="position: absolute;margin: 7px 30px;color: green;">Generating encrypted pdf</label>
                          <input type="hidden" class="form-control" id="isEncryptedValue" name="isEncryptedValue" value="1">
                      </div>
                      <?php } ?> 
        ...
        <div class="modal fade" id="editTtitle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              ...
              <div class="modal-header">
                  ...
                  <?php if($subdomain[0]=="demo"||$subdomain[0]=="konkankrishi"||$subdomain[0]=="test"){?>    
...
<script type="text/javascript">
    ...
    $('#previewPdf').on('change',function(){
       ...
       if(value==1){
          ...
          <?php if($subdomain[0]=="demo"){ ?>
            ...
            // added by rohit
            $('#is_encrypted_div').hide();
            // $('#encryption_type_div').hide();
            // added by rohit
    ...
    $('#isEncrypted').on('change',function(){
        var value = $('#isEncryptedValue').val();

        if(value==0){
            $('#isEncryptedValue').val(1);
            $('#isEncryptedtoggleText').html('<span style="color: green;margin-left: 5px;    margin-top: 10px;">Generating encrypted pdf.</span>');
            $('#encryption_type_div').show();
        }else{
            $('#isEncryptedValue').val(0);
            $('#isEncryptedtoggleText').html('<span style="color: #F28c28;margin-left: 5px;    margin-top: 10px;">Generating non encrypted pdf..</span>');
            $('#encryption_type_div').hide();
            // resetEncryptionType(); 
        }
    });
    ...
    function resetIsEncrypted() {
        $('#isEncrypted').prop('checked',true);
        $('#isEncryptedValue').val(1);
        $('#isEncryptedtoggleText').html('<span style="color: green;margin-left: 5px;    margin-top: 10px;">Generating encrypted pdf..</span>');
    }
  ...
  $('#btn_upload').click(function(){
      ...
      var isEncryptedValue=$('#isEncryptedValue').val();
      ...
      fd.append('isEncryptedValue',isEncryptedValue);
      ...
      $.ajax({
          ...
          success: function(response){
              ...
              if(res.type == 'Success'){                    
                  ...
                  $('#preview').html(res.msg+"<br /><input type='button' class='btn btn-primary progress-status' data-file='"+res.filename+"' data-templateid='"+res.template_id+"' data-pdfpage='"+res.pdf_page+"' data-unids='"+res.unids+"' data-progressfile='"+res.progressfile+"' data-previewPdf='"+previewPdf+"' data-isencryptedvalue='"+isEncryptedValue+"' value='Proceed' id='btn_proceed'>");
                  $('#btn_upload').hide();
                  $('#progress-file').hide();
                  $('#message-file').hide();
                  $('#progress-file').html('');
                  ...
                  if(subdomain == 'demo') {
                    ...
                    if(isEncryptedValue == 1) {
                        $('#isEncrypted').prop('checked', true);
                        $('#isEncryptedtoggleText').html('<span style="color: green;margin-left: 5px;    margin-top: 10px;">Generating encrypted pdf.</span>');
                    } else if(isEncryptedValue == 0) {
                        $('#isEncrypted').prop('checked', false);
                        $('#isEncryptedtoggleText').html('<span style="color: #F28c28;margin-left: 5px;    margin-top: 10px;">Generating non encrypted pdf..</span>');
                    }
                  }
  ...
  $(document.body).on('click', "#btn_proceed", function(){    
      ...
      var isEncryptedValue=$(this).data('isencryptedvalue');
      ...
      $.ajax({
          ...
          type: 'post',
          data: {'_token':'<?= csrf_token()?>',file:file, template_id:template_id, pdf_page:pdf_page, unids:unids, progressfile:progress_file,previewPdf:previewPdf,isEncryptedValue:isEncryptedValue},
          beforeSend : function() {
              $('#preview').html('Processing...');    
          },
          success: function(response){                
