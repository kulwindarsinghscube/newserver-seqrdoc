	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('pgconfig') }}</ol>
		...
		<div class="panel panel-primary" style="margin: 0 auto; width: 560px;">
            ...
            @if(App\Helpers\SitePermissionCheck::isPermitted('pgconfig.update'))
            @if(App\Helpers\RolePermissionCheck::isPermitted('pgconfig.update'))
...
$(document).ready(function() {	
	...
	// show payment method on select input
     var ajaxURL="{{ URL::route('pgconfig.index') }}";
   ...
   // on change payment method call featch data
	 $("#opt_wl").change(function(event) {
	 	var pg_id=$(this).val();
        var show_path="{{ URL::route('pgconfig_fetch_dropdown_value.show') }}";
        var token="{{ csrf_token() }}";
	 	$.ajax({
             url:show_path,
   ...
   $("#update_save").click(function(event) {
      ...
      var update_path="{{ URL::route('pgconfig.update') }}";
      var token="{{ csrf_token() }}";
