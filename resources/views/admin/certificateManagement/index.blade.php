				...
				<i class="fa fa-info-circle iconModalCss" title="User Manual" id="certificateManagementClick"></i>
                    ...
                    <div class="">
                        <ul class="nav nav-pills" id="pills-filter">
                            ...
                            @if(App\Helpers\SitePermissionCheck::isPermitted('certificateManagement.excelExport'))
                            @if(App\Helpers\RolePermissionCheck::isPermitted('certificateManagement.excelExport'))
                            <li style="float: right;">
                                <a href="{{ route('certificateManagement.excelExport') }}" style="background: #0052CC;padding-top:5px; color: #fff;border-radius: 2px;box-shadow: 2px 2px 5px #999;border: 1px solid transparent;height: 35px;" id="report" ><i class="fa fa-file"></i> Generate Report</a> 
                            </li>
                            @endif
                            @endif
                        </ul>
...
<script type="text/javascript">
    ...
    var oTable = $('#example').dataTable({
        ...
        "bServerSide": true,
        "autoWidth": true,
        "sAjaxSource": "{{URL::route('certificateManagement.index',['status'=>1])}}",
        "aaSorting": [
            [4, "desc"]
        ],
        "oLanguage": {
        ...
        
        {
            ...
            mRender: function(v, t, o) {
                ...
                var act_html = "<div class='btn-group'>"
                                ...
                                +"@if(App\Helpers\SitePermissionCheck::isPermitted('certificateManagement.generateQRCode')) @if(App\Helpers\RolePermissionCheck::isPermitted('certificateManagement.generateQRCode'))<a href='javascript:void(0);' onclick='generateQRCode("+ o['id'] +")' data-toggle='tooltip' title='Generate QR Code' data-placement='top' class='editrow'><i class='fa fa-info-circle fa-lg blue'></i></a>@endif @endif"

                                +"@if(App\Helpers\SitePermissionCheck::isPermitted('certificateManagement.update')) @if(App\Helpers\RolePermissionCheck::isPermitted('certificateManagement.update')) <label class='switch'><input type='checkbox' id ='action' data-id = '"+o.id+"' data-status='"+ o.status +"' ><span class='slider round'></label>@endif @endif"
        ...
        bootbox.confirm({
            ...
            callback: 
                function(result) {
                    if(result) {
                        $.ajax({
                            url: "{{ URL::route('certificateManagement.update') }}",
                            type: 'post',
                            dataType:'html',
                            data:{
                                id:id,
    ...
    function generateQRCode(id) {
        ...
        var id = id;
            ...
            $.ajax({
                url: "{{ URL::route('certificateManagement.generateQRCode') }}",
                type: 'post',
                dataType:'html',
                data:{
                    id:id,
