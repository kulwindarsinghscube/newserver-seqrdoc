...
<div class="container">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><i class="fa fa fa-file-excel-o"></i> Generate Passing Certificate
					<ol class="breadcrumb pull-right" style="background:transparent;font-size:14px;">{{ Breadcrumbs::render('generatemitadtpassingcertificates') }}</ol>
				</h1>
		...
		<form method="post" id="processExcelForm" class="form-horizontal" enctype="multipart/form-data" action="<?=route('passing-certificate.uploadfile')?>">
			...
			<div class="form-group">
		    	<label class="control-label col-sm-2" for="excel">Select template</label>
		    	<div class="col-sm-10">
		    		
		      		{!! Form::select('template_id', $templates, null, ['class' => 'form-control template_id']) !!}
		    	</div>
		  	</div>
		  	<div class="form-group">
...
function validateUpload() {
	...
	$.ajax({
		url:'<?= route('passing-certificate.validateexcel')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
...
function uploadfile(){
	...
	fd.append('_token',token);
	$.ajax({
		url:'<?= route('passing-certificate.uploadfile')?>',
	    type: "POST",
		dataType: "JSON",
		data:fd,
		processData: false,
