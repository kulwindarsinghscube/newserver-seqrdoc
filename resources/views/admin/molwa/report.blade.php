...
$(function() {
    $("#report_name").change(function() {
        $selected_val=$('option:selected', this).val(); //1, 4, 6
		if($selected_val=="1" || $selected_val=="4" || $selected_val=="6"){
			$('#is_alive option:contains("Alive")').attr("selected",true);
			$('#is_alive option:contains("Dead")').attr("disabled","disabled");
		}else{
			$('#is_alive option:contains("Alive")').attr("selected",false);
			$('#is_alive option:contains("Dead")').removeAttr("disabled","disabled");
		}
    });
});	
