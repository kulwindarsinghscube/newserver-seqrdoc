			...
			<div class="clearfix">
				<div id="">
					<div class="container-fluid">
						<div class="row">
							<div class="col-lg-12">
								<h1 class="page-header"><i class="fa fa-fw fa-file"></i> Re-Import & Print</h1>	
							</div>
						</div>						
                        ...
                        <div class="col-xs-12">                
                            <div class="card">                                
                                ...
                                <div class="row">
                                    <form method="post" id="processApiForm" class="form-horizontal" files="true" novalidate="novalidate">
                                    ...
                                    <!--<div class="col-xs-3" style="border-right: 2px solid green;">
                                        <div class="form-group">                                            
                                            ...
                                            <div class="col-sm-12">
                                                <label class="control-label" for="temp">Enter FF ID(s)</label>
                                                <textarea class="form-control" id="ffid" name="ffid"></textarea>
                                            </div>
                                        </div>                                
                                    </div>--> 
                                    <div class="col-xs-12">  
                                        <div class="form-group">                                            
                                            ...
                                            <div class="col-sm-6">
                                                ...
                                                <input type="text" class="form-control" id="log_name" name="log_name"><br />
                                                <label class="control-label">Upload FF IDs From Excel File</label>
                                                <input type="file" name="file_name" class="form-control" id="exampleInputFile" />
                                                <br /><b>Please Click <a href="{{ asset('secura/backend/sample_excel/FFids_import.xlsx') }}" download>HERE</a> To Download Sample Excel.</b><br />
												<label class="control-label">Reason</label> <br />
												<textarea name='log_reason' id='log_reason' class="form-control" rows='4'></textarea>
											</div>
                                        </div>         
                                    </div>
                                    ...
                                    <div class="col-xs-12"><div id="show_msg2"></div></div>
                                </div>                                
                                </div>                                
                            </div> 
                        </div>
						...
						<div class="">							
                            ...
                            <div class="col-sm-12">
                                ...
                                <blockquote><i class="fa fa-cog fa-lg blue"></i> icon: generate secured PDF. Show progress bar. Change status from RE-IMPORTED to RE-GENERATED.</blockquote>
                                <blockquote><i class="fa fa-file-pdf-o fa-sm red"></i> icon: is shown when the PDF file is ready for download</blockquote>
                                <blockquote><i class="fa fa-refresh fa-lg blue"></i> icon: will be available when status = RE-GENERATED. on click, change the status to RE-COMPLETED.</blockquote>
...
<script type="text/javascript">
	...
	var oTable = $('#example').DataTable({
        ...
        "sAjaxSource":"<?= URL::route('molwa-certificate.ReImportPrint',['status'=>1])?>",
        ...
        {mData: 'fresh_records',
            ...
            mRender: function(v, t, o) {
                ...
                buttons += '&nbsp;<a data-toggle="tooltip" title="Export" href="ExportToExcel/'+o['record_unique_id']+'/reimport" target="_blank"><i class="fa fa-file-excel-o fa-lg green" style="font-size:23px;"></i></a>';
                }
				return buttons;
            },         		   
        }, 
		...
		{mData: "idcard_status",bSortable: true,
            mRender: function(v, t, o) {
            	if(o['idcard_status']=='RE-IMPORTED'){ var initial=''; }else{ var initial='RE-'; }
                var buttons = initial+o['idcard_status'];
        ...
        {mData: "certificate_status",bSortable: true,
            mRender: function(v, t, o) {
            	if(o['certificate_status']=='RE-IMPORTED'){ var initial=''; }else{ var initial='RE-'; }
                var buttons = initial+o['certificate_status']; 
    ...
    */
    ...
    $('#processApiForm').validate({
        ...
        rules:{            
            //ffid: { required: '#api_option[value="1"]:checked'  },
            log_name:{required: '#api_option[value="2"]:checked'},
            file_name:{required: '#api_option[value="2"]:checked', extension:'xls|xlsx'}, 
			log_reason:{required: '#api_option[value="2"]:checked'}, 
        },
        messages:{            
            //ffid:{required:'Enter FF ID'},           
            log_name:{required:'Enter Name'},    
            file_name:{required:'Upload file', extension:'Select only Excel file'},
			log_reason:{required:'Enter Reason'},  
        },
    ...
    function fire_ajax_submit() {        
        ...
        $.ajax({
            ...
            url: '<?= route('molwa-certificate.REimportFfData')?>',
            data: data,       
            ...
            beforeSend:function(){
                ...
                interval = setInterval(function(){
                    $.getJSON('<?=route('molwa-certificate.getReProgess')?>', function(data) {
            ...
            success: function (resObj) {
                if(resObj.success == true){
                    ...
                    if(resObj.records == 'Found'){
                        $('#show_records').html("Fresh Records: "+resObj.fresh_records); 
                        $('#show_records').show(); 
                    }
                    $('#progress').hide();
                    $('#btn_import').show()
    ...
    oTable.on('click', '.btn_generate_id', function (e) {     
        ...
        $('#'+rid).hide();
        $('#'+msg).hide();   
        $('.btn_generate_id').hide();
        //var id=1;
        var token = "<?= csrf_token()?>";
        var interval = null;
        if (confirm('Do you want to generate ID Card?')) {
            $.ajax({
                type:'GET',
                url : "<?=route('molwa-certificate.IdReGenerate')?>",
                dataType:'json',
                ...
                beforeSend : function() {
                    ...
                    $('#'+msg).show();                   
                    interval = setInterval(function(){
                        $.getJSON('<?=route('molwa-certificate.getIdReProgess')?>', function(data) {
    ...
    oTable.on('click', '.btn_generate_certificate', function (e) {     
        ...
        $('#'+rid).hide();
        $('#'+msg).hide();   
        $('.btn_generate_certificate').hide();
        //var id=1;
        var token = "<?= csrf_token()?>";
        var interval = null;
        if (confirm('Do you want to generate Certificate?')) {
            $.ajax({
                type:'GET',
                url : "<?=route('molwa-certificate.CertificateReGenerate')?>",
                dataType:'json',
                data:{'id':id, '_token':token},            
                beforeSend : function() {
                    $('#'+rid).show();
                    $('#'+msg).show();                   
                    interval = setInterval(function(){
                        $.getJSON('<?=route('molwa-certificate.getCtReProgess')?>', function(data) {
    ...
    oTable.on('click', '.btn_status', function (e) {     
        ...
        if (confirm('If you have printed all the Pages of this lot then confirm.')) {
            ...
            $.ajax({
                type:'GET',
                url : "<?=route('molwa-certificate.StatusReComplete')?>",
                dataType:'json',
                ...
                success:function(response){
                    if(response.success==true){
                        id=response.uid;
                        col=response.col;
                        record_type="Reimport";
                        $.ajax({
                            type:'GET',
                            url : "<?=route('molwa-certificate.sendUpdateToMolwa')?>",
                            dataType:'json',
                            data:{'id':id, 'col':col, 'record_type':record_type, '_token':token},            
                            beforeSend : function() {
                                $('#'+rid).show();
                                $('#'+msg).show();                   
                                interval = setInterval(function(){
                                    $.getJSON('<?=route('molwa-certificate.getCtCompleteProgess')?>', function(data) {
                                        $('#'+rid).html('<div class="cbar" style="width:' + data.percent + '%">' + data.percent + '%</div>');
                                        $('#'+msg).html(data.show_msg);
                                        if(data.percent==100){clearInterval(interval);oTable.ajax.reload();}
                                    });
                                }, 1000);
                            },
                            success:function(response){
    ...
    $('.viewData').click(function (e) {
        ...
        var token = "<?= csrf_token()?>";
        $.get("<?=route('molwa-certificate.ViewImportDetails')?>",{'api_option':api_option, 'per_page':per_page, 'page_no':page_no, 'district':district_selector, 'upazila':upazila_selector, 'category':category_selector, '_token':token},function(data){
...
function format ( rowData,id ) { //rowData[1] 
    ...
    var token = "<?= csrf_token()?>";
    $.ajax( {
        url: "<?=route('molwa-certificate.ViewReImportFF')?>",
		type: "POST",
        data: {
            'id':id, '_token':token
        },
