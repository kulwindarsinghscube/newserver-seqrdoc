...
<body>
	<style>
	body {
        background-image: url('{{ ($subdomain[0] == "kmtc") ? asset("assets/media/auth/B4.jpg") : asset("assets/media/auth/bg4.jpg") }}');
	...
	<div class="row" style="margin-top: 15%;">
		<div class="col-xs-12">
			...
			<div class="col-md-4 col-sm-12 col-xs-12">
				...
				<img class="img-responsive center-block" src="{{url('assets/media/auth/logo.PNG')}}" style="width:37%;">
				<?php } ?>
				 @if ($subdomain[0] != 'kmtc')
				<h2 class="center-block" style="text-align: center; font-family: 'Abel', sans-serif; font-weight: bold; color:#fff;">
				   
				        Welcome to {{ $sitename }} SeQR Web App
				...
				<h2 class="center-block" style="text-align: center; font-family: 'Abel', sans-serif; font-weight: bold; color:#fff;font-size: xxx-large;">
				        {{ strtoupper($sitename) }} Certificate Portal
				</h2>
				@endif
			</div>
			<div class="col-md-4 col-sm-12 col-xs-12">
						...
						<p>
							<form method="POST" action="#" id="web_login">
								...
								<fieldset>
								  ...
								  <button type="submit" id="login" class="btn btn-lg {{($subdomain[0]=='kmtc')?'btn-tpsdi':'btn-primary'}} btn-block"><i class="fa fa-unlock"></i> Login</button>
								  </div>
...
$(document).ready(function(){
	...
	$("#login,#verify,#set_password").click(function(e){
		...
		e.preventDefault();		
		var login_url="{{ URL::route('inswebapp.login') }}";
		...
		$("#web_login").ajaxSubmit({
              ...
              success:function(data)
              {
                if(data.success==true)
                {
                   ...
                   window.location.href="{{ URL::route('insdocuments') }}";
	...
	$("#signup").click(function(e){
		...
		if (!$('.signupform').valid()) 
		{
			...
			var web_register="{{ URL::route('webapp.register') }}";
			var method_type="post";
			var token="{{ csrf_token() }}";
...
<script>
	...
	$("form#forgotpasswordform").submit(function(e){
		...
		if (!$('.forgotpasswordform').valid())
		{
			...
			$.ajax({
				url: "{{route('authVerificationLink')}}",
				type: 'POST',
				data: formData,
				beforeSend:function() {
                    $('#loader').show();
